...
#include "sandbird.h"
  ...
  const char *inet_ntop_sandbird(int af, const void *src, char *dst, socklen_t size) {
...
static int get_socket_address(sb_Socket sockfd, char *dst) {
  ...
  if (addr.sas.ss_family == AF_INET6) {
    inet_ntop_sandbird(AF_INET6, &addr.sai6.sin6_addr, dst, INET6_ADDRSTRLEN);
  } else {
    inet_ntop_sandbird(AF_INET, &addr.sai.sin_addr, dst, INET_ADDRSTRLEN);
  }
  return SB_ESUCCESS;
}
