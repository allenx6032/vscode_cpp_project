...
#define DYAD_VERSION "0.2.1"
  ...
  const char *inet_ntop_dyad(int af, const void *src, char *dst, socklen_t size) {
...
static void stream_initAddress(dyad_Stream *stream) {
  ...
  if (addr.sas.ss_family == AF_INET6) {
    ...
    inet_ntop_dyad(AF_INET6, &addr.sai6.sin6_addr, stream->address,
              INET6_ADDRSTRLEN);
    stream->port = ntohs(addr.sai6.sin6_port);
  } else {
    stream->address = dyad_realloc(NULL, INET_ADDRSTRLEN);
    inet_ntop_dyad(AF_INET, &addr.sai.sin_addr, stream->address, INET_ADDRSTRLEN);
