...
DE_IMPL const unsigned char *deccan_fs_get_file_content(SDL_RWops *file) {
    int64_t size = SDL_RWsize(file);

    if (size > 0) {
        unsigned char *content = deccan_alloc(sizeof(char), size + 1);
        SDL_RWread(file, content, sizeof(unsigned char), size + 1);
        content[size] = '\0';
