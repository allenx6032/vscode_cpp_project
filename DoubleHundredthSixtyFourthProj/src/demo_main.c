...
uint64_t count = 0;
deccan_timer_t demo_timer;
deccan_camera_t demo_camera;
...
void begin(deccan_scene_t *scene) {
    ...
    deccan_camera_init(&demo_camera, 0.1f, 100.0f);
}
...
void render(deccan_scene_t *scene) {
    ...
    demo_camera.cam.position = (vec3s){0.0f, 0.0f, 1.0f};
    deccan_camera_set_viewport(&demo_camera, viewport);
    deccan_camera_set_ortho(&demo_camera, 30.0f);
    deccan_scene_set_camera(scene, &demo_camera);
}
