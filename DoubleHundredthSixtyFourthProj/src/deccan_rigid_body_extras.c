...
DE_IMPL void deccan_comp_rigid_body_apply_force_world(deccan_object_t obj, vec2s force, vec2s point) {
...
DE_IMPL void deccan_comp_rigid_body_apply_force_local(deccan_object_t obj, vec2s force, vec2s point) {
    deccan_comp_rigid_body_t *rb = deccan_object_get_component(obj, "RigidBody");
    if (rb) {
...
DE_IMPL void deccan_comp_rigid_body_apply_impulse_world(deccan_object_t obj, vec2s force, vec2s point) {
    deccan_comp_rigid_body_t *rb = deccan_object_get_component(obj, "RigidBody");
    if (rb) {
...
DE_IMPL void deccan_comp_rigid_body_apply_impulse_local(deccan_object_t obj, vec2s force, vec2s point) {
    deccan_comp_rigid_body_t *rb = deccan_object_get_component(obj, "RigidBody");
    if (rb) {
