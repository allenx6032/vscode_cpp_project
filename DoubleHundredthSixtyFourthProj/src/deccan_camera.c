...
DE_IMPL vec3s deccan_camera_convert_screen_to_world(deccan_camera_t *camera, vec3s coord) {
    vec2s screen = deccan_core_get_resolution();

    return glms_unproject(
        (vec3s){ .x=coord.x, .y=screen.y - coord.y, .z=coord.z },
        glms_mat4_mul(camera->proj, camera->cam.view),
        (vec4s){ .x=0, .y=0, .z=screen.x, .w=screen.y });
}
