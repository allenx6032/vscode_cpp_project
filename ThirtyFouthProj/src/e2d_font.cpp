...
bool Font::load()
{
    if (!mbLoad)
    {
        ...
        if (fontData == nullptr)
        {
            LOG_ERROR << ("Font::load loadAsset failed!");
            return false;
        }
        FT_Error error = FT_New_Memory_Face(fLibrary, fontData->data(), fontData->size(), 0, &mFace);
        if (error == FT_Err_Unknown_File_Format)
        {
            LOG_ERROR << ("Font::load FT_New_Memory_Face file format error!");
            return false;
        }
        else if (error)
        {
            LOG_ERROR << ("Font::load FT_New_Memory_Face file error! code: ") << error;
            return false;
        }
...
SPtr<FontChar> Font::loadFontChar(wchar_t ch, bool bBold, bool bItalic)
{
    ...
    if (error)
    {
        LOG_ERROR << ("Font::loadFontChar FT_Load_Char error! ch: ") << ch;
        return nullptr;
    }
    if (bBold)
    {
        ...
        if (error)
        {
            LOG_ERROR << ("Font::loadFontChar FT_Outline_Embolden error! ch: ") << ch;
            return nullptr;
        }
    }
    if (bItalic)
    ...
    if (error)
    {
        LOG_ERROR << ("Font::loadFontChar FT_Render_Glyph error! ch: ") << ch;
        return nullptr;
    }
