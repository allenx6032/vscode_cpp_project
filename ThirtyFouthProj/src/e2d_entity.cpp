...
bool Entity::setup()
{
    ...
    {
        LOG_WARN << ("Entity::setup: create TransformComponent Failed!");
        return false;
    }
    return true;
}
...
bool Entity::addChild(SPtr<Entity> pChild)
{
    ...
    {
        LOG_WARN << ("Entity::addChild: a child with the name '") << pChild->getName() <<
            ("' already exists. Child gets added but beware, duplicate names can become the cause of problems.");
        return false;
    }
