...
namespace Easy2D
{
    ...
    SPtr<T> Entity::createAction()
    {
        ...
        {
            LOG_WARN << ("Entity::createAction: a child with the name '") << pAction->getName() <<
                ("' already exists. Action gets added but beware, duplicate names can become the cause of problems.");
            return nullptr;
        }
        if (!pAction->setup(std::dynamic_pointer_cast<Entity>(shared_from_this())))
        {
            LOG_WARN << ("Entity::createAction: setup '") << pAction->getName() << ("' failed!");
            return nullptr;
        }
    ...
    SPtr<T> Entity::createComponent()
    {
        ...
        {
            LOG_WARN << ("Entity::createComponent: a child with the name '") << pComponent->getName() <<
                ("' already exists. Component gets added but beware, duplicate names can become the cause of problems.");
            return nullptr;
        }
        if (!pComponent->setup(std::dynamic_pointer_cast<Entity>(shared_from_this())))
        {
            LOG_WARN << ("Entity::createComponent: setup '") << pComponent->getName() << ("' failed!");
            return nullptr;
        }
