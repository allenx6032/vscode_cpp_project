...
void Scene::DrawComputerCards(void)
{
	...
	if (game->status == GAMEOVER){//游戏结束，所有手牌正面显示
		...
		if (i){//先显示牌的背面
			...
			GetTextExtentPoint32(hdcScene, szText, wcslen((const wchar_t*)(void *)szText), &size);
			TextOut(hdcScene, sceneSize.cx - 50 - size.cx / 2,
				65 + (cardSize.cy - size.cy) / 2, szText, wcslen((const wchar_t*)(void *)szText));
		...
		if (i){//显示一张牌的背面
			...
			TextOut(hdcScene, 50 - size.cx / 2, 65 + (cardSize.cy - size.cy) / 2,
				szText, wcslen((const wchar_t*)(void *)szText));
