...
{
	printf("xskin_test  UIWindow\n");
	if(m_hWnd)
		return FALSE;
	printf("xskin_test  UIWindow\n");
	m_hWnd = ::CreateWindowEx(GetWndExStyle(), GetWndClassName(), NULL, GetWndStyle(),
		0, 0, 100, 100, m_hWnd, NULL, UIApplication::Instance()->GetHandle(), this);
	printf("xskin_test  UIWindow\n");
	if(m_hWnd)
	{
	...
	{
		printf("xskin_test  UIWindow FALSE\n");
		return FALSE;
	}
}
