...
{
	UISize size = GetPreferredSize();

	if(m_bkgnd.IsValid())
	{
		size.cx = max(size.cx, (long)m_bkgnd.Width());
		size.cy = max(size.cy, (long)m_bkgnd.Height());
	}
	if(m_foldIcon.IsValid())
	{
		size.cx += m_foldIcon.Width();
		size.cy = max(size.cy, (long)m_foldIcon.Height());
	}
	if(m_minusIcon.IsValid())
	{
		size.cx += m_minusIcon.Width();
		size.cy = max(size.cy, (long)m_minusIcon.Height());
	}
	if(m_minusIcon.IsValid())
	{
		size.cx += m_minusIcon.Width();
