...
// Warning: List must match 'enum XMLError'
    ...
    XMLError XMLDocument::LoadFile( const char* filename )
    {
        ...
        Clear();
#ifdef WIN32
        wchar_t *wideFileName = Utf8ToUcs2(filename);
        HANDLE fileHandle = ::CreateFileW(wideFileName,
        ...
        unsigned long long filelength = {highPart};
        filelength = (filelength<<32) | lowPart;
        ...
        DWORD  read = 0;
        ::ReadFile(fileHandle,_charBuffer, size, &read,nullptr);
        ::CloseHandle(fileHandle);
        if ( read != size ) {
            SetError( XML_ERROR_FILE_READ_ERROR, 0, 0 );
            return _errorID;
