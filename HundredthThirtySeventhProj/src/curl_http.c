                          ...
                          int socketindex)
{
  ...
  ptr = curlx_dyn_ptr(in);
  size = curlx_dyn_len(in);
...
static CURLcode add_haproxy_protocol_header(struct Curl_easy *data)
{
  ...
  DEBUGASSERT(data->conn);
  curlx_dyn_init(&req, DYN_HAXPROXY);

#ifdef USE_UNIX_SOCKETS
  if(data->conn->unix_domain_socket)
    /* the buffer is large enough to hold this! */
    result = curlx_dyn_addn(&req, STRCONST("PROXY UNKNOWN\r\n"));
  else {
#endif
  /* Emit the correct prefix for IPv6 */
  tcp_version = data->conn->bits.ipv6 ? "TCP6" : "TCP4";

  result = curlx_dyn_addf(&req, "PROXY %s %s %s %i %i\r\n",
                         tcp_version,
                        ...
                        CURLcode status, bool premature)
{
  ...
  curlx_dyn_free(&http->send_buffer);
                          ...
                          struct dynbuf *req)
{
  ...
  if(!data->state.disableexpect && Curl_use_http_1_1plus(data, conn) &&
    ...
    if(ptr) {
      ...
      result = curlx_dyn_addn(req, STRCONST("Expect: 100-continue\r\n"));
      if(!result)
                                    ...
                                    struct Curl_easy *handle)
{
  ...
  while(trailers) {
    ...
    if(ptr && *(ptr + 1) == ' ') {
      result = curlx_dyn_add(b, trailers->data);
      if(result)
        return result;
      result = curlx_dyn_add(b, endofline_native);
      if(result)
        return result;
    }
    else
  ...
  result = curlx_dyn_add(b, endofline_network);
  return result;
}
...
#define Curl_buffer_send(a,b,c,d,e) CURLE_OK
  ...
  case HTTPREQ_PUT: /* Let's PUT the data to the server! */
    ...
    if((http->postsize != -1) && !data->req.upload_chunky &&
      ...
      result = curlx_dyn_addf(r, "Content-Length: %" CURL_FORMAT_CURL_OFF_T
                             "\r\n", http->postsize);
      if(result)
        return result;
    }
  ...
  case HTTPREQ_POST_MIME:
    ...
    if(conn->bits.authneg) {
      /* nothing to post! */
      result = curlx_dyn_addn(r, STRCONST("Content-Length: 0\r\n\r\n"));
      if(result)
        return result;
  ...
  case HTTPREQ_POST:
    ...
    if(data->set.postfields) {
      ...
      if(conn->httpversion != 20 &&
        ...
        if(!data->req.upload_chunky) {
          ...
          result = curlx_dyn_addn(r, data->set.postfields,
          ...
          if(!result) {
            result = curlx_dyn_addn(r, STRCONST("\x30\x0d\x0a\x0d\x0a"));
            /* 0  CR  LF  CR  LF */
            included_body += 5;
          }
        }
...
#endif
    {
      ...
      if(data->req.upload_chunky && conn->bits.authneg) {
        ...
        result = curlx_dyn_addn(r, (char *)STRCONST("\x30\x0d\x0a\x0d\x0a"));
        /* 0  CR  LF  CR  LF */
        if(result)
          return result;
      }
    ...
    if(co) {
      ...
      while(co) {
        if(co->value) {
          ...
          if((curlx_dyn_len(r) + strlen(co->name) + strlen(co->value) + 1) >=
...
{
  struct SingleRequest *k = &data->req;
  const char *header = curlx_dyn_ptr(&data->state.headerb);
  size_t hlen = curlx_dyn_len(&data->state.headerb);
                                     ...
                                     bool *stop_reading)
{
  ...
  do {
    ...
    if(!end_ptr) {
      ...
      result = curlx_dyn_addn(&data->state.headerb, str_start, *nread);
      if(result)
        return result;
    ...
    *nread -= (ssize_t)rest_length;
    ...
    result = curlx_dyn_addn(&data->state.headerb, str_start, full_length);
    if(result)
      return result;
    ...
    /* headers are in network encoding so use 0x0a and 0x0d instead of '\n'
       and '\r' */
    headp = curlx_dyn_ptr(&data->state.headerb);
    if((0x0a == *headp) || (0x0d == *headp)) {
      ...
      headerlen = curlx_dyn_len(&data->state.headerb);
...
#define HEADER1 headp /* no conversion needed, just use headp */
    ...
    data->info.header_size += curlx_dyn_len(&data->state.headerb);
    data->req.headerbytecount += curlx_dyn_len(&data->state.headerb);
