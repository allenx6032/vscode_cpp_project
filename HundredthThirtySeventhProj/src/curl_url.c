...
#include "curl_multihandle.h"
#include "curl_dotdot.h"
...
CURLcode Curl_close(struct Curl_easy **datap)
{
  ...
  *datap = NULL;
  ...
  if(data->req.doh) {
    curlx_dyn_free(&data->req.doh->probe[0].serverdoh);
    curlx_dyn_free(&data->req.doh->probe[1].serverdoh);
