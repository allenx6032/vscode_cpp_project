...
static CURLcode connect_init(struct Curl_easy *data, bool reinit)
{
  ...
  if(!reinit) {
    ...
    curlx_dyn_init(&s->rcvbuf, DYN_PROXY_CONNECT_HEADERS);
...
void Curl_connect_done(struct Curl_easy *data)
{
  ...
  if(s && (s->tunnel_state != TUNNEL_EXIT)) {
    ...
    curlx_dyn_free(&s->rcvbuf);
    curlx_dyn_free(&s->req);
  ...
  do {
    ...
    { /* READING RESPONSE PHASE */
      ...
      while(s->keepon) {
        ...
        if(curlx_dyn_addn(&s->rcvbuf, &byte, 1)) {
        ...
        s->headerlines++;
        linep = curlx_dyn_ptr(&s->rcvbuf);
        perline = curlx_dyn_len(&s->rcvbuf); /* amount of bytes in this line */
  ...
  do {
    ...
    case TUNNEL_INIT:
      ...
      if(!Curl_checkProxyheaders(data, conn, STRCONST("User-Agent")) &&
         data->set.str[STRING_USERAGENT]) {
        struct dynbuf ua;
        curlx_dyn_init(&ua, DYN_HTTP_REQUEST);
        result = curlx_dyn_addf(&ua, "User-Agent: %s\r\n",
                               data->set.str[STRING_USERAGENT]);
        if(result)
          goto error;
        result = Curl_hyper_header(data, headers, curlx_dyn_ptr(&ua));
        if(result)
          goto error;
        curlx_dyn_free(&ua);
      }
