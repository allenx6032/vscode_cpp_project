...
#include "curl_urldata.h" /* it includes http_chunks.h */
...
void Curl_httpchunk_init(struct Curl_easy *data)
{
  ...
  curlx_dyn_init(&conn->trailer, DYN_H1_TRAILER);
}
  ...
  while(length) {
    ...
    case CHUNK_TRAILER:
      if((*datap == 0x0d) || (*datap == 0x0a)) {
        char *tr = curlx_dyn_ptr(&conn->trailer);
        /* this is the end of a trailer, but if the trailer was zero bytes
           there was no trailer and we move on */

        if(tr) {
          size_t trlen;
          result = curlx_dyn_addn(&conn->trailer, (char *)STRCONST("\x0d\x0a"));
          if(result)
            return CHUNKE_OUT_OF_MEMORY;

          tr = curlx_dyn_ptr(&conn->trailer);
          trlen = curlx_dyn_len(&conn->trailer);
        ...
        result = curlx_dyn_addn(&conn->trailer, datap, 1);
        if(result)
          return CHUNKE_OUT_OF_MEMORY;
      }
      datap++;
