  ...
  *block = 0; /* we're not blocking by default */
  ...
  do {
    ...
    case SSH_SFTP_READDIR_INIT:
      ...
      curlx_dyn_init(&sshp->readdir, PATH_MAX * 2);
        ...
        if(data->set.list_only) {
          ...
          if(!result) {
            if((sshp->readdir_attrs.flags & LIBSSH2_SFTP_ATTR_PERMISSIONS) &&
              ...
              curlx_dyn_init(&sshp->readdir_link, PATH_MAX);
    ...
    case SSH_SFTP_READDIR_LINK:
      rc =
        libssh2_sftp_symlink_ex(sshc->sftp_session,
                                ...
                                (int)curlx_dyn_len(&sshp->readdir_link),
                                sshp->readdir_filename,
    ...
    case SSH_SFTP_READDIR_BOTTOM:
      ...
      if(!result) {
        ...
        Curl_debug(data, CURLINFO_DATA_IN,
                   curlx_dyn_ptr(&sshp->readdir),
                   curlx_dyn_len(&sshp->readdir));
        data->req.bytecount += curlx_dyn_len(&sshp->readdir);
      }
      if(result) {
