...
#include "curl_urlapi-int.h"
#include "curl_hsts.h"
#include "curl_setopt.h"
                            ...
                            void *raw)
{
  ...
  size_t bytes_left = curlx_dyn_len(trailers_buf) -
    data->state.trailers_bytes_sent;
  size_t to_copy = (size*nitems < bytes_left) ? size*nitems : bytes_left;
  if(to_copy) {
    memcpy(buffer,
           curlx_dyn_ptr(trailers_buf) + data->state.trailers_bytes_sent,
           to_copy);
...
static size_t trailers_left(void *raw)
{
  struct Curl_easy *data = (struct Curl_easy *)raw;
  struct dynbuf *trailers_buf = &data->state.trailers_buf;
  return curlx_dyn_len(trailers_buf) - data->state.trailers_bytes_sent;
}
#endif
                             ...
                             size_t *nreadp)
{
  ...
  if(data->state.trailers_state == TRAILERS_INITIALIZED) {
    ...
    curlx_dyn_init(&data->state.trailers_buf, DYN_TRAILERS);
    ...
    if(result) {
      curlx_dyn_free(&data->state.trailers_buf);
    ...
    *didwhat |= KEEP_RECV;
    ...
    if(!k->header && (nread > 0 || is_empty_data)) {
      ...
      if((k->badheader == HEADER_PARTHEADER) && !k->ignorebody) {
        size_t headlen = curlx_dyn_len(&data->state.headerb);
