...
static const char *octet2str(const char *beg, const char *end)
{
  ...
  curlx_dyn_init(&buf, 3 * CURL_ASN1_MAX + 1);
  result = curlx_dyn_addn(&buf, "", 0);

  while(!result && beg < end)
    result = curlx_dyn_addf(&buf, "%02x:", (unsigned char) *beg++);

  return curlx_dyn_ptr(&buf);
}
