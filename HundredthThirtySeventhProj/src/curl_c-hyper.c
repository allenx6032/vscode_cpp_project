                             ...
                             size_t value_len)
{
  ...
  if(name_len) {
    if(curlx_dyn_addf(&data->state.headerb, "%.*s: %.*s\r\n",
                     (int) name_len, name, (int) value_len, value))
      return HYPER_ITER_BREAK;
  }
  else {
    if(curlx_dyn_addn(&data->state.headerb, STRCONST("\r\n")))
      return HYPER_ITER_BREAK;
  }
  len = curlx_dyn_len(&data->state.headerb);
                            ...
                            const uint8_t *reason, size_t rlen)
{
  ...
  result = curlx_dyn_addf(&data->state.headerb, "HTTP/%s %03d %.*s\r\n",
                         vstr,
                         (int)http_status,
                         (int)rlen, reason);
  if(result)
    return result;
  len = curlx_dyn_len(&data->state.headerb);
  Curl_debug(data, CURLINFO_HEADER_IN, curlx_dyn_ptr(&data->state.headerb),
             len);
  ...
  if(!data->state.hconnect || !data->set.suppress_connect_headers) {
    ...
    result = Curl_client_write(data, writetype,
                               curlx_dyn_ptr(&data->state.headerb), len);
    if(result)
      return result;
  }
  data->info.header_size += (long)len;
                               ...
                               hyper_request *req)
{
  ...
  curlx_dyn_init(&r, DYN_HTTP_REQUEST);
  ...
  if(h2 && hyper_request_set_uri_parts(req,
                                       ...
                                       /* path_and_query */
                                       (uint8_t *)curlx_dyn_uptr(&r),
                                       curlx_dyn_len(&r))) {
    failf(data, "error setting uri parts to hyper");
    result = CURLE_OUT_OF_MEMORY;
  }
  else if(!h2 && hyper_request_set_uri(req, (uint8_t *)curlx_dyn_uptr(&r),
                                       curlx_dyn_len(&r))) {
    failf(data, "error setting uri to hyper");
    result = CURLE_OUT_OF_MEMORY;
  }
  else
    result = debug_request(data, method, curlx_dyn_ptr(&r), h2);
                         ...
                         Curl_HttpReq httpreq)
{
  ...
  if((httpreq == HTTPREQ_GET) || (httpreq == HTTPREQ_HEAD))
    ...
    if(!result)
      result = Curl_hyper_header(data, headers, curlx_dyn_ptr(&req));
