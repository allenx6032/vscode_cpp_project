...
#endif
#include "curl_openssl.h"
#elif defined(USE_GNUTLS)
#include <ngtcp2/ngtcp2_crypto_gnutls.h>
#include "curl_gtls.h"
#elif defined(USE_WOLFSSL)
#include <ngtcp2/ngtcp2_crypto_wolfssl.h>
#include "curl_wolfssl.h"
#endif
...
#include "curl_vquic.h"
#include "curl_h2h3.h"
#include "curl_keylog.h"
#include "curl_vtls.h"
...
static CURLcode write_data(struct HTTP *stream, const void *mem, size_t memlen)
{
  ...
  if(ncopy)
    result = curlx_dyn_addn(&stream->overflow, buf, ncopy);
  return result;
}
...
{
  size_t overlen = curlx_dyn_len(&stream->overflow);
  size_t ncopy = CURLMIN(overlen, stream->len);
  if(ncopy > 0) {
    memcpy(stream->mem, curlx_dyn_ptr(&stream->overflow), ncopy);
    stream->len -= ncopy;
    stream->mem += ncopy;
    stream->memlen += ncopy;
    if(ncopy != overlen)
      /* make the buffer only keep the tail */
      (void)curlx_dyn_tail(&stream->overflow, overlen - ncopy);
    else
      curlx_dyn_reset(&stream->overflow);
  }
  return ncopy;
}
                             ...
                             size_t len)
{
  ...
  curlx_dyn_init(&stream->overflow, CURL_MAX_READ_SIZE);
...
void Curl_quic_done(struct Curl_easy *data, bool premature)
{
  ...
  if(data->conn->handler == &Curl_handler_http3) {
    /* only for HTTP/3 transfers */
    struct HTTP *stream = data->req.p.http;
    curlx_dyn_free(&stream->overflow);
    free(stream->h3out);
  }
}
...
bool Curl_quic_data_pending(const struct Curl_easy *data)
{
  ...
  return curlx_dyn_len(&stream->overflow) > 0;
}
