/*
     ...
     $Id: puAuxLargeInput.cxx 2068 2006-03-20 15:17:35Z fayjf $
*/


#include "plib_puAuxLocal.h"

UL_RTTI_DEF2(puaLargeInput,puInputBase,puGroup)
...
void puaLargeInput::normalizeCursors ( void )
{
...
void puaLargeInput::removeSelectRegion ( void )
{
...
puaLargeInput::puaLargeInput ( int x, int y, int w, int h, int arrows, int sl_width, int wrap_text ) :
...
void  puaLargeInput::selectEntireLine ( void )
{
...
void  puaLargeInput::addNewLine ( const char *l )
{
...
void  puaLargeInput::addText ( const char *l )
{
  char *text = getStringValue () ;
...
void  puaLargeInput::appendText ( const char *l )
{
  if ( !l ) return ;
...
void  puaLargeInput::removeText ( int start, int end )
{
  int str_val_len = strlen ( getStringValue () ) ;
...
void puaLargeInput::draw ( int dx, int dy )
{
...
void puaLargeInput::doHit ( int button, int updown, int x, int y )
{
...
int puaLargeInput::checkKey ( int key, int updown )
{
  ...
  if ( ! done && ! input_disabled )
  {
    ...
    case 0x18 /* ^X */ :  /* Cut or copy selected text */
      if ( select_start_position != select_end_position )
      {
        extern void puSetPasteBuffer ( const char *ch ) ;
        p = getStringValue () ;
...
void puaLargeInput::wrapText ( void )
{
