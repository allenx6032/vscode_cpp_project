...
static int	Direct3D9_GraphFilter_GetWorkTexture( int IsFloatType, int ChannelBitDepth, DWORD TexSizeW, DWORD TexSizeH, DWORD HandleIndex )
{
	...
	if( GRAPHCHK( GraphFilterSystemInfoD3D9.WorkDrawValidGrHandle[ IsFloatType ][ BitDepth ][ NPow ][ HandleIndex ], BaseImage ) )
	{
		...
		GraphFilterSystemInfoD3D9.WorkDrawValidGrHandle[ IsFloatType ][ BitDepth ][ NPow ][ HandleIndex ] = Graphics_Image_MakeGraph_UseGParam( &GParam, TexSize, TexSize, FALSE, FALSE, 0, FALSE ) ;
		if( GraphFilterSystemInfoD3D9.WorkDrawValidGrHandle[ IsFloatType ][ BitDepth ][ NPow ][ HandleIndex ] < 0 )
		{
		...
		NS_SetDeleteHandleFlag( GraphFilterSystemInfoD3D9.WorkDrawValidGrHandle[ IsFloatType ][ BitDepth ][ NPow ][ HandleIndex ], &GraphFilterSystemInfoD3D9.WorkDrawValidGrHandle[ IsFloatType ][ BitDepth ][ NPow ][ HandleIndex ] ) ;
	}

	return GraphFilterSystemInfoD3D9.WorkDrawValidGrHandle[ IsFloatType ][ BitDepth ][ NPow ][ HandleIndex ] ;
}
...
extern int	GraphFilter_D3D9_Bright_Clip_PF( GRAPHFILTER_INFO *Info, int CmpType, float CmpParam, int ClipFillFlag, COLOR_F *ClipFillColor, int IsPMA )
{
	...
	if( GraphFilterShaderHandle.BrightClipPS[ UseShader ][ UseFill ][ IsPMA ] < 0 )
	{
		...
		if( GraphFilterShaderHandle.BrightClipPS[ UseShader ][ UseFill ][ IsPMA ] < 0 )
		{
			...
			ConvString( PsoFileName[ UseShader ][ UseFill ][ IsPMA ], DX_CHARCODEFORMAT_ASCII, ( char * )PathUTF16LE, sizeof( PathUTF16LE ), DX_CHARCODEFORMAT_UTF16LE ) ;
			DXST_LOGFILEFMT_ADDUTF16LE(( "\xd5\x30\xa3\x30\xeb\x30\xbf\x30\xfc\x30\x28\x75\xb7\x30\xa7\x30\xfc\x30\xc0\x30\xfc\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x20\x00\x25\x00\x73\x00\x00"/*@ L"フィルター用シェーダーの作成に失敗しました %s" @*/, PathUTF16LE )) ;
			return -1 ;
		}
...
extern int	GraphFilter_D3D9_Bright_Scale_PF( GRAPHFILTER_INFO *Info, int BrightMin, int BrightMax, int IsPMA )
{
	...
	if( GraphFilterShaderHandle.BrightScalePS[ IsPMA ] < 0 )
	{
		...
		if( GraphFilterShaderHandle.BrightScalePS[ IsPMA ] < 0 )
		{
			...
			ConvString( PsoFileName[ IsPMA ], DX_CHARCODEFORMAT_ASCII, ( char * )PathUTF16LE, sizeof( PathUTF16LE ), DX_CHARCODEFORMAT_UTF16LE ) ;
			DXST_LOGFILEFMT_ADDUTF16LE(( "\xd5\x30\xa3\x30\xeb\x30\xbf\x30\xfc\x30\x28\x75\xb7\x30\xa7\x30\xfc\x30\xc0\x30\xfc\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x20\x00\x25\x00\x73\x00\x00"/*@ L"フィルター用シェーダーの作成に失敗しました %s" @*/, PathUTF16LE )) ;
			return -1 ;
		}
...
extern int	GraphFilter_D3D9_HSB_PF(         GRAPHFILTER_INFO *Info, int HueType, float Hue, float Saturation, float Bright, int IsPMA )
{
	...
	if( GraphFilterShaderHandle.HsbPS[ UseShader ][ IsPMA ] < 0 )
	{
		...
		if( GraphFilterShaderHandle.HsbPS[ UseShader ][ IsPMA ] < 0 )
		{
			...
			ConvString( PsoFileName[ UseShader ][ IsPMA ], DX_CHARCODEFORMAT_ASCII, ( char * )PathUTF16LE, sizeof( PathUTF16LE ), DX_CHARCODEFORMAT_UTF16LE ) ;
			DXST_LOGFILEFMT_ADDUTF16LE(( "\xd5\x30\xa3\x30\xeb\x30\xbf\x30\xfc\x30\x28\x75\xb7\x30\xa7\x30\xfc\x30\xc0\x30\xfc\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x20\x00\x25\x00\x73\x00\x00"/*@ L"フィルター用シェーダーの作成に失敗しました %s" @*/, PathUTF16LE )) ;
			return -1 ;
		}
		...
		if( SrcGraphDivFlag )
		{
			...
			*UseSrcGrHandle = Direct3D9_GraphFilter_GetWorkTexture( SrcTexFloatType, SrcTexChannelBitDepth, ( DWORD )TexSizeW, ( DWORD )TexSizeH, ( DWORD )UseSrcWorkHandleIndex ) ;
			...
			for( i = 0 ; i < SrcImage->Hard.DrawNum ; i ++ )
			{
				...
				Graphics_D3D9_DrawPrimitive2D( Vert, 4, DX_PRIMTYPE_TRIANGLESTRIP, SrcImage, TRUE, FALSE, FALSE, FALSE, i, FALSE ) ;
			}
		}
	}
