...
extern int Graphics_D3D9_Shader_Normal3DDraw_Setup( void )
{
	...
	if( GSYS.DrawSetting.ShadowMapDraw )
	{
		...
		PS           =  &SB3D->Base3D_ShadowMap_Normal_PS       [ IgnoreTextureAlpha ] ;
		PSAddress    = &SCB3D->Base3D_ShadowMap_Normal_PSAddress[ IgnoreTextureAlpha ] ;
	}
	else
	if( GD3D9.Device.State.Lighting )
	{
		...
		PS_PL        =  &SB3D->Base3D_PixelLighting_Normal_PS       [ ShadowMap ][ LightIndex84 ][ Specular ][ RenderBlendType ][ GD3D9.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ] ;
		PSAddress_PL = &SCB3D->Base3D_PixelLighting_Normal_PSAddress[ ShadowMap ][ LightIndex84 ][ Specular ][ RenderBlendType ][ GD3D9.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ] ;
		...
		PS           =  &SB3D->Base3D_VertexLighting_Normal_PS       [ ShaderModel ][ ShadowMap ][ LightIndex10 ][ Specular ][ RenderBlendType ][ GD3D9.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ] ;
		PSAddress    = &SCB3D->Base3D_VertexLighting_Normal_PSAddress[ ShaderModel ][ ShadowMap ][ LightIndex10 ][ Specular ][ RenderBlendType ][ GD3D9.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ] ;
	}
	else
	{
		// ライティングなし描画
		...
		PS           =  &SB3D->Base3D_NoLighting_Normal_PS       [ RenderBlendType ][ GD3D9.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ] ;
		PSAddress    = &SCB3D->Base3D_NoLighting_Normal_PSAddress[ RenderBlendType ][ GD3D9.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ] ;
	}
...
/*		if( GRH.UseAlphaChDrawShader )
		...
		if( GSYS.DrawSetting.UseShadowMapNum > 0 )
		{
			...
			for( i = 0 ; i < MAX_USE_SHADOWMAP_NUM ; i ++ )
			{
				BlendInfo.TextureStageInfo[ 8 + i ].TextureCoordIndex = 0 ;
				BlendInfo.TextureStageInfo[ 8 + i ].ResultTempARG = FALSE ;

				if( !SHADOWMAPCHKFULL( GSYS.DrawSetting.ShadowMap[ i ], ShadowMap ) )
				{
					StageTexture[ 8 + i ] = ShadowMap->PF->D3D9.Texture ;
