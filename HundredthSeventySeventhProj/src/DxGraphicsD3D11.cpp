...
extern int Graphics_D3D11_Shader_Normal3DDraw_Setup( void )
{
	...
	if( GSYS.DrawSetting.ShadowMapDraw )
	{
		...
		PS           =  &SB3D->Base3D_ShadowMap_Normal_PS     [ IgnoreTextureAlpha ][ AlphaTestMode ] ;
		PSAddress    = &SCB3D->Base3D_ShadowMap_Normal_PS_Code[ IgnoreTextureAlpha ][ AlphaTestMode ] ;
	}
	else
	if( GD3D11.Device.State.Lighting )
	{
		...
		PS_PL        =  &SB3D->Base3D_PixelLighting_Normal_PS     [ ShadowMap ][ LightIndex84 ][ RenderBlendType ][ GD3D11.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ][ AlphaTestMode ] ;
		PSAddress_PL = &SCB3D->Base3D_PixelLighting_Normal_PS_Code[ ShadowMap ][ LightIndex84 ][ RenderBlendType ][ GD3D11.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ][ AlphaTestMode ] ;
		...
		PS           =  &SB3D->Base3D_VertexLighting_Normal_PS     [ ShadowMap ][ LightIndex10 ][ RenderBlendType ][ GD3D11.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ][ AlphaTestMode ] ;
		PSAddress    = &SCB3D->Base3D_VertexLighting_Normal_PS_Code[ ShadowMap ][ LightIndex10 ][ RenderBlendType ][ GD3D11.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ][ AlphaTestMode ] ;
	}
	else
	{
		// ライティングなし描画
		...
		PS           =  &SB3D->Base3D_NoLighting_Normal_PS     [ RenderBlendType ][ GD3D11.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ][ AlphaTestMode ] ;
		PSAddress    = &SCB3D->Base3D_NoLighting_Normal_PS_Code[ RenderBlendType ][ GD3D11.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ][ AlphaTestMode ] ;
	}
...
// 描画先画面のセット
static	int		Graphics_Hardware_D3D11_SetDrawScreen_PF_Base( int DrawScreen, int OldScreenSurface, int OldScreenMipLevel, IMAGEDATA *NewTargetImage, IMAGEDATA *OldTargetImage, SHADOWMAPDATA *NewTargetShadowMap, SHADOWMAPDATA *OldTargetShadowMap, D_ID3D11RenderTargetView *pID3D11RenderTargetView )
{
...
{
	return Graphics_Hardware_D3D11_SetDrawScreen_PF_Base( DrawScreen, OldScreenSurface, OldScreenMipLevel, NewTargetImage, OldTargetImage, NewTargetShadowMap, OldTargetShadowMap, NULL ) ;
}
...
#endif
{
		...
		WinData.UseUpdateLayerdWindowFlag         == FALSE )
	{
		...
		WinAPIData.Win32Func.SetTimerFunc( NS_GetMainWindowHandle(), GRAPHICS_HARDWARE_D3D11_SCREENFLIPTIMER_ID, 32, (D_TIMERPROC)Graphics_Hardware_D3D11_ScreenFlipTimerProc ) ;
	}
}
...
extern int Graphics_Hardware_D3D11_SetDrawScreen_ID3D11RenderTargetView_PF( const void *pID3D11RenderTargetView )
{
...
//	int Result ;
	...
	if( GSYS.Setting.ValidHardware )
	{
		...
		Graphics_Hardware_D3D11_SetDrawScreen_PF_Base( DX_SCREEN_OTHER, OldScreenSurface, OldScreenMipLevel, NULL, OldImage, NULL, OldShadowMap, ( D_ID3D11RenderTargetView * )pID3D11RenderTargetView ) ;
	}
