...
// マウスの入力情報を保存する
static	void		StockMouseInputInfo(  int Button, int ClickX, int ClickY ) ;
static	void		StockMouseInputInfo2( int Button, int ClickX, int ClickY, int LogType ) ;
...
//				if( WinData.NotWindowVisibleFlag == TRUE ) AddExStyle |= WS_EX_TRANSPARENT ;
		...
		//		Rect.bottom += + 3;
			...
			else
			{
				...
				ShowWindow( WinData.MainWindow , WStyle_ShowWindow_Param[ WinData.WindowStyle ] ) ;
				UpdateWindow( WinData.MainWindow ) ;
			}
				...
				if( fp != NULL )
				{
					...
					if( DxSysData.LogFileTabStop == FALSE )
					{
						...
						for( i = 0 ; i < DxSysData.LogFileTabNum ; i ++ )
							String[i] = '\t' ;
						String[i] = '\0' ;
						WriteFile( fp, String, DxSysData.LogFileTabNum, &WriteSize, NULL ) ;
...
//					fputs( ErrorStr , fp ) ;
					...
					DxSysData.LogFileTabStop = ErrorStr[ lstrlenA( ErrorStr ) - 1 ] != '\n' ;
...
//				return DxLib_Error( DXSTRING( "ウインドウモード時にデスクトップの解像度が変更されました、終了します" ) ;
	...
	case WM_MBUTTONUP :
		...
		{
		case WM_LBUTTONDOWN :
			StockMouseInputInfo(  MOUSE_INPUT_LEFT,   ( short )LOWORD( lParam ), ( short )HIWORD( lParam ) ) ; 
			StockMouseInputInfo2( MOUSE_INPUT_LEFT,   ( short )LOWORD( lParam ), ( short )HIWORD( lParam ), MOUSE_INPUT_LOG_DOWN ) ; 
			break ;

		case WM_RBUTTONDOWN :
			StockMouseInputInfo(  MOUSE_INPUT_RIGHT,  ( short )LOWORD( lParam ), ( short )HIWORD( lParam ) ) ;
			StockMouseInputInfo2( MOUSE_INPUT_RIGHT,  ( short )LOWORD( lParam ), ( short )HIWORD( lParam ), MOUSE_INPUT_LOG_DOWN ) ;
			break ;

		case WM_MBUTTONDOWN :
			StockMouseInputInfo(  MOUSE_INPUT_MIDDLE, ( short )LOWORD( lParam ), ( short )HIWORD( lParam ) ) ;
			StockMouseInputInfo2( MOUSE_INPUT_MIDDLE, ( short )LOWORD( lParam ), ( short )HIWORD( lParam ), MOUSE_INPUT_LOG_DOWN ) ;
			break ;

		case WM_LBUTTONUP :
			StockMouseInputInfo2( MOUSE_INPUT_LEFT,   ( short )LOWORD( lParam ), ( short )HIWORD( lParam ), MOUSE_INPUT_LOG_UP ) ; 
			break ;

		case WM_RBUTTONUP :
			StockMouseInputInfo2( MOUSE_INPUT_RIGHT,  ( short )LOWORD( lParam ), ( short )HIWORD( lParam ), MOUSE_INPUT_LOG_UP ) ;
			break ;

		case WM_MBUTTONUP :
			StockMouseInputInfo2( MOUSE_INPUT_MIDDLE, ( short )LOWORD( lParam ), ( short )HIWORD( lParam ), MOUSE_INPUT_LOG_UP ) ;
			break ;
		}
#endif // DX_NON_INPUT
...
static	void		StockMouseInputInfo2( int Button, int ClickX, int ClickY, int LogType )
{
	...
	// ログの数が最大数に達していたら何もせず終了
	if( WinData.MouseInput2Num >= WIN_MOUSEINPUT_LOG_NUM ) return ;

	// 情報を保存
	MInput = &WinData.MouseInput2[ WinData.MouseInput2Num ] ;
	MInput->ClickX = ClickX ;
	MInput->ClickY = ClickY ;
	MInput->Button = Button ;
...
extern int NS_GetMouseInputLog2( int *Button, int *ClickX, int *ClickY, int *LogType, int LogDelete )
{
	...
	// ログが無かったら -1 を返す
	if( WinData.MouseInput2Num == 0 ) return -1 ;

	// 情報を返す
	MInput = &WinData.MouseInput2[ 0 ] ;
	if( Button  ) *Button  = MInput->Button ;
	if( ClickX  ) *ClickX  = MInput->ClickX ;
	if( ClickY  ) *ClickY  = MInput->ClickY ;
	...
	{
		WinData.MouseInput2Num -- ;
		if( WinData.MouseInput2Num )
			_MEMMOVE( WinData.MouseInput2, &WinData.MouseInput2[ 1 ], sizeof( WINMOUSEINPUT ) * WinData.MouseInput2Num ) ;
	}
