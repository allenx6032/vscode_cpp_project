...
extern int InitializeInputCharBuf( void )
{
	...
	if( WinData.WindowsVersion >= DX_WINDOWSVERSION_7 )
	{
		HRESULT hr ;
		D_TfClientId cid ;
		int Result = -1 ;
...
extern int NS_DrawIMEInputString( int x , int y , int SelectStringNum )
{
	...
	// 候補リストが出ている場合はその描画
	if( CharBuf.CandidateList && CharBuf.CandidateList->dwCount > 0 )
	{
		DWORD i ;
		RECT SelectRect ;
		int j ,h ;
...
extern int GetIMEInputModeStr_WCHAR_T( wchar_t *GetBuffer ) 
{
	...
	{
		ConvString( "\x68\x51\xd2\x89\xab\x30\xbf\x30\xab\x30\xca\x30\x00"/*@ L"全角カタカナ" @*/, DX_CHARCODEFORMAT_UTF16LE, ( char * )ZenkakuKanaString, sizeof( ZenkakuKanaString ), WCHAR_T_CHARCODEFORMAT ) ;
		ConvString( "\x4a\x53\xd2\x89\xab\x30\xbf\x30\xab\x30\xca\x30\x00"/*@ L"半角カタカナ" @*/, DX_CHARCODEFORMAT_UTF16LE, ( char * )HankakuKanaString, sizeof( HankakuKanaString ), WCHAR_T_CHARCODEFORMAT ) ;
		ConvString( "\x68\x51\xd2\x89\x72\x30\x89\x30\x4c\x30\x6a\x30\x00"/*@ L"全角ひらがな" @*/, DX_CHARCODEFORMAT_UTF16LE, ( char * )ZenkakuHiraString, sizeof( ZenkakuHiraString ), WCHAR_T_CHARCODEFORMAT ) ;
		ConvString( "\x68\x51\xd2\x89\xf1\x82\x70\x65\x00"/*@ L"全角英数" @*/, DX_CHARCODEFORMAT_UTF16LE, ( char * )ZenkakuEisuString, sizeof( ZenkakuEisuString ), WCHAR_T_CHARCODEFORMAT ) ;
		ConvString( "\x4a\x53\xd2\x89\xf1\x82\x70\x65\x00"/*@ L"半角英数" @*/, DX_CHARCODEFORMAT_UTF16LE, ( char * )HankakuEisuString, sizeof( HankakuEisuString ), WCHAR_T_CHARCODEFORMAT ) ;
		ConvString( "\x0d\x4e\x0e\x66\x6a\x30\x65\x51\x9b\x52\xe2\x30\xfc\x30\xc9\x30\x00"/*@ L"不明な入力モード" @*/, DX_CHARCODEFORMAT_UTF16LE, ( char * )FumeiString, sizeof( FumeiString ), WCHAR_T_CHARCODEFORMAT ) ;
