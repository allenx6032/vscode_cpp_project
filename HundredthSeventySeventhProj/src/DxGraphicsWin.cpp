...
extern int NS_GetDesktopScreenGraph( int x1, int y1, int x2, int y2, int GrHandle, int DestX, int DestY )
{
	...
	DesktopW = GetDeviceCaps( DesktopHDC, DESKTOPHORZRES ) ;
	DesktopH = GetDeviceCaps( DesktopHDC, DESKTOPVERTRES ) ;
	...
	// デスクトップイメージををDIBにコピー
	BitBlt( HMemDC, 0, 0, CaptureW, CaptureH, DesktopHDC, x1, y1, SRCCOPY ) ;
	ReleaseDC( DesktopHWND, DesktopHDC ) ;
	DesktopHDC = NULL ;
...
extern int NS_SetDrawScreen_ID3D11RenderTargetView( const void *pID3D11RenderTargetView )
{
	switch( GRAWIN.Setting.UseGraphicsAPI )
	{
#ifndef DX_NON_DIRECT3D11
	case GRAPHICS_API_DIRECT3D11_WIN32 :
		return Graphics_Hardware_D3D11_SetDrawScreen_ID3D11RenderTargetView_PF( pID3D11RenderTargetView ) ;
#endif // DX_NON_DIRECT3D11
...
#endif
{
		...
		WinData.UseUpdateLayerdWindowFlag         == FALSE )
	{
		...
		WinAPIData.Win32Func.SetTimerFunc( NS_GetMainWindowHandle(), GRAPHICS_SOFTWARE_SCREENFLIPTIMER_ID, 32, (D_TIMERPROC)Graphics_Software_ScreenFlipTimerProc ) ;
	}
}
...
)
{
	...
	if( GSYS.Screen.DisplayInfo != NULL )
	{
		...
		DisplayInfo->DesktopPixelSizeX     = GetDeviceCaps( hdc, DESKTOPHORZRES ) ;
		DisplayInfo->DesktopPixelSizeY     = GetDeviceCaps( hdc, DESKTOPVERTRES ) ;
