...
// Direct3D9 の２Ｄ描画用のシェーダーコードの初期化を行う
...
// Direct3D9 の２Ｄ描画用のシェーダーコードの後始末を行う
...
//	return 0 ;
		...
		( Shader->ValidVertexShader && Shader->ValidPixelShader ) )
	{
		...
		{
			DXST_ERRORLOGFMT_ADDUTF16LE(( "\xb7\x30\xa7\x30\xfc\x30\xc0\x30\xfc\x30\xb3\x30\xfc\x30\xc9\x30\x92\x30\x55\x5c\x8b\x95\x59\x30\x8b\x30\xe1\x30\xe2\x30\xea\x30\x18\x98\xdf\x57\x6e\x30\xba\x78\xdd\x4f\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"シェーダーコードを展開するメモリ領域の確保に失敗しました\n" @*/ )) ;
			GSYS.HardInfo.UseShader = FALSE ;
			goto ENDLABEL ;
		}
...
//				{
//					DXST_ERRORLOGFMT_ADDUTF16LE(( L"２Ｄ描画用基本頂点シェーダーオブジェクトの作成に失敗しました\n" )) ;
//					GSYS.HardInfo.UseShader = FALSE ;
//					goto ENDLABEL ;
//				}
//			}
...
//				{
//					DXST_ERRORLOGFMT_ADDUTF16LE(( L"３Ｄ描画用基本頂点シェーダーオブジェクトの作成に失敗しました\n" )) ;
//					GSYS.HardInfo.UseShader = FALSE ;
//					goto ENDLABEL ;
//				}
//			}
			...
			for( i = 0 ; i < 7 ; i ++ )
			{
				...
				for( j = 0 ; j < 2 ; j ++ )
				{
					...
					if( DXA_GetFileInfo( &ShaderCode->_2D.BaseShaderBinDxa, DX_CODEPAGE_ASCII, FileName, &Addr, &Size ) == 0 )
					{
						...
						{
							DXST_ERRORLOGFMT_ADDUTF16LE(( "\xfa\x57\x2c\x67\xb7\x30\xa7\x30\xfc\x30\xc0\x30\xfc\x30\xaa\x30\xd6\x30\xb8\x30\xa7\x30\xaf\x30\xc8\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"基本シェーダーオブジェクトの作成に失敗しました\n" @*/ )) ;
							GSYS.HardInfo.UseShader = FALSE ;
							goto ENDLABEL ;
						}
					}
			...
			{
				DXST_ERRORLOGFMT_ADDUTF16LE(( "\xde\x30\xb9\x30\xaf\x30\xe6\x51\x06\x74\x28\x75\xb7\x30\xa7\x30\xfc\x30\xc0\x30\xfc\x30\xaa\x30\xd6\x30\xb8\x30\xa7\x30\xaf\x30\xc8\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"マスク処理用シェーダーオブジェクトの作成に失敗しました\n" @*/ )) ;
				GSYS.HardInfo.UseShader = FALSE ;
				goto ENDLABEL ;
			}
		}
...
ENDLABEL:
	...
	{
		DXST_ERRORLOGFMT_ADDUTF16LE(( "\xd7\x30\xed\x30\xb0\x30\xe9\x30\xde\x30\xd6\x30\xeb\x30\xb7\x30\xa7\x30\xfc\x30\xc0\x30\xfc\x30\x92\x30\x7f\x4f\x28\x75\x57\x30\x7e\x30\x5b\x30\x93\x30\x0a\x00\x00"/*@ L"プログラマブルシェーダーを使用しません\n" @*/ )) ;
	}
	else
	{
		DXST_ERRORLOGFMT_ADDUTF16LE(( "\xd7\x30\xed\x30\xb0\x30\xe9\x30\xde\x30\xd6\x30\xeb\x30\xb7\x30\xa7\x30\xfc\x30\xc0\x30\xfc\x30\x92\x30\x7f\x4f\x28\x75\x57\x30\x7e\x30\x59\x30\x0a\x00\x00"/*@ L"プログラマブルシェーダーを使用します\n" @*/ )) ;
	}
...
extern	int		Graphics_D3D9_VertexBuffer_CreateObject( VERTEXBUFFERHANDLEDATA *VertexBuffer, int Restore )
{
	...
	if( Result < 0 )
	{
		DXST_ERRORLOG_ADDUTF16LE( "\x02\x98\xb9\x70\xd0\x30\xc3\x30\xd5\x30\xa1\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"頂点バッファの作成に失敗しました\n" @*/ ) ;
		return -1 ;
	}
...
extern	int		Graphics_D3D9_IndexBuffer_CreateObject( INDEXBUFFERHANDLEDATA *IndexBuffer, int Restore )
{
	...
	{
		DXST_ERRORLOG_ADDUTF16LE( "\xa4\x30\xf3\x30\xc7\x30\xc3\x30\xaf\x30\xb9\x30\xd0\x30\xc3\x30\xd5\x30\xa1\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"インデックスバッファの作成に失敗しました\n" @*/ ) ;
		return -1 ;
	}
...
extern	int		Graphics_D3D9_SetupSubBackBuffer( void )
{
	...
	GD3D9.Device.Screen.SubBackBufferTexture = NULL ;
	if( GRAWIN.Setting.FSAAMultiSampleCount == 0 && GRAWIN.Setting.FSAAMultiSampleQuality == 0 )
	{
	...
	if( GD3D9.Device.Screen.SubBackBufferSurface == NULL )
	{
		...
		hr = Direct3DDevice9_CreateRenderTarget(
			...
			( D_D3DMULTISAMPLE_TYPE )GRAWIN.Setting.FSAAMultiSampleCount,
			( DWORD )GRAWIN.Setting.FSAAMultiSampleQuality,
			FALSE,
			&GD3D9.Device.Screen.SubBackBufferSurface,
			NULL
		) ;
		...
		if( FAILED( hr ) )
		{
			DXST_ERRORLOG_ADDUTF16LE( "\xb5\x30\xd6\x30\xd0\x30\xc3\x30\xaf\x30\xd0\x30\xc3\x30\xd5\x30\xa1\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"サブバックバッファの作成に失敗しました\n" @*/ ) ;
			return -1 ;
		}
	}
...
extern	int		Graphics_D3D9_CreateZBuffer( void )
{
	...
	DXST_ERRORLOG_ADDUTF16LE( "\x3a\xff\xd0\x30\xc3\x30\xd5\x30\xa1\x30\x92\x30\x5c\x4f\x10\x62\x57\x30\x7e\x30\x59\x30\x2e\x00\x2e\x00\x2e\x00\x2e\x00\x20\x00\x00"/*@ L"Ｚバッファを作成します.... " @*/ ) ;
	...
	if( FAILED( hr ) )
	{
		DXST_ERRORLOG_ADDUTF16LE( "\x3a\xff\xd0\x30\xc3\x30\xd5\x30\xa1\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"Ｚバッファの作成に失敗しました\n" @*/ ) ;
		return -1 ;
	}
	...
	DXST_ERRORLOG_ADDUTF16LE( "\x10\x62\x9f\x52\x0a\x00\x00"/*@ L"成功\n" @*/ ) ;
...
extern int Graphics_D3D9_Device_Create()
{
	...
	{
		GRAWIN.Setting.FSAAMultiSampleCount = D_D3DMULTISAMPLE_NONE ;
		GRAWIN.Setting.FSAAMultiSampleQuality = 0 ;
	}
...
//	GD3D9.Device.Caps.DeviceCaps.PixelShaderVersion  = 0x000 ;
	...
	// 各種フォーマットを調べる
	{
		...
		default:
			DXST_ERRORLOGFMT_ADDA(( "\x83\x6f\x83\x62\x83\x4e\x83\x6f\x83\x62\x83\x74\x83\x40\x82\xaa\x91\xce\x89\x9e\x82\xb5\x82\xc4\x82\xa2\x82\xc8\x82\xa2\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67 %d \x82\xc5\x82\xb7"/*@ "バックバッファが対応していないフォーマット %d です" @*/, GD3D9.Device.Caps.ScreenFormat )) ;
			break ;
		}
		DXST_ERRORLOGFMT_ADDA(( "\x89\xe6\x96\xca\x82\xcc\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "画面のフォーマットは %s です" @*/, String )) ;

		// 16ビットＺバッファフォーマットの設定
		GD3D9.Device.Caps.ZBufferFormat[ ZBUFFER_FORMAT_16BIT ] = D_D3DFMT_D16 ;
		String = "D3DFMT_D16" ;
		DXST_ERRORLOGFMT_ADDA(( "16bit Z\x83\x6f\x83\x62\x83\x74\x83\x40\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "16bit Zバッファフォーマットは %s です" @*/, String )) ;
		...
		DXST_ERRORLOGFMT_ADDA(( "24bit Z\x83\x6f\x83\x62\x83\x74\x83\x40\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "24bit Zバッファフォーマットは %s です" @*/, String )) ;
		...
		DXST_ERRORLOGFMT_ADDA(( "32bit Z\x83\x6f\x83\x62\x83\x74\x83\x40\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "32bit Zバッファフォーマットは %s です" @*/, String )) ;
		...
		DXST_ERRORLOGFMT_ADDA(( "16bit \x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "16bit カラーフォーマットは %s です" @*/, String )) ;
		...
		DXST_ERRORLOGFMT_ADDA(( "32bit \x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "32bit カラーフォーマットは %s です" @*/, String )) ;
		...
		DXST_ERRORLOGFMT_ADDA(( "\x83\x41\x83\x8b\x83\x74\x83\x40\x95\x74\x82\xab 16bit \x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "アルファ付き 16bit カラーフォーマットは %s です" @*/, String )) ;
		...
		DXST_ERRORLOGFMT_ADDA(( "\x83\x41\x83\x8b\x83\x74\x83\x40\x95\x74\x82\xab 32bit \x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "アルファ付き 32bit カラーフォーマットは %s です" @*/, String )) ;
		...
		DXST_ERRORLOGFMT_ADDA(( "\x83\x41\x83\x8b\x83\x74\x83\x40\x83\x65\x83\x58\x83\x67\x97\x70 16bit \x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "アルファテスト用 16bit カラーフォーマットは %s です" @*/, String )) ;
		...
		DXST_ERRORLOGFMT_ADDA(( "\x82\x63\x82\x77\x82\x73\x82\x50\x83\x65\x83\x4e\x83\x58\x83\x60\x83\x83\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd%s"/*@ "ＤＸＴ１テクスチャフォーマットは%s" @*/, String )) ;
		...
		DXST_ERRORLOGFMT_ADDA(( "\x82\x63\x82\x77\x82\x73\x82\x51\x83\x65\x83\x4e\x83\x58\x83\x60\x83\x83\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd%s"/*@ "ＤＸＴ２テクスチャフォーマットは%s" @*/, String )) ;
		...
		DXST_ERRORLOGFMT_ADDA(( "\x82\x63\x82\x77\x82\x73\x82\x52\x83\x65\x83\x4e\x83\x58\x83\x60\x83\x83\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd%s"/*@ "ＤＸＴ３テクスチャフォーマットは%s" @*/, String )) ;
		...
		DXST_ERRORLOGFMT_ADDA(( "\x82\x63\x82\x77\x82\x73\x82\x53\x83\x65\x83\x4e\x83\x58\x83\x60\x83\x83\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd%s"/*@ "ＤＸＴ４テクスチャフォーマットは%s" @*/, String )) ;
		...
		DXST_ERRORLOGFMT_ADDA(( "\x82\x63\x82\x77\x82\x73\x82\x54\x83\x65\x83\x4e\x83\x58\x83\x60\x83\x83\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd%s"/*@ "ＤＸＴ５テクスチャフォーマットは%s" @*/, String )) ;
		...
		{
			DXST_ERRORLOGFMT_ADDA(( "\x95\x60\x89\xe6\x97\x70 16bit \x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "描画用 16bit カラーフォーマットは %s です" @*/, String )) ;
		}
		...
		{
			DXST_ERRORLOGFMT_ADDA(( "\x95\x60\x89\xe6\x97\x70 32bit \x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "描画用 32bit カラーフォーマットは %s です" @*/, String )) ;
		}
		...
		{
			DXST_ERRORLOGFMT_ADDA(( "\x95\x60\x89\xe6\x97\x70\x83\x41\x83\x8b\x83\x74\x83\x40\x95\x74\x82\xab 32bit \x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "描画用アルファ付き 32bit カラーフォーマットは %s です" @*/, String )) ;
		}
		...
		{
			DXST_ERRORLOGFMT_ADDA(( "\x95\x60\x89\xe6\x97\x70 ABGR \x90\xae\x90\x94 16 \x83\x72\x83\x62\x83\x67\x8c\x5e\x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "描画用 ABGR 整数 16 ビット型カラーフォーマットは %s です" @*/, String )) ;
		}
		...
		{
			DXST_ERRORLOGFMT_ADDA(( "\x95\x60\x89\xe6\x97\x70 ABGR \x95\x82\x93\xae\x8f\xac\x90\x94\x93\x5f 16 \x83\x72\x83\x62\x83\x67\x8c\x5e\x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "描画用 ABGR 浮動小数点 16 ビット型カラーフォーマットは %s です" @*/, String )) ;
		}
		...
		{
			DXST_ERRORLOGFMT_ADDA(( "\x95\x60\x89\xe6\x97\x70 ABGR \x95\x82\x93\xae\x8f\xac\x90\x94\x93\x5f 32 \x83\x72\x83\x62\x83\x67\x8c\x5e\x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "描画用 ABGR 浮動小数点 32 ビット型カラーフォーマットは %s です" @*/, String )) ;
		}
		...
		{
			DXST_ERRORLOGFMT_ADDA(( "\x95\x60\x89\xe6\x97\x70\x82\x50\x83\x60\x83\x83\x83\x93\x83\x6c\x83\x8b\x90\xae\x90\x94 8 \x83\x72\x83\x62\x83\x67\x8c\x5e\x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "描画用１チャンネル整数 8 ビット型カラーフォーマットは %s です" @*/, String )) ;
		}
		...
		{
			DXST_ERRORLOGFMT_ADDA(( "\x95\x60\x89\xe6\x97\x70\x82\x50\x83\x60\x83\x83\x83\x93\x83\x6c\x83\x8b\x90\xae\x90\x94 16 \x83\x72\x83\x62\x83\x67\x8c\x5e\x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "描画用１チャンネル整数 16 ビット型カラーフォーマットは %s です" @*/, String )) ;
		}
		...
		{
			DXST_ERRORLOGFMT_ADDA(( "\x95\x60\x89\xe6\x97\x70\x82\x50\x83\x60\x83\x83\x83\x93\x83\x6c\x83\x8b\x95\x82\x93\xae\x8f\xac\x90\x94\x93\x5f 16 \x83\x72\x83\x62\x83\x67\x8c\x5e\x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "描画用１チャンネル浮動小数点 16 ビット型カラーフォーマットは %s です" @*/, String )) ;
		}
		...
		{
			DXST_ERRORLOGFMT_ADDA(( "\x95\x60\x89\xe6\x97\x70\x82\x50\x83\x60\x83\x83\x83\x93\x83\x6c\x83\x8b\x95\x82\x93\xae\x8f\xac\x90\x94\x93\x5f 32 \x83\x72\x83\x62\x83\x67\x8c\x5e\x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "描画用１チャンネル浮動小数点 32 ビット型カラーフォーマットは %s です" @*/, String )) ;
		}
		...
		{
			DXST_ERRORLOGFMT_ADDA(( "\x95\x60\x89\xe6\x97\x70\x82\x51\x83\x60\x83\x83\x83\x93\x83\x6c\x83\x8b\x90\xae\x90\x94 8 \x83\x72\x83\x62\x83\x67\x8c\x5e\x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "描画用２チャンネル整数 8 ビット型カラーフォーマットは %s です" @*/, String )) ;
		}
		...
		{
			DXST_ERRORLOGFMT_ADDA(( "\x95\x60\x89\xe6\x97\x70\x82\x51\x83\x60\x83\x83\x83\x93\x83\x6c\x83\x8b\x90\xae\x90\x94 16 \x83\x72\x83\x62\x83\x67\x8c\x5e\x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "描画用２チャンネル整数 16 ビット型カラーフォーマットは %s です" @*/, String )) ;
		}
		...
		{
			DXST_ERRORLOGFMT_ADDA(( "\x95\x60\x89\xe6\x97\x70\x82\x51\x83\x60\x83\x83\x83\x93\x83\x6c\x83\x8b\x95\x82\x93\xae\x8f\xac\x90\x94\x93\x5f 16 \x83\x72\x83\x62\x83\x67\x8c\x5e\x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "描画用２チャンネル浮動小数点 16 ビット型カラーフォーマットは %s です" @*/, String )) ;
		}
		...
		{
			DXST_ERRORLOGFMT_ADDA(( "\x95\x60\x89\xe6\x97\x70\x82\x51\x83\x60\x83\x83\x83\x93\x83\x6c\x83\x8b\x95\x82\x93\xae\x8f\xac\x90\x94\x93\x5f 32 \x83\x72\x83\x62\x83\x67\x8c\x5e\x83\x4a\x83\x89\x81\x5b\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd %s \x82\xc5\x82\xb7"/*@ "描画用２チャンネル浮動小数点 32 ビット型カラーフォーマットは %s です" @*/, String )) ;
		}
	...
	// 情報出力
	{
		DXST_ERRORLOGFMT_ADDA(( "\x88\xea\x93\x78\x82\xc9\x95\x60\x89\xe6\x82\xc5\x82\xab\x82\xe9\x83\x76\x83\x8a\x83\x7e\x83\x65\x83\x42\x83\x75\x82\xcc\x8d\xc5\x91\xe5\x90\x94:%d"/*@ "一度に描画できるプリミティブの最大数:%d" @*/, GSYS.HardInfo.MaxPrimitiveCount )) ;
		DXST_ERRORLOGFMT_ADDA(( "\x91\xce\x89\x9e\x82\xb5\x82\xc4\x82\xa2\x82\xe9\x8d\xc5\x91\xe5\x92\xb8\x93\x5f\x83\x43\x83\x93\x83\x66\x83\x62\x83\x4e\x83\x58:%d"/*@ "対応している最大頂点インデックス:%d" @*/,     GSYS.HardInfo.MaxVertexIndex )) ;
		DXST_ERRORLOGFMT_ADDA(( "\x93\xaf\x8e\x9e\x82\xc9\x83\x8c\x83\x93\x83\x5f\x83\x8a\x83\x93\x83\x4f\x82\xc5\x82\xab\x82\xe9\x83\x6f\x83\x62\x83\x74\x83\x40\x82\xcc\x90\x94:%d"/*@ "同時にレンダリングできるバッファの数:%d" @*/, GSYS.HardInfo.RenderTargetNum )) ;
		DXST_ERRORLOGFMT_ADDA(( "\x8d\xc5\x91\xe5\x83\x65\x83\x4e\x83\x58\x83\x60\x83\x83\x83\x54\x83\x43\x83\x59\x81\x40\x95\x9d:%d \x8d\x82\x82\xb3:%d"/*@ "最大テクスチャサイズ　幅:%d 高さ:%d" @*/, GD3D9.Device.Caps.DeviceCaps.MaxTextureWidth, GD3D9.Device.Caps.DeviceCaps.MaxTextureHeight )) ;
		DXST_ERRORLOGFMT_ADDA(( "\x83\x65\x83\x4e\x83\x58\x83\x60\x83\x83\x83\x58\x83\x65\x81\x5b\x83\x57\x83\x65\x83\x93\x83\x7c\x83\x89\x83\x8a\x83\x8c\x83\x57\x83\x58\x83\x5e\x81\x46%s"/*@ "テクスチャステージテンポラリレジスタ：%s" @*/, GD3D9.Device.Caps.ValidTexTempRegFlag ? "\x8e\x67\x97\x70\x89\xc2"/*@ "使用可" @*/ : "\x8e\x67\x97\x70\x95\x73\x89\xc2"/*@ "使用不可" @*/ )) ;
		DXST_ERRORLOGFMT_ADDA(( "\x8c\xb8\x8e\x5a\x8d\x87\x90\xac\x82\xcc\x83\x6e\x81\x5b\x83\x68\x83\x45\x83\x46\x83\x41\x91\xce\x89\x9e\x81\x46%s"/*@ "減算合成のハードウェア対応：%s" @*/, GD3D9.Device.Caps.ValidDestBlendOp ? "\x83\x6c\x83\x43\x83\x65\x83\x42\x83\x75"/*@ "ネイティブ" @*/ : "\x83\x47\x83\x7e\x83\x85\x83\x8c\x81\x5b\x83\x67"/*@ "エミュレート" @*/ )) ;
		if( GD3D9.Device.Caps.VertexHardwareProcess )
		{
			DXST_ERRORLOGFMT_ADDA(( "\x83\x6e\x81\x5b\x83\x68\x83\x45\x83\x46\x83\x41\x92\xb8\x93\x5f\x83\x56\x83\x46\x81\x5b\x83\x5f\x81\x5b\x83\x6f\x81\x5b\x83\x57\x83\x87\x83\x93\x83\x52\x81\x5b\x83\x68\x81\x46%x"/*@ "ハードウェア頂点シェーダーバージョンコード：%x" @*/, GD3D9.Device.Caps.DeviceCaps.VertexShaderVersion & 0xffff )) ;
		...
		{
			DXST_ERRORLOGFMT_ADDA(( "\x83\x6e\x81\x5b\x83\x68\x83\x45\x83\x46\x83\x41\x92\xb8\x93\x5f\x83\x56\x83\x46\x81\x5b\x83\x5f\x81\x5b\x83\x6f\x81\x5b\x83\x57\x83\x87\x83\x93\x83\x52\x81\x5b\x83\x68\x81\x46%x"/*@ "ハードウェア頂点シェーダーバージョンコード：%x" @*/, GD3D9.Device.Shader.NativeVertexShaderVersion & 0xffff )) ;
			DXST_ERRORLOGFMT_ADDA(( "\x83\x47\x83\x7e\x83\x85\x83\x8c\x81\x5b\x83\x56\x83\x87\x83\x93\x92\xb8\x93\x5f\x83\x56\x83\x46\x81\x5b\x83\x5f\x81\x5b\x83\x6f\x81\x5b\x83\x57\x83\x87\x83\x93\x83\x52\x81\x5b\x83\x68\x81\x46%x"/*@ "エミュレーション頂点シェーダーバージョンコード：%x" @*/, GD3D9.Device.Caps.DeviceCaps.VertexShaderVersion & 0xffff )) ;
		}
		DXST_ERRORLOGFMT_ADDA(( "\x83\x73\x83\x4e\x83\x5a\x83\x8b\x83\x56\x83\x46\x81\x5b\x83\x5f\x81\x5b\x83\x6f\x81\x5b\x83\x57\x83\x87\x83\x93\x83\x52\x81\x5b\x83\x68\x81\x46%x"/*@ "ピクセルシェーダーバージョンコード：%x" @*/, GD3D9.Device.Caps.DeviceCaps.PixelShaderVersion & 0xffff )) ;
	}
	...
	// 描画先の情報を描画先をロックして読み取る場合と一時的な描画サーフェスを介して読み取る場合のどちらが高速かを調べる
	{
		...
		if( ( D_D3DMULTISAMPLE_TYPE )GRAWIN.Setting.FSAAMultiSampleCount != D_D3DMULTISAMPLE_NONE )
		{
			DXST_ERRORLOG_ADDUTF16LE( ""/*@ L"フルシーンアンチエイリアスを使用する場合はバックバッファはロックできません\n" @*/ ) ;
		}
		else
		{
			...
			if( Time2 == -1 || Time1 == -1 )
			{
				DXST_ERRORLOG_ADDUTF16LE( "\xd0\x30\xc3\x30\xaf\x30\xd0\x30\xc3\x30\xd5\x30\xa1\x30\x6f\x30\xed\x30\xc3\x30\xaf\x30\x67\x30\x4d\x30\x7e\x30\x5b\x30\x93\x30\x0a\x00\x00"/*@ L"バックバッファはロックできません\n" @*/ ) ;
			}
			else
			{
				// 高速な方を使用する
				...
				if( Time1 < Time2 )
				{
					...
					DXST_ERRORLOG_ADDUTF16LE( "\xd0\x30\xc3\x30\xaf\x30\xd0\x30\xc3\x30\xd5\x30\xa1\x30\xed\x30\xc3\x30\xaf\x30\x92\x30\x7f\x4f\x28\x75\x57\x30\x7e\x30\x59\x30\x0a\x00\x00"/*@ L"バックバッファロックを使用します\n" @*/ ) ;
				}
				else
				{
					DXST_ERRORLOG_ADDUTF16LE( "\xd0\x30\xc3\x30\xaf\x30\xd0\x30\xc3\x30\xd5\x30\xa1\x30\xed\x30\xc3\x30\xaf\x30\x92\x30\x7f\x4f\x28\x75\x57\x30\x7e\x30\x5b\x30\x93\x30\x0a\x00\x00"/*@ L"バックバッファロックを使用しません\n" @*/ ) ;
				}
			}
		}
	}
...
{
	if( Graphics_Hardware_CheckValid_PF() == 0 ) return 0 ;
...
extern	int		Graphics_D3D9_VertexDeclaration_Create( void )
{
	...
	{
		D_D3DVERTEXELEMENT9 VertElem_[ 10 ] ;
	...
	else
	{
		...
		if( Result != D_D3D_OK )
		{
			...
			if( OutPutLog == false )
			{
				...
				DXST_ERRORLOGFMT_ADDUTF16LE(( ""/*@ L"メモリテクスチャの作成に失敗しました  エラーコード:0x%08x SizeX:%d SizeY:%d Format:%d 総メモリ確保数:%d 総確保メモリサイズ:%dByte(%dKByte)" @*/,
					Result, MemTex->Width, MemTex->Height, GD3D9.Device.Caps.TextureFormat[ MemTex->Format ], NS_DxGetAllocNum(), NS_DxGetAllocSize(), NS_DxGetAllocSize() )) ;
				DXST_ERRORLOGFMT_ADDUTF16LE(( ""/*@ L"非管理テクスチャへのデータ転送用システムメモリ配置テクスチャの数：%d" @*/, GD3D9.SysMemTexSurf.TextureInitNum )) ;
	...
	if( Result != D_D3D_OK )
	{
		...
		if( OutPutLog == false )
		{
			...
			DXST_ERRORLOGFMT_ADDUTF16LE(( ""/*@ L"メモリサーフェスの作成に失敗しました PosNo.2  エラーコード:0x%08x SizeX:%d SizeY:%d Format:%d 総メモリ確保数:%d 総確保メモリサイズ:%dByte(%dKByte)" @*/,
				Result, MemSurface->Width, MemSurface->Height, MemSurface->Format, NS_DxGetAllocNum(), NS_DxGetAllocSize(), NS_DxGetAllocSize() / 1024 )) ;
			DXST_ERRORLOGFMT_ADDUTF16LE(( ""/*@ L"非管理テクスチャへのデータ転送用システムメモリ配置サーフェスの数：%d" @*/, GD3D9.SysMemTexSurf.TextureInitNum )) ;
...
//					if( VectorIntFlag == FALSE && GSYS.DrawSetting.DrawMode == DX_DRAWMODE_BILINEAR )
		...
		UseDiffuseRGB   = ( ParamFlag & DX_D3D9_DRAWPREP_DIFFUSERGB )   || ( GSYS.DrawSetting.bDrawBright & 0xffffff ) != 0xffffff ;
...
extern	int		Graphics_D3D9_DrawBillboard3D( VECTOR Pos, float cx, float cy, float Size, float Angle, IMAGEDATA *Image, IMAGEDATA *BlendImage, int TransFlag, int TurnFlag, int DrawFlag, RECT *DrawArea )
{
	...
	IMAGEDATA_HARD_VERT *BlendTexVect = NULL ;
	DWORD DiffuseColor ;
	int DrawTexNum ;
	int i ;
	int Flag ;
	...
	// サイズと座標関係の事前計算
	SizeX = Size ;
	SizeY = Size * ( float )Image->Height / ( float )Image->Width ;
	...
	for( i = 0 ; i < DrawTexNum ; i ++, DrawTex ++ )
	{
		...
		if( GD3D9.Device.DrawInfo.VertexType != VERTEXTYPE_BLENDTEX && DrawFlag == TRUE )
		{
			...
			if( Angle != 0.0 )
			{
				...
				SrcVec[1].y = SrcVec[0].y = ( -cy + Image->Height - TexVect[0].y ) * ScaleY ;
				SrcVec[3].y = SrcVec[2].y = ( -cy + Image->Height - TexVect[2].y ) * ScaleY ;
			...
			else
			{
				...
				SrcVec[0].y = ( -cy + Image->Height - TexVect[0].y ) * ScaleY ;
				SrcVec[2].y = ( -cy + Image->Height - TexVect[2].y ) * ScaleY ;
		...
		else
		{
			...
			case VERTEXTYPE_BLENDTEX :
				...
				// スクリーン座標に変換
				DrawVectB[0].pos.x = SrcVec2[0].x * GSYS.DrawSetting.BlendMatrixF.m[0][0] + SrcVec2[0].y * GSYS.DrawSetting.BlendMatrixF.m[1][0] + SrcVec2[0].z * GSYS.DrawSetting.BlendMatrixF.m[2][0] + GSYS.DrawSetting.BlendMatrixF.m[3][0]  ;
				DrawVectB[0].pos.y = SrcVec2[0].x * GSYS.DrawSetting.BlendMatrixF.m[0][1] + SrcVec2[0].y * GSYS.DrawSetting.BlendMatrixF.m[1][1] + SrcVec2[0].z * GSYS.DrawSetting.BlendMatrixF.m[2][1] + GSYS.DrawSetting.BlendMatrixF.m[3][1]  ;
				DrawVectB[0].pos.z = SrcVec2[0].x * GSYS.DrawSetting.BlendMatrixF.m[0][2] + SrcVec2[0].y * GSYS.DrawSetting.BlendMatrixF.m[1][2] + SrcVec2[0].z * GSYS.DrawSetting.BlendMatrixF.m[2][2] + GSYS.DrawSetting.BlendMatrixF.m[3][2]  ;
				DrawVectB[0].rhw   = SrcVec2[0].x * GSYS.DrawSetting.BlendMatrixF.m[0][3] + SrcVec2[0].y * GSYS.DrawSetting.BlendMatrixF.m[1][3] + SrcVec2[0].z * GSYS.DrawSetting.BlendMatrixF.m[2][3] + GSYS.DrawSetting.BlendMatrixF.m[3][3]  ;
				...
				DrawVectB[1].pos.x = ( SrcVec2[1].x * GSYS.DrawSetting.BlendMatrixF.m[0][0] + SrcVec2[1].y * GSYS.DrawSetting.BlendMatrixF.m[1][0] + SrcVec2[1].z * GSYS.DrawSetting.BlendMatrixF.m[2][0] + GSYS.DrawSetting.BlendMatrixF.m[3][0] ) * DrawVectB[0].rhw ;
				DrawVectB[1].pos.y = ( SrcVec2[1].x * GSYS.DrawSetting.BlendMatrixF.m[0][1] + SrcVec2[1].y * GSYS.DrawSetting.BlendMatrixF.m[1][1] + SrcVec2[1].z * GSYS.DrawSetting.BlendMatrixF.m[2][1] + GSYS.DrawSetting.BlendMatrixF.m[3][1] ) * DrawVectB[0].rhw ;
				DrawVectB[2].pos.x = ( SrcVec2[2].x * GSYS.DrawSetting.BlendMatrixF.m[0][0] + SrcVec2[2].y * GSYS.DrawSetting.BlendMatrixF.m[1][0] + SrcVec2[2].z * GSYS.DrawSetting.BlendMatrixF.m[2][0] + GSYS.DrawSetting.BlendMatrixF.m[3][0] ) * DrawVectB[0].rhw ;
				DrawVectB[2].pos.y = ( SrcVec2[2].x * GSYS.DrawSetting.BlendMatrixF.m[0][1] + SrcVec2[2].y * GSYS.DrawSetting.BlendMatrixF.m[1][1] + SrcVec2[2].z * GSYS.DrawSetting.BlendMatrixF.m[2][1] + GSYS.DrawSetting.BlendMatrixF.m[3][1] ) * DrawVectB[0].rhw ;
				DrawVectB[3].pos.x = ( SrcVec2[3].x * GSYS.DrawSetting.BlendMatrixF.m[0][0] + SrcVec2[3].y * GSYS.DrawSetting.BlendMatrixF.m[1][0] + SrcVec2[3].z * GSYS.DrawSetting.BlendMatrixF.m[2][0] + GSYS.DrawSetting.BlendMatrixF.m[3][0] ) * DrawVectB[0].rhw ;
				DrawVectB[3].pos.y = ( SrcVec2[3].x * GSYS.DrawSetting.BlendMatrixF.m[0][1] + SrcVec2[3].y * GSYS.DrawSetting.BlendMatrixF.m[1][1] + SrcVec2[3].z * GSYS.DrawSetting.BlendMatrixF.m[2][1] + GSYS.DrawSetting.BlendMatrixF.m[3][1] ) * DrawVectB[0].rhw ;
				...
				if( TurnFlag )
				{
					DrawVectB[5].u1 = DrawVectB[3].u1 = DrawVectB[1].u1 = ( TexVect[0].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D9.Device.DrawSetting.InvBlendTextureWidth ;
					DrawVectB[4].u1 = DrawVectB[2].u1 = DrawVectB[0].u1 = ( TexVect[1].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D9.Device.DrawSetting.InvBlendTextureWidth ;
				}
				else
				{
					DrawVectB[4].u1 = DrawVectB[2].u1 = DrawVectB[0].u1 = ( TexVect[0].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D9.Device.DrawSetting.InvBlendTextureWidth ;
					DrawVectB[5].u1 = DrawVectB[3].u1 = DrawVectB[1].u1 = ( TexVect[1].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D9.Device.DrawSetting.InvBlendTextureWidth ;
				}
				DrawVectB[4].u2 = DrawVectB[2].u2 = DrawVectB[0].u2 = TexVect[0].u ;
				DrawVectB[5].u2 = DrawVectB[3].u2 = DrawVectB[1].u2 = TexVect[1].u ;

				DrawVectB[5].v1 = DrawVectB[1].v1 = DrawVectB[0].v1 = ( TexVect[0].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D9.Device.DrawSetting.InvBlendTextureHeight ;
				DrawVectB[4].v1 = DrawVectB[3].v1 = DrawVectB[2].v1 = ( TexVect[2].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D9.Device.DrawSetting.InvBlendTextureHeight ;
			...
			case VERTEXTYPE_TEX :
				...
				// スクリーン座標に変換
				DrawVect[0].pos.x = SrcVec2[0].x * GSYS.DrawSetting.BlendMatrixF.m[0][0] + SrcVec2[0].y * GSYS.DrawSetting.BlendMatrixF.m[1][0] + SrcVec2[0].z * GSYS.DrawSetting.BlendMatrixF.m[2][0] + GSYS.DrawSetting.BlendMatrixF.m[3][0]  ;
				DrawVect[0].pos.y = SrcVec2[0].x * GSYS.DrawSetting.BlendMatrixF.m[0][1] + SrcVec2[0].y * GSYS.DrawSetting.BlendMatrixF.m[1][1] + SrcVec2[0].z * GSYS.DrawSetting.BlendMatrixF.m[2][1] + GSYS.DrawSetting.BlendMatrixF.m[3][1]  ;
				DrawVect[0].pos.z = SrcVec2[0].x * GSYS.DrawSetting.BlendMatrixF.m[0][2] + SrcVec2[0].y * GSYS.DrawSetting.BlendMatrixF.m[1][2] + SrcVec2[0].z * GSYS.DrawSetting.BlendMatrixF.m[2][2] + GSYS.DrawSetting.BlendMatrixF.m[3][2]  ;
				DrawVect[0].rhw   = SrcVec2[0].x * GSYS.DrawSetting.BlendMatrixF.m[0][3] + SrcVec2[0].y * GSYS.DrawSetting.BlendMatrixF.m[1][3] + SrcVec2[0].z * GSYS.DrawSetting.BlendMatrixF.m[2][3] + GSYS.DrawSetting.BlendMatrixF.m[3][3]  ;
				...
				DrawVect[1].pos.x = ( SrcVec2[1].x * GSYS.DrawSetting.BlendMatrixF.m[0][0] + SrcVec2[1].y * GSYS.DrawSetting.BlendMatrixF.m[1][0] + SrcVec2[1].z * GSYS.DrawSetting.BlendMatrixF.m[2][0] + GSYS.DrawSetting.BlendMatrixF.m[3][0] ) * DrawVect[0].rhw ;
				DrawVect[1].pos.y = ( SrcVec2[1].x * GSYS.DrawSetting.BlendMatrixF.m[0][1] + SrcVec2[1].y * GSYS.DrawSetting.BlendMatrixF.m[1][1] + SrcVec2[1].z * GSYS.DrawSetting.BlendMatrixF.m[2][1] + GSYS.DrawSetting.BlendMatrixF.m[3][1] ) * DrawVect[0].rhw ;
				DrawVect[2].pos.x = ( SrcVec2[2].x * GSYS.DrawSetting.BlendMatrixF.m[0][0] + SrcVec2[2].y * GSYS.DrawSetting.BlendMatrixF.m[1][0] + SrcVec2[2].z * GSYS.DrawSetting.BlendMatrixF.m[2][0] + GSYS.DrawSetting.BlendMatrixF.m[3][0] ) * DrawVect[0].rhw ;
				DrawVect[2].pos.y = ( SrcVec2[2].x * GSYS.DrawSetting.BlendMatrixF.m[0][1] + SrcVec2[2].y * GSYS.DrawSetting.BlendMatrixF.m[1][1] + SrcVec2[2].z * GSYS.DrawSetting.BlendMatrixF.m[2][1] + GSYS.DrawSetting.BlendMatrixF.m[3][1] ) * DrawVect[0].rhw ;
				DrawVect[3].pos.x = ( SrcVec2[3].x * GSYS.DrawSetting.BlendMatrixF.m[0][0] + SrcVec2[3].y * GSYS.DrawSetting.BlendMatrixF.m[1][0] + SrcVec2[3].z * GSYS.DrawSetting.BlendMatrixF.m[2][0] + GSYS.DrawSetting.BlendMatrixF.m[3][0] ) * DrawVect[0].rhw ;
				DrawVect[3].pos.y = ( SrcVec2[3].x * GSYS.DrawSetting.BlendMatrixF.m[0][1] + SrcVec2[3].y * GSYS.DrawSetting.BlendMatrixF.m[1][1] + SrcVec2[3].z * GSYS.DrawSetting.BlendMatrixF.m[2][1] + GSYS.DrawSetting.BlendMatrixF.m[3][1] ) * DrawVect[0].rhw ;
...
extern	int		Graphics_D3D9_DrawModiBillboard3D( VECTOR Pos, float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, IMAGEDATA *Image, IMAGEDATA *BlendImage, int TransFlag, int DrawFlag, RECT *DrawArea )
{
	...
	for( i = 0 ; i < DrawTexNum ; i ++, DrawTex ++ )
	{
		...
		if( GD3D9.Device.DrawInfo.VertexType != VERTEXTYPE_BLENDTEX && DrawFlag == TRUE )
		{
			...
			// ビルボード座標をワールド座標へ変換
			DrawVect3D[ 0 ].pos.x = x1 * GSYS.DrawSetting.BillboardMatrixF.m[0][0] + y1 * GSYS.DrawSetting.BillboardMatrixF.m[1][0] + Pos.x ;
			DrawVect3D[ 0 ].pos.y = x1 * GSYS.DrawSetting.BillboardMatrixF.m[0][1] + y1 * GSYS.DrawSetting.BillboardMatrixF.m[1][1] + Pos.y ;
			DrawVect3D[ 0 ].pos.z = x1 * GSYS.DrawSetting.BillboardMatrixF.m[0][2] + y1 * GSYS.DrawSetting.BillboardMatrixF.m[1][2] + Pos.z ;

			DrawVect3D[ 1 ].pos.x = x2 * GSYS.DrawSetting.BillboardMatrixF.m[0][0] + y2 * GSYS.DrawSetting.BillboardMatrixF.m[1][0] + Pos.x ;
			DrawVect3D[ 1 ].pos.y = x2 * GSYS.DrawSetting.BillboardMatrixF.m[0][1] + y2 * GSYS.DrawSetting.BillboardMatrixF.m[1][1] + Pos.y ;
			DrawVect3D[ 1 ].pos.z = x2 * GSYS.DrawSetting.BillboardMatrixF.m[0][2] + y2 * GSYS.DrawSetting.BillboardMatrixF.m[1][2] + Pos.z ;

			DrawVect3D[ 2 ].pos.x = x4 * GSYS.DrawSetting.BillboardMatrixF.m[0][0] + y4 * GSYS.DrawSetting.BillboardMatrixF.m[1][0] + Pos.x ;
			DrawVect3D[ 2 ].pos.y = x4 * GSYS.DrawSetting.BillboardMatrixF.m[0][1] + y4 * GSYS.DrawSetting.BillboardMatrixF.m[1][1] + Pos.y ;
			DrawVect3D[ 2 ].pos.z = x4 * GSYS.DrawSetting.BillboardMatrixF.m[0][2] + y4 * GSYS.DrawSetting.BillboardMatrixF.m[1][2] + Pos.z ;

			DrawVect3D[ 3 ].pos.x = x3 * GSYS.DrawSetting.BillboardMatrixF.m[0][0] + y3 * GSYS.DrawSetting.BillboardMatrixF.m[1][0] + Pos.x ;
			DrawVect3D[ 3 ].pos.y = x3 * GSYS.DrawSetting.BillboardMatrixF.m[0][1] + y3 * GSYS.DrawSetting.BillboardMatrixF.m[1][1] + Pos.y ;
			DrawVect3D[ 3 ].pos.z = x3 * GSYS.DrawSetting.BillboardMatrixF.m[0][2] + y3 * GSYS.DrawSetting.BillboardMatrixF.m[1][2] + Pos.z ;
...
extern	int		Graphics_D3D9_DrawGraph( int x, int y, float xf, float yf, IMAGEDATA *Image, IMAGEDATA *BlendImage, int TransFlag, int IntFlag )
{
	...
	IMAGEDATA_ORIG			*Orig ;
	IMAGEDATA_HARD_VERT	*TexVect ;
	IMAGEDATA_HARD_VERT	*BlendTexVect ;
	DWORD					DiffuseColor ;
	int						DrawTexNum ;
	int						i ;
	int						j ;
	...
	if( IN_DRAWAREA_CHECK( x, y, x + Image->Width, y + Image->Height ) )
...
//		y + Image->Height < GSYS.DrawSetting.DrawArea.bottom && y > GSYS.DrawSetting.DrawArea.right )
	...
	else
	{
		...
		// 完全にはみ出ていたら何もせず終了
		if( OUT_DRAWAREA_CHECK( x, y, x + Image->Width, y + Image->Height ) ) return 0 ;
...
//			y > GSYS.DrawSetting.DrawArea.bottom || y + Image->Height < GSYS.DrawSetting.DrawArea.top ) return 0 ;
		...
		case VERTEXTYPE_BLENDTEX :
			...
			for( i = 0 ; i < DrawTexNum ; i ++, DrawTex ++ )
			{
				...
				DrawVectB[2].u1 = DrawVectB[0].u1 = ( TexVect[0].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D9.Device.DrawSetting.InvBlendTextureWidth ;
				DrawVectB[3].u1 = DrawVectB[1].u1 = ( TexVect[1].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D9.Device.DrawSetting.InvBlendTextureWidth ;
				DrawVectB[1].v1 = DrawVectB[0].v1 = ( TexVect[0].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D9.Device.DrawSetting.InvBlendTextureHeight ;
				DrawVectB[3].v1 = DrawVectB[2].v1 = ( TexVect[2].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D9.Device.DrawSetting.InvBlendTextureHeight ;
...
//	if( x2 - x1 == Image->Width && y2 - y1 == Image->Height ) return G_D3D_DrawGraph( x1, y1, GraphData, TransFlag ) ; 	
	...
	// 拡大率を取得しておく
	GraphExRateX = ( x2f - x1f ) / Image->Width ;
	GraphExRateY = ( y2f - y1f ) / Image->Height ;
	...
	if( IN_DRAWAREA_CHECK( x1, y1, x2, y2 ) )
	{
		...
		case VERTEXTYPE_BLENDTEX :	// ブレンドテクスチャを使用する
			...
			for( i = 0 ; i < DrawTexNum ; i ++, DrawTex ++ )
			{
				...
				VectDataB[4].u1 = VectDataB[2].u1 = VectDataB[0].u1 = ( TexVect[0].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D9.Device.DrawSetting.InvBlendTextureWidth ;
				VectDataB[5].u1 = VectDataB[3].u1 = VectDataB[1].u1 = ( TexVect[1].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D9.Device.DrawSetting.InvBlendTextureWidth ;
				VectDataB[5].v1 = VectDataB[1].v1 = VectDataB[0].v1 = ( TexVect[0].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D9.Device.DrawSetting.InvBlendTextureHeight ;
				VectDataB[4].v1 = VectDataB[3].v1 = VectDataB[2].v1 = ( TexVect[2].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D9.Device.DrawSetting.InvBlendTextureHeight ;
	...
	else
	{
		...
		case VERTEXTYPE_BLENDTEX :	// ブレンドテクスチャを使用する
			...
			for( i = 0 ; i < DrawTexNum ; i ++, DrawTex ++ )
			{
				...
				VectDataB[2].u1 = VectDataB[0].u1 = ( TexVect[0].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D9.Device.DrawSetting.InvBlendTextureWidth ;
				VectDataB[3].u1 = VectDataB[1].u1 = ( TexVect[1].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D9.Device.DrawSetting.InvBlendTextureWidth ;
				VectDataB[1].v1 = VectDataB[0].v1 = ( TexVect[0].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D9.Device.DrawSetting.InvBlendTextureHeight ;
				VectDataB[3].v1 = VectDataB[2].v1 = ( TexVect[2].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D9.Device.DrawSetting.InvBlendTextureHeight ;
...
//	if( _FABS( (float)Angle ) < 0.0001 && ExRate < 1.001 && ExRate > 0.999 ) Adj = TRUE ;
		...
		if( fx < 0.00001f && fy < 0.00001f )
		{
			if( ExRate == 1.0 )
			{
				if( hx == hy )
				{
					if( hx == 0 )
					{
						CenX = ( float )(Image->Width >> 1) ;
						CenY = ( float )(Image->Height >> 1) ;
						xf   = ( float )x - 0.5F ;
						yf   = ( float )y - 0.5F ;
					}
					else
					{
						CenX = ( float )Image->Width * 0.5F ;
						CenY = ( float )Image->Height * 0.5F ;
						xf   = ( float )x ;
						yf   = ( float )y ;
						FastCode = 1;
					}
	...
	case VERTEXTYPE_BLENDTEX :	// ブレンドテクスチャを使用する
		...
		if( DrawTexNum == 1 && FastCode == 1 )
		{
			...
			VectB[4].u1 = VectB[2].u1 = VectB[0].u1 = ( TexVect[0].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D9.Device.DrawSetting.InvBlendTextureWidth ;
			VectB[5].u1 = VectB[3].u1 = VectB[1].u1 = ( TexVect[1].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D9.Device.DrawSetting.InvBlendTextureWidth ;
			VectB[5].v1 = VectB[1].v1 = VectB[0].v1 = ( TexVect[0].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D9.Device.DrawSetting.InvBlendTextureHeight ;
			VectB[4].v1 = VectB[3].v1 = VectB[2].v1 = ( TexVect[2].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D9.Device.DrawSetting.InvBlendTextureHeight ;
		...
		else
		{
			...
			for( i = 0 ; i < DrawTexNum ; i ++, DrawTex ++ )
			{
				...
				VectB[0].u1 = ( TexVect[0].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D9.Device.DrawSetting.InvBlendTextureWidth ;
				VectB[1].u1 = ( TexVect[1].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D9.Device.DrawSetting.InvBlendTextureWidth ;
				VectB[2].u1 = ( TexVect[2].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D9.Device.DrawSetting.InvBlendTextureWidth ;
				VectB[3].u1 = ( TexVect[3].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D9.Device.DrawSetting.InvBlendTextureWidth ;
				VectB[4].u1 = VectB[2].u1 ;
				VectB[5].u1 = VectB[1].u1 ;
				
				VectB[0].v1 = ( TexVect[0].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D9.Device.DrawSetting.InvBlendTextureHeight ;
				VectB[1].v1 = ( TexVect[1].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D9.Device.DrawSetting.InvBlendTextureHeight ;
				VectB[2].v1 = ( TexVect[2].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D9.Device.DrawSetting.InvBlendTextureHeight ;
				VectB[3].v1 = ( TexVect[3].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D9.Device.DrawSetting.InvBlendTextureHeight ;
...
extern	int		Graphics_D3D9_DrawModiGraph( int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, IMAGEDATA *Image, IMAGEDATA *BlendImage, int TransFlag, bool SimpleDrawFlag )
{
	...
	GraphSizeX = ( float )Image->Width ;  GraphSizeY = ( float )Image->Height ;
		...
		for( i = 0 ; i < DrawTexNum ; i ++, DrawTex ++ )
		{
			...
			*((DWORD *)&VectDataB[3].pos.z) = drawz ;
			...
			for( j = 0 ; j < 4 ; j ++, VectB ++, TexVect ++ )
			{
				...
				VectB->u1 = ( TexVect->x - BlendTexVect->x + GSYS.DrawSetting.BlendGraphX ) * GD3D9.Device.DrawSetting.InvBlendTextureWidth ;
				VectB->v1 = ( TexVect->y - BlendTexVect->y + GSYS.DrawSetting.BlendGraphY ) * GD3D9.Device.DrawSetting.InvBlendTextureHeight ;
			...
			{
				// プリミティブの描画
				Direct3DDevice9_DrawPrimitiveUP( D_D3DPT_TRIANGLESTRIP, 2, VectDataB, sizeof( VERTEX_BLENDTEX_2D ) ) ;
			}
			else
			{
				// Ｘ方向反転時の補正
...
//		Graphics_D3D9_DeviceState_SetFVF( VERTEXFVF_2D ) ;
		...
		for( i = 0 ; i < DrawTexNum ; i ++, DrawTex ++ )
		{
			...
			*((DWORD *)&VectData[3].pos.z) = drawz;
			...
			{
				// プリミティブの描画
				Direct3DDevice9_DrawPrimitiveUP( D_D3DPT_TRIANGLESTRIP, 2, VectData, sizeof( VERTEX_2D ) ) ;
			}
			else
			{
				// Ｘ方向反転時の補正
...
//		h = 1.0F / (float)Image->Height ;
		w = 1.0F / (double)Image->Width ;
		h = 1.0F / (double)Image->Height ;
...
extern	int		Graphics_D3D9_DrawFillBox( int x1, int y1, int x2, int y2, unsigned int Color )
{
	...
	// クリッピング処理
	{
		...
		drect.top    = y1 ;
		drect.bottom = y2 ;
		RectClipping( &drect, &GSYS.DrawSetting.DrawArea ) ;
	...
	Color = ( ( ( ( ( 	GD3D9.Device.DrawInfo.DiffuseColor & 0x00ff0000 ) >> 16 ) * Red 	) / 255 ) << 16 ) |
...
//	Graphics_D3D9_RenderVertex( D3D.Direct3DDeviceHandle ) ;
	...
	// 色その他ステータスのセット
	NS_GetColor2( Color, &Red, &Green, &Blue ) ;
	Color = ( ( ( ( (	GD3D9.Device.DrawInfo.DiffuseColor & 0xff0000 ) >> 16 ) * Red )	/ 255 ) << 16 ) |
			( ( ( ( (	GD3D9.Device.DrawInfo.DiffuseColor & 0x00ff00 ) >> 8  ) * Green )	/ 255 ) << 8 ) |
			( ( ( (		GD3D9.Device.DrawInfo.DiffuseColor & 0x0000ff )       ) * Blue )	/ 255 ) | 
			(			GD3D9.Device.DrawInfo.DiffuseColor & 0xff000000 ) ;
...
extern	int		Graphics_D3D9_DrawLine3D( VECTOR Pos1, VECTOR Pos2, unsigned int Color, int DrawFlag, RECT *DrawArea )
{
	...
	else
	{
		...
		// スクリーン座標に変換
		TempVect[0].pos.x = Pos1.x * GSYS.DrawSetting.BlendMatrixF.m[0][0] + Pos1.y * GSYS.DrawSetting.BlendMatrixF.m[1][0] + Pos1.z * GSYS.DrawSetting.BlendMatrixF.m[2][0] + GSYS.DrawSetting.BlendMatrixF.m[3][0]  ;
		TempVect[0].pos.y = Pos1.x * GSYS.DrawSetting.BlendMatrixF.m[0][1] + Pos1.y * GSYS.DrawSetting.BlendMatrixF.m[1][1] + Pos1.z * GSYS.DrawSetting.BlendMatrixF.m[2][1] + GSYS.DrawSetting.BlendMatrixF.m[3][1]  ;
		TempVect[0].pos.z = Pos1.x * GSYS.DrawSetting.BlendMatrixF.m[0][2] + Pos1.y * GSYS.DrawSetting.BlendMatrixF.m[1][2] + Pos1.z * GSYS.DrawSetting.BlendMatrixF.m[2][2] + GSYS.DrawSetting.BlendMatrixF.m[3][2]  ;
		TempVect[0].rhw   = Pos1.x * GSYS.DrawSetting.BlendMatrixF.m[0][3] + Pos1.y * GSYS.DrawSetting.BlendMatrixF.m[1][3] + Pos1.z * GSYS.DrawSetting.BlendMatrixF.m[2][3] + GSYS.DrawSetting.BlendMatrixF.m[3][3]  ;

		TempVect[1].pos.x = Pos2.x * GSYS.DrawSetting.BlendMatrixF.m[0][0] + Pos2.y * GSYS.DrawSetting.BlendMatrixF.m[1][0] + Pos2.z * GSYS.DrawSetting.BlendMatrixF.m[2][0] + GSYS.DrawSetting.BlendMatrixF.m[3][0]  ;
		TempVect[1].pos.y = Pos2.x * GSYS.DrawSetting.BlendMatrixF.m[0][1] + Pos2.y * GSYS.DrawSetting.BlendMatrixF.m[1][1] + Pos2.z * GSYS.DrawSetting.BlendMatrixF.m[2][1] + GSYS.DrawSetting.BlendMatrixF.m[3][1]  ;
		TempVect[1].pos.z = Pos2.x * GSYS.DrawSetting.BlendMatrixF.m[0][2] + Pos2.y * GSYS.DrawSetting.BlendMatrixF.m[1][2] + Pos2.z * GSYS.DrawSetting.BlendMatrixF.m[2][2] + GSYS.DrawSetting.BlendMatrixF.m[3][2]  ;
		TempVect[1].rhw   = Pos2.x * GSYS.DrawSetting.BlendMatrixF.m[0][3] + Pos2.y * GSYS.DrawSetting.BlendMatrixF.m[1][3] + Pos2.z * GSYS.DrawSetting.BlendMatrixF.m[2][3] + GSYS.DrawSetting.BlendMatrixF.m[3][3]  ;
...
//	int j ;
	...
	VectBuf = ( VERTEX_NOTEX_2D * )GSYS.Resource.CommonBuffer[ 0 ] ;
...
//	Graphics_D3D9_DeviceState_SetFVF( VERTEXFVF_NOTEX_2D ) ;
	...
	if( VectNum )
	{
		Direct3DDevice9_DrawPrimitiveUP( D_D3DPT_LINELIST, VectNum / 2, VectBuf, sizeof( VERTEX_NOTEX_2D ) ) ;
	}
...
#define DX_3D_CIRCLE_PSET																\
			...
			{																			\
				...
				*((DWORD *)&VectBuf[ VectNum ].pos.z) = drawz ;							\
				...
				if( VectNum >= CIRCLE_VERTEX_NUM )										\
				{																		\
					Direct3DDevice9_DrawPrimitiveUP( D_D3DPT_POINTLIST, VectNum, VectBuf, sizeof( VERTEX_NOTEX_2D ) ) ;\
					VectNum = 0 ;														\
				}																		\
			}
...
extern	int		Graphics_D3D9_DrawTriangle3D( VECTOR Pos1, VECTOR Pos2, VECTOR Pos3, unsigned int Color, int FillFlag, int DrawFlag, RECT *DrawArea )
{
	...
	else
	{
		...
		TempVect[2].pos.x = Pos3.x * GSYS.DrawSetting.BlendMatrixF.m[0][0] + Pos3.y * GSYS.DrawSetting.BlendMatrixF.m[1][0] + Pos3.z * GSYS.DrawSetting.BlendMatrixF.m[2][0] + GSYS.DrawSetting.BlendMatrixF.m[3][0]  ;
		TempVect[2].pos.y = Pos3.x * GSYS.DrawSetting.BlendMatrixF.m[0][1] + Pos3.y * GSYS.DrawSetting.BlendMatrixF.m[1][1] + Pos3.z * GSYS.DrawSetting.BlendMatrixF.m[2][1] + GSYS.DrawSetting.BlendMatrixF.m[3][1]  ;
		TempVect[2].pos.z = Pos3.x * GSYS.DrawSetting.BlendMatrixF.m[0][2] + Pos3.y * GSYS.DrawSetting.BlendMatrixF.m[1][2] + Pos3.z * GSYS.DrawSetting.BlendMatrixF.m[2][2] + GSYS.DrawSetting.BlendMatrixF.m[3][2]  ;
		TempVect[2].rhw   = Pos3.x * GSYS.DrawSetting.BlendMatrixF.m[0][3] + Pos3.y * GSYS.DrawSetting.BlendMatrixF.m[1][3] + Pos3.z * GSYS.DrawSetting.BlendMatrixF.m[2][3] + GSYS.DrawSetting.BlendMatrixF.m[3][3]  ;
...
//	Graphics_D3D9_DeviceState_SetFVF( VERTEXFVF_NOTEX_2D ) ;
	...
	if( FillFlag )
	{
		...
		// 反転処理判定
		{
			...
			{
				// プリミティブの描画
				Direct3DDevice9_DrawPrimitiveUP( D_D3DPT_TRIANGLESTRIP, 2, VectData, sizeof( VERTEX_NOTEX_2D ) ) ;
			}
			else
			{
				// Ｘ方向反転時の補正
	...
	else
	{
		...
		// プリミティブの描画
		Direct3DDevice9_DrawPrimitiveUP( D_D3DPT_LINESTRIP, 4, VectData, sizeof( VERTEX_NOTEX_2D ) ) ;
	}
...
extern	int		Graphics_D3D9_DrawPixel( int x, int y, unsigned int Color )
{
	...
	{
		NS_GetColor2( Color, &Red, &Green, &Blue ) ;
		VectData->color =	( ( ( ( (	GD3D9.Device.DrawInfo.DiffuseColor & 0x00ff0000 ) >> 16 ) * Red   )	/ 255 ) << 16 ) |
							( ( ( ( (	GD3D9.Device.DrawInfo.DiffuseColor & 0x0000ff00 ) >> 8  ) * Green )	/ 255 ) << 8  ) |
							( ( ( (		GD3D9.Device.DrawInfo.DiffuseColor & 0x000000ff )       ) * Blue  )	/ 255 ) | 
							(			GD3D9.Device.DrawInfo.DiffuseColor & 0xff000000 ) ;
	}
...
extern	int		Graphics_D3D9_DrawPixel3D( VECTOR Pos, unsigned int Color, int DrawFlag, RECT *DrawArea )
{
	...
	if( DrawFlag )
	{
		...
		{
			*( ( DWORD * )&VectData->b ) =	( ( ( ( (	GD3D9.Device.DrawInfo.DiffuseColor & 0x00ff0000 ) >> 16 ) * Red   )	/ 255 ) << 16 ) |
											( ( ( ( (	GD3D9.Device.DrawInfo.DiffuseColor & 0x0000ff00 ) >> 8  ) * Green )	/ 255 ) << 8  ) |
											( ( ( (		GD3D9.Device.DrawInfo.DiffuseColor & 0x000000ff )       ) * Blue  )	/ 255 ) | 
											(			GD3D9.Device.DrawInfo.DiffuseColor & 0xff000000 ) ;
		}
	...
	else
	{
		...
		// スクリーン座標に変換
		TempVect.pos.x = Pos.x * GSYS.DrawSetting.BlendMatrixF.m[0][0] + Pos.y * GSYS.DrawSetting.BlendMatrixF.m[1][0] + Pos.z * GSYS.DrawSetting.BlendMatrixF.m[2][0] + GSYS.DrawSetting.BlendMatrixF.m[3][0]  ;
		TempVect.pos.y = Pos.x * GSYS.DrawSetting.BlendMatrixF.m[0][1] + Pos.y * GSYS.DrawSetting.BlendMatrixF.m[1][1] + Pos.z * GSYS.DrawSetting.BlendMatrixF.m[2][1] + GSYS.DrawSetting.BlendMatrixF.m[3][1]  ;
		TempVect.pos.z = Pos.x * GSYS.DrawSetting.BlendMatrixF.m[0][2] + Pos.y * GSYS.DrawSetting.BlendMatrixF.m[1][2] + Pos.z * GSYS.DrawSetting.BlendMatrixF.m[2][2] + GSYS.DrawSetting.BlendMatrixF.m[3][2]  ;
		TempVect.rhw   = Pos.x * GSYS.DrawSetting.BlendMatrixF.m[0][3] + Pos.y * GSYS.DrawSetting.BlendMatrixF.m[1][3] + Pos.z * GSYS.DrawSetting.BlendMatrixF.m[2][3] + GSYS.DrawSetting.BlendMatrixF.m[3][3]  ;
...
//	Graphics_D3D9_DeviceState_SetFVF( VERTEXFVF_NOTEX_2D ) ;
	...
	for( i = 0 ; i < ( UINT )Num ; i ++, PointData ++ )
	{
		...
		*((DWORD *)&Vect->pos.z) = drawz ;
		...
		{
			// 描画
			Direct3DDevice9_DrawPrimitiveUP( D_D3DPT_POINTLIST, j, VectData, sizeof( VERTEX_NOTEX_2D ) ) ;
			j = 0 ;
			Vect = VectData ;
		}
	}
...
//	Graphics_D3D9_DeviceState_SetFVF( VERTEXFVF_NOTEX_2D ) ;
	...
	for( i = 0 ; i < ( DWORD )Num ; i ++, LineData ++ )
	{
		...
		*((DWORD *)&Vect[1].pos.z) = drawz;
		...
		{
			// 描画
			Direct3DDevice9_DrawPrimitiveUP( D_D3DPT_LINELIST, j, VectData, sizeof( VERTEX_NOTEX_2D ) ) ;
			j = 0 ;
			Vect = VectData ;
		}
	}
...
extern	int		Graphics_D3D9_DrawPrimitive2D( VERTEX_2D *Vertex, int VertexNum, int PrimitiveType, IMAGEDATA *Image, int TransFlag, int BillboardFlag, int Is3D, int TurnFlag, int TextureNo )
{
	int Flag ;
	int i ;
	VERTEX_2D *Vert ;
	...
	if( Image && ( DWORD_PTR )Image <= 0xffffffff )
	{
...
//	DXST_ERRORLOGFMT_ADDUTF16LE(( L"確保メモリ数:%d  確保メモリ総サイズ:%dByte(%dKByte)", NS_DxGetAllocNum(), NS_DxGetAllocSize(), NS_DxGetAllocSize() / 1024 )) ;
...
extern	int		Graphics_Hardware_D3D9_SetMultiThreadFlag_PF( int Flag )
{
	if( GD3D9.Device.DrawSetting.CancelSettingEqualCheck == TRUE )
		return -1 ;

	GD3D9.Setting.UseMultiThread = Flag ;
	...
	else
	{
		...
		if( TargetScreenImage )
		{
			...
			if( TargetZBufferImage != NULL &&
				TargetZBufferImage->Width >= TargetScreenImage->Width &&
				TargetZBufferImage->Height >= TargetScreenImage->Height )
			{
			...
			if( TargetScreenImage->Width  > GD3D9.Device.Screen.ZBufferSizeX || 
				TargetScreenImage->Height > GD3D9.Device.Screen.ZBufferSizeY )
			{
		...
		else
		{
			// 描画先Ｚバッファがあって、且つ描画先以上のサイズを持つ場合は描画先Ｚバッファを使用する
			if( TargetZBufferImage != NULL &&
				TargetZBufferImage->Width  >= GSYS.DrawSetting.DrawSizeX &&
				TargetZBufferImage->Height >= GSYS.DrawSetting.DrawSizeY )
			{
				Direct3DDevice9_SetDepthStencilSurface( TargetZBufferImage->Hard.Draw[ 0 ].Tex->PF->D3D9.ZBuffer ) ;
			}
			else
...
/*
		...
		{
			DXST_ERRORLOG_ADDUTF16LE( L"バックバッファロック用システムメモリサーフェスの作成に失敗しました\n" ) ;
			Direct3D9_ObjectRelease( RTSurface ) ;
			return -1 ;
		}
*/
...
extern	int		Graphics_Hardware_D3D9_GraphLock_PF( IMAGEDATA *Image, COLORDATA **ColorDataP, int WriteOnly )
{
	...
	Image->LockImagePitch = ( DWORD )( Image->Width * ColorData->PixelByte ) ;
	Image->LockImage = ( BYTE * )DXALLOC( Image->Height * Image->LockImagePitch ) ;
	if( Image->LockImage == NULL )
	{
		DXST_ERRORLOG_ADDUTF16LE( "\xed\x30\xc3\x30\xaf\x30\x28\x75\xc6\x30\xf3\x30\xdd\x30\xe9\x30\xea\x30\xa4\x30\xe1\x30\xfc\x30\xb8\x30\x3c\x68\x0d\x7d\x28\x75\xe1\x30\xe2\x30\xea\x30\x6e\x30\xba\x78\xdd\x4f\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"ロック用テンポラリイメージ格納用メモリの確保に失敗しました\n" @*/ ) ;
		return -1 ;
	}
	...
	if( WriteOnly == FALSE )
	{
		...
		for( i = 0 ; i < Image->Hard.DrawNum ; i ++, DrawTex ++ )
		{
			...
			if( Direct3D9_IsExObject() || Image->Orig->FormatDesc.DrawValidFlag )
			{
				...
				{
					DXST_ERRORLOG_ADDUTF16LE( "\xed\x30\xc3\x30\xaf\x30\x28\x75\xcf\x63\x3b\x75\xef\x53\xfd\x80\xb5\x30\xfc\x30\xd5\x30\xa7\x30\xb9\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"ロック用描画可能サーフェスの作成に失敗しました\n" @*/ ) ;
					return -1 ;
				}
				...
				{
					DXST_ERRORLOG_ADDUTF16LE( "\xed\x30\xc3\x30\xaf\x30\x28\x75\xb7\x30\xb9\x30\xc6\x30\xe0\x30\xe1\x30\xe2\x30\xea\x30\xb5\x30\xfc\x30\xd5\x30\xa7\x30\xb9\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"ロック用システムメモリサーフェスの作成に失敗しました\n" @*/ ) ;
					Direct3D9_ObjectRelease( RTSurface ) ;
					return -1 ;
				}
			...
			Dest = Image->LockImage + DrawTex->DrawPosX * ColorData->PixelByte + DrawTex->DrawPosY * Image->LockImagePitch ;
			WidthByte = ( DWORD )( DrawTex->Width * ColorData->PixelByte ) ;
			for( i = 0 ; i < DrawTex->Height ; i ++, Dest += Image->LockImagePitch, Src += LockRect.Pitch )
			{
...
extern	int		Graphics_Hardware_D3D9_GraphUnlock_PF( IMAGEDATA *Image )
{
	...
	for( i = 0 ; i < Image->Hard.DrawNum ; i ++, DrawTex ++ )
	{
		...
		if( Direct3D9_IsExObject() || Image->Orig->FormatDesc.DrawValidFlag )
		{
			...
			if( Direct3DDevice9_CreateOffscreenPlainSurface( ( UINT )DrawTex->Width, ( UINT )DrawTex->Height, GD3D9.Device.Caps.TextureFormat[ Image->Orig->ColorFormat ], D_D3DPOOL_SYSTEMMEM, &SMSurface, NULL ) != D_D3D_OK )
			{
			...
			// データを転送
			Src       = Image->LockImage + ColorData->PixelByte * DrawTex->DrawPosX + Image->LockImagePitch * DrawTex->DrawPosY ;
			Dest      = ( BYTE * )LockRect.pBits ;
			WidthByte = ( DWORD )( Image->Width * ColorData->PixelByte ) ;
			for( j = 0 ; j < DrawTex->Height ; j ++, Src += Image->LockImagePitch, Dest += LockRect.Pitch )
			{
				_MEMCPY( Dest, Src, WidthByte ) ;
			}
		...
		else
		{
			...
			// 転送
			Src       = Image->LockImage + DrawTex->DrawPosX * ColorData->PixelByte + DrawTex->DrawPosY * Image->LockImagePitch ;
			Dest      = ( BYTE * )LockRect.pBits ;
			WidthByte = ( DWORD )( DrawTex->Width * ColorData->PixelByte ) ;
			for( i = 0 ; i < DrawTex->Height ; i ++, Src += Image->LockImagePitch, Dest += LockRect.Pitch )
			{
				_MEMCPY( Dest, Src, WidthByte ) ;
			}
...
extern	int		Graphics_Hardware_D3D9_CopyGraphZBufferImage_PF( IMAGEDATA *DestImage, IMAGEDATA *SrcImage )
{
	...
	{
		DXST_ERRORLOG_ADDUTF16LE( "\x5a\x00\xd0\x30\xc3\x30\xd5\x30\xa1\x30\xfc\x30\xa4\x30\xe1\x30\xfc\x30\xb8\x30\x6e\x30\xe2\x8e\x01\x90\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"Zバッファーイメージの転送に失敗しました\n" @*/ ) ;
		return -1 ;
	}
...
extern	int		Graphics_Hardware_D3D9_FillGraph_PF( IMAGEDATA *Image, int Red, int Green, int Blue, int Alpha, int ASyncThread )
{
	...
	else
	{
		...
		if( NS_CreateARGB8ColorBaseImage( Image->Width, Image->Height, &TempImage ) == 0 )
		{
			NS_FillBaseImage( &TempImage, Red, Green, Blue, Alpha ) ;
			NS_BltBmpOrGraphImageToGraph( NULL, NULL, NULL, FALSE, &TempImage, NULL, 0, 0, Image->HandleInfo.Handle ) ;
...
)
{
	...
	POINT				DestPoint ;
	int					UseSysMemSurfaceIndex = -1 ;
	D_IDirect3DSurface9	*DestSurface ;
		  ...
		  ( RgbBaseImage->ColorData.Format == DX_BASEIMAGE_FORMAT_DXT5 && GD3D9.Device.Caps.TextureFormat[ Orig->ColorFormat ] == D_D3DFMT_DXT5 ) ) )
	{
		if( Orig->FormatDesc.CubeMapTextureFlag )
		{
			...
			for( i = 0 ; i < CUBEMAP_SURFACE_NUM ; i ++ )
			{
				...
				for( j = 0 ; j < Orig->Hard.MipMapCount ; j ++ )
				{
					...
					if( hr != D_D3D_OK )
					{
						DXST_ERRORLOG_ADDUTF16LE( "\xad\x30\xe5\x30\xfc\x30\xd6\x30\xde\x30\xc3\x30\xd7\x30\xdf\x30\xc3\x30\xd7\x30\xde\x30\xc3\x30\xd7\x30\xc6\x30\xaf\x30\xb9\x30\xc1\x30\xe3\x30\x6e\x30\xed\x30\xc3\x30\xaf\x30\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"キューブマップミップマップテクスチャのロックに失敗しました\n" @*/ ) ;
						return -1 ;
					}
		...
		else
		{
			...
			for( j = 0 ; j < Orig->Hard.MipMapCount ; j ++ )
			{
				...
				if( hr != D_D3D_OK )
				{
					DXST_ERRORLOG_ADDUTF16LE( "\xdf\x30\xc3\x30\xd7\x30\xde\x30\xc3\x30\xd7\x30\xc6\x30\xaf\x30\xb9\x30\xc1\x30\xe3\x30\x6e\x30\xed\x30\xc3\x30\xaf\x30\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"ミップマップテクスチャのロックに失敗しました\n" @*/ ) ;
					return -1 ;
				}
	...
	else
	{
		...
		{
			DXST_ERRORLOG_ADDUTF16LE( ""/*@ L"DXTフォーマットのテクスチャに標準形式のイメージを転送することはできません\n" @*/ ) ;
		}
	}
...
//	DestRectT.left, DestRectT.top, DestRectT.right, DestRectT.bottom )) ;
	...
	for( i = 0 ; i < ImageNum ; i ++ )
	{
		...
		if( hr != D_D3D_OK )
		{
			DXST_ERRORLOG_ADDUTF16LE( "\xc6\x30\xaf\x30\xb9\x30\xc1\x30\xe3\x30\x6e\x30\xed\x30\xc3\x30\xaf\x30\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"テクスチャのロックに失敗しました\n" @*/ ) ;
			return -1  ;
		}
...
)
{
	...
	if( Orig->FormatDesc.DrawValidFlag )
	{
		for( i = 0 ; i < ImageNum ; i ++ )
		{
			...
			{
				DXST_ERRORLOG_ADDUTF16LE( "\x3b\x75\xcf\x50\xe2\x8e\x01\x90\x28\x75\x6e\x30\xb7\x30\xb9\x30\xc6\x30\xe0\x30\xe1\x30\xe2\x30\xea\x30\xc6\x30\xaf\x30\xb9\x30\xc1\x30\xe3\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"画像転送用のシステムメモリテクスチャの作成に失敗しました\n" @*/ ) ;
				goto ERR ;
			}
		...
		for( i = 0 ; i < Image->Hard.DrawNum ; i ++, DrawTex ++ )
		{
			...
			if( Orig->FormatDesc.DrawValidFlag == TRUE )
			{
				...
				for( j = 0 ; j < ImageNum ; j ++ )
				{
					...
					{
						DXST_ERRORLOG_ADDUTF16LE( "\xcf\x63\x3b\x75\xef\x53\xfd\x80\xc6\x30\xaf\x30\xb9\x30\xc1\x30\xe3\x30\x78\x30\x6e\x30\x3b\x75\xcf\x50\x6e\x30\xe2\x8e\x01\x90\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"描画可能テクスチャへの画像の転送に失敗しました\n" @*/ ) ;
						goto ERR ;
					}
				}
			}
...
//		if( OrigTex->PF->D3D9.Texture != NULL ) continue ;
		...
		if( Orig->FormatDesc.DrawValidFlag )
		{
			...
			if( Orig->FormatDesc.CubeMapTextureFlag == FALSE )
			{
				...
				if( Orig->FormatDesc.MSSamples != 0 && OrigTex->PF->D3D9.RenderTargetSurface == NULL )
				{
					// レンダリングターゲットサーフェスの作成
					...
					{
						DXST_ERRORLOG_ADDUTF16LE( "\xf8\x66\x4d\x30\xbc\x8f\x7f\x30\x02\x5c\x28\x75\xb5\x30\xfc\x30\xd5\x30\xa7\x30\xb9\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"書き込み専用サーフェスの作成に失敗しました\n" @*/ ) ;
						return -1 ;
					}
				}

				if( ( ( D_D3DMULTISAMPLE_TYPE )GRAWIN.Setting.FSAAMultiSampleCount == D_D3DMULTISAMPLE_NONE && Orig->FormatDesc.MSSamples != 0 ) ||
					( ( D_D3DMULTISAMPLE_TYPE )GRAWIN.Setting.FSAAMultiSampleCount != D_D3DMULTISAMPLE_NONE && Orig->FormatDesc.MSSamples == 0 ) ||
					Orig->ZBufferFlag  )
				{
		...
		if( Orig->FormatDesc.CubeMapTextureFlag )
		{
			if( OrigTex->PF->D3D9.CubeTexture == NULL )
			{
				...
				{
					DXST_ERRORLOG_ADDUTF16LE( "\xad\x30\xe5\x30\xfc\x30\xd6\x30\xde\x30\xc3\x30\xd7\x30\xc6\x30\xaf\x30\xb9\x30\xc1\x30\xe3\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"キューブマップテクスチャの作成に失敗しました\n" @*/ ) ;
					return -1 ;
				}
			}
		}
		else
		{
			if( OrigTex->PF->D3D9.Texture == NULL )
			{
				...
				{
					DXST_ERRORLOG_ADDUTF16LE( "\xc6\x30\xaf\x30\xb9\x30\xc1\x30\xe3\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"テクスチャの作成に失敗しました\n" @*/ ) ;
					return -1 ;
				}
			}
		}
		...
		/*if( ( Orig->FormatDesc.DrawValidFlag == FALSE && Orig->FormatDesc.UseManagedTextureFlag == FALSE ) && OrigTex->MemTexture == NULL )
		{
				...
				) != D_D3D_OK )
			{
				...
				DXST_ERRORLOG_ADDUTF16LE( L"メモリテクスチャの作成に失敗しました\n" ) ;
				return -1 ;
			}
		}*/
		...
		if( Orig->FormatDesc.CubeMapTextureFlag )
		{
			for( j = 0 ; j < CUBEMAP_SURFACE_NUM ; j ++ )
			{
				if( OrigTex->PF->D3D9.Surface[ j ] == NULL )
				{
					...
					{
						DXST_ERRORLOG_ADDUTF16LE( "\xad\x30\xe5\x30\xfc\x30\xd6\x30\xc6\x30\xaf\x30\xb9\x30\xc1\x30\xe3\x30\x6e\x30\xb5\x30\xfc\x30\xd5\x30\xa7\x30\xb9\x30\xa2\x30\xc9\x30\xec\x30\xb9\x30\x6e\x30\xd6\x53\x97\x5f\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"キューブテクスチャのサーフェスアドレスの取得に失敗しました\n" @*/ ) ;
						return -1 ;
					}
				}
			}
		...
		else
		{
			if( OrigTex->PF->D3D9.Surface[ 0 ] == NULL )
			{
				...
				{
					DXST_ERRORLOG_ADDUTF16LE( "\xc6\x30\xaf\x30\xb9\x30\xc1\x30\xe3\x30\x6e\x30\xb5\x30\xfc\x30\xd5\x30\xa7\x30\xb9\x30\xa2\x30\xc9\x30\xec\x30\xb9\x30\x6e\x30\xd6\x53\x97\x5f\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"テクスチャのサーフェスアドレスの取得に失敗しました\n" @*/ ) ;
					return -1 ;
				}
			}
		}
...
extern	int		Graphics_Hardware_D3D9_VertexBuffer_SetData_PF( VERTEXBUFFERHANDLEDATA *VertexBuffer, int SetIndex, const void *VertexData, int VertexNum )
{
	...
	if( Direct3DVertexBuffer9_Lock(
			...
			0 ) != D_D3D_OK )
	{
		DXST_ERRORLOG_ADDUTF16LE( "\x02\x98\xb9\x70\xd0\x30\xc3\x30\xd5\x30\xa1\x30\x6e\x30\xed\x30\xc3\x30\xaf\x30\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"頂点バッファのロックに失敗しました\n" @*/ ) ;
		return -1 ;
	}
...
extern	int		Graphics_Hardware_D3D9_IndexBuffer_SetData_PF( INDEXBUFFERHANDLEDATA *IndexBuffer, int SetIndex, const void *IndexData, int IndexNum )
{
	...
	if( Direct3DIndexBuffer9_Lock(
			...
			0 ) != D_D3D_OK )
	{
		DXST_ERRORLOG_ADDUTF16LE( "\xa4\x30\xf3\x30\xc7\x30\xc3\x30\xaf\x30\xb9\x30\xd0\x30\xc3\x30\xd5\x30\xa1\x30\x6e\x30\xed\x30\xc3\x30\xaf\x30\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"インデックスバッファのロックに失敗しました\n" @*/ ) ;
		return -1 ;
	}
...
extern	int		Graphics_Hardware_D3D9_ShadowMap_CreateTexture_PF( SHADOWMAPDATA *ShadowMap, int ASyncThread )
{
	...
	if( ShadowMap->PF->D3D9.ZBufferSurface == NULL )
	{
		...
		{
			DXST_ERRORLOG_ADDUTF16LE( "\xb7\x30\xe3\x30\xc9\x30\xa6\x30\xde\x30\xc3\x30\xd7\x30\x28\x75\xf1\x6d\xa6\x5e\xb5\x30\xfc\x30\xd5\x30\xa7\x30\xb9\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"シャドウマップ用深度サーフェスの作成に失敗しました\n" @*/ ) ;
			return -1 ;
		}
	}
	...
	if( ShadowMap->PF->D3D9.Texture == NULL )
	{
		...
		{
			DXST_ERRORLOG_ADDUTF16LE( "\xb7\x30\xe3\x30\xc9\x30\xa6\x30\xde\x30\xc3\x30\xd7\x30\x28\x75\xc6\x30\xaf\x30\xb9\x30\xc1\x30\xe3\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"シャドウマップ用テクスチャの作成に失敗しました\n" @*/ ) ;
			return -1 ;
		}
...
extern	int		Graphics_Hardware_D3D9_Shader_Create_PF( int ShaderHandle, int IsVertexShader, void *Image, int ImageSize, int ImageAfterFree, int ASyncThread )
{
	...
	if( IsVertexShader )
	{
		...
		{
			DXST_ERRORLOG_ADDUTF16LE( "\x02\x98\xb9\x70\xb7\x30\xa7\x30\xfc\x30\xc0\x30\xfc\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"頂点シェーダーの作成に失敗しました\n" @*/ ) ;
			goto ERR ;
		}
	}
	else
	{
		...
		{
			DXST_ERRORLOG_ADDUTF16LE( "\xd4\x30\xaf\x30\xbb\x30\xeb\x30\xb7\x30\xa7\x30\xfc\x30\xc0\x30\xfc\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"ピクセルシェーダーの作成に失敗しました\n" @*/ ) ;
			goto ERR ;
		}
	}
...
//		{
//			DXST_ERRORLOG_ADDUTF16LE( L"Paint 用システムメモリサーフェスの作成に失敗しました\n" ) ;
//			return -1 ;
//		}
//
