...
static void MV1MultiplyRotateMatrix( MATRIX *DestBuffer, VECTOR *Rotate, int RotateOrder ) ;			// 回転行列を乗算する
...
static bool MV1MakeMeshNormals( MV1_MODEL_R *ReadModel, MV1_MESH_R *Mesh )
{
	...
	{
		_SINCOS( ( float )( 89.5f / 180.0f * DX_PI ), &Sin, &SmoothCos ) ;
	}
	else
	{
		_SINCOS( Mesh->Container->SmoothingAngle, &Sin, &SmoothCos ) ;
	}
