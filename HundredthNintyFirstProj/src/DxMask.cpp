...
extern int NS_GraphImageBltToMask( const BASEIMAGE *BaseImage, int ImageX, int ImageY, int MaskHandle )
{
	...
	// BASEIMAGE がフルカラー( 24bit )ではなかった場合はフルカラー画像に変換
...
)
{
	...
	Bmp = CreateDIBGraph_WCHAR_T( FileName, FALSE, &SrcColor ) ;
	...
	// マスク保存用サーフェスの作成
	if( Mask_CreateSurface( &Mask->SrcData, &Mask->SrcDataPitch, bm.bmWidth, bm.bmHeight, &Mask->TransMode ) < 0 )
	{
		DeleteObject( Bmp ) ;
		return -1 ;
	}
