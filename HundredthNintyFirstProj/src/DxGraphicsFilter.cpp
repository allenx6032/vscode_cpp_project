...
static int GraphFilter_GetSoftImage( int GrHandle, BASEIMAGE *BaseImage, RECT *GetRect )
{
	...
	if( Image->Orig->FormatDesc.TextureFlag )
	{
		...
		for( i = 0 ; i < Image->Hard.DrawNum ; i ++, HardDraw ++ )
		{
			...
			if( Graphics_Screen_LockDrawScreen( &SrcRect, &SurfaceImage, GrHandle, 0, TRUE, i ) < 0 )
			{
	...
	{
		if( Graphics_Screen_LockDrawScreen( GetRect, &SurfaceImage, GrHandle, 0, TRUE, 0 ) < 0 )
		{
			NS_ReleaseBaseImage( BaseImage ) ;
			return -1 ;
		}
   ...
   va_list ParamList )
{
	...
	{
		DestImageWidth = DestImage->Width ;
		DestImageHeight = DestImage->Height ;
	}
	else
	...
	for( Info.Pass = 0 ; Info.Pass < Info.PassNum ; Info.Pass ++ )
	{
		...
		if( IsBlend )
		{
			...
			{
			case DX_GRAPH_BLEND_RGBA_SELECT_MIX :
				FilterResult = GraphBlend_RGBA_Select_Mix( &Info, Param.RGBA_R, Param.RGBA_G, Param.RGBA_B, Param.RGBA_A ) ; 
				break ;
		...
		else
		{
			...
			case DX_GRAPH_FILTER_BRIGHT_CLIP :
				FilterResult = GraphFilter_Bright_Clip( &Info, Param.Bright_Clip_CmpType, Param.Bright_Clip_CmpParam, Param.Bright_Clip_ClipFillFlag, &Param.Bright_Clip_ClipFillColor ) ;
				break ;

			case DX_GRAPH_FILTER_HSB :
				FilterResult = GraphFilter_HSB( &Info, Param.Hsb_HueType, Param.Hsb_Hue, Param.Hsb_Saturation, Param.Hsb_Bright ) ;
				break ;
			...
			case DX_GRAPH_FILTER_LEVEL :
				FilterResult = GraphFilter_Level( &Info, Param.Lv_Min, Param.Lv_Max, Param.Lv_Gamma, Param.Lv_AfterMin, Param.Lv_AfterMax ) ;
				break ;

			case DX_GRAPH_FILTER_TWO_COLOR :
				FilterResult = GraphFilter_TwoColor( &Info, Param.TC_Threshold, &Param.TC_LowColor, &Param.TC_HighColor ) ;
				break ;

			case DX_GRAPH_FILTER_GRADIENT_MAP :
				FilterResult = GraphFilter_GradientMap( &Info, Param.GM_MapGrHandle, Param.GM_Reverse ) ;
				break ;
...
extern int GraphFilter_Bright_Clip( GRAPHFILTER_INFO *Info, int CmpType, float CmpParam, int ClipFillFlag, COLOR_F *ClipFillColor )
{
	...
	if( GSYS.HardInfo.UseShader == TRUE )
	{
		GraphFilter_Bright_Clip_PF( Info, CmpType, CmpParam, ClipFillFlag, ClipFillColor ) ;
	}
	else
	{
		// シェーダーが使えない場合
...
extern int GraphFilter_HSB( GRAPHFILTER_INFO *Info, int HueType, float Hue, float Saturation, float Bright )
{
	...
	if( GSYS.HardInfo.UseShader == TRUE )
	{
		GraphFilter_HSB_PF( Info, HueType, Hue, Saturation, Bright ) ;
	}
	else
	{
		// シェーダーが使えない場合
...
extern int GraphFilter_Level( GRAPHFILTER_INFO *Info, float Min, float Max, float Gamma, float AfterMin, float AfterMax )
{
	...
	if( GSYS.HardInfo.UseShader == TRUE )
	{
		...
		GraphFilter_Level_PF( Info, Min, Max, Gamma, AfterMin, AfterMax ) ;
	}
	else
	{
		// シェーダーが使えない場合
...
extern int GraphFilter_TwoColor( GRAPHFILTER_INFO *Info, float Threshold, COLOR_F *LowColor, COLOR_F *HighColor )
{
	...
	if( GSYS.HardInfo.UseShader == TRUE )
	{
		GraphFilter_TwoColor_PF( Info, Threshold, LowColor, HighColor ) ;
	}
	else
	{
		// シェーダーが使えない場合
...
extern int	GraphFilter_GradientMap( GRAPHFILTER_INFO *Info, int MapGrHandle, int Reverse )
{
	...
	if( GSYS.HardInfo.UseShader == TRUE )
	{
		GraphFilter_GradientMap_PF( Info, MapGrHandle, Reverse ) ;
	}
	else
	{
		// シェーダーが使えない場合
		...
		case DX_GRAPH_BLEND_PMA_MULTIPLE_A_ONLY :
			{
				...
				do
				{
					...
					do
					{
						...
						{
							LIN_B = ( int )( ( Src[ 0 ] << 8 ) / Src[ 3 ] ) ; if( LIN_B > 255 ) LIN_B = 255 ;
							LIN_G = ( int )( ( Src[ 1 ] << 8 ) / Src[ 3 ] ) ; if( LIN_G > 255 ) LIN_G = 255 ;
							LIN_R = ( int )( ( Src[ 2 ] << 8 ) / Src[ 3 ] ) ; if( LIN_R > 255 ) LIN_R = 255 ;

							Dest[ 0 ] = ( BYTE )( ( LIN_B * Dest[ 3 ] ) >> 8 ) ;
							Dest[ 1 ] = ( BYTE )( ( LIN_G * Dest[ 3 ] ) >> 8 ) ;
							Dest[ 2 ] = ( BYTE )( ( LIN_R * Dest[ 3 ] ) >> 8 ) ;
						}
...
extern int	GraphBlend_RGBA_Select_Mix( GRAPHFILTER_INFO *Info, int SelectR, int SelectG, int SelectB, int SelectA )
{
	...
	if( GSYS.HardInfo.UseShader == TRUE )
	{
		GraphBlend_RGBA_Select_Mix_PF( Info, SelectR, SelectG, SelectB, SelectA ) ;
	}
	else
	{
		// シェーダーが使えない場合
