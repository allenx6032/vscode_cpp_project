...
// Ｏｇｇ用読み込み関数
static size_t StreamRead( void *Buffer, size_t Size, size_t Count, void *DataP )
{
...
// Ｏｇｇ用６４ビット版シーク関数
static int StreamSeek64_wrap( void *DataP, ogg_int64_t Offset, int SeekMode )
{
	STREAMDATA *StreamP = ( STREAMDATA * )DataP ;
...
// Ｏｇｇ用クローズ関数
static int StreamClose( void * /*StreamP*/ )
{
	// 普通にクローズが成功したとして返す
	return 0 ;
}
...
// Ｏｇｇ用のデータ位置取得関数
static long StreamTell( void *DataP )
{
	STREAMDATA *StreamP = ( STREAMDATA * )DataP ;
	if( StreamP == NULL ) return -1 ;
...
int th_ov_open(FILE *f,OggVorbis_File *vf,char *initial,long ibytes){
	ov_callbacks callbacks = {
		(size_t (*)(void *, size_t, size_t, void *))StreamRead,
		(int    (*)(void *, ogg_int64_t, int))StreamSeek64_wrap,
		(int    (*)(void *))StreamClose,
		(long   (*)(void *))StreamTell
	};
...
//	SOUNDCONV_OGG *oggdata = &SoundConv->OggTypeData ;
	...
	// コールバック関数を用意する
	ogfunc.read_func = ( size_t ( * )( void *, size_t, size_t, void * ) )StreamRead ;
	ogfunc.seek_func = ( int  ( * )( void *, ogg_int64_t, int ) )StreamSeek64_wrap ;
	ogfunc.close_func = ( int ( * )( void * ) )StreamClose ;
	ogfunc.tell_func = ( long ( * )( void * ) )StreamTell ;
...
int TheoraDecode_SetupImage( DWORD_PTR Handle, int BaseImage, int YUVImage, int ASyncThread )
{
	DECODE_THEORA *DT = ( DECODE_THEORA * )Handle ;
	volatile THEORA_STOCKFRAME *Stock ;
	unsigned char *d, *ys, *us, *vs;
	...
	if( BaseImage && DT->BaseImageSetup == 0 )
	{
			...
			Stock->YHeight == Stock->UVHeight * 2 )
		{
			...
			{
				for( j = 0; j < w; j ++, d += 8, ys += 2, us ++, vs ++ )
				{
					uvr =                         YUVTable[ RV ][ *vs ] ;
					uvg = YUVTable[ GU ][ *us ] + YUVTable[ GV ][ *vs ] ;
					uvb = YUVTable[ BU ][ *us ]                         ;
		...
		else
		{
			...
			for( i = 0; i < Stock->YHeight; i++, d += dadd, ys += ysadd )
			{
				...
				{
					us = ( BYTE * )Stock->UBuffer + ( i / 2 ) * Stock->UVStride + ( j / 2 );
					vs = ( BYTE * )Stock->VBuffer + ( i / 2 ) * Stock->UVStride + ( j / 2 );

					r = (int)(1.164f * (*ys-16)                      + 1.596f * (*vs-128));
					g = (int)(1.164f * (*ys-16) - 0.391f * (*us-128) - 0.813f * (*vs-128));
					b = (int)(1.164f * (*ys-16) + 2.018f * (*us-128));
...
const D_IDirect3DSurface9 *TheoraDecode_GetYUVImage( DWORD_PTR Handle )
{
	DECODE_THEORA *DT = ( DECODE_THEORA * )Handle ;
