...
typedef struct tagGRAPHICS_HARDWEARE_DIRECT3D11_BASE_SIMPLE_VERTEXSHADER_INFO
{
...
} GRAPHICS_HARDWEARE_DIRECT3D11_BASE_SIMPLE_VERTEXSHADER_INFO ;
...
// 標準描画用の複雑な処理を行わない頂点シェーダーの情報
static GRAPHICS_HARDWEARE_DIRECT3D11_BASE_SIMPLE_VERTEXSHADER_INFO g_BaseSimpleVertexShaderInfo[ D3D11_VERTEX_INPUTLAYOUT_NUM ] =
{
	...
	// D3D11_VERTEX_INPUTLAYOUT_SHADER_3D( VERTEX3DSHADER )
	{
		...
		sizeof( VERTEX3DSHADER ),
		{
			...
			{ "TEXCOORD",     2, D_DXGI_FORMAT_R32G32_FLOAT,       0, 88, D_D3D11_INPUT_PER_VERTEX_DATA, 0 },
			{ NULL },
		},
	},
} ;
...
//	return 0 ;
	...
	DXST_ERRORLOG_ADDUTF16LE( "\xb7\x30\xa7\x30\xfc\x30\xc0\x30\xfc\x30\xb3\x30\xfc\x30\xc9\x30\xa2\x95\xc2\x4f\x6e\x30\x1d\x52\x1f\x67\x16\x53\x2e\x00\x2e\x00\x2e\x00\x2e\x00\x20\x00\x00"/*@ L"シェーダーコード関係の初期化.... " @*/ ) ;
...
extern int Graphics_D3D11_Shader_Normal3DDraw_Setup( void )
{
	...
	if( GD3D11.Device.State.Lighting )
	{
		...
		PS_PL        =  &SB3D->Base3D_PixelLighting_Normal_PS     [ ShadowMap ][ LightIndex84 ][ AlphaTestMode ] ;
		PSAddress_PL = &SCB3D->Base3D_PixelLighting_Normal_PS_Code[ ShadowMap ][ LightIndex84 ][ AlphaTestMode ] ;
		...
		PS           =  &SB3D->Base3D_VertexLighting_Normal_PS     [ ShadowMap ][ LightIndex10 ][ AlphaTestMode ] ;
		PSAddress    = &SCB3D->Base3D_VertexLighting_Normal_PS_Code[ ShadowMap ][ LightIndex10 ][ AlphaTestMode ] ;
	}
	else
	{
		// ライティングなし描画
		...
		PS           =  &SB3D->Base3D_NoLighting_Normal_PS     [ AlphaTestMode ] ;
		PSAddress    = &SCB3D->Base3D_NoLighting_Normal_PS_Code[ AlphaTestMode ] ;
	}
...
extern	int Graphics_D3D11_VertexBuffer_CreateObject( VERTEXBUFFERHANDLEDATA *VertexBuffer, int Restore )
{
	...
	if( FAILED( hr ) )
	{
		DXST_ERRORLOGFMT_ADDUTF16LE(( "\x02\x98\xb9\x70\xd0\x30\xc3\x30\xd5\x30\xa1\x30\xcf\x30\xf3\x30\xc9\x30\xeb\x30\x28\x75\x6e\x30\x02\x98\xb9\x70\xd0\x30\xc3\x30\xd5\x30\xa1\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"頂点バッファハンドル用の頂点バッファの作成に失敗しました\n" @*/ )) ;
		return -1 ;
	}
...
extern	int		Graphics_D3D11_SetupSubBackBuffer( void )
{
	...
	if( GRAWIN.Setting.FSAAMultiSampleCount > 1 )
	{
		...
		{
			Texture2DDesc.SampleDesc.Count		= ( UINT )GRAWIN.Setting.FSAAMultiSampleCount ;
			Texture2DDesc.SampleDesc.Quality	= ( UINT )GRAWIN.Setting.FSAAMultiSampleQuality ;
...
*/
		...
		if( IsMainTarget )
		{
			if( NS_GetWindowModeFlag() == TRUE )
			{
				...
				DXST_ERRORLOG_ADDUTF16LE( "\x5b\x00\xa6\x30\xa4\x30\xf3\x30\xc9\x30\xa6\x30\xe2\x30\xfc\x30\xc9\x30\x5d\x00\x20\x00\x00"/*@ L"[ウインドウモード] " @*/ ) ;
			}
			else
			{
				...
				DXST_ERRORLOG_ADDUTF16LE( "\x5b\x00\xd5\x30\xeb\x30\xb9\x30\xaf\x30\xea\x30\xfc\x30\xf3\x30\xe2\x30\xfc\x30\xc9\x30\x5d\x00\x20\x00\x00"/*@ L"[フルスクリーンモード] " @*/ ) ;
			}
		}
		else
		{
		...
		if( GRAWIN.Setting.FSAAMultiSampleCount > 1 )
		{
			...
			{
				SwapChainDesc.SampleDesc.Count		  = ( UINT )GRAWIN.Setting.FSAAMultiSampleCount ;
				SwapChainDesc.SampleDesc.Quality	  = ( UINT )GRAWIN.Setting.FSAAMultiSampleQuality ;
		...
		GRAWIN.Setting.FSAAMultiSampleCount   = ( int )SwapChainDesc.SampleDesc.Count ;
		GRAWIN.Setting.FSAAMultiSampleQuality = ( int )SwapChainDesc.SampleDesc.Quality ;

		DXST_ERRORLOG_ADDUTF16LE( "\x49\x00\x44\x00\x58\x00\x47\x00\x49\x00\x53\x00\x77\x00\x61\x00\x70\x00\x43\x00\x68\x00\x61\x00\x69\x00\x6e\x00\x20\x00\x92\x30\x5c\x4f\x10\x62\x57\x30\x7e\x30\x59\x30\x2e\x00\x2e\x00\x2e\x00\x2e\x00\x20\x00\x00"/*@ L"IDXGISwapChain を作成します.... " @*/ ) ;
...
//		DXST_ERRORLOGFMT_ADDUTF16LE(( L"CreateSwapChain Result:%d", Result )) ;
		...
		{
			DXST_ERRORLOG_ADDUTF16LE( "\x31\x59\x57\x65\x0a\x00\x00"/*@ L"失敗\n" @*/ ) ;
			goto ERR ;
		}
	...
	DXST_ERRORLOG_ADDUTF16LE( "\x49\x00\x44\x00\x58\x00\x47\x00\x49\x00\x4f\x00\x75\x00\x74\x00\x70\x00\x75\x00\x74\x00\x20\x00\x92\x30\xd6\x53\x97\x5f\x57\x30\x7e\x30\x59\x30\x2e\x00\x2e\x00\x2e\x00\x2e\x00\x20\x00\x00"/*@ L"IDXGIOutput を取得します.... " @*/ ) ;
...
extern int Graphics_D3D11_OutputWindow_ResizeBuffer( int Index, int NewSizeX, int NewSizeY )
{
	...
	if( hr != S_OK )
	{
		DXST_ERRORLOG_ADDUTF16LE( "\xd0\x30\xc3\x30\xaf\x30\xd0\x30\xc3\x30\xd5\x30\xa1\x30\x6e\x30\xb5\x30\xa4\x30\xba\x30\x09\x59\xf4\x66\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"バックバッファのサイズ変更に失敗しました\n" @*/ ) ;
		return -1 ;
	}
...
extern	int		Graphics_D3D11_CreateDepthBuffer( void )
{
	...
	DXST_ERRORLOG_ADDUTF16LE( "\xf1\x6d\xa6\x5e\xd0\x30\xc3\x30\xd5\x30\xa1\x30\x92\x30\x5c\x4f\x10\x62\x57\x30\x7e\x30\x59\x30\x2e\x00\x2e\x00\x2e\x00\x2e\x00\x20\x00\x00"/*@ L"深度バッファを作成します.... " @*/ ) ;
...
//		Texture2DDesc.Format             = GD3D11.Device.Caps.DepthBufferTexture2DFormat[ index ] ;
		...
		if( FAILED( hr ) )
		{
			DXST_ERRORLOG_ADDUTF16LE( "\xf1\x6d\xa6\x5e\xd0\x30\xc3\x30\xd5\x30\xa1\x30\x28\x75\x20\x00\x54\x00\x65\x00\x78\x00\x74\x00\x75\x00\x72\x00\x65\x00\x32\x00\x44\x00\x20\x00\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"深度バッファ用 Texture2D の作成に失敗しました\n" @*/ ) ;
			return -1 ;
		}
	}
...
extern int Graphics_D3D11_StretchRect_Initialize( void )
{
	...
	DXST_ERRORLOG_ADDUTF16LE( "\x3b\x75\xcf\x50\x6e\x30\x58\x53\x14\x7d\xe2\x8e\x01\x90\xe6\x51\x06\x74\x6e\x30\x1d\x52\x1f\x67\x16\x53\x2e\x00\x2e\x00\x2e\x00\x20\x00\x00"/*@ L"画像の単純転送処理の初期化... " @*/ ) ;
...
)
{
	...
	D_D3D11_BOX            DestBox ;
	UINT                   Offsets = 0 ;
	UINT                   VertexStride = sizeof( VertexData[ 0 ] ) ;
	RECT                   SrcRectTemp ;
	RECT                   DestRectTemp ;
	RECT                   BlendRectTemp ;
	...
	Graphics_D3D11_CommonVertexBuffer_Setup( sizeof( VertexData ) ) ;
	...
	// 入力レイアウトをセット
	D3D11DeviceContext_IASetInputLayout( GD3D11.Device.InputLayout.BaseStretchRectInputLayout ) ;

	// ピクセルシェーダーをセット
	D3D11DeviceContext_PSSetShader( PixelShader  != NULL ? PixelShader  : GD3D11.Device.Shader.Base.StretchRect_PS, NULL, 0 ) ;

	// 頂点シェーダーをセット
	D3D11DeviceContext_VSSetShader( VertexShader != NULL ? VertexShader : GD3D11.Device.Shader.Base.StretchRect_VS, NULL, 0 ) ;
...
)
{
	...
	D_ID3D11Texture2D				*DrawValidTexture    = NULL ;
	...
	*MapTempTexture = NULL ;
	...
	// ロック対象がマルチサンプル対応か、ミップマップ付きの場合は同サイズの非マルチサンプルテクスチャに転送
		...
		TargetTexture2DDesc.MipLevels        != 1 )
	{
		...
		Texture2DDesc.SampleDesc.Count   = 1 ;
		Texture2DDesc.SampleDesc.Quality = 0 ;
		hr = D3D11Device_CreateTexture2D_ASync( &Texture2DDesc, NULL, &DrawValidTexture ) ;
		if( FAILED( hr ) )
		{
			DXST_ERRORLOG_ADDUTF16LE( "\xc6\x30\xaf\x30\xb9\x30\xc1\x30\xe3\x30\xfc\x30\xde\x30\xc3\x30\xd7\x30\x28\x75\xcf\x63\x3b\x75\xef\x53\xfd\x80\xc6\x30\xaf\x30\xb9\x30\xc1\x30\xe3\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"テクスチャーマップ用描画可能テクスチャの作成に失敗しました\n" @*/ ) ;
			goto ERR ;
		}
	...
	// 転送用テクスチャに描画対象画像を転送
	{
		...
		if( FAILED( hr ) )
		{
			DXST_ERRORLOG_ADDUTF16LE( "\xcf\x63\x3b\x75\xfe\x5b\x61\x8c\xd0\x30\xc3\x30\xd5\x30\xa1\x30\xad\x8a\x7f\x30\xd6\x53\x8a\x30\x28\x75\x6e\x30\xc6\x30\xaf\x30\xb9\x30\xc1\x30\xe3\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"描画対象バッファ読み取り用のテクスチャの作成に失敗しました\n" @*/ ) ;
			goto ERR ;
		}

		D3D11DeviceContext_CopyResource( *MapTempTexture, DrawValidTexture ? DrawValidTexture : TargetTexture ) ;
	...
	if( DrawValidTexture != NULL )
	{
		Direct3D11_Release_Texture2D( DrawValidTexture ) ;
		DrawValidTexture = NULL ;
	}
...
//		GSYS.HardInfo.TextureSizeNonPow2Conditional = FALSE ;
	...
	// 各種フォーマットを調べる
	{
		...
		DXST_ERRORLOGFMT_ADDA(( "\x82\x63\x82\x77\x82\x73\x82\x51\x83\x65\x83\x4e\x83\x58\x83\x60\x83\x83\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd\x8e\x67\x82\xa6\x82\xdc\x82\xb9\x82\xf1"/*@ "ＤＸＴ２テクスチャフォーマットは使えません" @*/ )) ;
		...
		DXST_ERRORLOGFMT_ADDA(( "\x82\x63\x82\x77\x82\x73\x82\x53\x83\x65\x83\x4e\x83\x58\x83\x60\x83\x83\x83\x74\x83\x48\x81\x5b\x83\x7d\x83\x62\x83\x67\x82\xcd\x8e\x67\x82\xa6\x82\xdc\x82\xb9\x82\xf1"/*@ "ＤＸＴ４テクスチャフォーマットは使えません" @*/ )) ;
...
//			( FormatSupport & D_D3D11_FORMAT_SUPPORT_RENDER_TARGET ) == 0 )
//		{
//			DXST_ERRORLOG_ADDUTF16LE( L"使用できる描画可能カラーフォーマットが見つかりませんでした\n" ) ;
//			return -1 ;
//		}
//		GD3D11.Device.Caps.TextureFormat[ DX_GRAPHICSIMAGE_FORMAT_3D_DRAWVALID_RGB16       ] = D_DXGI_FORMAT_R8G8B8A8_UNORM ;
//		GD3D11.Device.Caps.TextureFormat[ DX_GRAPHICSIMAGE_FORMAT_3D_DRAWVALID_RGB32       ] = D_DXGI_FORMAT_R8G8B8A8_UNORM ;
//		GD3D11.Device.Caps.TextureFormat[ DX_GRAPHICSIMAGE_FORMAT_3D_DRAWVALID_ALPHA_RGB32 ] = D_DXGI_FORMAT_R8G8B8A8_UNORM ;
//		String = "DXGI_FORMAT_R8G8B8A8_UNORM" ;
//		DXST_ERRORLOGFMT_ADDA(( "描画用カラーフォーマットは %s です", String )) ;
	...
	// 情報出力
	{
		...
		DXST_ERRORLOGFMT_ADDA(( "\x8e\x67\x97\x70\x82\xb7\x82\xe9\x8b\x40\x94\x5c\x83\x8c\x83\x78\x83\x8b:%s"/*@ "使用する機能レベル:%s" @*/, String )) ;
//		DXST_ERRORLOGFMT_ADDA(( "対応している最大頂点インデックス:%d",     GSYS.HardInfo.MaxVertexIndex )) ;
		...
		DXST_ERRORLOGFMT_ADDA(( "\x8d\xc5\x91\xe5\x83\x65\x83\x4e\x83\x58\x83\x60\x83\x83\x83\x54\x83\x43\x83\x59\x81\x40\x95\x9d:%d \x8d\x82\x82\xb3:%d"/*@ "最大テクスチャサイズ　幅:%d 高さ:%d" @*/, GSYS.HardInfo.MaxTextureWidth, GSYS.HardInfo.MaxTextureHeight )) ;
	}
...
extern	int		Graphics_D3D11_Device_Initialize( void )
{
	...
	// 描画ステータスのセット
	{
		...
		// ビューポートを設定
		{
			...
			GD3D11.Device.State.Viewport.TopLeftX	= 0 ;
			GD3D11.Device.State.Viewport.TopLeftY	= 0 ;
			GD3D11.Device.State.Viewport.Width		= ( FLOAT )GSYS.Screen.MainScreenSizeX ;
			GD3D11.Device.State.Viewport.Height		= ( FLOAT )GSYS.Screen.MainScreenSizeY ;
			GD3D11.Device.State.Viewport.MinDepth	= 0.0f ;
			GD3D11.Device.State.Viewport.MaxDepth	= 1.0f ;
...
{
	DX_D3D11_VS_CONST_BUFFER_OTHERMATRIX *ConstOtherMatrix = ( DX_D3D11_VS_CONST_BUFFER_OTHERMATRIX * )GD3D11.Device.Shader.Constant.ConstBuffer_VS_OtherMatrix->SysmemBuffer ;
...
{
	DX_D3D11_PS_CONST_BUFFER_BASE *ConstantPSBase = ( DX_D3D11_PS_CONST_BUFFER_BASE * )GD3D11.Device.Shader.Constant.ConstBuffer_PS_Base->SysmemBuffer ;
...
extern int Graphics_D3D11_DeviceState_SetPSShaderResouceView( int StartSlot, int Num, D_ID3D11ShaderResourceView * const *ppShaderResourceViews )
{
	...
	if( Change == FALSE && GD3D11.Device.DrawSetting.CancelSettingEqualCheck == FALSE )
	{
		return 0 ;
	}

	D3D11DeviceContext_PSSetShaderResources( ( UINT )StartSlot, ( UINT )Num, ppShaderResourceViews ) ;
...
{
	DXST_ERRORLOG_ADDUTF16LE( "\x04\x54\x2e\x7a\xb7\x30\xa7\x30\xfc\x30\xc0\x30\xfc\x30\x28\x75\x9a\x5b\x70\x65\xd0\x30\xc3\x30\xd5\x30\xa1\x30\x6e\x30\x5c\x4f\x10\x62\x2e\x00\x2e\x00\x2e\x00\x2e\x00\x20\x00\x00"/*@ L"各種シェーダー用定数バッファの作成.... " @*/ ) ;
...
extern	CONSTANTBUFFER_DIRECT3D11 *Graphics_D3D11_ConstantBuffer_Create( DWORD Size, int ASyncThread )
{
	...
	if( Size % 16 != 0 )
	{
		DXST_ERRORLOGFMT_ADDUTF16LE(( "\x9a\x5b\x70\x65\xd0\x30\xc3\x30\xd5\x30\xa1\x30\x6e\x30\xb5\x30\xa4\x30\xba\x30\x6f\x30\x11\xff\x16\xff\x6e\x30\x0d\x50\x70\x65\x67\x30\x42\x30\x8b\x30\xc5\x5f\x81\x89\x4c\x30\x42\x30\x8a\x30\x7e\x30\x59\x30\x0a\x00\x00"/*@ L"定数バッファのサイズは１６の倍数である必要があります\n" @*/ )) ;
		goto ERR ;
	}
	...
	if( NewBuffer == NULL )
	{
		DXST_ERRORLOGFMT_ADDUTF16LE(( "\x9a\x5b\x70\x65\xd0\x30\xc3\x30\xd5\x30\xa1\x30\xc5\x60\x31\x58\x92\x30\x3c\x68\x0d\x7d\x59\x30\x8b\x30\xe1\x30\xe2\x30\xea\x30\x18\x98\xdf\x57\x6e\x30\xba\x78\xdd\x4f\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"定数バッファ情報を格納するメモリ領域の確保に失敗しました\n" @*/ )) ;
		goto ERR ;
	}
...
extern int Graphics_D3D11_InputLayout_Base_Create( void )
{
	...
	int							ElementNum ;
	HRESULT						hr ;
	GRAPHICS_HARDWEARE_DIRECT3D11_BASE_SIMPLE_VERTEXSHADER_INFO *BSVI ;
	GRAPHICS_HARDWARE_DIRECT3D11_SHADERCODE_INFO                *Code ;

	// 既に作成されていたときのために削除処理を行う
	Graphics_D3D11_InputLayout_Base_Terminate() ;

	DXST_ERRORLOG_ADDUTF16LE( "\x04\x54\x2e\x7a\x20\x00\x49\x00\x44\x00\x33\x00\x44\x00\x31\x00\x31\x00\x49\x00\x6e\x00\x70\x00\x75\x00\x74\x00\x4c\x00\x61\x00\x79\x00\x6f\x00\x75\x00\x74\x00\x20\x00\x6e\x30\x5c\x4f\x10\x62\x2e\x00\x2e\x00\x2e\x00\x2e\x00\x20\x00\x00"/*@ L"各種 ID3D11InputLayout の作成.... " @*/ ) ;
...
extern	int		Graphics_D3D11_NormalDrawVertexBuffer_Create( void )
{
	...
	DXST_ERRORLOG_ADDUTF16LE( "\x19\x6a\x96\x6e\xcf\x63\x3b\x75\x28\x75\x6e\x30\x02\x98\xb9\x70\xd0\x30\xc3\x30\xd5\x30\xa1\x30\x6e\x30\x5c\x4f\x10\x62\x2e\x00\x2e\x00\x2e\x00\x2e\x00\x20\x00\x00"/*@ L"標準描画用の頂点バッファの作成.... " @*/ ) ;
...
extern	int		Graphics_D3D11_DrawBillboard3D( VECTOR Pos, float cx, float cy, float Size, float Angle, IMAGEDATA *Image, IMAGEDATA *BlendImage, int TransFlag, int TurnFlag, int DrawFlag, RECT *DrawArea )
{
	...
	for( i = 0 ; i < DrawTexNum ; i ++, DrawTex ++ )
	{
		...
		if( GD3D11.Device.DrawInfo.VertexType != VERTEXTYPE_BLENDTEX && DrawFlag == TRUE )
		{
			...
			*( ( DWORD * )&DrawVect3D[1].b ) = 
			*( ( DWORD * )&DrawVect3D[2].b ) = 
			*( ( DWORD * )&DrawVect3D[3].b ) = 
			*( ( DWORD * )&DrawVect3D[4].b ) = 
			*( ( DWORD * )&DrawVect3D[5].b ) = DiffuseColor ;
		...
		else
		{
			...
			case VERTEXTYPE_BLENDTEX :
				...
				DrawVectB[3].color	= 
				DrawVectB[4].color	= 
				DrawVectB[5].color	= DiffuseColor ;

				if( TurnFlag )
				{
					DrawVectB[5].u1 = DrawVectB[3].u1 = DrawVectB[1].u1 = ( TexVect[0].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D11.Device.DrawSetting.InvBlendTextureWidth ;
					DrawVectB[4].u1 = DrawVectB[2].u1 = DrawVectB[0].u1 = ( TexVect[1].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D11.Device.DrawSetting.InvBlendTextureWidth ;
				}
				else
				{
					DrawVectB[4].u1 = DrawVectB[2].u1 = DrawVectB[0].u1 = ( TexVect[0].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D11.Device.DrawSetting.InvBlendTextureWidth ;
					DrawVectB[5].u1 = DrawVectB[3].u1 = DrawVectB[1].u1 = ( TexVect[1].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D11.Device.DrawSetting.InvBlendTextureWidth ;
				}
				DrawVectB[4].u2 = DrawVectB[2].u2 = DrawVectB[0].u2 = TexVect[0].u ;
				DrawVectB[5].u2 = DrawVectB[3].u2 = DrawVectB[1].u2 = TexVect[1].u ;

				DrawVectB[5].v1 = DrawVectB[1].v1 = DrawVectB[0].v1 = ( TexVect[0].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D11.Device.DrawSetting.InvBlendTextureHeight ;
				DrawVectB[4].v1 = DrawVectB[3].v1 = DrawVectB[2].v1 = ( TexVect[2].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D11.Device.DrawSetting.InvBlendTextureHeight ;
...
//		y + Image->Height < GSYS.DrawSetting.DrawArea.bottom && y > GSYS.DrawSetting.DrawArea.right )
	{
		...
		if( GD3D11.Device.DrawInfo.VertexType == VERTEXTYPE_BLENDTEX )
		{
			...
			for( i = 0 ; i < DrawTexNum ; i ++, DrawTex ++ )
			{
				...
				DrawVectB[3].color = 
				DrawVectB[4].color = 
				DrawVectB[5].color = ( DWORD )DiffuseColor ;
...
//			y > GSYS.DrawSetting.DrawArea.bottom || y + Image->Height < GSYS.DrawSetting.DrawArea.top ) return 0 ;
		...
		case VERTEXTYPE_BLENDTEX :
			...
			for( i = 0 ; i < DrawTexNum ; i ++, DrawTex ++ )
			{
				...
				DrawVectB[2].u1 = DrawVectB[0].u1 = ( TexVect[0].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D11.Device.DrawSetting.InvBlendTextureWidth ;
				DrawVectB[3].u1 = DrawVectB[1].u1 = ( TexVect[1].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D11.Device.DrawSetting.InvBlendTextureWidth ;
				DrawVectB[1].v1 = DrawVectB[0].v1 = ( TexVect[0].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D11.Device.DrawSetting.InvBlendTextureHeight ;
				DrawVectB[3].v1 = DrawVectB[2].v1 = ( TexVect[2].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D11.Device.DrawSetting.InvBlendTextureHeight ;
	...
	if( IN_DRAWAREA_CHECK( x1, y1, x2, y2 ) )
	{
		...
		case VERTEXTYPE_BLENDTEX :	// ブレンドテクスチャを使用する
			...
			for( i = 0 ; i < DrawTexNum ; i ++, DrawTex ++ )
			{
				...
				VectDataB[3].color = 
				VectDataB[4].color = 
				VectDataB[5].color = ( DWORD )DiffuseColor ;
				...
				VectDataB[4].u1 = VectDataB[2].u1 = VectDataB[0].u1 = ( TexVect[0].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D11.Device.DrawSetting.InvBlendTextureWidth ;
				VectDataB[5].u1 = VectDataB[3].u1 = VectDataB[1].u1 = ( TexVect[1].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D11.Device.DrawSetting.InvBlendTextureWidth ;
				VectDataB[5].v1 = VectDataB[1].v1 = VectDataB[0].v1 = ( TexVect[0].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D11.Device.DrawSetting.InvBlendTextureHeight ;
				VectDataB[4].v1 = VectDataB[3].v1 = VectDataB[2].v1 = ( TexVect[2].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D11.Device.DrawSetting.InvBlendTextureHeight ;
		...
		case VERTEXTYPE_TEX :	// ブレンドテクスチャを使用しない
			...
			for(;;)
			{
				...
				Vect[3].color = 
				Vect[4].color = 
				Vect[5].color = ( DWORD )DiffuseColor ;
	...
	else
	{
		...
		case VERTEXTYPE_BLENDTEX :	// ブレンドテクスチャを使用する
			...
			for( i = 0 ; i < DrawTexNum ; i ++, DrawTex ++ )
			{
				...
				VectDataB[2].u1 = VectDataB[0].u1 = ( TexVect[0].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D11.Device.DrawSetting.InvBlendTextureWidth ;
				VectDataB[3].u1 = VectDataB[1].u1 = ( TexVect[1].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D11.Device.DrawSetting.InvBlendTextureWidth ;
				VectDataB[1].v1 = VectDataB[0].v1 = ( TexVect[0].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D11.Device.DrawSetting.InvBlendTextureHeight ;
				VectDataB[3].v1 = VectDataB[2].v1 = ( TexVect[2].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D11.Device.DrawSetting.InvBlendTextureHeight ;
...
//		yf -= 0.5F ;
	...
	case VERTEXTYPE_BLENDTEX :	// ブレンドテクスチャを使用する
		...
		if( DrawTexNum == 1 && FastCode == 1 )
		{
			...
			VectB[3].color = 
			VectB[4].color = 
			VectB[5].color = ( DWORD )DiffuseColor ;
			...
			VectB[4].u1 = VectB[2].u1 = VectB[0].u1 = ( TexVect[0].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D11.Device.DrawSetting.InvBlendTextureWidth ;
			VectB[5].u1 = VectB[3].u1 = VectB[1].u1 = ( TexVect[1].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D11.Device.DrawSetting.InvBlendTextureWidth ;
			VectB[5].v1 = VectB[1].v1 = VectB[0].v1 = ( TexVect[0].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D11.Device.DrawSetting.InvBlendTextureHeight ;
			VectB[4].v1 = VectB[3].v1 = VectB[2].v1 = ( TexVect[2].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D11.Device.DrawSetting.InvBlendTextureHeight ;
		...
		else
		{
			...
			for( i = 0 ; i < DrawTexNum ; i ++, DrawTex ++ )
			{
				...
				VectB[0].u1 = ( TexVect[0].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D11.Device.DrawSetting.InvBlendTextureWidth ;
				VectB[1].u1 = ( TexVect[1].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D11.Device.DrawSetting.InvBlendTextureWidth ;
				VectB[2].u1 = ( TexVect[2].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D11.Device.DrawSetting.InvBlendTextureWidth ;
				VectB[3].u1 = ( TexVect[3].x - BlendTexVect[0].x + GSYS.DrawSetting.BlendGraphX ) * GD3D11.Device.DrawSetting.InvBlendTextureWidth ;
				VectB[4].u1 = VectB[2].u1 ;
				VectB[5].u1 = VectB[1].u1 ;
				
				VectB[0].v1 = ( TexVect[0].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D11.Device.DrawSetting.InvBlendTextureHeight ;
				VectB[1].v1 = ( TexVect[1].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D11.Device.DrawSetting.InvBlendTextureHeight ;
				VectB[2].v1 = ( TexVect[2].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D11.Device.DrawSetting.InvBlendTextureHeight ;
				VectB[3].v1 = ( TexVect[3].y - BlendTexVect[0].y + GSYS.DrawSetting.BlendGraphY ) * GD3D11.Device.DrawSetting.InvBlendTextureHeight ;
...
//	xf4 = (float)x4 - 0.5F ; yf4 = (float)y4 - 0.5F ; 
	...
	case VERTEXTYPE_BLENDTEX :	// ブレンドテクスチャを使用する
		...
		for( i = 0 ; i < DrawTexNum ; i ++, DrawTex ++ )
		{
			...
			*((DWORD *)&DrawVectB[1].pos.z) =
			*((DWORD *)&DrawVectB[2].pos.z) =
			*((DWORD *)&DrawVectB[3].pos.z) = drawz ;

			DrawVectB[0].rhw =
			DrawVectB[1].rhw =
			DrawVectB[2].rhw =
			DrawVectB[3].rhw = 1.0f ;
			...
			for( j = 0 ; j < 4 ; j ++, VectB ++, TexVect ++ )
			{
				...
				VectB->u1 = ( TexVect->x - BlendTexVect->x + GSYS.DrawSetting.BlendGraphX ) * GD3D11.Device.DrawSetting.InvBlendTextureWidth ;
				VectB->v1 = ( TexVect->y - BlendTexVect->y + GSYS.DrawSetting.BlendGraphY ) * GD3D11.Device.DrawSetting.InvBlendTextureHeight ;
			...
			// 反転判定処理
			Reverse0 =  ( DrawVectB[ 1 ].pos.x - DrawVectB[ 0 ].pos.x ) * ( DrawVectB[ 2 ].pos.y - DrawVectB[ 0 ].pos.y ) -
						( DrawVectB[ 2 ].pos.x - DrawVectB[ 0 ].pos.x ) * ( DrawVectB[ 1 ].pos.y - DrawVectB[ 0 ].pos.y ) ;
			Reverse1 = -( DrawVectB[ 1 ].pos.x - DrawVectB[ 3 ].pos.x ) * ( DrawVectB[ 2 ].pos.y - DrawVectB[ 3 ].pos.y ) +
						( DrawVectB[ 1 ].pos.y - DrawVectB[ 3 ].pos.y ) * ( DrawVectB[ 2 ].pos.x - DrawVectB[ 3 ].pos.x ) ;
			if( Reverse0 <= 0 && Reverse1 <= 0 )
			{
				VectBufB = DrawVectB[ 1 ] ; DrawVectB[ 1 ] = DrawVectB[ 2 ] ; DrawVectB[ 2 ] = VectBufB ;
			}
	...
	case VERTEXTYPE_TEX :	// ブレンドテクスチャを使用しない
		...
		for( i = 0 ; i < DrawTexNum ; i ++, DrawTex ++ )
		{
			...
			// 頂点データをセットする
			DrawVect[ 0 ].pos.x = TexVect[ 0 ].x ;
			DrawVect[ 0 ].pos.y = TexVect[ 0 ].y ;
			DrawVect[ 1 ].pos.x = TexVect[ 1 ].x ;
			DrawVect[ 1 ].pos.y = TexVect[ 1 ].y ;
			DrawVect[ 2 ].pos.x = TexVect[ 2 ].x ;
			DrawVect[ 2 ].pos.y = TexVect[ 2 ].y ;
			DrawVect[ 3 ].pos.x = TexVect[ 3 ].x ;
			DrawVect[ 3 ].pos.y = TexVect[ 3 ].y ;

			DrawVect[ 0 ].u = TexVect[ 0 ].u ;
			DrawVect[ 0 ].v = TexVect[ 0 ].v ;
			DrawVect[ 1 ].u = TexVect[ 1 ].u ;
			DrawVect[ 1 ].v = TexVect[ 1 ].v ;
			DrawVect[ 2 ].u = TexVect[ 2 ].u ;
			DrawVect[ 2 ].v = TexVect[ 2 ].v ;
			DrawVect[ 3 ].u = TexVect[ 3 ].u ;
			DrawVect[ 3 ].v = TexVect[ 3 ].v ;
			...
			*((DWORD *)&DrawVect[3].pos.z) = drawz;
			...
			// 反転判定処理
			Reverse0 =  ( DrawVect[ 1 ].pos.x - DrawVect[ 0 ].pos.x ) * ( DrawVect[ 2 ].pos.y - DrawVect[ 0 ].pos.y ) -
						( DrawVect[ 2 ].pos.x - DrawVect[ 0 ].pos.x ) * ( DrawVect[ 1 ].pos.y - DrawVect[ 0 ].pos.y ) ;
			Reverse1 = -( DrawVect[ 1 ].pos.x - DrawVect[ 3 ].pos.x ) * ( DrawVect[ 2 ].pos.y - DrawVect[ 3 ].pos.y ) +
						( DrawVect[ 1 ].pos.y - DrawVect[ 3 ].pos.y ) * ( DrawVect[ 2 ].pos.x - DrawVect[ 3 ].pos.x ) ;
			if( Reverse0 <= 0 && Reverse1 <= 0 )
			{
				VectBuf = DrawVect[ 1 ] ; DrawVect[ 1 ] = DrawVect[ 2 ] ; DrawVect[ 2 ] = VectBuf ;
			}
...
//			xxx3 = xxx1 - 0.5F ;	yyy3 = yyy1 - 0.5F ;
		...
		// 頂点追加処理
		D3D11_Graphics_SetPlaneVertexHardware( VectData2, tdn, tdn ) ;
...
extern	int		Graphics_D3D11_DrawLineBox( int x1, int y1, int x2, int y2, unsigned int Color )
{
	...
	*((DWORD *)&VectData[18].pos.z) = *((DWORD *)&VectData[19].pos.z) = *((DWORD *)&VectData[20].pos.z) = *((DWORD *)&VectData[21].pos.z) = *((DWORD *)&VectData[22].pos.z) = *((DWORD *)&VectData[23].pos.z) = drawz;
	...
	VectData[ 0].rhw = VectData[ 1].rhw = VectData[ 2].rhw = VectData[ 3].rhw = VectData[ 4].rhw = VectData[ 5].rhw =
	VectData[ 6].rhw = VectData[ 7].rhw = VectData[ 8].rhw = VectData[ 9].rhw = VectData[10].rhw = VectData[11].rhw =
	VectData[12].rhw = VectData[13].rhw = VectData[14].rhw = VectData[15].rhw = VectData[16].rhw = VectData[17].rhw =
	VectData[18].rhw = VectData[19].rhw = VectData[20].rhw = VectData[21].rhw = VectData[22].rhw = VectData[23].rhw = 1.0f ;
...
#define DX_D3D11_CIRCLE_THICKNESS											\
	...
	if( x2 > DrawRect.right - 1 ) x2 = DrawRect.right - 1 ;					\
																			\
	VectBuf[ VectNum ].pos.x  = ( float )x1 + 0.1f ;						\
	VectBuf[ VectNum ].pos.y  = y1f + 0.1f ;								\
	VectBuf[ VectNum ].color = Color ;										\
	VectBuf[ VectNum ].rhw   = 1.0f ;										\
	*((DWORD *)&VectBuf[ VectNum ].pos.z) = drawz ;							\
	VectNum ++ ;															\
																			\
	VectBuf[ VectNum ].pos.x =  ( float )( x2 + 1 ) + 0.1f ;				\
	VectBuf[ VectNum ].pos.y =  y1f + 0.1f ;								\
	VectBuf[ VectNum ].color = Color ;										\
	VectBuf[ VectNum ].rhw   = 1.0f ;										\
	*((DWORD *)&VectBuf[ VectNum ].pos.z) = drawz ;							\
	VectNum  ++ ;															\
																			\
	if( VectNum >= D3D11_CIRCLE_VERTEX_NUM )								\
	{																		\
		Graphics_D3D11_CommonBuffer_DrawPrimitive( D3D11_VERTEX_INPUTLAYOUT_NOTEX_2D, D_D3D11_PRIMITIVE_TOPOLOGY_LINELIST, VectBuf, VectNum ) ;\
		VectNum = 0 ;														\
	}																		
...
#define DX_D3D11_CIRCLE																	\
			...
			{																			\
				...
				if( x2 > DrawRect.right - 1 ) x2 = DrawRect.right - 1 ;					\
																						\
				VectBuf[ VectNum ].pos.x  = ( float )x1 + 0.1f ;						\
				VectBuf[ VectNum ].pos.y  = ( float )y1 + 0.1f ;						\
				VectBuf[ VectNum ].color = Color ;										\
				VectBuf[ VectNum ].rhw   = 1.0f ;										\
				*((DWORD *)&VectBuf[ VectNum ].pos.z) = drawz ;							\
				VectNum ++ ;															\
...
#define DX_D3D11_CIRCLE_PSET															\
			...
			{																			\
				...
				*((DWORD *)&VectBuf[ VectNum ].pos.z) = drawz ;							\
				...
				if( VectNum >= D3D11_CIRCLE_VERTEX_NUM )								\
				{																		\
					Graphics_D3D11_CommonBuffer_DrawPrimitive( D3D11_VERTEX_INPUTLAYOUT_NOTEX_2D, D_D3D11_PRIMITIVE_TOPOLOGY_POINTLIST, VectBuf, VectNum ) ;\
					VectNum = 0 ;														\
				}																		\
			}
...
extern	int		Graphics_D3D11_DrawPrimitive2D( VERTEX_2D *Vertex, int VertexNum, int PrimitiveType, IMAGEDATA *Image, int TransFlag, int BillboardFlag, int Is3D, int TurnFlag, int TextureNo )
{
	int Flag ;
	int i ;
	VERTEX_2D *Vert ;
...
extern void Graphics_D3D11_DrawPreparationToShader( int ParamFlag, int Is2D )
{
	...
	else
	{
		...
		Graphics_D3D11_DeviceState_SetAlphaTestRef( 0 ) ;
		Graphics_D3D11_DeviceState_SetAlphaTestCmpMode( DX_CMP_GREATER ) ;
	}
...
{
	Graphics_D3D11_DeviceState_SetMaxAnisotropy( MaxAnisotropy ) ;
...
//#endif
		...
		if( TargetScreenImage )
		{
			...
			if( TargetScreenImage->Width  > GD3D11.Device.Screen.DepthBufferSizeX || 
				TargetScreenImage->Height > GD3D11.Device.Screen.DepthBufferSizeY )
			{
...
#endif
{
		...
		WinData.UseUpdateLayerdWindowFlag         == FALSE )
	{
		...
		SetTimer( NS_GetMainWindowHandle(), GRAPHICS_HARDWARE_D3D11_SCREENFLIPTIMER_ID, 32, (TIMERPROC)Graphics_Hardware_D3D11_ScreenFlipTimerProc ) ;
	}
}
...
extern	int		Graphics_Hardware_D3D11_LockDrawScreenBuffer_PF( RECT *LockRect, BASEIMAGE *BaseImage, int TargetScreen, IMAGEDATA *TargetImage, int TargetScreenSurface, int ReadOnly, int TargetScreenTextureNo )
{
	...
	{
		DXST_ERRORLOG_ADDUTF16LE( "\xcf\x63\x3b\x75\xfe\x5b\x61\x8c\xd0\x30\xc3\x30\xd5\x30\xa1\x30\x6e\x30\xed\x30\xc3\x30\xaf\x30\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"描画対象バッファのロックに失敗しました\n" @*/ ) ;
		return -1 ;
	}
		...
		if( NS_GetWindowModeFlag() == TRUE )
		{
			...
			else
			{
				...
				else
				{
					...
					DestRect.right  = _DTOL( GSYS.Screen.MainScreenSizeX * ExRateX ) ;
					DestRect.bottom = _DTOL( GSYS.Screen.MainScreenSizeY * ExRateY ) ;
...
extern	int		Graphics_Hardware_D3D11_GraphLock_PF( IMAGEDATA *Image, COLORDATA **ColorDataP, int WriteOnly )
{
	...
	// テンポラリバッファを確保する
	Image->LockImagePitch	= ( DWORD )( ColorData->PixelByte * Image->Width ) ;
	Image->LockImage		= ( BYTE * )DXALLOC( Image->LockImagePitch * Image->Height ) ;
	...
	if( WriteOnly == FALSE )
	{
		...
		for( i = 0 ; i < Image->Hard.DrawNum ; i ++, DrawTex ++ )
		{
			...
			// 転送
			Src  = ( BYTE * )MapBaseImage.GraphData + DrawTex->UsePosX  * ColorData->PixelByte + DrawTex->UsePosY  * MapBaseImage.Pitch ;
			...
			for( i = 0 ; i < DrawTex->Height ; i ++, Dest += Image->LockImagePitch, Src += MapBaseImage.Pitch )
			{
...
extern	int		Graphics_Hardware_D3D11_GraphUnlock_PF( IMAGEDATA *Image )
{
	...
	BaseImage.Pitch          = ColorData->PixelByte * Image->Width ;
...
)
{
		...
		RgbBaseImage->MipMapCount >= Orig->Hard.MipMapCount )
	{
		...
		SrcScale    = ( DWORD )( RgbBaseImage->ColorData.ColorBitDepth * ( IsDXTFormat ? 4 : 1 ) / 8 ) ;
		...
		if( Orig->FormatDesc.DrawValidFlag )
		{
			...
			for( j = 0 ; j < RTVNum ; j ++ )
			{
				...
				if( OrigTex->PF->D3D11.TextureRTV[ j ] == NULL )
				{
					...
					hr = D3D11Device_CreateRenderTargetView_ASync( OrigTex->PF->D3D11.Texture, &RTVDesc, &OrigTex->PF->D3D11.TextureRTV[ j ], ASyncThread ) ;
					if( FAILED( hr ) )
					{
			...
			if( ( ( GRAWIN.Setting.FSAAMultiSampleCount == 1 && Samples >  1 ) ||
				  ( GRAWIN.Setting.FSAAMultiSampleCount >  1 && Samples == 1 ) ||
				  Orig->ZBufferFlag ) )
			{
				...
				if( FAILED( hr ) )
				{
					DXST_ERRORLOG_ADDUTF16LE( "\xf1\x6d\xa6\x5e\xd0\x30\xc3\x30\xd5\x30\xa1\x30\x28\x75\xf1\x6d\xa6\x5e\xb9\x30\xc6\x30\xf3\x30\xb7\x30\xeb\x30\xd3\x30\xe5\x30\xfc\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"深度バッファ用深度ステンシルビューの作成に失敗しました\n" @*/ ) ;
					return -1 ;
				}
			}
			else
...
extern	int		Graphics_Hardware_D3D11_ReleaseOrigTexture_PF( IMAGEDATA_ORIG *Orig )
{
	...
	for( i = 0 ; i < Orig->Hard.TexNum ; i ++, OrigTex ++ )
	{
		...
		{
			Direct3D11_Release_ShaderResourceView( OrigTex->PF->D3D11.TextureSRV ) ;
...
extern	int		Graphics_Hardware_D3D11_ShadowMap_CreateTexture_PF( SHADOWMAPDATA *ShadowMap, int ASyncThread )
{
	...
	if( ShadowMap->PF->D3D11.DepthBuffer == NULL )
	{
		...
		if( FAILED( hr ) )
		{
			DXST_ERRORLOG_ADDUTF16LE( "\xb7\x30\xe3\x30\xc9\x30\xa6\x30\xde\x30\xc3\x30\xd7\x30\x28\x75\xf1\x6d\xa6\x5e\xd0\x30\xc3\x30\xd5\x30\xa1\x30\xc6\x30\xaf\x30\xb9\x30\xc1\x30\xe3\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"シャドウマップ用深度バッファテクスチャの作成に失敗しました\n" @*/ ) ;
			return -1 ;
		}
		...
		if( FAILED( hr ) )
		{
			DXST_ERRORLOG_ADDUTF16LE( "\xb7\x30\xe3\x30\xc9\x30\xa6\x30\xde\x30\xc3\x30\xd7\x30\x28\x75\xf1\x6d\xa6\x5e\xb9\x30\xc6\x30\xf3\x30\xb7\x30\xeb\x30\xd3\x30\xe5\x30\xfc\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"シャドウマップ用深度ステンシルビューの作成に失敗しました\n" @*/ ) ;
			return -1 ;
		}
	}
...
{
	DX_D3D11_PS_CONST_BUFFER_SHADOWMAP *ConstantPSShadowMap = ( DX_D3D11_PS_CONST_BUFFER_SHADOWMAP * )GD3D11.Device.Shader.Constant.ConstBuffer_PS_ShadowMap->SysmemBuffer ;
	SHADOWMAPDATA *ShadowMap ;
	LIGHT_HANDLE  *pLH ;
	int           CheckNum ;
	int           i ;
