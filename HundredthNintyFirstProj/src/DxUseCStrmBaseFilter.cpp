...
//コンストラクタ
D_CMovieRender::D_CMovieRender( IUnknown * pUnk, HRESULT *phr ) : D_CBaseVideoRenderer( CLSID_MovieRenderer, NAME("Movie Renderer"), pUnk, phr), SetMediaTypeFlag( 0 ), ImageBuffer( NULL ), TempBaseImage( NULL ), UseTempBaseImage( 0 )
{
	ASSERT(phr) ;
	if( phr )
	{
...
void D_CMovieRender::AllocImageBuffer()
{
	...
	if( UseTempBaseImage == 1 && TempBaseImage == NULL )
	{
...
HRESULT D_CMovieRender::DoRenderSample( D_IMediaSample * pSample )
{
	...
	if( ImageBuffer )
	{
		...
		if( UseTempBaseImage != 0 )
		{
			...
			Src = ( BYTE * )TempBaseImage->GraphData ;
		}
		...
		{
			_MEMCPY( ImageBuffer, Src, Pitch * Height ) ;
		}
