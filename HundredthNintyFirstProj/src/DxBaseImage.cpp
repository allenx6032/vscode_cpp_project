...
// 画像読み込み関数
static	int LoadBmpImage( STREAMDATA *Stream, BASEIMAGE *BaseImage ) ;						// ＢＭＰ画像の読みこみ
/*
#ifndef DX_NON_JPEGREAD
static	int LoadJpegImage( STREAMDATA *Src, BASEIMAGE *BaseImage ) ;						// ＪＰＥＧ画像の読みこみ
#endif
*/
/*
#ifndef DX_NON_PNGREAD
static	int LoadPngImage( STREAMDATA *Src, BASEIMAGE *BaseImage ) ;							// ＰＮＧ画像の読みこみ
#endif
*/
static	int LoadArgbImage( STREAMDATA *Src, BASEIMAGE *BaseImage ) ;						// ＡＲＧＢ画像の読みこみ

#ifndef DX_NON_TGA
static	int LoadTargaImage( STREAMDATA *Src, BASEIMAGE *BaseImage ) ;						// ＴＧＡ画像の読みこみ
#endif

static	int LoadDDSImage( STREAMDATA *Src, BASEIMAGE *BaseImage ) ;							// ＤＤＳ画像の読みこみ
...
// 画像読み込み関数配列
int ( *DefaultImageLoadFunc4[] )( STREAMDATA *Src, BASEIMAGE *BaseImage ) =
{
	LoadBmpImage ,
#ifndef DX_NON_JPEGREAD
	LoadJpegImage ,
...
extern int NS_CreateBmpInfo( BITMAPINFO *BmpInfo, int Width, int Height, int Pitch, const void *SrcGrData, void **DestGrData )
{
	...
	if( DPitch == SPitch )
	{
		memcpy( DestData, SrcData, ( size_t )( Height * DPitch ) ) ;
	}
	else
	{
		for( i = 0 ; i < Height ; i ++, SrcData += SPitch, DestData += DPitch )
			memcpy( DestData, SrcData, ( size_t )SPitch ) ;
	}
...
// ＢＭＰ画像の読みこみ
static int LoadBmpImage( STREAMDATA *Stream, BASEIMAGE *BaseImage )
{
	void				*sp ;
	BYTE				*GData = NULL ;
...
RLE8END:;
		...
		// BASEIMAGE のヘッダを構築
		{
			...
			memcpy( BaseImage->ColorData.Palette, BmpInfo2->bmiColors, 256 * sizeof( RGBQUAD ) ) ;
			for( i = 0 ; i < 256 ; i ++ )
...
RLE4END :;
		...
		// BASEIMAGE のヘッダを構築
		{
			...
			// カラー情報のセット
			NS_CreatePaletteColorData( &BaseImage->ColorData ) ;
			memcpy( BaseImage->ColorData.Palette, BmpInfo2->bmiColors, 16 * sizeof( RGBQUAD ) ) ;
			for( i = 0 ; i < 16 ; i ++ )
				BaseImage->ColorData.Palette[i].Alpha = 0 ;
		}
	}
		...
		( BmpInfo2->bmiHeader.biBitCount == 32 ||  BmpInfo2->bmiHeader.biBitCount == 24 || BmpInfo2->bmiHeader.biBitCount == 8 ) )
	{
		...
		// BASEIMAGE のヘッダを構築
		{
			...
			if( BmpInfo2->bmiHeader.biBitCount == 8 )
			{
				...
				// カラー情報のセット
				NS_CreatePaletteColorData( &BaseImage->ColorData ) ;
				memcpy( BaseImage->ColorData.Palette, BmpInfo2->bmiColors, ( BmpInfo2->bmiHeader.biBitCount == 8 ? 256 : 16 ) * sizeof( RGBQUAD ) ) ;
				for( i = 0 ; i < 256 ; i ++ )
					BaseImage->ColorData.Palette[i].Alpha = 0 ;
			}
			else
...
// ＡＲＧＢ画像の読みこみ
static int LoadArgbImage( STREAMDATA *Src, BASEIMAGE *BaseImage )
{
	ArgbHeader Head ;
	void *sp ;
	STREAMDATASHRED *sstr ;
...
// ＴＧＡ画像の読みこみ
static int LoadTargaImage( STREAMDATA *Src, BASEIMAGE *BaseImage )
{
	_TGAHEADER head ;
//	_TGAEXTENSION ext ;
	_TGAFOOTER foot ;
...
// ＤＤＳ画像の読みこみ
static int LoadDDSImage( STREAMDATA *Src, BASEIMAGE *BaseImage )
{
	void            *sp ;
	STREAMDATASHRED *sstr ;
	DDSFILEHEADER   head ;
