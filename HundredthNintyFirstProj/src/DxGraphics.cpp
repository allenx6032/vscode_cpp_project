	...
	if( SPitch != DPitch )
	{
		// RGBデータの作成
		{
			...
			for( i = 0 ; i < Height ; i ++, SData += SPitch, DData += DPitch )
				memcpy( DData, SData, ( size_t )SPitch ) ;
			DData = DestData ;
		}
		...
		if( AlphaData != NULL )
		{
			...
			for( i = 0 ; i < Height ; i ++, SData += SPitch, AData += DPitch )
				memcpy( AData, SData, ( size_t )SPitch ) ;
			AData = DestData ;
		}
	}
	else
...
// グラフィックのサイズを得る
extern	int NS_GetGraphSize(unsigned int GrHandle, int *SizeXBuf, int *SizeYBuf )
{
	IMAGEDATA *Image ;
	SHADOWMAPDATA *ShadowMap ;
...
extern int NS_ScreenCopy( void )
{
	...
	{
		Graphics_Hardware_ScreenCopy_PF() ;
	}
...
extern int Graphics_Image_TerminateHandle( HANDLEINFO *HandleInfo )
{
	...
	for( i = 0 ; i < GSYS.HardInfo.RenderTargetNum ; i ++ )
	{
		...
		{
			NS_SetRenderTargetToShader( i, ( int )( i == 0 ? DX_SCREEN_FRONT : -1 ) ) ;
		}
	}
...
)
{
	...
	GParam->CreateGraphGParam.InitHandleGParam.AlphaChannelImageCreateFlag = Movie->A8R8G8B8Flag && Movie->pMovieImage->ImageType == 1 ;
...
extern int Graphics_Other_AllocCommonBuffer( int Index, DWORD Size )
{
	...
	GSYS.Resource.CommonBuffer[ Index ] = DXALLOC( Size ) ;
