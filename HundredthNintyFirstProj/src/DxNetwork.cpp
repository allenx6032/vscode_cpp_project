		...
		if( ProcessMessage() == 0 && GetActiveFlag() == TRUE && HTTP_GetDownloadedFileSize( DHandle ) != -1 )
		{
			...
			if( GetNowCount() - StartTime > 1000 )
			{
				...
				// ダウンロードメーターの描画
				{
					...
					_SPRINTF( tempstr, "%.1f%%", ( double )HTTP_GetDownloadedFileSize( DHandle ) * 100 / HTTP_GetDownloadFileSize( DHandle ) ) ;
				...
				if( RecvSize != 0 )
				{
					...
					_SPRINTF( tempstr, Message3, timestr ) ;
				...
				// ダウンロード速度の描画
				{
					...
					_SPRINTF( tempstr, Message4, numstr1, unit1 ) ;
					width = GetDrawStringWidthToHandle( tempstr, lstrlenA( tempstr ), Font ) ;
				...
				// ダウンロード完了サイズの描画
				{
					...
					_SPRINTF( tempstr, Message5, numstr1, unit1, numstr2, unit2 ) ;
					width = GetDrawStringWidthToHandle( tempstr, lstrlenA( tempstr ), Font ) ;
								...
								char *PathBuf, char *FileNameBuf, int *PortBuf )
{
	...
	if( UseProxy == TRUE )
	{
		...
		if( tempport != -1 ) _SPRINTF( PathBuf + lstrlenA( PathBuf ), ":%d", tempport ) ;
