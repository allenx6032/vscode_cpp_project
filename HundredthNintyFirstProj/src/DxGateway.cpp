...
// エラー処理関数
extern int ErrorLogAdd( const TCHAR *ErrorStr )
{
	int Result ;
	DXFUNC_BEGIN
	Result = NS_ErrorLogAdd( ErrorStr ) ;
	DXFUNC_END
	return Result ;
}
...
extern int AppLogAdd( const TCHAR *String, ... )
{
	...
	DXFUNC_BEGIN
	Result = NS_ErrorLogAdd( StringBuf ) ;
	DXFUNC_END
	return Result ;
}
...
extern int InputStringToCustom( int x, int y, int BufLength, TCHAR *StrBuffer, int CancelValidFlag, int SingleCharOnlyFlag, int NumCharOnlyFlag, int DoubleCharOnlyFlag )
{
	int Result ;
...
extern int MakeKeyInput( int MaxStrLength, int CancelValidFlag, int SingleCharOnlyFlag, int NumCharOnlyFlag, int DoubleCharOnlyFlag )
{
	int Result ;
	DXFUNC_BEGIN
	Result = NS_MakeKeyInput( MaxStrLength, CancelValidFlag, SingleCharOnlyFlag, NumCharOnlyFlag, DoubleCharOnlyFlag ) ;
	DXFUNC_END
	return Result ;
}
...
// 特殊用途関係
extern const D_IDirect3DDevice9* GetUseDirect3DDevice9( void )
{
	const D_IDirect3DDevice9 * Result ;
	DXFUNC_BEGIN
	Result = NS_GetUseDirect3DDevice9( ) ;
	DXFUNC_END
	return Result ;
}
extern const D_IDirect3DSurface9* GetUseDirect3D9BackBufferSurface( void )
{
	const D_IDirect3DSurface9 * Result ;
	DXFUNC_BEGIN
...
extern	int			SetRenderTargetToShader( int TargetIndex, int DrawScreen, int SurfaceIndex )
{
	int Result ;
	DXFUNC_BEGIN
	Result = NS_SetRenderTargetToShader( TargetIndex, DrawScreen, SurfaceIndex ) ;
	DXFUNC_END
	return Result ;
}
...
extern	const BASEIMAGE *GetMovieBaseImageToGraph( int GraphHandle, int *ImageUpdateFlag )
{
	...
	DXFUNC_BEGIN
	Result = NS_GetMovieBaseImageToGraph( GraphHandle, ImageUpdateFlag ) ;
	DXFUNC_END
	return Result ;
}
...
extern int SetFontSpaceToHandle( int Point, int FontHandle )
{
	int Result ;
	DXFUNC_BEGIN
	Result = NS_SetFontSpaceToHandle( Point, FontHandle ) ;
	DXFUNC_END
	return Result ;
}
...
extern	int		MakePAL8ColorSoftImage( int SizeX, int SizeY )
{
	int Result ;
	DXFUNC_BEGIN
	Result = NS_MakePAL8ColorSoftImage(  SizeX,  SizeY ) ;
	DXFUNC_END
	return Result ;
}
...
extern	int		CreatePAL8ColorBaseImage( int SizeX, int SizeY, BASEIMAGE *BaseImage )
{
	int Result ;
	DXFUNC_BEGIN
	Result = NS_CreatePAL8ColorBaseImage(  SizeX,  SizeY, BaseImage );
	DXFUNC_END
	return Result ;
}
...
extern	int		ConvertNormalFormatBaseImage( BASEIMAGE *BaseImage )
{
	int Result ;
	DXFUNC_BEGIN
	Result = NS_ConvertNormalFormatBaseImage( BaseImage );
	DXFUNC_END
	return Result ;
}
...
{
	int Result ;
	Result = NS_CreatePal8ColorData( ColorDataBuf ) ;
	return Result ;
}
...
extern int LoadSoundMemByMemImage( const void *FileImageBuffer, int ImageSize, int UnionHandle )
{
	int Result ;
	DXFUNC_BEGIN
	Result = NS_LoadSoundMemByMemImage( FileImageBuffer, ImageSize, UnionHandle ) ;
	DXFUNC_END
	return Result ;
}
...
extern	int			MakeSoftSoundCustom( int ChannelNum, int BitsPerSample, int SamplesPerSec, int SampleNum )
{
	int Result ;
	DXFUNC_BEGIN
	Result = NS_MakeSoftSoundCustom(  ChannelNum,  BitsPerSample,  SamplesPerSec,  SampleNum ) ;
	DXFUNC_END
	return Result ;
}
...
extern	int			GetSoftSoundFormat( int SoftSoundHandle, int *Channels, int *BitsPerSample, int *SamplesPerSec )
{
	int Result ;
	DXFUNC_BEGIN
	Result = NS_GetSoftSoundFormat(  SoftSoundHandle, Channels, BitsPerSample, SamplesPerSec ) ;
	DXFUNC_END
	return Result ;
}
