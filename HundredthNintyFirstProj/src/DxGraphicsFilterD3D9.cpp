...
static int	Direct3D9_GraphFilter_GetWorkTexture( int IsFloatType, DWORD TexSizeW, DWORD TexSizeH, DWORD HandleIndex ) ;		// フィルター作業用のテクスチャハンドルを取得する
...
static int  Direct3D9_BasePolygonDraw( GRAPHFILTER_INFO *Info, int ScaleDivNum = 1 ) ;

// プログラム -----------------------------------------------------------------

// フィルター作業用のテクスチャハンドルを取得する
static int	Direct3D9_GraphFilter_GetWorkTexture( int IsFloatType, DWORD TexSizeW, DWORD TexSizeH, DWORD HandleIndex )
{
	IMAGEDATA *BaseImage ;
	DWORD       TexSizeBig ;
	DWORD       NPow ;
	...
	if( GRAPHCHK( GraphFilterSystemInfoD3D9.WorkDrawValidGrHandle[ IsFloatType ][ NPow ][ HandleIndex ], BaseImage ) )
	{
		...
		GraphFilterSystemInfoD3D9.WorkDrawValidGrHandle[ IsFloatType ][ NPow ][ HandleIndex ] = Graphics_Image_MakeGraph_UseGParam( &GParam, TexSize, TexSize, FALSE, FALSE ) ;
		if( GraphFilterSystemInfoD3D9.WorkDrawValidGrHandle[ IsFloatType ][ NPow ][ HandleIndex ] < 0 )
		{
		...
		NS_SetDeleteHandleFlag( GraphFilterSystemInfoD3D9.WorkDrawValidGrHandle[ IsFloatType ][ NPow ][ HandleIndex ], &GraphFilterSystemInfoD3D9.WorkDrawValidGrHandle[ IsFloatType ][ NPow ][ HandleIndex ] ) ;
	}

	return GraphFilterSystemInfoD3D9.WorkDrawValidGrHandle[ IsFloatType ][ NPow ][ HandleIndex ] ;
}
...
static int Direct3D9_MemLoadShaderCode( const char *ShaderName, bool IsVertexShader )
{
	...
	if( GD3D9.ShaderCode._2D._2DShaderInitializeFlag == FALSE ) return -1 ;

	if( DXA_GetFileInfo( &GD3D9.ShaderCode._2D.FilterShaderBinDxa, DX_CODEPAGE_ASCII, ShaderName, &Addr, &Size ) < 0 ) return -1 ;

	FileImage = ( BYTE * )DXA_GetFileImage( &GD3D9.ShaderCode._2D.FilterShaderBinDxa ) + Addr ;
...
extern int	GraphFilter_D3D9_Bright_Clip_PF( GRAPHFILTER_INFO *Info, int CmpType, float CmpParam, int ClipFillFlag, COLOR_F *ClipFillColor )
{
	...
	{
		GraphFilterShaderHandle.BrightClipPS[ UseShader ][ UseFill ] = Direct3D9_MemLoadShaderCode( PsoFileName[ UseShader ][ UseFill ], false ) ;
		if( GraphFilterShaderHandle.BrightClipPS[ UseShader ][ UseFill ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	NS_SetUsePixelShader( GraphFilterShaderHandle.BrightClipPS[ UseShader ][ UseFill ] ) ;
...
extern int	GraphFilter_D3D9_HSB_PF(         GRAPHFILTER_INFO *Info, int HueType, float Hue, float Saturation, float Bright )
{
	...
	{
		GraphFilterShaderHandle.HsbPS[ UseShader ] = Direct3D9_MemLoadShaderCode( PsoFileName[ UseShader ], false ) ;
		if( GraphFilterShaderHandle.HsbPS[ UseShader ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	NS_SetUsePixelShader( GraphFilterShaderHandle.HsbPS[ UseShader ] ) ;
...
extern int	GraphFilter_D3D9_Invert_PF(      GRAPHFILTER_INFO *Info )
{
	...
	{
		GraphFilterShaderHandle.InvertPS = Direct3D9_MemLoadShaderCode( PsoFileName[ 0 ], false ) ;
		if( GraphFilterShaderHandle.InvertPS < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	NS_SetUsePixelShader( GraphFilterShaderHandle.InvertPS ) ;
...
extern int	GraphFilter_D3D9_Level_PF(       GRAPHFILTER_INFO *Info, float Min, float Max, float Gamma, float AfterMin, float AfterMax )
{
	...
	{
		GraphFilterShaderHandle.LevelPS = Direct3D9_MemLoadShaderCode( PsoFileName[ 0 ], false ) ;
		if( GraphFilterShaderHandle.LevelPS < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	NS_SetUsePixelShader( GraphFilterShaderHandle.LevelPS ) ;
...
extern int	GraphFilter_D3D9_TwoColor_PF(    GRAPHFILTER_INFO *Info, float Threshold, COLOR_F *LowColor, COLOR_F *HighColor )
{
	...
	{
		GraphFilterShaderHandle.TwoColorPS = Direct3D9_MemLoadShaderCode( PsoFileName[ 0 ], false ) ;
		if( GraphFilterShaderHandle.TwoColorPS < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	NS_SetUsePixelShader( GraphFilterShaderHandle.TwoColorPS ) ;
...
extern int	GraphFilter_D3D9_GradientMap_PF( GRAPHFILTER_INFO *Info, int MapGrHandle, int Reverse )
{
	...
	{
		GraphFilterShaderHandle.GradientMapPS[ UseShader ] = Direct3D9_MemLoadShaderCode( PsoFileName[ UseShader ], false ) ;
		if( GraphFilterShaderHandle.GradientMapPS[ UseShader ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
	...
	NS_SetUsePixelShader( GraphFilterShaderHandle.GradientMapPS[ UseShader ] ) ;
...
extern int	GraphBlend_D3D9_RGBA_Select_Mix_PF( GRAPHFILTER_INFO *Info, int SelectR, int SelectG, int SelectB, int SelectA )
{
	...
	if( SrcOnlyFlag )
	{
		...
		{
			if( GD3D9.ShaderCode._2D.RGBAMixS_ShaderPackImage != NULL )
			{
				*PixelShaderHandle = NS_LoadPixelShaderFromMem( GD3D9.ShaderCode._2D.RGBAMixS_ShaderAddress[ SelectR ][ SelectG ][ SelectB ][ SelectA ], GD3D9.ShaderCode._2D.RGBAMixS_ShaderSize[ SelectR ][ SelectG ][ SelectB ][ SelectA ] ) ;
				if( *PixelShaderHandle < 0 )
				{
	...
	else
	{
		...
		if( *PixelShaderHandle < 0 )
		{
			*PixelShaderHandle = Direct3D9_MemLoadShaderCode( PsoFileName[ 0 ], false ) ;
			if( *PixelShaderHandle < 0 )
			{
				ConvString( PsoFileName[ 0 ], DX_CODEPAGE_ASCII, ( char * )PathUTF16LE, DX_CODEPAGE_UTF16LE ) ;
				DXST_ERRORLOGFMT_ADDUTF16LE(( "\xd5\x30\xa3\x30\xeb\x30\xbf\x30\xfc\x30\x28\x75\xb7\x30\xa7\x30\xfc\x30\xc0\x30\xfc\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x20\x00\x25\x00\x73\x00\x00"/*@ L"フィルター用シェーダーの作成に失敗しました %s" @*/, PathUTF16LE )) ;
...
static int Direct3D9_BasePolygonDraw( GRAPHFILTER_INFO *Info, int ScaleDivNum )
{
...
extern int GraphFilter_D3D9_DestGraphSetup_PF( GRAPHFILTER_INFO *Info, int *UseSrcGrHandle, int *UseDestGrHandle )
{
	...
	else
	{
		...
		if( SrcEqDestOrDestNonDrawValidHandle || AlwaysUseDestWorkHandle )
		{
			...
			*UseDestGrHandle = Direct3D9_GraphFilter_GetWorkTexture( DestTexFloatType, ( DWORD )TexSizeW, ( DWORD )TexSizeH, ( DWORD )UseDestWorkHandleIndex ) ;
		...
		if( SrcGraphDivFlag )
		{
			...
			*UseSrcGrHandle = Direct3D9_GraphFilter_GetWorkTexture( SrcTexFloatType, ( DWORD )TexSizeW, ( DWORD )TexSizeH, ( DWORD )UseSrcWorkHandleIndex ) ;
			if( *UseSrcGrHandle < 0 )
				return -1 ;
			...
			for( i = 0 ; i < SrcImage->Hard.DrawNum ; i ++ )
			{
				...
				Graphics_D3D9_DrawPrimitive2D( Vert, 4, DX_PRIMTYPE_TRIANGLESTRIP, SrcImage, TRUE, FALSE, FALSE, FALSE, i ) ;
			}
		}
	}
