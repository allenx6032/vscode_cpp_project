...
extern	const D_IDirect3DDevice9* NS_GetUseDirect3DDevice9( void )
{
	switch( GRAWIN.Setting.UseGraphicsAPI )
	{
#ifndef DX_NON_DIRECT3D11
...
extern const D_IDirect3DSurface9* NS_GetUseDirect3D9BackBufferSurface( void )
{
	switch( GRAWIN.Setting.UseGraphicsAPI )
	{
#ifndef DX_NON_DIRECT3D11
...
// 裏画面の内容を表画面に描画する
extern	int		Graphics_Hardware_ScreenCopy_PF( void )
{
	switch( GRAWIN.Setting.UseGraphicsAPI )
	{
#ifndef DX_NON_DIRECT3D11
...
//				DXST_ERRORLOGFMT_ADDA(( "デバイス名 %s  画面モード %d  %dx%d %dbit %dHz", DeviceName, i, ModeData->Width, ModeData->Height, ModeData->ColorBitDepth, ModeData->RefreshRate )) ;
				ModeData ++ ;
			}
		}
	}
