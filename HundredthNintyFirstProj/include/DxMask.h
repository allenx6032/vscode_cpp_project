...
extern	int			Mask_LoadDivMask_UseGParam(          const wchar_t *FileName,                  int AllNum, int XNum, int YNum, int XSize, int YSize, int *HandleBuf, int ASyncLoadFlag = FALSE ) ;	// マスクを画像ファイルから分割読みこみ
...
extern	int			Mask_CreateDivMaskFromMem_UseGParam( const void *FileImage, int FileImageSize, int AllNum, int XNum, int YNum, int XSize, int YSize, int *HandleBuf, int ASyncLoadFlag = FALSE ) ;	// マスクをメモリ上の画像ファイルイメージから分割読みこみ
