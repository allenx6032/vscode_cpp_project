...
//	int											UseShader ;						// プログラマブルシェーダーを使用するかどうか( TRUE:使用する  FALSE:使用しない )
	...
	GRAPHICS_HARDWARE_DIRECT3D9_SHADER_2D		_2D ;							// ２Ｄ描画処理用のシェーダーを纏めた構造体
...
// Direct3D9 の ２Ｄ描画用のシェーダーコードの元データを纏めた構造体
struct GRAPHICS_HARDWARE_DIRECT3D9_SHADERCODE_2D
{
	int						_2DShaderInitializeFlag ;				// シェーダーバイナリのセットアップが完了しているかどうかのフラグ( TRUE:完了している  FALSE:完了していない )
...
{
	GRAPHICS_HARDWARE_DIRECT3D9_SHADERCODE_2D		_2D ;				// ２Ｄ描画処理用のシェーダーコードを纏めた構造体
...
// シェーダーコード関係
extern	int		Graphics_D3D9_ShaderCode_2D_Initialize( void ) ;				// Direct3D9 の２Ｄ描画用のシェーダーコードの初期化を行う
extern	int		Graphics_D3D9_ShaderCode_2D_Terminate( void ) ;					// Direct3D9 の２Ｄ描画用のシェーダーコードの後始末を行う
...
extern	int		Graphics_D3D9_DrawSetting_SetIgnoreDrawGraphColor( int EnableFlag ) ;							// 描画時の画像の色を無視するかどうかを設定する
...
// Direct3D9 を使った描画関係
extern	int		Graphics_D3D9_DrawBillboard3D( VECTOR Pos, float cx, float cy, float Size, float Angle, IMAGEDATA *Image, IMAGEDATA *BlendImage, int TransFlag, int TurnFlag, int DrawFlag = TRUE, RECT *DrawArea = NULL ) ;	// ハードウエアアクセラレータ使用版 DrawBillboard3D
...
extern	int		Graphics_D3D9_DrawRotaGraph( int x, int y, float xf, float yf, double ExRate, double Angle, IMAGEDATA *Image, IMAGEDATA *BlendImage, int TransFlag, int TurnFlag, int IntFlag ) ;				// ハードウエアアクセラレータ使用版 DrawRotaGraph
...
extern	int		Graphics_D3D9_DrawCircle( int x, int y, int r, unsigned int Color, int FillFlag ) ;																		// ハードウエアアクセラレータ使用版 DrawCircle
extern	int		Graphics_D3D9_DrawOval( int x, int y, int rx, int ry, unsigned int Color, int FillFlag ) ;																// ハードウエアアクセラレータ使用版 DrawOval
...
// 環境依存描画設定関係
extern	int		Graphics_Hardware_D3D9_SetRenderTargetToShader_PF( int TargetIndex, int DrawScreen, int SurfaceIndex ) ;	// シェーダー描画での描画先を設定する
...
extern	int		Graphics_Hardware_D3D9_SetMultiThreadFlag_PF( int Flag ) ;									// DirectDraw や Direct3D の協調レベルをマルチスレッド対応にするかどうかをセットする
...
extern	int		Graphics_Hardware_D3D9_SetDrawScreen_PF( int DrawScreen, int OldScreenSurface, IMAGEDATA *NewTargetImage, IMAGEDATA *OldTargetImage, SHADOWMAPDATA *NewTargetShadowMap, SHADOWMAPDATA *OldTargetShadowMap ) ;						// 描画先画面のセット
...
extern	int		Graphics_Hardware_D3D9_LockDrawScreenBuffer_PF( RECT *LockRect, BASEIMAGE *BaseImage, int TargetScreen, IMAGEDATA *TargetImage, int TargetScreenSurface, int ReadOnly, int TargetScreenTextureNo ) ;	// 描画先バッファをロックする
...
extern	int		Graphics_Hardware_D3D9_SetScreenFlipTargetWindow_PF( HWND TargetWindow ) ;									// ScreenFlip で画像を転送する先のウインドウを設定する( NULL を指定すると設定解除 )
...
extern	int		Graphics_Hardware_D3D9_GetDrawScreenGraphBase_PF( IMAGEDATA *Image, IMAGEDATA *TargetImage, int TargetScreen, int TargetScreenSurface, int TargetScreenWidth, int TargetScreenHeight, int x1, int y1, int x2, int y2, int destX, int destY ) ;		// 描画可能画像やバックバッファから指定領域のグラフィックを取得する
