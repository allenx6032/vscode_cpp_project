...
//	D_ID3D11Texture2D *				CubeMapTexture ;						// キューブマップテクスチャ
//	D_ID3D11ShaderResourceView *	CubeMapTextureSRV ;						// キューブマップテクスチャ用シェーダーリソースビュー
//	D_ID3D11RenderTargetView *		CubeMapTextureRTV[ CUBEMAP_SURFACE_NUM ] ;	// キューブマップ用レンダリングターゲットビュー

//	D_ID3D11Texture2D *				RenderTarget ;							// レンダリングターゲットテクスチャ
//	D_ID3D11RenderTargetView *		RenderTargetRTV ;						// レンダリングターゲットテクスチャ用レンダリングターゲットビュー
...
//	D_IDirect3DSurface9	*			DepthBuffer ;							// 深度バッファ
} ;
...
struct GRAPHICS_HARDWARE_DIRECT3D11_SHADER_BASE3D
{
	...
	// /*[ スペキュラの有無( 0:無し 1:有り ) ]*/
	// [ ピクセルシェーダーのアルファテストモード ]
	D_ID3D11PixelShader						*Base3D_PixelLighting_Normal_PS[ 2 ]/*[ 5 ]*//*[ 2 ]*//*[ 2 ]*/[ 84 ]/*[ 2 ]*/[ DIRECT3D11_PS_ALPHATEST_NUM ] ;
	...
	// /*[ マルチテクスチャのブレンドモード、MV1_LAYERBLEND_TYPE_TRANSLUCENT などに +1 したもの ( 0:マルチテクスチャ無し  1:αブレンド  2:加算ブレンド  3:乗算ブレンド  4:乗算ブレンド×2 ) ]*/
	// [ ピクセルシェーダーのアルファテストモード ]
	D_ID3D11PixelShader						*Base3D_NoLighting_Normal_PS/*[ 5 ]*/[ DIRECT3D11_PS_ALPHATEST_NUM ] ;
	...
	// /*[ スペキュラの有無( 0:無し 1:有り ) ]*/
	// [ ピクセルシェーダーのアルファテストモード ]
	D_ID3D11PixelShader						*Base3D_VertexLighting_Normal_PS[ 2 ]/*[ 5 ]*//*[ 2 ]*//*[ 2 ]*/[ 10 ]/*[ 2 ]*/[ DIRECT3D11_PS_ALPHATEST_NUM ] ;
} ;
...
struct GRAPHICS_HARDDATA_DIRECT3D11_DEVICE_STATE
{
	...
	D_ID3D11SamplerState *					DefaultSamplerStateArray[ DIRECT3D11_DEFAULTRS_FILTER_NUM ][ D_D3D11_TEXTURE_ADDRESS_MIRROR_ONCE + 1 ][ D_D3D11_TEXTURE_ADDRESS_MIRROR_ONCE + 1 ][ D_D3D11_TEXTURE_ADDRESS_MIRROR_ONCE + 1 ][ 2 /* MaxAnisotropy  要素番号0 = 1  要素番号1 = 16 */ ] ;	// よく使う設定のサンプラーステートの配列
...
struct GRAPHICS_HARDWARE_DIRECT3D11_SHADERCODE_BASE3D
{
	...
	// /*[ スペキュラの有無( 0:無し 1:有り ) ]*/
	// [ ピクセルシェーダーのアルファテストモード ]
	GRAPHICS_HARDWARE_DIRECT3D11_SHADERCODE_INFO Base3D_PixelLighting_Normal_PS_Code[ 2 ]/*[ 5 ]*//*[ 2 ]*//*[ 2 ]*/[ 84 ]/*[ 2 ]*/[ DIRECT3D11_PS_ALPHATEST_NUM ] ;
	...
	// /*[ マルチテクスチャのブレンドモード、MV1_LAYERBLEND_TYPE_TRANSLUCENT などに +1 したもの ( 0:マルチテクスチャ無し  1:αブレンド  2:加算ブレンド  3:乗算ブレンド  4:乗算ブレンド×2 ) ]*/
	// [ ピクセルシェーダーのアルファテストモード ]
	GRAPHICS_HARDWARE_DIRECT3D11_SHADERCODE_INFO Base3D_NoLighting_Normal_PS_Code/*[ 5 ]*/[ DIRECT3D11_PS_ALPHATEST_NUM ] ;
	...
	// /*[ スペキュラの有無( 0:無し 1:有り ) ]*/
	// [ ピクセルシェーダーのアルファテストモード ]
	GRAPHICS_HARDWARE_DIRECT3D11_SHADERCODE_INFO Base3D_VertexLighting_Normal_PS_Code[ 2 ]/*[ 5 ]*//*[ 2 ]*//*[ 2 ]*/[ 10 ]/*[ 2 ]*/[ DIRECT3D11_PS_ALPHATEST_NUM ] ;
} ;
...
extern	int		Graphics_D3D11_DrawSetting_SetIgnoreDrawGraphColor( int EnableFlag ) ;							// 描画時の画像の色を無視するかどうかを設定する
...
// Direct3D11 を使った描画関係
extern	int		Graphics_D3D11_DrawBillboard3D( VECTOR Pos, float cx, float cy, float Size, float Angle, IMAGEDATA *Image, IMAGEDATA *BlendImage, int TransFlag, int TurnFlag, int DrawFlag = TRUE, RECT *DrawArea = NULL ) ;	// ハードウエアアクセラレータ使用版 DrawBillboard3D
...
extern	int		Graphics_D3D11_DrawRotaGraph( int x, int y, float xf, float yf, double ExRate, double Angle, IMAGEDATA *Image, IMAGEDATA *BlendImage, int TransFlag, int TurnFlag, int IntFlag ) ;				// ハードウエアアクセラレータ使用版 DrawRotaGraph
...
extern	int		Graphics_D3D11_DrawCircle( int x, int y, int r, unsigned int Color, int FillFlag ) ;																		// ハードウエアアクセラレータ使用版 DrawCircle
extern	int		Graphics_D3D11_DrawOval( int x, int y, int rx, int ry, unsigned int Color, int FillFlag ) ;																// ハードウエアアクセラレータ使用版 DrawOval
...
// 環境依存描画設定関係
extern	int		Graphics_Hardware_D3D11_SetRenderTargetToShader_PF( int TargetIndex, int DrawScreen, int SurfaceIndex ) ;	// シェーダー描画での描画先を設定する
...
extern	int		Graphics_Hardware_D3D11_SetDrawScreen_PF( int DrawScreen, int OldScreenSurface, IMAGEDATA *NewTargetImage, IMAGEDATA *OldTargetImage, SHADOWMAPDATA *NewTargetShadowMap, SHADOWMAPDATA *OldTargetShadowMap ) ;						// 描画先画面のセット
...
extern	int		Graphics_Hardware_D3D11_LockDrawScreenBuffer_PF( RECT *LockRect, BASEIMAGE *BaseImage, int TargetScreen, IMAGEDATA *TargetImage, int TargetScreenSurface, int ReadOnly, int TargetScreenTextureNo ) ;	// 描画先バッファをロックする
...
extern	int		Graphics_Hardware_D3D11_SetScreenFlipTargetWindow_PF( HWND TargetWindow ) ;									// ScreenFlip で画像を転送する先のウインドウを設定する( NULL を指定すると設定解除 )
...
extern	int		Graphics_Hardware_D3D11_GetDrawScreenGraphBase_PF( IMAGEDATA *Image, IMAGEDATA *TargetImage, int TargetScreen, int TargetScreenSurface, int TargetScreenWidth, int TargetScreenHeight, int x1, int y1, int x2, int y2, int destX, int destY ) ;		// 描画可能画像やバックバッファから指定領域のグラフィックを取得する
