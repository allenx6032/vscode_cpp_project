...
renderer* renderer_new(asset_hndl options) {
  ...
  renderer* dr = malloc(sizeof(renderer));
  ...
  /* Lights */
  dr->dyn_lights_num = 0;
  for(int i = 0; i < RENDERER_MAX_DYN_LIGHTS; i++) {
    dr->dyn_light[i] = NULL;
  }
...
void renderer_add_dyn_light(renderer* dr, light* l) {
  ...
  if (dr->dyn_lights_num == RENDERER_MAX_DYN_LIGHTS) {
...
static void shadow_mapper_transforms(renderer* dr, int i, mat4* view, mat4* proj, float* nearclip, float* farclip) {
...
static void render_shadows_vegetation(renderer* dr, int i, instance_object* io) {
...
static void render_shadows_static(renderer* dr, int i, static_object* s) {
...
static void render_shadows_instance(renderer* dr, int i, instance_object* io) {
...
static void render_shadows_animated(renderer* dr, int i, animated_object* ao) {
...
static void render_shadows_landscape_blobtree(renderer* dr, int i, shader* shader, landscape_blobtree* lbt, terrain* terr) {
...
static void render_shadows_landscape(renderer* dr, int i, landscape* l) {
...
static void render_shadows(renderer* dr) {
...
void render_ellipsoid(renderer* dr, ellipsoid e);
void render_plane(renderer* dr, plane p);

static void render_clear(renderer* dr) {
...
static void render_cmesh(renderer* dr, cmesh* cm, mat4 world) {
...
static void render_static(renderer* dr, static_object* so) {
  ...
  if (config_bool(asset_hndl_ptr(&dr->options), "render_colmeshes")) {
    ...
    cmesh_counter = 0;
    if (!asset_hndl_isnull(&so->collision_body)) {
...
static void render_skin(renderer* dr, instance_object* io) {
...
static void render_instance(renderer* dr, instance_object* io) {
  ...
  if (config_bool(asset_hndl_ptr(&dr->options), "render_colmeshes")) {
    ...
    if (!asset_hndl_isnull(&io->collision_body)) {
  ...
  if(r->is_rigged) { error("Instance object is rigged!"); }
...
static void render_vegetation(renderer* dr, instance_object* io) {
...
static void render_axis(renderer* dr, mat4 world) {

  vec4 x_pos = mat4_mul_vec4(world, vec4_new(1,0,0,1));
  vec4 y_pos = mat4_mul_vec4(world, vec4_new(0,1,0,1));
  vec4 z_pos = mat4_mul_vec4(world, vec4_new(0,0,1,1));
...
static void render_frame(renderer* dr, frame* f, mat4 world) {
  
  for (int i = 0; i < f->joint_count; i++) {
    render_axis(dr, mat4_mul_mat4(world, f->transforms[i]));
  }
  
  for (int i = 0; i < f->joint_count; i++) {
    
    int p = f->joint_parents[i];
    if (p == -1) { continue; }
    
    vec3 j0 = mat4_mul_vec3(f->transforms[i], vec3_zero());
    vec3 j1 = mat4_mul_vec3(f->transforms[p], vec3_zero());
    vec3 col = vec3_black();
    
    float line[6] = { j0.x, j0.y, j0.z, j1.x, j1.y, j1.z };
    ...
    glPointSize(10.0);
      ...
      shader_program_enable_attribute(shader, "vPosition",  3, 3, &line);
      shader_program_enable_attribute(shader, "vColor",     3, 3, &col);
...
static void render_animated(renderer* dr, animated_object* ao) {
  ...
  if (config_bool(asset_hndl_ptr(&dr->options), "render_skeletons")) {
    render_frame(dr, ao->pose, world);
  }
  
}

static void render_landscape_blobtree(renderer* dr, shader* shader, landscape_blobtree* lbt, terrain* terr) {
...
static void render_landscape(renderer* dr, landscape* l) {
...
void render_light(renderer* dr, light* l) {
...
void render_frustum(renderer* dr, frustum f) {
...
void render_line(renderer* dr, vec3 line_start, vec3 line_end, vec3 line_color, float line_thickness) {
...
void render_point(renderer* dr, vec3 position, vec3 color, float size) {
...
void render_paint_circle(renderer* dr, mat4 axis, float radius) {
...
static void render_skies(renderer* dr) {
...
static void render_sea(renderer* dr) {
...
static void render_compose_low(renderer* dr) {
...
static void render_compose_high(renderer* dr) {
  ...
  float light_power[RENDERER_MAX_LIGHTS];
  float light_falloff[RENDERER_MAX_LIGHTS];
  vec3 light_position[RENDERER_MAX_LIGHTS];
  vec3 light_target[RENDERER_MAX_LIGHTS];
  vec3 light_diffuse[RENDERER_MAX_LIGHTS];
  vec3 light_ambient[RENDERER_MAX_LIGHTS];
  vec3 light_specular[RENDERER_MAX_LIGHTS];
...
static void render_compose(renderer* dr) {
...
static void render_particles(renderer* dr) {
...
static void render_tonemap(renderer* dr) {
...
static void render_post1(renderer* dr) {
