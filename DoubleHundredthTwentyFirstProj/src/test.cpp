...
{
	intptr_t ids[] = { 1 };
	float xs[] = { static_cast<float>(s_mouseX) };
	float ys[] = { static_cast<float>(s_mouseY) };
...
int main(void)
{
	...
	float scale = fmin((rt.right - rt.left) / WIN_WIDTH, (rt.bottom - rt.top - titleBarHeight - 14) / WIN_HEIGHT);
	...
	{
		printf("glfwWindowShouldClose while!");
		/* Render Frame */
