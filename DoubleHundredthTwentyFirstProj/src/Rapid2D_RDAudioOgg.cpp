...
int decodeOgg(RDData *data, RDPcmData *pcm)
{
    ...
    ogg_buffer buffer;
    buffer.curPtr = buffer.filePtr = data->getBuffer();
    ...
    pcm->size = ov_pcm_total(&ov, -1) * vi->channels * 2;
    pcm->data = (RD_uchar *)malloc(pcm->size);
    ...
    long status = 0;
    while (size < pcm->size) {
        status = ov_read(&ov, (char *)(pcm->data + size), pcm->size - size, 0, 2, 1, &section);
        if (status > 0) {
            size += status;
        } else if (status < 0) {
            return -1;
