//-----------------------------------------------------------------
// Game Engine Object
// C++ Source - GameEngine.cpp - version v7_01
//-----------------------------------------------------------------
...
DWORD GameEngine::KeybThreadProc()
{
	while (m_KeybRunning)
	{
		GetFocus();																					// for some reason the next GetFocus() doesn't work without this GetFocus() call
		if (m_KeyListPtr != nullptr && GetFocus() == m_Window)
		{
			int count{};
