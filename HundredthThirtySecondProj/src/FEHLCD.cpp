...
void FEHLCD::WriteAt(std::string str, int x, int y)
{
    WriteAt(str.c_str(), x, y);
}
...
void FEHLCD::WriteLine(std::string str)
{
    WriteLine(str.c_str());
}
...
unsigned int FEHLCD::ScaleColor(unsigned int color, float scale)
{
    unsigned int r = (color >> 16) % 0x100;
    unsigned int g = (color >> 8) % 0x100;
    unsigned int b = color % 0x100;
    
    r = ((unsigned int)((float)r*scale));
    r = r >= 0x100 ? 0xff : r;

    g = ((unsigned int)((float)g*scale));
    g = g >= 0x100 ? 0xff : g;

    b = ((unsigned int)((float)b*scale));
    b = b >= 0x100 ? 0xff : b;
