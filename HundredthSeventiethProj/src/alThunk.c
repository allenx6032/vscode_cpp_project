...
static CRITICAL_SECTION_T g_ThunkLock;

void alThunkInit(void)
{
    InitializeCriticalSectionT(&g_ThunkLock);
    g_ThunkArraySize = 1;
...
void alThunkExit(void)
{
    ...
    g_ThunkArray = NULL;
    g_ThunkArraySize = 0;
    DeleteCriticalSectionT(&g_ThunkLock);
}
...
ALuint alThunkAddEntry(ALvoid *ptr)
{
    ...
    EnterCriticalSectionT(&g_ThunkLock);
    ...
    if(index == g_ThunkArraySize)
    {
        ...
        if(!NewList)
        {
            LeaveCriticalSectionT(&g_ThunkLock);
