...
NVGcontext* nvgCreateInternal(NVGparams* params) {
  ...
  ctx->fontImages[0] = ctx->params.renderCreateTexture(
      ctx->params.userPtr, NVG_TEXTURE_ALPHA, fontParams.width,
...
void nvgBeginFrame(NVGcontext* ctx, float windowWidth, float windowHeight,
  ...
  /*	printf("Tris: draws:%d  fill:%d  stroke:%d  text:%d  TOT:%d\n",
		  ctx->drawCallCount, ctx->fillTriCount, ctx->strokeTriCount,
     ctx->textTriCount,
...
int nvgCreateImageMem(NVGcontext* ctx, int imageFlags, unsigned char* data,
		      int ndata) {
  int		 w, h, n, image;
...
int nvgCreateImageRGBA(NVGcontext* ctx, int w, int h, int imageFlags,
  ...
  return ctx->params.renderCreateTexture(ctx->params.userPtr, NVG_TEXTURE_RGBA,
					 w, h, imageFlags, data);
}
...
NVGpaint nvgImagePattern(NVGcontext* ctx, float cx, float cy, float w, float h,
...
static void nvg__isectRects(float* dst, float ax, float ay, float aw, float ah,
...
void nvgIntersectScissor(NVGcontext* ctx, float x, float y, float w, float h) {
  ...
  // Intersect rects.
  nvg__isectRects(rect, pxform[4] - tex, pxform[5] - tey, tex * 2, tey * 2, x,
		  y, w, h);
...
static float nvg__distPtSeg(float x, float y, float px, float py, float qx,
			    float qy) {
...
static void nvg__appendCommands(NVGcontext* ctx, float* vals, int nvals) {
  ...
  while (i < nvals) {
    ...
    switch (cmd) {
    case NVG_MOVETO:
      nvgTransformPoint(&vals[i + 1], &vals[i + 2], state->xform, vals[i + 1],
			vals[i + 2]);
      i += 3;
      break;
    case NVG_LINETO:
    ...
    case NVG_BEZIERTO:
      nvgTransformPoint(&vals[i + 1], &vals[i + 2], state->xform, vals[i + 1],
			vals[i + 2]);
      nvgTransformPoint(&vals[i + 3], &vals[i + 4], state->xform, vals[i + 3],
			vals[i + 4]);
      nvgTransformPoint(&vals[i + 5], &vals[i + 6], state->xform, vals[i + 5],
			vals[i + 6]);
      i += 7;
      break;
    case NVG_CLOSE:
...
static float nvg__triarea2(float ax, float ay, float bx, float by, float cx,
			   float cy) {
  float abx = bx - ax;
  float aby = by - ay;
  float acx = cx - ax;
...
static void nvg__tesselateBezier(NVGcontext* ctx, float x1, float y1, float x2,
  ...
  /*	if (nvg__absf(x1+x3-x2-x2) + nvg__absf(y1+y3-y2-y2) +
     nvg__absf(x2+x4-x3-x3) + nvg__absf(y2+y4-y3-y3) < ctx->tessTol) {
		  nvg__addPoint(ctx, x4, y4, type);
		  return;
	  }*/
  ...
  nvg__tesselateBezier(ctx, x1, y1, x12, y12, x123, y123, x1234, y1234,
		       level + 1, 0);
  nvg__tesselateBezier(ctx, x1234, y1234, x234, y234, x34, y34, x4, y4,
		       level + 1, type);
}
...
static void nvg__flattenPaths(NVGcontext* ctx) {
  ...
  while (i < ctx->ncommands) {
	...
	p   = &ctx->commands[i + 5];
	nvg__tesselateBezier(ctx, last->x, last->y, cp1[0], cp1[1], cp2[0],
			     cp2[1], p[0], p[1], 0, NVG_PT_CORNER);
      }
      i += 7;
      break;
    case NVG_CLOSE:
...
static NVGvertex* nvg__roundJoin(NVGvertex* dst, NVGpoint* p0, NVGpoint* p1,
  ...
  if (p1->flags & NVG_PT_LEFT) {
    ...
    nvg__chooseBevel(p1->flags & NVG_PR_INNERBEVEL, p0, p1, lw, &lx0, &ly0,
		     &lx1, &ly1);
    a0 = atan2f(-dly0, -dlx0);
    a1 = atan2f(-dly1, -dlx1);
    if (a1 > a0)
    ...
    nvg__chooseBevel(p1->flags & NVG_PR_INNERBEVEL, p0, p1, -rw, &rx0, &ry0,
		     &rx1, &ry1);
    a0 = atan2f(dly0, dlx0);
    a1 = atan2f(dly1, dlx1);
    if (a1 < a0)
...
static NVGvertex* nvg__buttCapStart(NVGvertex* dst, NVGpoint* p, float dx,
				    float dy, float w, float d, float aa,
				    float u0, float u1) {
  float px  = p->x - dx * d;
  float py  = p->y - dy * d;
  float dlx = dy;
...
static NVGvertex* nvg__buttCapEnd(NVGvertex* dst, NVGpoint* p, float dx,
				  float dy, float w, float d, float aa,
				  float u0, float u1) {
  float px  = p->x + dx * d;
  float py  = p->y + dy * d;
...
static NVGvertex* nvg__roundCapStart(NVGvertex* dst, NVGpoint* p, float dx,
...
static NVGvertex* nvg__roundCapEnd(NVGvertex* dst, NVGpoint* p, float dx,
				   float dy, float w, int ncap, float aa,
				   float u0, float u1) {
  int	i;
  float px  = p->x;
...
static int nvg__expandStroke(NVGcontext* ctx, float w, float fringe,
			     int lineCap, int lineJoin, float miterLimit) {
  NVGpathCache* cache = ctx->cache;
  NVGvertex*	verts;
  NVGvertex*	dst;
  int		cverts, i, j;
  ...
  int		ncap = nvg__curveDivs(
	w, NVG_PI, ctx->tessTol); // Calculate divisions per half circle.
...
static int nvg__expandFill(NVGcontext* ctx, float w, int lineJoin,
  ...
  for (i = 0; i < cache->npaths; i++) {
	...
	if (p1->flags & NVG_PT_BEVEL) {
	  ...
	  nvg__vset(dst, p1->x + (p1->dmx * woff), p1->y + (p1->dmy * woff),
		    0.5f, 1);
	  dst++;
	}
	p0 = p1++;
...
void nvgArcTo(NVGcontext* ctx, float x1, float y1, float x2, float y2,
  ...
  if (nvg__cross(dx0, dy0, dx1, dy1) > 0.0f) {
    ...
    // a0/NVG_PI*180.0f, a1/NVG_PI*180.0f);
  } else {
    cx	= x1 + dx0 * d + -dy0 * radius;
    cy	= y1 + dy0 * d + dx0 * radius;
    a0	= nvg__atan2f(-dx0, dy0);
...
void nvgArc(NVGcontext* ctx, float cx, float cy, float r, float a0, float a1,
  ...
  ndivs =
      nvg__maxi(1, nvg__mini((int)(nvg__absf(da) / (NVG_PI * 0.5f) + 0.5f), 5));
...
void nvgRect(NVGcontext* ctx, float x, float y, float w, float h) {
  float vals[] = {NVG_MOVETO, x,	  y,	    NVG_LINETO, x,
		  y + h,      NVG_LINETO, x + w,    y + h,	NVG_LINETO,
		  x + w,      y,	  NVG_CLOSE};
  nvg__appendCommands(ctx, vals, NVG_COUNTOF(vals));
}
...
void nvgStroke(NVGcontext* ctx) {
  ...
  if (ctx->params.edgeAntiAlias && state->shapeAntiAlias)
    nvg__expandStroke(ctx, strokeWidth * 0.5f, ctx->fringeWidth, state->lineCap,
...
int nvgAddFallbackFont(NVGcontext* ctx, const char* baseFont,
  ...
  return nvgAddFallbackFontId(ctx, nvgFindFont(ctx, baseFont),
			      nvgFindFont(ctx, fallbackFont));
}
...
static float nvg__getFontScale(NVGstate* state) {
  return nvg__minf(nvg__quantize(nvg__getAverageScale(state->xform), 0.01f),
		   4.0f);
}
...
float nvgText(NVGcontext* ctx, float x, float y, const char* string,
  ...
  while (fonsTextIterNext(ctx->fs, &iter, &q)) {
    ...
    prevIter = iter;
    // Transform corners.
    nvgTransformPoint(&c[0], &c[1], state->xform, q.x0 * invscale,
		      q.y0 * invscale);
    nvgTransformPoint(&c[2], &c[3], state->xform, q.x1 * invscale,
		      q.y0 * invscale);
    nvgTransformPoint(&c[4], &c[5], state->xform, q.x1 * invscale,
		      q.y1 * invscale);
    nvgTransformPoint(&c[6], &c[7], state->xform, q.x0 * invscale,
		      q.y1 * invscale);
    // Create triangles
    if (nverts + 6 <= cverts) {
...
void nvgTextBox(NVGcontext* ctx, float x, float y, float breakRowWidth,
  ...
  int	     haling =
      state->textAlign & (NVG_ALIGN_LEFT | NVG_ALIGN_CENTER | NVG_ALIGN_RIGHT);
  int	valign = state->textAlign & (NVG_ALIGN_TOP | NVG_ALIGN_MIDDLE |
  ...
  while (
	...
	nvgText(ctx, x + breakRowWidth * 0.5f - row->width * 0.5f, y,
		row->start, row->end);
...
int nvgTextGlyphPositions(NVGcontext* ctx, float x, float y, const char* string,
  ...
  while (fonsTextIterNext(ctx->fs, &iter, &q)) {
	...
	nvg__allocTextAtlas(ctx)) { // can not retrieve glyph?
      iter = prevIter;
      fonsTextIterNext(ctx->fs, &iter, &q); // try again
    }
    prevIter		 = iter;
...
int nvgTextBreakLines(NVGcontext* ctx, const char* string, const char* end,
  ...
  while (fonsTextIterNext(ctx->fs, &iter, &q)) {
	...
	if (((ptype == NVG_CHAR || ptype == NVG_CJK_CHAR) &&
	     type == NVG_SPACE) ||
	    type == NVG_CJK_CHAR) {
	  breakEnd   = iter.str;
	  breakWidth = rowWidth;
	...
	if ((ptype == NVG_SPACE &&
	     (type == NVG_CHAR || type == NVG_CJK_CHAR)) ||
	    type == NVG_CJK_CHAR) {
	  wordStart  = iter.str;
	  wordStartX = iter.x;
	  wordMinX   = q.x0 - rowStartX;
