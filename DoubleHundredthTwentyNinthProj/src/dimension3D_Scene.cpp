...
namespace dim
{
	...
	void Scene::display()
	{
		...
		ImGui::Image((ImTextureID)frame_buffer.get_texture().get_id(), ImVec2(static_cast<float>(temp.x), static_cast<float>(temp.y)), ImVec2(0.f, 1.f), ImVec2(1.f, 0.f));
		ImGui::SetCursorPos(ImVec2(8.f, 27.f));
		ImGui::Image((ImTextureID)render_texture.getTexture().getNativeHandle(),
