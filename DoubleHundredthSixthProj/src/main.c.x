...
game_state_t* init() {
    ...
    game->window = SDL_CreateWindow("Rasterizer", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, WIDTH, HEIGHT,SDL_WINDOW_SHOWN|SDL_WINDOW_RESIZABLE);
    if (game->window  == NULL) {
	...
	if (game->renderer == NULL) {
		fprintf(stderr, "ERROR: Renderer couldn't be createdr: %s.\n", SDL_GetError());
		exit(-1);
	}
...
void render(game_state_t* game) {
    ...
    //     exit(-1);
    // }
    // if (!SDL_CreateTexture(game->renderer, game->texture, NULL, NULL)) {
    //     fprintf(stderr, "ERROR: Texture couldn't be rendered: %s\n", SDL_GetError());
    //     exit(-1);
    // }
