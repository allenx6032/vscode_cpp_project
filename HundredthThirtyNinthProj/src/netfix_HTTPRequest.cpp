...
#include "netfix_HTTPRequest.h"
#include "netfix_Exception.h"
#include "netfix_Constants.h"
#include "netfix_Files.h"
...
std::string HTTPRequest::secureRequest(std::string content, SOCKET *sock) {
    ...
    */
    ...
    WOLFSSL *conn  = wolfSSL_new(ssl_ctx);
    wolfSSL_set_fd(conn, *sock);

    WOLFSSL* ssl;
    if((ssl = wolfSSL_new(ssl_ctx))==NULL)  {
        std::cout << "failsdsd" << std::endl;
        int errcode = wolfSSL_get_error(conn, errorCode);
