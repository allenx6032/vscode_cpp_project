...
#include "quick_PlayerWin.h"

#include "lua_cocos2dx_CCLuaEngine.h"
#include "quick_PlayerLuaCore.h"
#include "quick_PlayerLuaCoreManual.hpp"

// for network
#include "quick_cocos2dx_extra.h"
#include "quick_CCHTTPRequest.h"
...
void PlayerWin::loadLuaConfig()
{
    ...
    // load player.lua
    string playerCoreFilePath = quickRootPath + "welcome/src/player.lua";
...
int PlayerWin::run()
{
    ...
    g_oldWindowProc = (WNDPROC)SetWindowLong(_hwnd, 
    GWLP_WNDPROC // GWL_WNDPROC 32bit GWLP_WNDPROC 64bit
    , (LONG)(size_t)PlayerWin::windowProc);
...
std::string PlayerWin::removeFilename(const std::string& path) 
{
    // 统一处理路径分隔符为 '/'
