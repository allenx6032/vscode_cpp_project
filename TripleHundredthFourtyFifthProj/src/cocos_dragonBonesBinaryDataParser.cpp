...
DragonBonesData* BinaryDataParser::parseDragonBonesData(const char* rawData, float scale)
{
    ...
    // new buff to make Parse happy
    void *buff = malloc(headerLength + 1);
    memset(buff, 0, headerLength + 1);
    memcpy(buff, headerBytes, headerLength);
    rapidjson::Document document;
    document.Parse((const char *)buff);
    free(buff);
