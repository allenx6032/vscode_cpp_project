...
function WelcomeScene:createSamples(node)
	self.samples = dofile(cc.player.quickRootPath .. "samples/samples.lua")
    ...
    for i,v in ipairs(self.samples) do
		...
		local image = cc.player.quickRootPath .. v.path .. "/logo.png"
...
function WelcomeScene:openDemoWithPath(path)
    ...
    local configPath = cc.player.quickRootPath .. path .. "/src/config.lua"
    local projectConfig = ProjectConfig:new()
    -- projectConfig:setProjectDir(cc.player.quickRootPath .. "quick/" .. path)
    projectConfig:setProjectDir(cc.player.quickRootPath .. path)
