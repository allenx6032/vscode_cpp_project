  ...
  if(!(newstate & KEEP_RECV_PAUSE)) {
    ...
    if(data->state.tempcount) {
      ...
      for(i = 0; i < data->state.tempcount; i++) {
        ...
        curlx_dyn_init(&data->state.tempwrite[i].b, DYN_PAUSE_BUFFER);
      }
      ...
      for(i = 0; i < count; i++) {
        ...
        if(!result)
          result = Curl_client_write(data, writebuf[i].type,
                                     curlx_dyn_ptr(&writebuf[i].b),
                                     curlx_dyn_len(&writebuf[i].b));
        curlx_dyn_free(&writebuf[i].b);
      }
