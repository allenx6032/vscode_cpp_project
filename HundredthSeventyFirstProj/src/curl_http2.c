...
{
  if(http) {
    curlx_dyn_free(&http->header_recvbuf);
...
static struct Curl_easy *duphandle(struct Curl_easy *data)
{
  ...
  if(second) {
    ...
    if(!http) {
      ...
      curlx_dyn_init(&http->header_recvbuf, DYN_H2_HEADERS);
                        ...
                        const nghttp2_push_promise *frame)
{
  ...
  if(data->multi->push_cb) {
    ...
    curlx_dyn_init(&newstream->header_recvbuf, DYN_H2_HEADERS);
    curlx_dyn_init(&newstream->trailer_recvbuf, DYN_H2_TRAILERS);
  }
  else {
                         ...
                         void *userp)
{
  ...
  case NGHTTP2_HEADERS:
    ...
    result = curlx_dyn_addn(&stream->header_recvbuf, STRCONST("\r\n"));
    if(result)
      return NGHTTP2_ERR_CALLBACK_FAILURE;

    left = curlx_dyn_len(&stream->header_recvbuf) -
      stream->nread_header_recvbuf;
    ncopy = CURLMIN(stream->len, left);

    memcpy(&stream->mem[stream->memlen],
           curlx_dyn_ptr(&stream->header_recvbuf) +
           stream->nread_header_recvbuf,
           ncopy);
                     ...
                     void *userp)
{
  ...
  if(stream->bodystarted) {
    ...
    H2BUGF(infof(data_s, "h2 trailer: %.*s: %.*s", namelen, name, valuelen,
                 value));
    result = curlx_dyn_addf(&stream->trailer_recvbuf,
                           "%.*s: %.*s\r\n", namelen, name,
                           valuelen, value);
    if(result)
      return NGHTTP2_ERR_CALLBACK_FAILURE;
  ...
  if(namelen == sizeof(H2H3_PSEUDO_STATUS) - 1 &&
    ...
    if(result)
      return NGHTTP2_ERR_CALLBACK_FAILURE;
    result = curlx_dyn_addn(&stream->header_recvbuf, STRCONST("HTTP/2 "));
    if(result)
      return NGHTTP2_ERR_CALLBACK_FAILURE;
    result = curlx_dyn_addn(&stream->header_recvbuf, value, valuelen);
    if(result)
      return NGHTTP2_ERR_CALLBACK_FAILURE;
  ...
  result = curlx_dyn_addn(&stream->header_recvbuf, name, namelen);
  if(result)
    return NGHTTP2_ERR_CALLBACK_FAILURE;
  result = curlx_dyn_addn(&stream->header_recvbuf, STRCONST(": "));
  if(result)
...
void Curl_http2_done(struct Curl_easy *data, bool premature)
{
  ...
  /* there might be allocated resources done before this got the 'h2' pointer
     setup */
  curlx_dyn_free(&http->header_recvbuf);
  curlx_dyn_free(&http->trailer_recvbuf);
                                         ...
                                         struct HTTP *stream, CURLcode *err)
{
  ...
  if(curlx_dyn_len(&stream->trailer_recvbuf)) {
    char *trailp = curlx_dyn_ptr(&stream->trailer_recvbuf);
    char *lf;
                          ...
                          char *mem, size_t len, CURLcode *err)
{
  ...
  if(stream->bodystarted &&
     stream->nread_header_recvbuf < curlx_dyn_len(&stream->header_recvbuf)) {
    /* If there is header data pending for this stream to return, do that */
    size_t left =
      curlx_dyn_len(&stream->header_recvbuf) - stream->nread_header_recvbuf;
    size_t ncopy = CURLMIN(len, left);
    memcpy(mem, curlx_dyn_ptr(&stream->header_recvbuf) +
           stream->nread_header_recvbuf, ncopy);
    stream->nread_header_recvbuf += ncopy;
                          ...
                          struct connectdata *conn)
{
  ...
  curlx_dyn_init(&stream->header_recvbuf, DYN_H2_HEADERS);
  curlx_dyn_init(&stream->trailer_recvbuf, DYN_H2_TRAILERS);
  ...
  if(result) {
    curlx_dyn_free(&stream->header_recvbuf);
    return result;
  }
