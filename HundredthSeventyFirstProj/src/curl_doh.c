...
doh_write_cb(const void *contents, size_t size, size_t nmemb, void *userp)
{
  ...
  if(curlx_dyn_addn(mem, contents, realsize))
    return 0;
                         ...
                         struct curl_slist *headers)
{
  ...
  p->dnstype = dnstype;
  curlx_dyn_init(&p->serverdoh, DYN_DOH_RESPONSE);
  ...
  *dnsp = NULL; /* defaults to no response */
  ...
  if(!dohp->probe[DOH_PROBE_SLOT_IPADDR_V4].easy &&
    ...
    for(slot = 0; slot < DOH_PROBE_SLOTS; slot++) {
      ...
      if(!p->dnstype)
        continue;
      rc[slot] = doh_decode(curlx_dyn_uptr(&p->serverdoh),
                            curlx_dyn_len(&p->serverdoh),
                            p->dnstype,
                            &de);
      curlx_dyn_free(&p->serverdoh);
      if(rc[slot]) {
