...
//                : D_CTransInPlaceFilter( TEXT("SampleGrabber"), (D_IUnknown*) pOuter, 
                : D_CTransInPlaceFilter( "SampleGrabber", (D_IUnknown*) pOuter, 
...
HRESULT D_SoundConvertMFByteStream::Initialize( void )
{
	...
	ASyncThreadEvent = CreateEvent( NULL, FALSE, FALSE, NULL ) ;
	if( ASyncThreadEvent == NULL )
	{
		return E_FAIL;
	}

	ASyncThread = CreateThread(
		NULL,
		0,
...
DWORD WINAPI D_SoundConvertMFByteStream::AsyncThread( void *pContext )
{
	...
	for(;;)
	{
		for(;;)
		{
			Result = WaitForSingleObject( pThis->ASyncThreadEvent, 16 ) ;
