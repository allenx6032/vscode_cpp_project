...
#include "NintendoSwitch/DxGraphicsNSW.h"
...
// アンチエイリアス付きの線描画用のテクスチャのサイズ
#define LINE_TEX_SIZE				(8)

// アンチエイリアス付きの線矩形描画用のテクスチャのサイズ
...
\
	...
	{\
		FColor = ( 0xff000000 ) | ( ( 0xff - r ) << 16 ) | ( ( 0xff - g ) << 8 ) | ( 0xff - b ) ;\
	}\
	else\
	{\
...
// 16x16 の アンチエイリアス付きの線矩形描画用の画像情報
...
static int  Graphics_Software_DrawRotaGraphFast(    float x, float y, float  ExRate, float  Angle,                                  IMAGEDATA *Image, int TransFlag, int TurnFlag ) ;		// ソフトウエアレンダリング版 DrawRotaGraphFast
...
static int  Graphics_Software_DrawCircle(           int x, int y, int r,                                            unsigned int Color, int FillFlag, int Rx_One_Minus = FALSE, int Ry_One_Minus = FALSE ) ;		// ソフトウエアレンダリング版 DrawCircle
static int  Graphics_Software_DrawOval(             int x, int y, int rx, int ry,                                   unsigned int Color, int FillFlag, int Rx_One_Minus = FALSE, int Ry_One_Minus = FALSE ) ;		// ソフトウエアレンダリング版 DrawOval
...
__inline static int Graphics_Image_CheckBlendGraphSize( IMAGEDATA *GraphData )
{
	...
	// ハードウエアを使用する場合は大きさの制限無し
	if( GSYS.Setting.ValidHardware )
		return 0 ;

	// ソフトウエアの場合はブレンド画像座標モードは DX_BLENDGRAPH_POSMODE_DRAWGRAPH のみ対応
...
static void Graphics_Image_DefaultRestoreGraphFunction( void )
{
	...
	DXST_LOGFILE_ADDUTF16LE( "\xb0\x30\xe9\x30\xd5\x30\xa3\x30\xc3\x30\xaf\x30\x92\x30\xa9\x5f\x30\x5e\x57\x30\x7e\x30\x59\x30\x0a\x00\x00"/*@ L"グラフィックを復帰します\n" @*/ ) ;
	...
	if( HandleManageArray[ DX_HANDLETYPE_GRAPH ].InitializeFlag )
	{
		...
		//			DXST_LOGFILEFMT_ADDW(( L"i:%d フルパス:%s がロードできませんでした", i, Image->FilePath )) ;
		//			DXST_LOGFILEFMT_ADDW(( L"ファイル:%s をロードします", Image->FilePath->String )) ;
		//			DXST_LOGFILEFMT_ADDW(( L"反転フラグ:%d  ファイル:%s をロードします", Image->ReadBase->ReverseFlag, Image->ReadBase->FileName )) ;
		...
		R1 :
			...
			if( Image->ReadBase->MemImage )
			{
					...
					Image->ReadBase->ConvertPremultipliedAlpha != MemBackImage->ReadBase->ConvertPremultipliedAlpha )
				{
					...
					if( LoadHr == -1 )
					{
						DXST_LOGFILEFMT_ADDUTF16LE(( "\xe1\x30\xe2\x30\xea\x30\x4b\x30\x89\x30\x6e\x30\x3b\x75\xcf\x50\xc7\x30\xfc\x30\xbf\x30\x6e\x30\xed\x30\xfc\x30\x4c\x30\x67\x30\x4d\x30\x7e\x30\x5b\x30\x93\x30\x67\x30\x57\x30\x5f\x30\x00"/*@ L"メモリからの画像データのローができませんでした" @*/ )) ;
						goto R2 ;
					}
	...
	DXST_LOGFILE_ADDUTF16LE( "\xb0\x30\xe9\x30\xd5\x30\xa3\x30\xc3\x30\xaf\x30\x6e\x30\xa9\x5f\x30\x5e\x4c\x30\x8c\x5b\x86\x4e\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"グラフィックの復帰が完了しました\n" @*/ ) ;
}
...
static int Graphics_Software_Initialize( void )
{
	...
	{
		return DXST_LOGFILE_ADDUTF16LE( "\xee\x4e\x3b\x75\x62\x97\x28\x75\x6e\x30\x20\x00\x4d\x00\x45\x00\x4d\x00\x49\x00\x4d\x00\x47\x00\x20\x00\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"仮画面用の MEMIMG の作成に失敗しました\n" @*/ ) ;
	}
	...
	if( MakeMemImgScreen(
		...
		GSYS.Screen.MainScreenColorBitDepth == 16 ? 0 : 1 ) < 0 )
	{
		return DXST_LOGFILE_ADDUTF16LE( "\xdc\x88\xa9\x52\x28\x75\x6e\x30\x20\x00\x4d\x00\x45\x00\x4d\x00\x49\x00\x4d\x00\x47\x00\x20\x00\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"補助用の MEMIMG の作成に失敗しました\n" @*/ ) ;
	}
...
static int  Graphics_Software_DrawRotaGraph( float x, float y, double ExRate, double Angle, IMAGEDATA *Image, int TransFlag, int TurnFlag )
{
	DrawRotationMemImg( GSYS.SoftRender.TargetMemImg, &Image->Soft.MemImg,
		_FTOL( x ), _FTOL( y ), ( float )Angle, ( float )ExRate, ( float )ExRate, TransFlag, TurnFlag, GSYS.SoftRender.BlendMemImg ) ;

	// 終了
	return 0 ;
}

// ソフトウエアレンダリング版 DrawRotaGraphFast
...
{
	DrawRotationMemImg( GSYS.SoftRender.TargetMemImg, &Image->Soft.MemImg,
		_FTOL( x ), _FTOL( y ), Angle, ExRate, ExRate, TransFlag, TurnFlag, GSYS.SoftRender.BlendMemImg ) ;
...
static int DrawCircleLineAA( float x, float y, float r, int posnum, unsigned int Color, float LineThickness )
{
	...
	float Sin, Cos ;
	static float LineUV[ 4 ] = { 0.0f, 3.0f / LINE_TEX_SIZE, 5.0f / LINE_TEX_SIZE, 1.0f };
	float PosX[ CIRCLE_MAX_POSNUM + 1 ][ 4 ];
	float PosY[ CIRCLE_MAX_POSNUM + 1 ][ 4 ];
...
#define SETDRAWRECTCODE\
	...
	DRAWRECT_DRAWAREA_CLIP
extern	int NS_DrawOval_Rect( int x1, int y1, int x2, int y2, unsigned int Color, int FillFlag )
{
	int Ret = -1 ;
	int Flag ;
	int w, h ;
	...
	{
		DRAW_DEF(
			Graphics_Hardware_DrawCircle_PF( x, y, rx, Color, FillFlag, Rx_One_Minus, Ry_One_Minus ),
			Graphics_Software_DrawCircle(    x, y, rx, Color, FillFlag, Rx_One_Minus, Ry_One_Minus ),
			SETDRAWRECTCODE,
			Ret,
			Flag
		)
	...
	{
		DRAW_DEF(
			Graphics_Hardware_DrawOval_PF( x, y, rx, ry, Color, FillFlag, Rx_One_Minus, Ry_One_Minus ),
			Graphics_Software_DrawOval(    x, y, rx, ry, Color, FillFlag, Rx_One_Minus, Ry_One_Minus ),
			SETDRAWRECTCODE,
			Ret,
			Flag
		)
...
static int DrawOvalFillAA( float x, float y, float rx, float ry, int posnum, unsigned int Color )
{
	...
	float Sin, Cos ;
	static float LineUV[ 4 ] = { 0.0f, 3.0f / LINE_TEX_SIZE, 5.0f / LINE_TEX_SIZE, 1.0f };
	float PosX[ CIRCLE_MAX_POSNUM + 1 ][ 2 ];
	float PosY[ CIRCLE_MAX_POSNUM + 1 ][ 2 ];
...
static int DrawRoundRectLineAA( float x1, float y1, float x2, float y2, float rx, float ry, int posnum, unsigned int Color, float LineThickness )
{
	...
	if( LineThickness <= 1.0f )
	{
		...
		Vertex[ 0 ].x = PosX[ 0 ][ 0          ][ 0 ] ;	Vertex[ 0 ].y = PosY[ 0 ][ 0          ][ 0 ] ;
		Vertex[ 1 ].x = PosX[ 3 ][ posnum - 1 ][ 0 ] ;	Vertex[ 1 ].y = PosY[ 3 ][ posnum - 1 ][ 0 ] ;
		Vertex[ 2 ].x = PosX[ 0 ][ 0          ][ 1 ] ;	Vertex[ 2 ].y = PosY[ 0 ][ 0          ][ 1 ] ;
		Vertex[ 3 ].x = PosX[ 3 ][ posnum - 1 ][ 1 ] ;	Vertex[ 3 ].y = PosY[ 3 ][ posnum - 1 ][ 1 ] ;
	...
	else
	{
		...
		for( j = 0 ; j < 3 ; j ++ )
		{
			Vertex[ 0 ].x = PosX[ 0 ][ 0          ][ j + 0 ] ;	Vertex[ 0 ].y = PosY[ 0 ][ 0          ][ j + 0 ] ;
			Vertex[ 1 ].x = PosX[ 3 ][ posnum - 1 ][ j + 0 ] ;	Vertex[ 1 ].y = PosY[ 3 ][ posnum - 1 ][ j + 0 ] ;
			Vertex[ 2 ].x = PosX[ 0 ][ 0          ][ j + 1 ] ;	Vertex[ 2 ].y = PosY[ 0 ][ 0          ][ j + 1 ] ;
			Vertex[ 3 ].x = PosX[ 3 ][ posnum - 1 ][ j + 1 ] ;	Vertex[ 3 ].y = PosY[ 3 ][ posnum - 1 ][ j + 1 ] ;
...
static int DrawRoundRectFillAA( float x1, float y1, float x2, float y2, float rx, float ry, int posnum, unsigned int Color )
{
	...
	Vertex[ 2 ].x = PosX[ 1 ][ 0          ][ 1 ] ;	Vertex[ 2 ].y = PosY[ 1 ][ 0          ][ 1 ] ;
	Vertex[ 3 ].x = PosX[ 0 ][ posnum - 1 ][ 1 ] ;	Vertex[ 3 ].y = PosY[ 0 ][ posnum - 1 ][ 1 ] ;
	Vertex[ 4 ].x = PosX[ 1 ][ 0          ][ 1 ] ;	Vertex[ 4 ].y = PosY[ 1 ][ 0          ][ 1 ] ;
	Vertex[ 5 ].x = PosX[ 0 ][ posnum - 1 ][ 0 ] ;	Vertex[ 5 ].y = PosY[ 0 ][ posnum - 1 ][ 0 ] ;
	...
	Vertex[ 0 ].x = PosX[ 1 ][ 0          ][ 1 ] ;	Vertex[ 0 ].y = PosY[ 1 ][ 0          ][ 1 ] ;
	Vertex[ 1 ].x = PosX[ 0 ][ posnum - 1 ][ 1 ] ;	Vertex[ 1 ].y = PosY[ 0 ][ posnum - 1 ][ 1 ] ;
	...
	Vertex[ 3 ].u = LineUV[ 1 ] ;	Vertex[ 3 ].v = LineUV[ 1 ] ;
	Vertex[ 4 ].u = LineUV[ 1 ] ;	Vertex[ 4 ].v = LineUV[ 1 ] ;
	Vertex[ 5 ].u = LineUV[ 1 ] ;	Vertex[ 5 ].v = LineUV[ 1 ] ;
	...
	Vertex[ 2 ].x = PosX[ 2 ][ 0          ][ 1 ] ;	Vertex[ 2 ].y = PosY[ 2 ][ 0          ][ 1 ] ;
	Vertex[ 3 ].x = PosX[ 1 ][ posnum - 1 ][ 1 ] ;	Vertex[ 3 ].y = PosY[ 1 ][ posnum - 1 ][ 1 ] ;
	Vertex[ 4 ].x = PosX[ 2 ][ 0          ][ 1 ] ;	Vertex[ 4 ].y = PosY[ 2 ][ 0          ][ 1 ] ;
	Vertex[ 5 ].x = PosX[ 1 ][ posnum - 1 ][ 0 ] ;	Vertex[ 5 ].y = PosY[ 1 ][ posnum - 1 ][ 0 ] ;
	...
	Vertex[ 0 ].x = PosX[ 2 ][ 0          ][ 1 ] ;	Vertex[ 0 ].y = PosY[ 2 ][ 0          ][ 1 ] ;
	Vertex[ 1 ].x = PosX[ 1 ][ posnum - 1 ][ 1 ] ;	Vertex[ 1 ].y = PosY[ 1 ][ posnum - 1 ][ 1 ] ;
	...
	Vertex[ 5 ].x = PosX[ 1 ][ posnum - 1 ][ 1 ] ;	Vertex[ 5 ].y = PosY[ 1 ][ posnum - 1 ][ 1 ] ;
	...
	Vertex[ 2 ].x = PosX[ 3 ][ 0          ][ 1 ] ;	Vertex[ 2 ].y = PosY[ 3 ][ 0          ][ 1 ] ;
	Vertex[ 3 ].x = PosX[ 2 ][ posnum - 1 ][ 1 ] ;	Vertex[ 3 ].y = PosY[ 2 ][ posnum - 1 ][ 1 ] ;
	Vertex[ 4 ].x = PosX[ 3 ][ 0          ][ 1 ] ;	Vertex[ 4 ].y = PosY[ 3 ][ 0          ][ 1 ] ;
	Vertex[ 5 ].x = PosX[ 2 ][ posnum - 1 ][ 0 ] ;	Vertex[ 5 ].y = PosY[ 2 ][ posnum - 1 ][ 0 ] ;
	...
	Vertex[ 0 ].x = PosX[ 3 ][ 0          ][ 1 ] ;	Vertex[ 0 ].y = PosY[ 3 ][ 0          ][ 1 ] ;
	Vertex[ 1 ].x = PosX[ 2 ][ posnum - 1 ][ 1 ] ;	Vertex[ 1 ].y = PosY[ 2 ][ posnum - 1 ][ 1 ] ;
	...
	Vertex[ 5 ].x = PosX[ 2 ][ posnum - 1 ][ 1 ] ;	Vertex[ 5 ].y = PosY[ 2 ][ posnum - 1 ][ 1 ] ;
	...
	Vertex[ 2 ].x = PosX[ 0 ][ 0          ][ 1 ] ;	Vertex[ 2 ].y = PosY[ 0 ][ 0          ][ 1 ] ;
	Vertex[ 3 ].x = PosX[ 3 ][ posnum - 1 ][ 1 ] ;	Vertex[ 3 ].y = PosY[ 3 ][ posnum - 1 ][ 1 ] ;
	Vertex[ 4 ].x = PosX[ 0 ][ 0          ][ 1 ] ;	Vertex[ 4 ].y = PosY[ 0 ][ 0          ][ 1 ] ;
	Vertex[ 5 ].x = PosX[ 3 ][ posnum - 1 ][ 0 ] ;	Vertex[ 5 ].y = PosY[ 3 ][ posnum - 1 ][ 0 ] ;
	...
	Vertex[ 0 ].x = PosX[ 0 ][ 0          ][ 1 ] ;	Vertex[ 0 ].y = PosY[ 0 ][ 0          ][ 1 ] ;
	Vertex[ 1 ].x = PosX[ 3 ][ posnum - 1 ][ 1 ] ;	Vertex[ 1 ].y = PosY[ 3 ][ posnum - 1 ][ 1 ] ;
	Vertex[ 2 ].x = PosXBase[ 0 ] ;					Vertex[ 2 ].y = PosYBase[ 0 ] ;
	Vertex[ 3 ].x = PosXBase[ 3 ] ;					Vertex[ 3 ].y = PosYBase[ 3 ] ;
	Vertex[ 4 ].x = PosXBase[ 0 ] ;					Vertex[ 4 ].y = PosYBase[ 0 ] ;
	Vertex[ 5 ].x = PosX[ 3 ][ posnum - 1 ][ 1 ] ;	Vertex[ 5 ].y = PosY[ 3 ][ posnum - 1 ][ 1 ] ;
	...
	Vertex[ 0 ].x = PosXBase[ 2 ] ;	Vertex[ 0 ].y = PosYBase[ 2 ] ;
	Vertex[ 1 ].x = PosXBase[ 3 ] ;	Vertex[ 1 ].y = PosYBase[ 3 ] ;
	Vertex[ 2 ].x = PosXBase[ 1 ] ;	Vertex[ 2 ].y = PosYBase[ 1 ] ;
	Vertex[ 3 ].x = PosXBase[ 0 ] ;	Vertex[ 3 ].y = PosYBase[ 0 ] ;
	Vertex[ 4 ].x = PosXBase[ 1 ] ;	Vertex[ 4 ].y = PosYBase[ 1 ] ;
	Vertex[ 5 ].x = PosXBase[ 3 ] ;	Vertex[ 5 ].y = PosYBase[ 3 ] ;
...
extern	int NS_DrawRotaGraphFastToZBuffer( int x, int y, float ExRate, float Angle, int GrHandle, int WriteZMode /* DX_ZWRITE_MASK 等 */, int TurnFlag )
{
	DrawZBuffer_Pre( WriteZMode );
...
extern	int NS_DrawRotaGraphFast2ToZBuffer(  int x, int y, int cx, int cy, float ExtRate,                   float Angle, int GrHandle, int WriteZMode /* DX_ZWRITE_MASK 等 */ , int TurnFlag )
{
	DrawZBuffer_Pre( WriteZMode );
...
extern	int NS_DrawRotaGraphFast3ToZBuffer(  int x, int y, int cx, int cy, float ExtRateX, float ExtRateY, float Angle, int GrHandle, int WriteZMode /* DX_ZWRITE_MASK 等 */ , int TurnFlag )
{
	DrawZBuffer_Pre( WriteZMode );
...
\
	...
	{\
		if( Vert->pos.x - 1.0F < MinX ) MinX = Vert->pos.x - 1.0F ;\
		if( Vert->pos.x + 1.0F > MaxX ) MaxX = Vert->pos.x + 1.0F ;\
\
		if( Vert->pos.y - 1.0F < MinY ) MinY = Vert->pos.y - 1.0F ;\
		if( Vert->pos.y + 1.0F > MaxY ) MaxY = Vert->pos.y + 1.0F ;\
	}\
\
...
// ブレンド画像の適応座標モードを設定する
extern int NS_SetBlendGraphPositionMode( int BlendGraphPositionMode /* DX_BLENDGRAPH_POSMODE_DRAWGRAPH など */ )
{
...
extern	int NS_GetTransformPositionD( VECTOR_D *LocalPos, double *x, double *y )
{
	...
	VectorTransform4D(
		&ScreenPos, &w,
		LocalPos,   &w,
		&GSYS.DrawSetting.Blend3DMatrix
	) ;
	w = 1.0 / w ;
...
extern VECTOR NS_ConvScreenPosToWorldPos( VECTOR ScreenPos )
{
	...
	if( GSYS.DrawSetting.ValidInverseBlend3DMatrix == FALSE )
	{
		...
		ConvertMatrixDtoF( &GSYS.DrawSetting.InverseBlend3DMatrixF, &GSYS.DrawSetting.InverseBlend3DMatrix ) ;
...
extern VECTOR_D NS_ConvScreenPosToWorldPos_ZLinearD( VECTOR_D ScreenPos )
{
	...
	if( GSYS.DrawSetting.ValidConvScreenPosToWorldPosMatrix == FALSE )
	{
		...
		if( CreateInverseMatrixD( &GSYS.DrawSetting.ConvScreenPosToWorldPosMatrix, &TempMatrix ) < 0 )
		{
			return VGetD( -1.0, -1.0, -1.0 ) ;
		}
...
extern COLOR_F NS_GetPixelF( int x, int y )
{
	...
	else
	{
		...
		NS_GetColor5( ColorData, Color, &ri, &gi, &bi, &ai ) ;

		Result.r = ( float )ri / 255.0f ;
		Result.g = ( float )gi / 255.0f ;
		Result.b = ( float )bi / 255.0f ;
		Result.a = ( float )ai / 255.0f ;
		return Result ;
	}
}
...
extern const GRAPHICSSYS_DISPLAYINFO *Graphics_GetRectMatchDisplayInfo( const RECT *Rect )
{
	...
	for( i = 0 ; i < GSYS.Screen.DisplayNum ; i ++, DisplayInfo ++ )
	{
		int MatchWidth = 0 ;
		int MatchHeight = 0 ;

		if( Rect->left <= DisplayInfo->DesktopRect.left && DisplayInfo->DesktopRect.left < Rect->right )
		{
			MatchWidth = ( DisplayInfo->DesktopRect.right < Rect->right ? DisplayInfo->DesktopRect.right : Rect->right ) - DisplayInfo->DesktopRect.left ;
		}
		else
		if( DisplayInfo->DesktopRect.left <= Rect->left && Rect->left < DisplayInfo->DesktopRect.right )
		{
			MatchWidth = ( DisplayInfo->DesktopRect.right < Rect->right ? DisplayInfo->DesktopRect.right : Rect->right ) - Rect->left ;
		}
		else
		{
			continue ;
		...
		if( Rect->top <= DisplayInfo->DesktopRect.top && DisplayInfo->DesktopRect.top < Rect->bottom )
		{
			MatchHeight = ( DisplayInfo->DesktopRect.bottom < Rect->bottom ? DisplayInfo->DesktopRect.bottom : Rect->bottom ) - DisplayInfo->DesktopRect.top ;
		}
		else
		if( DisplayInfo->DesktopRect.top <= Rect->top && Rect->top < DisplayInfo->DesktopRect.bottom )
		{
			MatchHeight = ( DisplayInfo->DesktopRect.bottom < Rect->bottom ? DisplayInfo->DesktopRect.bottom : Rect->bottom ) - Rect->top ;
		}
		else
		{
			continue ;
		...
		if( MaxMatchIndex == -1 || MatchWidth * MatchHeight > MaxMatchAreaSize )
		{
			MaxMatchIndex = i ;
			MaxMatchAreaSize = MatchWidth * MatchHeight ;
		}
	}
...
extern int NS_GetFullScreenResolutionMode( int *ResolutionMode, int *UseResolutionMode )
{
	...
	{
		*ResolutionMode = GSYS.Screen.FullScreenResolutionMode ;
	}

	if( UseResolutionMode != NULL )
	{
...
extern int NS_SetFullScreenScalingMode( int ScalingMode /* DX_FSSCALINGMODE_NEAREST 等 */ , int FitScaling )
{
...
extern int SaveDrawScreen_WCHAR_T( int x1, int y1, int x2, int y2, const wchar_t *FileName, int SaveType, int Jpeg_Quality, int Jpeg_Sample2x1, int Png_CompressionLevel )
{
	...
	LockRect.right  = x2 ;
	LockRect.bottom = y2 ;
	if( Graphics_Screen_LockDrawScreen( &LockRect, &BaseImage, -1, -1, -1, TRUE, 0 ) < 0 )
	{
		return -1 ;
	}
...
{
#ifdef UNICODE
	return SaveDrawValidGraph_WCHAR_T(
	...
	Result = SaveDrawValidGraph_WCHAR_T(
		GrHandle, x1, y1, x2, y2, UseFileNameBuffer, SaveType, Jpeg_Quality, Jpeg_Sample2x1, Png_CompressionLevel
	) ;
...
extern int SaveDrawValidGraph_WCHAR_T( int GrHandle, int x1, int y1, int x2, int y2, const wchar_t *FileName, int SaveType, int Jpeg_Quality, int Jpeg_Sample2x1, int Png_CompressionLevel )
{
	IMAGEDATA *Image ;
	BASEIMAGE BaseImage ;
	RECT LockRect ;
	...
	// サイズが画面いっぱいで、かつ DDS ファイルでの保存で、テクスチャの場合は専用の処理を行う
		...
		SaveType == DX_IMAGESAVETYPE_DDS && Image->Orig->FormatDesc.TextureFlag == TRUE )
	{
		...
		{
			for( j = 0 ; j < MipMapCount ; j ++, k ++ )
			{
				int Width, Height ;

				Graphics_Image_CalcMipSize( Image->WidthI, Image->HeightI, j, &Width, &Height ) ;

				// 描画対象にできるグラフィックハンドルをロック
				LockRect.left   = 0 ;
				LockRect.top    = 0 ;
				LockRect.right  = Width ;
				LockRect.bottom = Height ;
				if( Graphics_Screen_LockDrawScreen( &LockRect, &BaseImage, GrHandle, i, j, TRUE, 0 ) < 0 )
				{
					DXFREE( BaseImageDim ) ;
					BaseImageDim = NULL ;
	...
	LockRect.right  = x2 ;
	LockRect.bottom = y2 ;
	if( Graphics_Screen_LockDrawScreen( &LockRect, &BaseImage, GrHandle, 0, 0, TRUE, 0 ) < 0 )
	{
		return -1 ;
	}
...
extern int SaveDrawValidGraphToJPEG_WCHAR_T( int GrHandle, int x1, int y1, int x2, int y2, const wchar_t *FileName, int Quality, int Sample2x1 )
{
...
//		return 0 ;
	...
	// ターゲットが 0 だった場合は描画可能サイズの取得
	if( TargetIndex == 0 )
	{
		NS_GetGraphSize( DrawScreen, &GSYS.DrawSetting.DrawSizeX, &GSYS.DrawSetting.DrawSizeY ) ;
		if( MipLevel > 0 )
		{
			Graphics_Image_CalcMipSize( GSYS.DrawSetting.DrawSizeX, GSYS.DrawSetting.DrawSizeY, MipLevel, &GSYS.DrawSetting.DrawSizeX, &GSYS.DrawSetting.DrawSizeY ) ;
		}
	}
	...
	// ターゲットが 0 だった場合は描画範囲の設定やカメラの設定を行う
	if( TargetIndex == 0 )
	{
...
extern int NS_CalcPolygonBinormalAndTangentsToShader( VERTEX3DSHADER *Vertex, int PolygonNum )
{
	...
	if( Indices == NULL )
	{
		DXST_LOGFILEFMT_ADDUTF16LE(( "\x02\x98\xb9\x70\xa4\x30\xf3\x30\xc7\x30\xc3\x30\xaf\x30\xb9\x30\x92\x30\x3c\x68\x0d\x7d\x59\x30\x8b\x30\xe1\x30\xe2\x30\xea\x30\x18\x98\xdf\x57\x6e\x30\xba\x78\xdd\x4f\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"頂点インデックスを格納するメモリ領域の確保に失敗しました\n" @*/ )) ;
		return -1 ;
	}
...
extern int NS_DrawModiStringToZBuffer( int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, int WriteZMode /* DX_ZWRITE_MASK 等 */ , int VerticalFlag , const TCHAR *String )
{
	DrawZBuffer_Pre( WriteZMode );
...
extern int NS_DrawModiStringToHandleToZBuffer( int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, int FontHandle, int WriteZMode /* DX_ZWRITE_MASK 等 */ , int VerticalFlag , const TCHAR *String )
{
	DrawZBuffer_Pre( WriteZMode );
...
extern int NS_DrawModiFormatStringToZBuffer( int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, int WriteZMode /* DX_ZWRITE_MASK 等 */ , int VerticalFlag, const TCHAR *FormatString , ... )
{
	va_list VaList ;
	TCHAR String[ 2048 ] ;
...
extern int NS_DrawModiFormatStringToHandleToZBuffer( int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4, int FontHandle, int WriteZMode /* DX_ZWRITE_MASK 等 */ , int VerticalFlag, const TCHAR *FormatString, ... )
{
	va_list VaList ;
	TCHAR String[ 2048 ] ;
...
extern int Graphics_Screen_ChangeMode( int ScreenSizeX, int ScreenSizeY, int ColorBitDepth, int ChangeWindowFlag, int RefreshRate )
{
	...
	DXST_LOGFILE_ADDUTF16LE( "\x3b\x75\x62\x97\xe2\x30\xfc\x30\xc9\x30\x09\x59\xf4\x66\xe6\x51\x06\x74\x92\x30\x8b\x95\xcb\x59\x57\x30\x7e\x30\x59\x30\x0a\x00\x00"/*@ L"画面モード変更処理を開始します\n" @*/ ) ;
	DXST_LOGFILE_TABADD ;
	...
	if( GSYS.Setting.ChangeScreenModeNotGraphicsSystemFlag )
	{
		...
		DXST_LOGFILE_TABSUB ;
		DXST_LOGFILE_ADDUTF16LE( "\x3b\x75\x62\x97\xe2\x30\xfc\x30\xc9\x30\x09\x59\xf4\x66\xe6\x51\x06\x74\x6f\x30\x63\x6b\x38\x5e\x6b\x30\x42\x7d\x86\x4e\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"画面モード変更処理は正常に終了しました\n" @*/ ) ;
	}
	else
	{
		// リセットする場合
...
)
{
	...
	if( Format->TextureFlag == TRUE )
	{
		...
		// 描画可能画像の場合はチャンネル数とチャンネル辺りのビット数、浮動小数点型かどうかなどを設定する
		if( GParam->DrawValidImageCreateFlag == TRUE )
		{
...
) 
{
	...
	IsDXT =
		( Format.BaseFormat == DX_BASEIMAGE_FORMAT_DXT1 ||
		  ...
		  Format.BaseFormat == DX_BASEIMAGE_FORMAT_DXT3 ||
		  Format.BaseFormat == DX_BASEIMAGE_FORMAT_DXT4 ||
		  Format.BaseFormat == DX_BASEIMAGE_FORMAT_DXT5 ) ? TRUE : FALSE ;
...
*/
	...
	else
	{
		...
		if( Orig->FormatDesc.BlendGraphFlag == TRUE )
		{
			...
			{
				DXST_LOGFILE_ADDUTF16LE( "\xd6\x30\xec\x30\xf3\x30\xc9\x30\x3b\x75\xcf\x50\x28\x75\xe1\x30\xe2\x30\xea\x30\xa4\x30\xe1\x30\xfc\x30\xb8\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"ブレンド画像用メモリイメージの作成に失敗しました\n" @*/ ) ;
				goto ERR ;
			}
		}
		else
		{
			...
			{
				DXST_LOGFILE_ADDUTF16LE( "\xe1\x30\xe2\x30\xea\x30\xa4\x30\xe1\x30\xfc\x30\xb8\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"メモリイメージの作成に失敗しました\n" @*/ ) ;
				goto ERR ;
			}
		}
	}
...
extern int Graphics_Image_ListUpTexSize( int Size, short *SizeList, int NotDivFlag, int Pow2Flag, int MaxTextureSize, int IsDXT )
{
	...
	if( MinTexSize >= Size )
	{
		if( SizeList ) SizeList[ 0 ] = MinTexSize ;
		return 1 ;
	}
...
extern int Graphics_Image_InitializeDrawInfo( int GrHandle, int IsFloat, int ASyncThread )
{
	...
	if( Orig->FormatDesc.TextureFlag )
	{
		...
		if( Num == 0 )
		{
			DXST_LOGFILE_ADDUTF16LE( "\xc6\x30\xaf\x30\xb9\x30\xc1\x30\xe3\x30\xcf\x63\x3b\x75\xc5\x60\x31\x58\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"テクスチャ描画情報の作成に失敗しました\n" @*/ ) ;
			return -1 ;
		}
		...
		if( Num > 4 )
		{
			...
			{
				DXST_LOGFILE_ADDUTF16LE( "\x3b\x75\xcf\x50\xcf\x63\x3b\x75\xc5\x60\x31\x58\x3c\x68\x0d\x7d\x28\x75\xe1\x30\xe2\x30\xea\x30\x6e\x30\xba\x78\xdd\x4f\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"画像描画情報格納用メモリの確保に失敗しました\n" @*/ ) ;
				return -1 ;
			}
...
)
{
	...
	// ハンドルの初期化
	{
		...
		UsePaletteFlag = RgbBaseImage->ColorData.PixelByte  == 1 && 
						 ( GSYS.Setting.ValidHardware || RgbBaseImage->ColorData.AlphaWidth == 0 ) &&
						 AlphaBaseImage == NULL &&
		...
		// BaseImage のカラー情報でチャンネル数やチャンネルビット数が指定されている場合は GParam に値を反映する
...
)
{
	...
	if( MovieHandle < 0 )
	{
		DXST_LOGFILEFMT_ADDW(( L"Image File Load Error : %s \n", GraphName )) ;
		return -1 ;
	}
	...
	if( Movie->TheoraFlag && /* Movie->RightAlpha == FALSE && */ GSYS.HardInfo.OggTheoraUseDrawValidGraph )
	{
		...
		GParam->CreateGraphGParam.InitHandleGParam.CreateDrawValidGraphMipLevels = 0 ;
...
#ifndef DX_NON_DSHOW_MOVIE
		|| ( Movie->PF.pMovieImage != NULL && Movie->PF.pMovieImage->ImageType != 1 )
#endif // DX_NON_DSHOW_MOVIE
		) )
	{
		GParam->CreateGraphGParam.InitHandleGParam.DrawValidImageCreateFlag = TRUE ;
	...
	{
		DXST_LOGFILEFMT_ADDW(( L"Graph Handle Setup Error : %s", GraphName )) ;
		return -1 ;
	}
	...
	else
	{
		...
		else
		{
			...
			if( MemImage )
			{
				...
				if( Image->ReadBase == NULL )
				{
					DXST_LOGFILE_ADDUTF16LE( "\xe2\x8e\x01\x90\x59\x30\x8b\x30\x3b\x75\xcf\x50\xc7\x30\xfc\x30\xbf\x30\x92\x30\xdd\x4f\x58\x5b\x59\x30\x8b\x30\xe1\x30\xe2\x30\xea\x30\x18\x98\xdf\x57\x6e\x30\xba\x78\xdd\x4f\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"転送する画像データを保存するメモリ領域の確保に失敗しました\n" @*/ ) ;
					return -1 ;
				}
...
//			else
		...
		// Theora を使用していて、画像の右側がアルファ情報ではなく、且つ YUY2 フォーマットを使用しているかで処理を分岐
...
)
{
	IMAGEDATA		*Image ;
	IMAGEDATA_ORIG	*Orig ;
	RECT			SrcRectTemp = *SrcRect ;
	int				SrcWidth ;
	int				SrcHeight ;
	int				TempBaseRGBIsDXT = FALSE ;
	int				TempBaseAlphaIsDXT = FALSE ;
	BASEIMAGE		TempBaseRGB ;
	...
	TempBaseRGBIsDXT =
		( TempBaseRGB.ColorData.Format == DX_BASEIMAGE_FORMAT_DXT1 ||
		  ...
		  TempBaseRGB.ColorData.Format == DX_BASEIMAGE_FORMAT_DXT5 ) ? TRUE : FALSE ;

	// DXT 圧縮の場合は転送元の画像は 4 の倍数となる
	if( TempBaseRGBIsDXT )
	{
		TempBaseRGB.Width  = ( TempBaseRGB.Width  + 3 ) / 4 * 4 ;
		TempBaseRGB.Height = ( TempBaseRGB.Height + 3 ) / 4 * 4 ;
	}
	...
	if( AlphaBaseImage )
	{
		...
		if( TempBaseAlphaIsDXT )
		{
			...
			TempBaseAlpha.Height = ( TempBaseAlpha.Height + 3 ) / 4 * 4 ;
		}
	}
	...
	if( TempBaseRGBIsDXT )
	{
		...
		SrcRectTemp.top    = ( SrcRectTemp.top    + 3 ) / 4 * 4 ;
		SrcRectTemp.right  = ( SrcRectTemp.right  + 3 ) / 4 * 4 ;
		SrcRectTemp.bottom = ( SrcRectTemp.bottom + 3 ) / 4 * 4 ;
	}
	SrcWidth  = SrcRectTemp.right  - SrcRectTemp.left ;
	SrcHeight = SrcRectTemp.bottom - SrcRectTemp.top ;
	if( SrcWidth <= 0 || SrcHeight <= 0 )
	{
		return 0 ;
	}
	...
	// BASEIMAGE が DXT で、転送先が DXT では無い場合はここで標準フォーマットに変換する
	{
		...
		// テクスチャではない場合はＤＸＴフォーマットの場合は標準フォーマットに変換する
		...
		if( Hr1 < 0 || Hr2 < 0 )
		{
			DXST_LOGFILE_ADDUTF16LE( "\x19\x6a\x96\x6e\xa4\x30\xe1\x30\xfc\x30\xb8\x30\x28\x75\x6e\x30\xe1\x30\xe2\x30\xea\x30\xba\x78\xdd\x4f\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"標準イメージ用のメモリ確保に失敗しました\n" @*/ ) ;
			goto ERR ;
		}
	}
...
)
{
	...
	if( IsDXT )
	{
		...
		DestRect->top    = ( DestRect->top    + 3 ) / 4 * 4 ;
		DestRect->right  = ( DestRect->right  + 3 ) / 4 * 4 ;
		DestRect->bottom = ( DestRect->bottom + 3 ) / 4 * 4 ;

		MoveRect->left   = ( MoveRect->left   + 3 ) / 4 * 4 ;
		MoveRect->top    = ( MoveRect->top    + 3 ) / 4 * 4 ;
		MoveRect->right  = ( MoveRect->right  + 3 ) / 4 * 4 ;
...
// アンチエイリアス付きの線を描画するためのテクスチャハンドルを取得する( 太さ1ピクセル用 )
...
// アンチエイリアス付きの線矩形を描画するためのテクスチャハンドルを取得する
...
// アンチエイリアス付きの線矩形を描画するためのテクスチャハンドルを取得する( 太さ1ピクセル用 )
		...
		if( Param->ReCreateFlag == FALSE && ( Param->FileName != NULL || Param->RgbMemImage != NULL ) )
		{
			...
			if( Param->FileName != NULL )
			{
				...
				if( fp == 0 )
				{
					...
					DXST_LOGFILEFMT_ADDUTF16LE(( "\x3b\x75\xcf\x50\xd5\x30\xa1\x30\xa4\x30\xeb\x30\x20\x00\x25\x00\x73\x00\x20\x00\x4c\x30\x42\x30\x8a\x30\x7e\x30\x5b\x30\x93\x30\x00"/*@ L"画像ファイル %s がありません" @*/, FullPath_UTF16LE )) ;
					return -1 ;
				}
				DX_FCLOSE( fp ) ;
			}
...
#else	// DX_NON_MOVIE
			{
				if( Param->FileName != NULL )
				{
					...
					DXST_LOGFILEFMT_ADDUTF16LE(( "\x3b\x75\xcf\x50\xd5\x30\xa1\x30\xa4\x30\xeb\x30\x20\x00\x25\x00\x73\x00\x20\x00\x6e\x30\xaa\x30\xfc\x30\xd7\x30\xf3\x30\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x00"/*@ L"画像ファイル %s のオープンに失敗しました" @*/, FullPath_UTF16LE )) ;
				}
				return -1 ;
			}
#endif	// DX_NON_MOVIE
...
)
{
	...
	if( LParam.LoadHr == -1 )
	{
		if( Param->FileName != NULL )
		{
			...
			DXST_LOGFILEFMT_ADDUTF16LE(( "\x3b\x75\xcf\x50\xd5\x30\xa1\x30\xa4\x30\xeb\x30\x20\x00\x25\x00\x73\x00\x20\x00\x6e\x30\xed\x30\xfc\x30\xc9\x30\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x00"/*@ L"画像ファイル %s のロードに失敗しました" @*/, FullPath_UTF16LE )) ;
		}
		return -1 ;
	}
...
)
{
	...
	if( ReCreateFlag )
	{
		...
		if( SizeX != BaseImage->Width || SizeY != BaseImage->Height )
		{
			return -1 ;
		}

		if( AlphaBaseImage != NULL && ( SizeX != AlphaBaseImage->Width || SizeY != AlphaBaseImage->Height ) )
		{
			return -1 ;
		}
...
extern void Graphics_Image_InitSetupGraphHandleGParam( SETUP_GRAPHHANDLE_GPARAM *GParam )
{
	...
	GParam->CreateDrawValidGraphMipLevels		= GSYS.CreateImage.DrawValidMipLevels ;
...
//	GSYS.NotDrawFlag = FALSE ;
	...
	CreateIdentityMatrixD( &GSYS.DrawSetting.Blend3DMatrix ) ;
	CreateIdentityMatrix(  &GSYS.DrawSetting.Blend3DMatrixF ) ;
	CreateIdentityMatrixD( &GSYS.DrawSetting.InverseBlend3DMatrix ) ;
	CreateIdentityMatrix(  &GSYS.DrawSetting.InverseBlend3DMatrixF ) ;
...
{
	if( GSYS.Setting.ValidHardware == FALSE ) return ;
	...
	if( GSYS.DrawSetting.MatchHardware_2DMatrix == FALSE )
	{
		...
		// Graphics_Hardware_SetTransformToProjection_PF を呼ぶ前に MatchHardware_2DMatrix を TRUE にしておく必要あり
		GSYS.DrawSetting.MatchHardware_2DMatrix = TRUE ;

		// 記憶域を共有している射影行列を無効にする
		GSYS.DrawSetting.MatchHardware3DMatrix = FALSE ;
		GSYS.DrawSetting.MatchHardwareProjectionMatrix = FALSE ;
...
// SetDrawScreen 実行時に行う描画範囲設定、カメラ設定を行う
extern int Graphics_DrawSetting_SetupDefaultDrawAreaAndCamera( int OldScreen )
{
	// 描画領域を更新
	{
		...
		NS_GetGraphSize( GSYS.DrawSetting.TargetScreen[ 0 ], &NewWidth, &NewHeight ) ;
		NS_GetGraphSize( OldScreen,                          &OldWidth, &OldHeight ) ;
...
extern void Graphics_DrawSetting_RefreshBlend3DTransformMatrix( void )
{
	...
	CreateMultiplyMatrixD( &GSYS.DrawSetting.Blend3DMatrix, &GSYS.DrawSetting.WorldMatrix, &GSYS.DrawSetting.ViewProjectionViewportMatrix ) ;
	ConvertMatrixDtoF( &GSYS.DrawSetting.Blend3DMatrixF, &GSYS.DrawSetting.Blend3DMatrix ) ;
	...
	GSYS.DrawSetting.ValidInverseBlend3DMatrix          = FALSE ;
	GSYS.DrawSetting.ValidBlend3DMatrix                 = TRUE ;
...
extern int Graphics_DrawSetting_GetScreenDrawSettingInfo( SCREENDRAWSETTINGINFO *ScreenDrawSettingInfo )
{
	...
	ScreenDrawSettingInfo->BackgroundGreen		= GSYS.Screen.BackgroundGreen ;
	ScreenDrawSettingInfo->BackgroundBlue		= GSYS.Screen.BackgroundBlue ;
	ScreenDrawSettingInfo->DrawBlendMode		= GSYS.DrawSetting.BlendMode ;
	ScreenDrawSettingInfo->DrawBlendParam		= GSYS.DrawSetting.BlendParam ;
	ScreenDrawSettingInfo->DrawMode				= GSYS.DrawSetting.DrawMode ;
...
{
	// 保存しておいた設定を反映させる
	NS_SetBackgroundColor( ScreenDrawSettingInfo->BackgroundRed, ScreenDrawSettingInfo->BackgroundGreen, ScreenDrawSettingInfo->BackgroundBlue ) ;
...
extern int Graphics_Light_AddHandle( void )
{
	...
	if( ( pLH = *ppLH = ( LIGHT_HANDLE * )DXCALLOC( sizeof( LIGHT_HANDLE ) ) ) == NULL )
		return DXST_LOGFILE_ADDUTF16LE( "\xe9\x30\xa4\x30\xc8\x30\x28\x75\x6e\x30\xe1\x30\xe2\x30\xea\x30\x6e\x30\xba\x78\xdd\x4f\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x00"/*@ L"ライト用のメモリの確保に失敗しました" @*/ ) ;
...
extern int Graphics_VertexBuffer_SetupHandle( int VertexBufHandle, int VertexNum, int VertexType /* DX_VERTEX_TYPE_NORMAL_3D 等 */ )
{
	...
	default :
		DXST_LOGFILE_ADDUTF16LE( "\x02\x98\xb9\x70\xbf\x30\xa4\x30\xd7\x30\x6e\x30\x24\x50\x4c\x30\x0d\x4e\x63\x6b\x67\x30\x59\x30\x0a\x00\x00"/*@ L"頂点タイプの値が不正です\n" @*/ ) ;
		return -1 ;
	}
...
extern int Graphics_IndexBuffer_SetupHandle( int IndexBufHandle, int IndexNum, int IndexType /* DX_INDEX_TYPE_16BIT 等 */ )
{
	...
	default :
		DXST_LOGFILE_ADDUTF16LE( "\xa4\x30\xf3\x30\xc7\x30\xc3\x30\xaf\x30\xb9\x30\xbf\x30\xa4\x30\xd7\x30\x6e\x30\x24\x50\x4c\x30\x0d\x4e\x63\x6b\x67\x30\x59\x30\x0a\x00\x00"/*@ L"インデックスタイプの値が不正です\n" @*/ ) ;
		return -1 ;
	}
