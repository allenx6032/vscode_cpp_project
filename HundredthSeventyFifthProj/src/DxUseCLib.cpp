...
//	if( color_type == PNG_COLOR_TYPE_PALETTE )					png_set_expand( png_ptr ) ;		// パレット使用画像データの自動展開指定
	...
	// ８ビット以下のグレースケール画像の場合は出力画像のピクセルフォーマットをフルカラーにする
...
//			if( info_ptr->channels == 4 )
			...
			if( png_get_valid( png_ptr, info_ptr, PNG_INFO_tRNS ) )
			{
				...
				for( i = 0 ; i < BaseImage->Height; i ++, SrcBase += rowbytes, DestBase += BaseImage->Pitch )
				{
					BYTE *Src  = SrcBase ;
					BYTE *Dest = DestBase ;
					for( j = 0 ; j < BaseImage->Width ; j ++, Src += 3, Dest += 4 )
					{
						Dest[ 0 ] = Src[ 0 ] ;
						Dest[ 1 ] = Src[ 1 ] ;
						Dest[ 2 ] = Src[ 2 ] ;

						if( Src[ 0 ] == trans_values->blue &&
							Src[ 1 ] == trans_values->green &&
							Src[ 2 ] == trans_values->red )
						{
							Dest[ 3 ] = 0 ;
						}
						else
