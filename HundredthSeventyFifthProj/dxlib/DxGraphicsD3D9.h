...
struct GRAPHICS_HARDDATA_DIRECT3D9_SCREENINFO
{
	...
	D_D3DFORMAT				SubBackBufferTexturePixelFormat ;		// サブバックバッファテクスチャのピクセルフォーマット
...
extern	int		Graphics_D3D9_CompileShader( const char *FileName, const char *EntryPoint, const char *Profile, const char *SubProfile, const D_D3DXMACRO *MacroTable, D_ID3DXBuffer **ppShader ) ;										// Direct3D9 のシェーダーソースをコンパイルする
extern	int		Graphics_D3D9_CompileVertexShader( const char *FileName, const char *EntryPoint, const D_D3DXMACRO *MacroTable, D_IDirect3DVertexShader9 **VertexShader, int ShaderModel3Only = FALSE, int ShaderModel2Only = FALSE ) ;	// Direct3D9 のシェーダーソースをコンパイルして頂点シェーダーを作成する
extern	int		Graphics_D3D9_CompilePixelShader( const char *FileName, const char *EntryPoint, const D_D3DXMACRO *MacroTable, D_IDirect3DPixelShader9 **PixelShader, int ShaderModel3Only = FALSE, int ShaderModel2Only = FALSE ) ;	// Direct3D9 のシェーダーソースをコンパイルしてピクセルシェーダーを作成する
...
// 描画設定関係関数
extern	int		Graphics_D3D9_DrawSetting_SetDrawBlendMode( int BlendMode, int BlendEnable, int BlendRGBSrc, int BlendRGBDest, int BlendRGBOp, int BlendASrc, int BlendADest, int BlendAOp, int AlphaTestValidFlag, int AlphaChannelValidFlag ) ;	// 描画ブレンドモードの設定
extern	int		Graphics_D3D9_DrawSetting_SetIgnoreDrawGraphColor( int EnableFlag ) ;							// 描画時の画像のＲＧＢを無視するかどうかを設定する
extern	int		Graphics_D3D9_DrawSetting_SetWriteAlphaChannelFlag(	int NotFlag ) ;								// 描画先のアルファチャンネルの内容を書き換えるかを設定する
...
extern	int		Graphics_D3D9_DrawSetting_SetLightNoAngleAttenuation( int NoAngleAttenuation ) ;				// ライティング計算の角度減衰を行わないかを設定する
...
extern	int		Graphics_D3D9_DrawBoxSet( const RECTDATA *RectData, int Num ) ;																							// ハードウエアアクセラレータ使用版 DrawBoxSet
...
extern	int		Graphics_Hardware_D3D9_SetDrawCustomBlendMode_PF( int BlendEnable, int SrcBlendRGB, int DestBlendRGB, int BlendOpRGB, int SrcBlendA, int DestBlendA, int BlendOpA, int BlendParam ) ; // カスタムブレンドモードを設定する
...
extern	int		Graphics_Hardware_D3D9_SetDrawAddColor_PF( int Red, int Green, int Blue ) ;									// 描画色に加算する色をセット
...
extern	int		Graphics_Hardware_D3D9_GetVideoMemorySize_PF( DWORD *AllSize, DWORD *FreeSize ) ;			// ビデオメモリの容量を得る
...
extern	int		Graphics_Hardware_D3D9_Light_SetNoAngleAttenuation_PF( int NoAngleAttenuation ) ;								// ライトの計算で角度減衰を行わないようにするかどうかを設定する
...
// 環境依存シェーダー関係
extern	int		Graphics_Hardware_D3D9_Shader_Create_PF( int ShaderHandle, int ShaderType, void *Image, int ImageSize, int ImageAfterFree, int ASyncThread ) ;		// シェーダーハンドルを初期化する
