...
struct MQO_MODEL
{
	int Version ;								// バージョン( 1:MQOファイル  2:MQOZファイル )
	int CharCodeFormat ;						// テキストデータの文字フォーマット
	char *Text ;								// テキストデータ
...
static int GetNextString( MQO_MODEL *Model, char *Buffer, size_t BufferSize ) ;		// 次の意味のある文字列を取得する( 1:終端に達した  0:終端には達していない -1:エラー )
static int GetString( MQO_MODEL *Model, char *Buffer, size_t BufferSize ) ;			// 文字列を取得する( -1:エラー )
...
static int SkipChunk( MQO_MODEL *Model )
{
	...
	{
		GetNextString( Model, Buffer, sizeof( Buffer ) ) ;
		SkipSpace( Model ) ;
	}
	if( ENDCHECK( Model ) )
		return -1 ;
...
static float GetFloat( MQO_MODEL *Model )
{
	...
	// 文字列を取得する
	if( GetNextString( Model, Buffer, sizeof( Buffer ) ) != 0 )
		return -1 ;
...
// 文字列を取得する( -1:エラー )
static int GetString( MQO_MODEL *Model, char *Buffer, size_t BufferSize )
{
	int Len ;

	// 文字列取得
	if( GetNextString( Model, Buffer, BufferSize ) != 0 )
		return -1 ;
	Len = ( int )CL_strlen( Model->CharCodeFormat, Buffer ) ;
...
static int GetNextString( MQO_MODEL *Model, char *Buffer, size_t BufferSize )
{
	int PutBytes ;
	int CharBytes = 0 ;
	DWORD CharCode ;
	...
	case '\"' :		// 文字列
		// 次のダブルコーテーションまで取得
		Model->TextNow ++ ;
		Address += PutCharCode( '\"', Model->CharCodeFormat, &Buffer[ Address ], BufferSize - Address ) ;
		while( !ENDCHECK( Model ) )
		{
			CharCode = GetCharCode( Model->TextNow, Model->CharCodeFormat, &CharBytes ) ;
			if( CharCode == '\"' )
			{
				break ;
			}

			PutBytes = PutCharCode( CharCode, Model->CharCodeFormat, &Buffer[ Address ], BufferSize - Address ) ;
			Address += PutBytes ;
			Model->TextNow += CharBytes ;
		}
		if( ENDCHECK( Model ) )
			return -1 ;
		...
		// 終端文字のセット
		Address += PutCharCode( '\"', Model->CharCodeFormat, &Buffer[ Address ], BufferSize - Address ) ;
		Address += PutCharCode( '\0', Model->CharCodeFormat, &Buffer[ Address ], BufferSize - Address ) ;
	...
	case '{' :
	case '}' :
		Address += PutCharCode( *Model->TextNow, Model->CharCodeFormat, &Buffer[ Address ], BufferSize - Address ) ;
		Address += PutCharCode( '\0', Model->CharCodeFormat, &Buffer[ Address ], BufferSize - Address ) ;
		Model->TextNow ++ ;
		break ;
	...
	case '5' : case '6' : case '7' : case '8' : case '9' :
		while( !ENDCHECK( Model ) && 
			( *Model->TextNow == '-' || *Model->TextNow == '.' || *Model->TextNow == 'e' ||
...
extern int MV1LoadModelToMQO( const MV1_MODEL_LOAD_PARAM *LoadParam, int ASyncThread )
{
		...
		_MEMCMP( LoadParam->DataBuffer, "Metasequoia Document\nFormat Text Ver 1.1\n", 22 ) != 0 )
	{
		...
		if( ( ( BYTE * )LoadParam->DataBuffer )[ 0 ] == 0x50 &&
			( ( BYTE * )LoadParam->DataBuffer )[ 1 ] == 0x4B &&
			( ( BYTE * )LoadParam->DataBuffer )[ 2 ] == 0x03 &&
			( ( BYTE * )LoadParam->DataBuffer )[ 3 ] == 0x04 )
		{
			BYTE *Src = ( BYTE * )LoadParam->DataBuffer ;
			char *FileNameP ;
			WORD FileNameSize = 0 ;
			WORD ExeFieldSize = 0 ;
			size_t PressDataSize = 0 ;
			size_t OriginalSize = 0 ;
			BYTE *PressDataAddress = NULL ;
			int Success = FALSE ;

			// zip圧縮された mqoファイルを解凍する

			// 拡張子が mqo のファイルを探す
			while( Src[ 0 ] == 0x50 && Src[ 1 ] == 0x4B && Src[ 2 ] == 0x03 && Src[ 3 ] == 0x04 )
			{
				PressDataSize = ( size_t )*( ( DWORD * )&Src[ 18 ] ) ;
				OriginalSize = ( size_t )*( ( DWORD * )&Src[ 22 ] ) ;
				FileNameSize = *( ( WORD * )&Src[ 26 ] ) ;
				ExeFieldSize = *( ( WORD * )&Src[ 28 ] ) ;
				FileNameP = ( char * )&Src[ 30 ] ;
				PressDataAddress = &Src[ 30 + FileNameSize + ExeFieldSize ] ;

				if( FileNameP[ FileNameSize - 4 ] == '.' &&
					FileNameP[ FileNameSize - 3 ] == 'm' &&
					FileNameP[ FileNameSize - 2 ] == 'q' &&
					FileNameP[ FileNameSize - 1 ] == 'o' )
				{
					Success = TRUE ;
					break ;
				}

				Src += 30 + FileNameSize + ExeFieldSize + PressDataSize ;
			}

			// 拡張子が mqo のファイルが無かったら MQOZ ファイルではないと判断
			if( Success == FALSE )
			{
				return -1 ;
			}
			...
			// MQOファイルを格納するメモリを確保
			MQOZFileBuffer = DXALLOC( OriginalSize ) ;
			if( MQOZFileBuffer == NULL )
			{
				DXST_LOGFILEFMT_ADDUTF16LE(( ""/*@ L"MQO Load Error : 解凍した MQO ファイルを格納するメモリの確保に失敗しました\n" @*/ )) ;
				goto ENDLABEL ;
			}

			// MQOファイルを解凍
			MQOZ_ZIP_uncompress( MQOZFileBuffer, OriginalSize, PressDataAddress, PressDataSize ) ;

			// 使用するテキストデータを変更
			MqoModel.Text = ( char * )MQOZFileBuffer ;
			MqoModel.TextSize = ( int )OriginalSize ;
			MqoModel.TextNow = MqoModel.Text + 60 ;
			MqoModel.Version = 1 ;
			MqoModel.CharCodeFormat = DX_CHARCODEFORMAT_UTF8 ;
		}
		else
		{
			goto ENDLABEL ;
	...
	RModel.CharCodeFormat = MqoModel.CharCodeFormat ;
	...
	{
		if( GetNextString( &MqoModel, String, sizeof( String ) ) == -1 )
		{
		...
		if( CL_strcmp( MqoModel.CharCodeFormat, String, "Material" ) != 0 )
		{
			SkipChunk( &MqoModel ) ;
			continue ;
		}
		...
		// 括弧の中に入る
		GetNextString( &MqoModel, String, sizeof( String ) ) ;
		if( String[ 0 ] != '{' )
		{
		...
		{
			// マテリアルの名前を取得
			if( GetString( &MqoModel, String, sizeof( String ) ) == -1 )
			{
			...
			for(;;)
			{
				...
				// 両面ポリゴン
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "dbls" ) == 0 )
				{
					GetNextString( &MqoModel, String, sizeof( String ) ) ;
					if( String[ 0 ] != '(' )
					{
				...
				// カラー
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "col" ) == 0 )
				{
					GetNextString( &MqoModel, String, sizeof( String ) ) ;
					if( String[ 0 ] != '(' )
					{
				...
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "dif" ) == 0 )
				{
					GetNextString( &MqoModel, String, sizeof( String ) ) ;
					if( String[ 0 ] != '(' )
					{
				...
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "amb" ) == 0 )
				{
					GetNextString( &MqoModel, String, sizeof( String ) ) ;
					if( String[ 0 ] != '(' )
					{
				...
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "emi" ) == 0 )
				{
					GetNextString( &MqoModel, String, sizeof( String ) ) ;
					if( String[ 0 ] != '(' )
					{
				...
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "spc" ) == 0 )
				{
					GetNextString( &MqoModel, String, sizeof( String ) ) ;
					if( String[ 0 ] != '(' )
					{
				...
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "power" ) == 0 )
				{
					GetNextString( &MqoModel, String, sizeof( String ) ) ;
					if( String[ 0 ] != '(' )
					{
				...
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "tex" ) == 0 )
				{
					GetNextString( &MqoModel, String, sizeof( String ) ) ;
					if( String[ 0 ] != '(' )
					{
				...
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "aplane" ) == 0 )
				{
					GetNextString( &MqoModel, String, sizeof( String ) ) ;
					if( String[ 0 ] != '(' )
					{
				...
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "bump" ) == 0 )
				{
					GetNextString( &MqoModel, String, sizeof( String ) ) ;
					if( String[ 0 ] != '(' )
					{
	...
	while( SkipSpace( &MqoModel ) == 0 )
	{
		...
		// オブジェクトチャンク
		if( CL_strcmp( MqoModel.CharCodeFormat, String, "Object" ) == 0 )
		{
			char FrameName[ 512 ] ;
			...
			// オブジェクト名を取得
			if( GetString( &MqoModel, FrameName, sizeof( String ) ) != 0 )
			{
			...
			if( CL_strcmp( MqoModel.CharCodeFormat, String, "depth" ) == 0 )
			{
...
//			Frame->IgnoreParentTransform = 1 ;
			...
			for(;;)
			{
				...
				// 閉じ括弧だったら終了
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "}" ) == 0 )
				{
					break ;
				}
				else
				// ビジブル値だった場合
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "visible" ) == 0 )
				{
					Frame->Visible = GetInt( &MqoModel ) == 15 ? 1 : 0 ;
				}
				else
				// シェーディングだった場合
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "shading" ) == 0 )
				{
					EnableShading = 1;
				...
				// 拡大値だった場合
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "scale" ) == 0 )
				{
				...
				// 回転値だった場合
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "rotation" ) == 0 )
				{
				...
				// 平行移動値だった場合
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "translation" ) == 0 )
				{
				...
				// スムージング角度情報だった場合
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "facet" ) == 0 )
				{
				...
				// 鏡面処理だった場合
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "mirror" ) == 0 )
				{
					Mirror = GetInt( &MqoModel ) ;
				}
				else
				// 鏡面処理の角度だった場合
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "mirror_axis" ) == 0 )
				{
					Mirror_Axis = GetInt( &MqoModel ) ;
				}
				else
				// 頂点情報だった場合
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "vertex" ) == 0 )
				{
					// 頂点数を取得
				...
				if( CL_strcmp( MqoModel.CharCodeFormat, String, "face" ) == 0 )
				{
					int FaceNum ;
					int MaxIndexNum ;
					...
					// 面が持つ頂点の最大数を取得する
					{
						char *TempPointer = MqoModel.TextNow ;
						int IndexNum ;
						int Result ;
						...
						{
							// インデックスの数を取得
							IndexNum = GetInt( &MqoModel ) ;
							if( IndexNum > MaxIndexNum )
							{
								MaxIndexNum = IndexNum ;
							}
						...
						// 検索位置を元に戻す
						MqoModel.TextNow = TempPointer ;
					}

					// 面情報を格納するメモリ領域の確保
					if( MV1RSetupMeshFaceBuffer( &RModel, Mesh, FaceNum, MaxIndexNum ) < 0 )
					{
					...
					// ＵＶ値を格納するためのメモリ領域の確保
					Mesh->UVNum[ 0 ] = Mesh->FaceNum * MaxIndexNum ;
					...
					Mesh->VertexColorNum = Mesh->FaceNum * MaxIndexNum ;
...
*/
		...
		if( CL_strcmp( MqoModel.CharCodeFormat, String, "Eof" ) == 0 )
		{
			break ;
		}
		else
