...
#define DDS_ALPHA_MODE_UNKNOWN					(0x0)			// アルファチャネルのコンテンツは不明です。これはレガシーファイルの値で、通常は「ストレート」アルファと見なされます。
#define DDS_ALPHA_MODE_STRAIGHT					(0x1)			// アルファチャネルコンテンツはすべてストレートアルファを使用するものと想定されます。
#define DDS_ALPHA_MODE_PREMULTIPLIED			(0x2)			// アルファチャネルコンテンツは、事前乗算されたアルファを使用しています。この情報を示す唯一のレガシーファイル形式は、「DX2」および「DX4」です。
#define DDS_ALPHA_MODE_OPAQUE					(0x3)			// アルファチャネルコンテンツはすべて完全に不透明に設定されます。
#define DDS_ALPHA_MODE_CUSTOM					(0x4)			// アルファチャネルコンテンツは、4番目のチャネルとして使用されており、透明度（ストレートまたは事前乗算）を表すことを意図していません。
...
struct DDS_HEADER_DXT10
{
	...
	DWORD	dwMiscFlag ;				// その他フラグ( DDS_RESOURCE_MISC_TEXTURECUBE( 0x04 )でキューブマップ
	DWORD	dwArraySize ;				// キューブマップの場合の配列長
	DWORD	dwMiscFlag2 ;				// DDS_ALPHA_MODE_UNKNOWN 等
} ;
...
static	int LoadTargaImage( STREAMDATA *Src, BASEIMAGE *BaseImage, int GetFormatOnly ) ;					// ＴＧＡ画像の読みこみ
		...
		int LoadDDSImage( STREAMDATA *Src, BASEIMAGE *BaseImage, int GetFormatOnly ) ;						// ＤＤＳ画像の読みこみ
...
END :
	...
	else
	{
		...
		// もし ConvertNormalFormatBaseImage フラグが立っていたら変換する
...
extern int CreateGraphImageType2_UseGParam( CREATEBASEIMAGETYPE2_GPARAM *GParam, STREAMDATA *Src, BASEIMAGE *Dest )
{
	...
	// 環境依存のデフォルト関数を後から実行する設定ではなかった場合はここで終了
	if( BASEIM.PlatformLoadFunctionAfterFlag == FALSE )
	{
		return -1 ;
	}
