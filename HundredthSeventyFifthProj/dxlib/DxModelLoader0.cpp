...
extern int PStrGet( X_PSTRING *PStrBuf, char *StrBuf, size_t BufferBytes )
{
	...
	while( PStrBuf->StrOffset < PStrBuf->StrSize )
	{
			...
			PStrBuf->StrBuf[PStrBuf->StrOffset] == '#' )
		{
			...
			if( PStrBuf->StrOffset >= PStrBuf->StrSize )
				return -1 ;
		}
...
extern int PStrKakkoSkip( X_PSTRING *PString )
{
	...
	if( PString->StrOffset >= PString->StrSize )
		return -1 ;
	PString->StrOffset ++ ;
	KakkoNum = 1 ;
...
{
	if( PStrBuf->StrSize <= PStrBuf->StrOffset )
		return -1 ;
...
*/
				// テクスチャファイル名が無い場合は無効
				if( MT->TextureFileName != NULL && ( MT->TextureFileName->filename.String == NULL || MT->TextureFileName->filename.String[ 0 ] == '\0' ) )
				{
					MT->TextureFileName = NULL ;
				}
...
static int TempString( X_PSTRING *PStr, X_MODEL *Model, X_STRING *String )
{
	...
	if( !PStr->binf )
	{
		...
		// ダブルコーテーションが２回連続している場合は処理を分岐
		if( PStr->StrBuf[ PStr->StrOffset ] == '\"' )
		{
			StrB[ 0 ] = '\0' ;
		}
		else
	...
	else
	{
		...
		if( PStr->StrOffset >= PStr->StrSize )
			return -1 ;
