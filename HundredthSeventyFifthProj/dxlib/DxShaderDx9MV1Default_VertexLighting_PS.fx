...
*/
	...
	TextureDiffuseColor = tex2D( DiffuseMapTexture, PSInput.TexCoords0_1.xy ) ;
		...
		Temp = 0.29900f * cfMaterial.Specular.r + 0.58700f * cfMaterial.Specular.g + 0.11400f * cfMaterial.Specular.b ;

	#endif // MAT_PARAM_SPEC_LUMINANCE

	#if MAT_PARAM_SPEC_POWER

		Temp = cfMaterial.Power.x ;

	#endif // MAT_PARAM_SPEC_POWER

	#if MAT_CALC_UNORM

		PSOutput.Color0.rgb = saturate( ( Temp - cfMaterial.Power.y ) / ( cfMaterial.Power.z - cfMaterial.Power.y ) );

	#endif // MAT_CALC_UNORM

	#if MAT_CALC_CLIP_UNORM

		if( Temp < cfMaterial.Power.w )
		{
...
// ライト2の処理 +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++( 終了 )
	...
	// アルファ乗算カラー
	if( cfMulAlphaColorAddColor.x > 0.5f )
	{
