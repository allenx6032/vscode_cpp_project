...
extern int NS_ChangeStreamFunctionW( const STREAMDATASHREDTYPE2W *StreamThreadW )
{
	...
	if( StreamThreadW == NULL )
	{
...
#endif
		{
			...
			StreamFunctionW.IsDXA		= ReadOnlyFileAccessIsDXA ;
		...
		else
		{
			...
			StreamFunctionW.IsDXA		= DXA_DIR_IsDXA ;
...
extern DWORD_PTR StreamOpenW( const char *PathW, int UseCacheFlag, int BlockReadFlag, int UseASyncReadFlag )
{
	return StreamFunctionW.Open( ( wchar_t * )PathW, UseCacheFlag, BlockReadFlag, UseASyncReadFlag ) ;
}
...
extern int StreamIsDXA( DWORD_PTR Handle )
{
	return StreamFunctionW.IsDXA( Handle ) ;
}
...
extern int ReadOnlyFileAccessIsDXA( DWORD_PTR /* Handle */ )
{
	return 0 ;
}
