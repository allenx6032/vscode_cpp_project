...
//	int							SrcDataValidSize ;		// 変換元のデータの有効なサイズ
	LONGLONG					SrcDataPosition ;		// 変換元のデータの変換が完了したサイズ
	LONGLONG					DestDataSampleNum ;		// 変換後のデータのサンプル数( -1:以外の場合のみ有効 )

	LONGLONG					BeginSeekCompSrcSize ;	// シーク予定位置までサブACMハンドルで変換しておく処理で、既に変換が完了している圧縮データのサイズ
	LONGLONG					BeginSeekPosition ;		// シーク予定位置までサブACMハンドルで変換しておく処理で、既に変換が完了している展開後データのサイズ(シーク予定位置は SOUNDCONV 関数の SeekLockPosition メンバー)
...
extern	int      SetSampleTimeSoundConvert_ACM(    struct SOUNDCONV *SoundConv, LONGLONG SampleTime ) ;			// 変換処理の位置を変更する( サンプル単位 )
extern	LONGLONG GetSoundConvertDestSize_Fast_ACM( struct SOUNDCONV *SoundConv ) ;								// 変換後の大凡のデータサイズを得る
#endif
...
extern	int      SetSampleTimeSoundConvert_DSMP3(    struct SOUNDCONV *SoundConv, LONGLONG SampleTime ) ;		// 変換処理の位置を変更する( サンプル単位 )
extern	LONGLONG GetSoundConvertDestSize_Fast_DSMP3( struct SOUNDCONV *SoundConv ) ;							// 変換後の大凡のデータサイズを得る
#endif
#endif
#endif
...
extern	int      SetSampleTimeSoundConvert_MF(    SOUNDCONV *SoundConv, LONGLONG SampleTime ) ;					// 変換処理の位置を変更する( サンプル単位 )
extern	LONGLONG GetSoundConvertDestSize_Fast_MF( SOUNDCONV *SoundConv ) ;										// 変換後の大凡のデータサイズを得る
