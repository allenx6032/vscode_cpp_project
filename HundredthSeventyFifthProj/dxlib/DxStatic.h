...
extern	ULONGLONG	NS_GetNowSysPerformanceCount(	void ) ;									// OSが提供する高精度カウンタの現在の値を得る
extern	ULONGLONG	NS_GetSysPerformanceFrequency(	void ) ;									// OSが提供する高精度カウンタの周波数( 1秒辺りのカウント数 )を得る
extern	ULONGLONG	NS_ConvSysPerformanceCountToSeconds(		ULONGLONG Count ) ;				// OSが提供する高精度カウンタの値を秒の値に変換する
extern	ULONGLONG	NS_ConvSysPerformanceCountToMilliSeconds(	ULONGLONG Count ) ;				// OSが提供する高精度カウンタの値をミリ秒の値に変換する
extern	ULONGLONG	NS_ConvSysPerformanceCountToMicroSeconds(	ULONGLONG Count ) ;				// OSが提供する高精度カウンタの値をマイクロ秒の値に変換する
extern	ULONGLONG	NS_ConvSysPerformanceCountToNanoSeconds(	ULONGLONG Count ) ;				// OSが提供する高精度カウンタの値をナノ秒の値に変換する
extern	ULONGLONG	NS_ConvSecondsToSysPerformanceCount(		ULONGLONG Seconds ) ;			// 秒の値をOSが提供する高精度カウンタの値に変換する
extern	ULONGLONG	NS_ConvMilliSecondsToSysPerformanceCount(	ULONGLONG MilliSeconds ) ;		// ミリ秒の値をOSが提供する高精度カウンタの値に変換する
extern	ULONGLONG	NS_ConvMicroSecondsToSysPerformanceCount(	ULONGLONG MicroSeconds ) ;		// マイクロ秒の値をOSが提供する高精度カウンタの値に変換する
extern	ULONGLONG	NS_ConvNanoSecondsToSysPerformanceCount(	ULONGLONG NanoSeconds ) ;		// ナノ秒の値をOSが提供する高精度カウンタの値に変換する
...
extern	DWORD		NS_GetMersenneTwisterRand( void ) ;										// メルセンヌ・ツイスターアルゴリズムで生成された乱数値を無加工で取得する

extern	DWORD_PTR	NS_CreateRandHandle( int Seed ) ;											// 乱数ハンドルを作成する( 戻り値　0以外:乱数ハンドル　0:エラー )
extern	int			NS_DeleteRandHandle( DWORD_PTR RandHandle ) ;								// 乱数ハンドルを削除する
extern	int			NS_SRandHandle( DWORD_PTR RandHandle, int Seed ) ;							// 乱数ハンドルの初期値を再設定する
extern	int			NS_GetRandHandle( DWORD_PTR RandHandle, int RandMax ) ;						// 乱数ハンドルを使用して乱数を取得する( RandMax : 返って来る値の最大値 )
extern	DWORD		NS_GetMersenneTwisterRandHandle( DWORD_PTR RandHandle ) ;					// 乱数ハンドルを使用してメルセンヌ・ツイスターアルゴリズムで生成された乱数値を無加工で取得する
...
// iniファイル関係
extern	int			NS_GetPrivateProfileStringDx(                 const TCHAR *AppName,                       const TCHAR *KeyName,                       const TCHAR *Default,                       TCHAR *ReturnedStringBuffer, size_t ReturnedStringBufferBytes, const TCHAR *IniFilePath,                           int IniFileCharCodeFormat = -1 /* DX_CHARCODEFORMAT_SHIFTJIS 等、-1 でデフォルト */ ) ;		// GetPrivateProfileString のＤＸライブラリ版
...
extern	int			NS_GetPrivateProfileIntDx(                    const TCHAR *AppName,                       const TCHAR *KeyName,                       int          Default,                                                                                      const TCHAR *IniFilePath,                           int IniFileCharCodeFormat = -1 /* DX_CHARCODEFORMAT_SHIFTJIS 等、-1 でデフォルト */ ) ;		// GetPrivateProfileInt のＤＸライブラリ版
extern	int			NS_GetPrivateProfileIntDxWithStrLen(          const TCHAR *AppName, size_t AppNameLength, const TCHAR *KeyName, size_t KeyNameLength, int          Default,                                                                                      const TCHAR *IniFilePath, size_t IniFilePathLength, int IniFileCharCodeFormat = -1 /* DX_CHARCODEFORMAT_SHIFTJIS 等、-1 でデフォルト */ ) ;		// GetPrivateProfileInt のＤＸライブラリ版
...
extern	int			NS_GetPrivateProfileIntDxForMem(              const TCHAR *AppName,                       const TCHAR *KeyName,                       int          Default,                                                                                      const void *IniFileImage, size_t IniFileImageBytes, int IniFileCharCodeFormat = -1 /* DX_CHARCODEFORMAT_SHIFTJIS 等、-1 でデフォルト */ ) ;		// GetPrivateProfileIntDx のメモリから読み込む版
extern	int			NS_GetPrivateProfileIntDxForMemWithStrLen(    const TCHAR *AppName, size_t AppNameLength, const TCHAR *KeyName, size_t KeyNameLength, int          Default,                                                                                      const void *IniFileImage, size_t IniFileImageBytes, int IniFileCharCodeFormat = -1 /* DX_CHARCODEFORMAT_SHIFTJIS 等、-1 でデフォルト */ ) ;		// GetPrivateProfileIntDx のメモリから読み込む版
...
extern	int			NS_SetOutApplicationSystemLogValidFlag(      int Flag ) ;													// ログファイル( Log.txt ) にＤＸライブラリ内部のログ出力を行うかどうか設定する( TRUE:ＤＸライブラリ内部のログ出力を行う( デフォルト )  FALSE:ＤＸライブラリ内部のログ出力を行わない )
...
extern	int			NS_setPrintColorDx(     int Color, int EdgeColor = 0 ) ;													// printf や puts で表示する文字列の色を指定する
#endif
...
extern	int			NS_SetASyncLoadFinishCallback(	int Handle, void ( *Callback )( int Handle, void *Data ), void *Data ) ;	// ハンドルの非同期読み込み処理が完了したら呼ばれる関数をセットする
extern	int			NS_WaitHandleASyncLoad(					int Handle ) ;								// 指定のハンドルの非同期読み込み処理が終了するまで待つ
extern	int			NS_WaitHandleASyncLoadAll(				void ) ;									// 全ての非同期読み込みデータが読み込み終わるまで待つ
...
// 便利関数
extern	int			NS_GetResourceInfo(		const TCHAR *ResourceName , const TCHAR *ResourceType , void **DataPointerP , size_t *DataSizeP ) ;		// 指定のリソースを取得する( -1:失敗  0:成功 )
extern	int			NS_GetResourceInfoWithStrLen(	const TCHAR *ResourceName, size_t ResourceNameLength, const TCHAR *ResourceType, size_t ResourceTypeLength, void **DataPointerP , size_t *DataSizeP ) ;		// 指定の名前、タイプのリソースのアドレスとサイズを取得する( 戻り値  -1:失敗  0:成功 )
...
extern	int			NS_GetDefaultState(				int *SizeX , int *SizeY , int *ColorBitDepth, int *RefreshRate = NULL , int *LeftTopX = NULL , int *LeftTopY = NULL, int *PixelSizeX = NULL , int *PixelSizeY = NULL , int *XDpi = NULL , int *YDpi = NULL ) ;		// 起動時のデスクトップの画面モードを取得する
extern	int			NS_GetMonitorDpi(					int *XDpi, int *YDpi, int MonitorIndex = -1 ) ;			// モニターのDPIを取得する
...
extern	int			NS_GetWindowOSVersion(				void ) ;												// WindowsOSのバージョンを取得する( 戻り値 : DX_WINDOWSVERSION_10 など )
...
extern	int			NS_CheckWindowMaximizeButtonInput(	int StateResetFlag = TRUE ) ;							// メインウインドウの最大化ボタンが押されたかどうかを取得する、SetWindowMaximizeButtonBehavior( 1 ); が実行されている場合のみ有効な関数( StateResetFlag  TRUE = 押された状態をリセットする   FALSE = 押された状態をリセットしない )
...
extern	int			NS_SetWindowTextDX(						const TCHAR *WindowText                          ) ;								// メインウインドウのウインドウテキストを変更する
...
extern	int			NS_SetWindowMaximizeButtonBehavior(		int BehaviorType ) ;																// メインウインドウの最大化ボタンが押されたときの挙動を設定する( BehaviorType 0=標準動作 1=標準動作は行わず、最大化ボタンが押されたかどうかは CheckWindowMaximizeButtonInput で判定する )
...
extern	int			NS_SetUseBorderlessWindowFlag(				int Flag ) ;																		// メインウインドウをボーダーレスウィンドウにするかどうかを設定する( TRUE:ボーダーレスウィンドウ　FALSE:通常ウィンドウ )
...
extern	int			NS_SetUseMouseEventTransparentWindowFlag(	int Flag ) ;																		// SetUseUpdateLayerdWindowFlag( TRUE ); の設定の際にマウス関係のイベントを背後のウィンドウに透過させるかを設定する( TRUE:透過する   FALSE:透過しない( デフォルト ) )
...
extern	int			NS_SetWindows10_WM_CHAR_CancelTime(		int MilliSecond ) ;																	// Windows10 で WM_CHAR で短時間に連続して同じ文字が入力された場合の無効扱いにする時間を設定する( MilliSecond のミリ秒以内に連続して同じ文字が入力された場合に無効にする、MilliSecond の値をマイナスにするとデフォルトの設定に戻る )
...
extern	int			NS_GetDragFilePath(			TCHAR *FilePathBuffer, int FilePathBufferBytes DEFAULTPARAM( = -1 ) ) ;								// ドラッグ＆ドロップされたファイル名を取得する( -1:取得できなかった  0:取得できた )
...
#endif // DX_COMPILE_TYPE_C_LANGUAGE
extern	int			NS_GetMenuItemIDDX(				const TCHAR *ItemName ) ;																			// メニューの項目名からＩＤを取得する
...
extern	int			NS_SetMouseDispIgnoreMenuFlag(	int IgnoreMenuFlag ) ;										// マウスポインタの表示状態についてメニューの表示状態を無視するかどうかを設定する( TRUE:メニューの表示状態を無視する  FALSE:メニューの表示状態を考慮する( デフォルト ) )
...
extern	int				NS_GetTouchInput( int InputNo, int *PositionX, int *PositionY, int *ID = NULL , int *Device = NULL, float *Pressure = NULL ) ;		// タッチの情報を取得する
...
extern	void*		NS_DxAllocAligned(					               size_t AllocSize , size_t Alignment , const char *File = NULL , int Line = -1 ) ;	// 指定のサイズのメモリを確保する( アライン指定版 )( AllocSize:確保するメモリのサイズ( 単位:byte )  Alignment:確保するメモリ領域のアドレス値の倍数  File:DxAllocを呼んだソースファイル名( デバッグ用 )  Line:DxAllocを呼んだソースファイル中の行番号( デバッグ用 )　　戻り値  NULL:メモリの確保失敗   NULL以外:確保したメモリ領域の先頭アドレス )
extern	void*		NS_DxCalloc(						size_t AllocSize , const char *File = NULL , int Line = -1 ) ;					// メモリを確保して０で初期化する
extern	void*		NS_DxCallocAligned(				               size_t AllocSize , size_t Alignment , const char *File = NULL , int Line = -1 ) ;	// 指定のサイズのメモリを確保して、０で埋める、初期化後に０で埋める以外は DxAllocAligned と動作は同じ
extern	void*		NS_DxRealloc(						void *Memory , size_t AllocSize , const char *File = NULL , int Line = -1 ) ;	// メモリの再確保を行う
extern	void*		NS_DxReallocAligned(				void *Memory , size_t AllocSize , size_t Alignment , const char *File = NULL , int Line = -1 ) ;	// メモリの再確保を行う( アライン指定版 )( Memory:再確保を行うメモリ領域の先頭アドレス( DxAlloc の戻り値 )  AllocSize:新しい確保サイズ   Alignment:新しく確保するメモリ領域のアドレス値の倍数　FileとLine の説明は DxAlloc の注釈の通り 　戻り値 NULL:メモリの再確保失敗　NULL以外:再確保した新しいメモリ領域の先頭アドレス　)
...
extern	int			NS_GetUseCharCodeFormat( void ) ;															// 文字列の引数の文字コード形式を取得する( 戻り値：文字コード形式( DX_CHARCODEFORMAT_SHIFTJIS 等 ) )( UNICODE版では無効 )
...
extern	int			NS_DrawIMEInputExtendString(		int x, int y, double ExRateX, double ExRateY, int SelectStringNum , int DrawCandidateList DEFAULTPARAM( = TRUE ) ) ;	// 画面上に入力中の文字列を描画する( 拡大率付き )
...
extern	int			NS_SetIMEInputString(				const TCHAR *String ) ;																											// IMEで入力中の文字列を変更する( IMEで文字列を入力中ではなかった場合は何も起こりません )
extern	int			NS_SetIMEInputStringWithStrLen(	const TCHAR *String, size_t StringLength ) ;																					// IMEで入力中の文字列を変更する( IMEで文字列を入力中ではなかった場合は何も起こりません )
...
extern	int			NS_DrawKeyInputExtendString(		int x, int y, double ExRateX, double ExRateY, int InputHandle , int DrawCandidateList DEFAULTPARAM( = TRUE ) ) ;				// キー入力ハンドルの入力中情報の描画( 拡大率指定付き )
...
extern	LONGLONG	NS_FileRead_size_handle(				int FileHandle ) ;												// ファイルのサイズを取得する( ファイルハンドル使用版 )
...
extern	int			NS_GetHitKeyStateAllEx(					int *KeyStateArray ) ;													// すべてのキーの押下状態を取得する( KeyStateBuf:int型256個分の配列の先頭アドレス、配列の各要素の状態　0:押されていない　1：押された１回め　2以上：押され続けている回数　-1：押されて離された１回め　-2以下:押されて離されてからの回数 )
extern	int			NS_GetJoypadNum(							void ) ;																// ジョイパッドが接続されている数を取得する
extern	int			NS_GetJoypadButtonNum(						int InputType ) ;														// ジョイパッドのボタンの数を取得する
...
extern	int			NS_ReSetupInputSystem(						void ) ;																// 入力システムの再セットアップを行う
...
extern	int			NS_GetJoypadType(							int PadIndex ) ;														// ジョイパッドのタイプを取得する( 戻り値：ジョイパッドタイプ( DX_PADTYPE_XBOX_360 など ) )
...
extern	int			NS_LoadDivGraph(						const TCHAR *FileName,                        int AllNum, int XNum, int YNum, int   XSize, int   YSize, int *HandleArray, int NotUse3DFlag DEFAULTPARAM( = FALSE ) , int   XStride DEFAULTPARAM( = 0 ) , int   YStride DEFAULTPARAM( = 0 ) ) ;	// 画像ファイルを分割してグラフィックハンドルを作成する
extern	int			NS_LoadDivGraphWithStrLen(				const TCHAR *FileName, size_t FileNameLength, int AllNum, int XNum, int YNum, int   XSize, int   YSize, int *HandleArray, int NotUse3DFlag DEFAULTPARAM( = FALSE ) , int   XStride DEFAULTPARAM( = 0 ) , int   YStride DEFAULTPARAM( = 0 ) ) ;	// 画像ファイルを分割してグラフィックハンドルを作成する
extern	int			NS_LoadDivGraphF(						const TCHAR *FileName,                        int AllNum, int XNum, int YNum, float XSize, float YSize, int *HandleArray, int NotUse3DFlag DEFAULTPARAM( = FALSE ) , float XStride DEFAULTPARAM( = 0 ) , float YStride DEFAULTPARAM( = 0 ) ) ;	// 画像ファイルを分割してグラフィックハンドルを作成する
extern	int			NS_LoadDivGraphFWithStrLen(			const TCHAR *FileName, size_t FileNameLength, int AllNum, int XNum, int YNum, float XSize, float YSize, int *HandleArray, int NotUse3DFlag DEFAULTPARAM( = FALSE ) , float XStride DEFAULTPARAM( = 0 ) , float YStride DEFAULTPARAM( = 0 ) ) ;	// 画像ファイルを分割してグラフィックハンドルを作成する
extern	int			NS_LoadDivBmpToGraph(					const TCHAR *FileName,                        int AllNum, int XNum, int YNum, int   SizeX, int   SizeY, int *HandleArray, int TextureFlag, int ReverseFlag , int   XStride DEFAULTPARAM( = 0 ) , int   YStride DEFAULTPARAM( = 0 ) ) ;			// 画像ファイルを分割してグラフィックハンドルを作成する
extern	int			NS_LoadDivBmpToGraphWithStrLen(		const TCHAR *FileName, size_t FileNameLength, int AllNum, int XNum, int YNum, int   SizeX, int   SizeY, int *HandleArray, int TextureFlag, int ReverseFlag , int   XStride DEFAULTPARAM( = 0 ) , int   YStride DEFAULTPARAM( = 0 ) ) ;			// 画像ファイルを分割してグラフィックハンドルを作成する
extern	int			NS_LoadDivBmpToGraphF(					const TCHAR *FileName,                        int AllNum, int XNum, int YNum, float SizeX, float SizeY, int *HandleArray, int TextureFlag, int ReverseFlag , float XStride DEFAULTPARAM( = 0 ) , float YStride DEFAULTPARAM( = 0 ) ) ;			// 画像ファイルを分割してグラフィックハンドルを作成する
extern	int			NS_LoadDivBmpToGraphFWithStrLen(		const TCHAR *FileName, size_t FileNameLength, int AllNum, int XNum, int YNum, float SizeX, float SizeY, int *HandleArray, int TextureFlag, int ReverseFlag , float XStride DEFAULTPARAM( = 0 ) , float YStride DEFAULTPARAM( = 0 ) ) ;			// 画像ファイルを分割してグラフィックハンドルを作成する
extern	int			NS_LoadReverseDivGraph(				const TCHAR *FileName,                        int AllNum, int XNum, int YNum, int   XSize, int   YSize, int *HandleArray, int NotUse3DFlag DEFAULTPARAM( = FALSE ) , int   XStride DEFAULTPARAM( = 0 ) , int   YStride DEFAULTPARAM( = 0 ) ) ;	// 画像ファイルを反転したものを分割してグラフィックハンドルを作成する
extern	int			NS_LoadReverseDivGraphWithStrLen(		const TCHAR *FileName, size_t FileNameLength, int AllNum, int XNum, int YNum, int   XSize, int   YSize, int *HandleArray, int NotUse3DFlag DEFAULTPARAM( = FALSE ) , int   XStride DEFAULTPARAM( = 0 ) , int   YStride DEFAULTPARAM( = 0 ) ) ;	// 画像ファイルを反転したものを分割してグラフィックハンドルを作成する
extern	int			NS_LoadReverseDivGraphF(				const TCHAR *FileName,                        int AllNum, int XNum, int YNum, float XSize, float YSize, int *HandleArray, int NotUse3DFlag DEFAULTPARAM( = FALSE ) , float XStride DEFAULTPARAM( = 0 ) , float YStride DEFAULTPARAM( = 0 ) ) ;	// 画像ファイルを反転したものを分割してグラフィックハンドルを作成する
extern	int			NS_LoadReverseDivGraphFWithStrLen(		const TCHAR *FileName, size_t FileNameLength, int AllNum, int XNum, int YNum, float XSize, float YSize, int *HandleArray, int NotUse3DFlag DEFAULTPARAM( = FALSE ) , float XStride DEFAULTPARAM( = 0 ) , float YStride DEFAULTPARAM( = 0 ) ) ;	// 画像ファイルを反転したものを分割してグラフィックハンドルを作成する
...
#endif // DX_COMPILE_TYPE_C_LANGUAGE
extern	int			NS_LoadGraphToResource2(				const TCHAR *ResourceName,                            const TCHAR *ResourceType                            ) ;																				// 画像リソースからグラフィックハンドルを作成する
extern	int			NS_LoadGraphToResourceWithStrLen2(		const TCHAR *ResourceName, size_t ResourceNameLength, const TCHAR *ResourceType, size_t ResourceTypeLength ) ;																				// 画像リソースからグラフィックハンドルを作成する
extern	int			NS_LoadDivGraphToResource2(			const TCHAR *ResourceName,                            const TCHAR *ResourceType,                            int AllNum, int XNum, int YNum, int   XSize, int   YSize, int *HandleArray ) ;	// 画像リソースを分割してグラフィックハンドルを作成する
extern	int			NS_LoadDivGraphToResourceWithStrLen2(	const TCHAR *ResourceName, size_t ResourceNameLength, const TCHAR *ResourceType, size_t ResourceTypeLength, int AllNum, int XNum, int YNum, int   XSize, int   YSize, int *HandleArray ) ;	// 画像リソースを分割してグラフィックハンドルを作成する
extern	int			NS_LoadDivGraphFToResource2(			const TCHAR *ResourceName,                            const TCHAR *ResourceType,                            int AllNum, int XNum, int YNum, float XSize, float YSize, int *HandleArray ) ;	// 画像リソースを分割してグラフィックハンドルを作成する( float型 )
extern	int			NS_LoadDivGraphFToResourceWithStrLen2(	const TCHAR *ResourceName, size_t ResourceNameLength, const TCHAR *ResourceType, size_t ResourceTypeLength, int AllNum, int XNum, int YNum, float XSize, float YSize, int *HandleArray ) ;	// 画像リソースを分割してグラフィックハンドルを作成する( float型 )
...
extern	int			NS_CreateGraphFromGraphImage(			const BASEIMAGE *RgbBaseImage,                                                                                                                  int TextureFlag DEFAULTPARAM( = TRUE ) , int ReverseFlag DEFAULTPARAM( = FALSE ) ) ;																// 基本イメージデータからグラフィックハンドルを作成する
extern	int			NS_ReCreateGraphFromGraphImage(		const BASEIMAGE *RgbBaseImage,                                  int GrHandle,                                                                   int TextureFlag DEFAULTPARAM( = TRUE ) , int ReverseFlag DEFAULTPARAM( = FALSE ) ) ;																// 基本イメージデータから既存のグラフィックハンドルにデータを転送する
extern	int			NS_CreateDivGraphFromGraphImage(		      BASEIMAGE *RgbBaseImage,                                  int AllNum, int XNum, int YNum, int   SizeX, int   SizeY,       int *HandleArray, int TextureFlag DEFAULTPARAM( = TRUE ) , int ReverseFlag DEFAULTPARAM( = FALSE ) ) ;																// 基本イメージデータから分割グラフィックハンドルを作成する
extern	int			NS_CreateDivGraphFFromGraphImage(		      BASEIMAGE *RgbBaseImage,                                  int AllNum, int XNum, int YNum, float SizeX, float SizeY,       int *HandleArray, int TextureFlag DEFAULTPARAM( = TRUE ) , int ReverseFlag DEFAULTPARAM( = FALSE ) ) ;																// 基本イメージデータから分割グラフィックハンドルを作成する( float版 )
extern	int			NS_ReCreateDivGraphFromGraphImage(		      BASEIMAGE *RgbBaseImage,                                  int AllNum, int XNum, int YNum, int   SizeX, int   SizeY, const int *HandleArray, int TextureFlag DEFAULTPARAM( = TRUE ) , int ReverseFlag DEFAULTPARAM( = FALSE ) ) ;																// 基本イメージデータから既存の分割グラフィックハンドルにデータを転送する
extern	int			NS_ReCreateDivGraphFFromGraphImage(	      BASEIMAGE *RgbBaseImage,                                  int AllNum, int XNum, int YNum, float SizeX, float SizeY, const int *HandleArray, int TextureFlag DEFAULTPARAM( = TRUE ) , int ReverseFlag DEFAULTPARAM( = FALSE ) ) ;																// 基本イメージデータから既存の分割グラフィックハンドルにデータを転送する( float版 )
#ifndef DX_COMPILE_TYPE_C_LANGUAGE
extern	int			NS_CreateGraphFromGraphImage(			const BASEIMAGE *RgbBaseImage, const BASEIMAGE *AlphaBaseImage,                                                                                 int TextureFlag DEFAULTPARAM( = TRUE ) , int ReverseFlag DEFAULTPARAM( = FALSE ) ) ;																// 基本イメージデータからグラフィックハンドルを作成する
extern	int			NS_ReCreateGraphFromGraphImage(		const BASEIMAGE *RgbBaseImage, const BASEIMAGE *AlphaBaseImage, int GrHandle,                                                                   int TextureFlag DEFAULTPARAM( = TRUE ) , int ReverseFlag DEFAULTPARAM( = FALSE ) ) ;																// 基本イメージデータから既存のグラフィックハンドルにデータを転送する
extern	int			NS_CreateDivGraphFromGraphImage(		      BASEIMAGE *RgbBaseImage, const BASEIMAGE *AlphaBaseImage, int AllNum, int XNum, int YNum, int   SizeX, int   SizeY,       int *HandleArray, int TextureFlag DEFAULTPARAM( = TRUE ) , int ReverseFlag DEFAULTPARAM( = FALSE ) ) ;																// 基本イメージデータから分割グラフィックハンドルを作成する
extern	int			NS_CreateDivGraphFFromGraphImage(		      BASEIMAGE *RgbBaseImage, const BASEIMAGE *AlphaBaseImage, int AllNum, int XNum, int YNum, float SizeX, float SizeY,       int *HandleArray, int TextureFlag DEFAULTPARAM( = TRUE ) , int ReverseFlag DEFAULTPARAM( = FALSE ) ) ;																// 基本イメージデータから分割グラフィックハンドルを作成する( float版 )
extern	int			NS_ReCreateDivGraphFromGraphImage(		      BASEIMAGE *RgbBaseImage, const BASEIMAGE *AlphaBaseImage, int AllNum, int XNum, int YNum, int   SizeX, int   SizeY, const int *HandleArray, int TextureFlag DEFAULTPARAM( = TRUE ) , int ReverseFlag DEFAULTPARAM( = FALSE ) ) ;																// 基本イメージデータから既存の分割グラフィックハンドルにデータを転送する
extern	int			NS_ReCreateDivGraphFFromGraphImage(	      BASEIMAGE *RgbBaseImage, const BASEIMAGE *AlphaBaseImage, int AllNum, int XNum, int YNum, float SizeX, float SizeY, const int *HandleArray, int TextureFlag DEFAULTPARAM( = TRUE ) , int ReverseFlag DEFAULTPARAM( = FALSE ) ) ;																// 基本イメージデータから既存の分割グラフィックハンドルにデータを転送する( float版 )
#endif // DX_COMPILE_TYPE_C_LANGUAGE
extern	int			NS_CreateGraphFromGraphImage2(			const BASEIMAGE *RgbBaseImage, const BASEIMAGE *AlphaBaseImage,                                                                                 int TextureFlag DEFAULTPARAM( = TRUE ) , int ReverseFlag DEFAULTPARAM( = FALSE ) ) ;																// 基本イメージデータからグラフィックハンドルを作成する
extern	int			NS_ReCreateGraphFromGraphImage2(		const BASEIMAGE *RgbBaseImage, const BASEIMAGE *AlphaBaseImage, int GrHandle,                                                                   int TextureFlag DEFAULTPARAM( = TRUE ) , int ReverseFlag DEFAULTPARAM( = FALSE ) ) ;																// 基本イメージデータから既存のグラフィックハンドルにデータを転送する
extern	int			NS_CreateDivGraphFromGraphImage2(		      BASEIMAGE *RgbBaseImage, const BASEIMAGE *AlphaBaseImage, int AllNum, int XNum, int YNum, int   SizeX, int   SizeY,       int *HandleArray, int TextureFlag DEFAULTPARAM( = TRUE ) , int ReverseFlag DEFAULTPARAM( = FALSE ) ) ;																// 基本イメージデータから分割グラフィックハンドルを作成する
extern	int			NS_CreateDivGraphFFromGraphImage2(		      BASEIMAGE *RgbBaseImage, const BASEIMAGE *AlphaBaseImage, int AllNum, int XNum, int YNum, float SizeX, float SizeY,       int *HandleArray, int TextureFlag DEFAULTPARAM( = TRUE ) , int ReverseFlag DEFAULTPARAM( = FALSE ) ) ;																// 基本イメージデータから分割グラフィックハンドルを作成する( float版 )
extern	int			NS_ReCreateDivGraphFromGraphImage2(	      BASEIMAGE *RgbBaseImage, const BASEIMAGE *AlphaBaseImage, int AllNum, int XNum, int YNum, int   SizeX, int   SizeY, const int *HandleArray, int TextureFlag DEFAULTPARAM( = TRUE ) , int ReverseFlag DEFAULTPARAM( = FALSE ) ) ;																// 基本イメージデータから既存の分割グラフィックハンドルにデータを転送する
extern	int			NS_ReCreateDivGraphFFromGraphImage2(	      BASEIMAGE *RgbBaseImage, const BASEIMAGE *AlphaBaseImage, int AllNum, int XNum, int YNum, float SizeX, float SizeY, const int *HandleArray, int TextureFlag DEFAULTPARAM( = TRUE ) , int ReverseFlag DEFAULTPARAM( = FALSE ) ) ;																// 基本イメージデータから既存の分割グラフィックハンドルにデータを転送する( float版 )
...
extern	int			NS_ReCreateDivGraphFromSoftImage(  int SIHandle, int AllNum, int XNum, int YNum, int   SizeX, int   SizeY, const int *HandleArray ) ;										// ソフトウエアで扱うイメージから既存の分割グラフィックハンドルにデータを転送する
extern	int			NS_ReCreateDivGraphFFromSoftImage( int SIHandle, int AllNum, int XNum, int YNum, float SizeX, float SizeY, const int *HandleArray ) ;										// ソフトウエアで扱うイメージから既存の分割グラフィックハンドルにデータを転送する( float版 )
...
extern	int			NS_ReCreateDivGraphFromBaseImage(		      BASEIMAGE *BaseImage, int AllNum, int XNum, int YNum, int   SizeX, int   SizeY, const int *HandleArray ) ;						// 基本イメージデータから既存の分割グラフィックハンドルにデータを転送する
extern	int			NS_ReCreateDivGraphFFromBaseImage(	          BASEIMAGE *BaseImage, int AllNum, int XNum, int YNum, float SizeX, float SizeY, const int *HandleArray ) ;						// 基本イメージデータから既存の分割グラフィックハンドルにデータを転送する( float版 )
...
extern	int			NS_SetCreateGraphInitGraphDelete(				int Flag ) ;									// 作成するグラフィックハンドルを InitGraph() で削除されるかを設定する( Flag  TRUE:InitGraphで削除される(デフォルト)  FALSE:InitGraphで削除されない )
extern	int			NS_GetCreateGraphInitGraphDelete(				void ) ;										// 作成するグラフィックハンドルを InitGraph() で削除されるかを取得する( Flag  TRUE:InitGraphで削除される(デフォルト)  FALSE:InitGraphで削除されない )
extern	int			NS_SetCreateGraphHandle(						int GrHandle ) ;								// 作成するグラフィックハンドルのハンドル値を設定する、０以下の値を渡すと設定解除( 存在しないグラフィックハンドルの値の場合のみ有効 )
extern	int			NS_GetCreateGraphHandle(						void ) ;										// 作成するグラフィックハンドルのハンドル値を取得する
extern	int			NS_SetCreateDivGraphHandle(					const int *HandleArray, int HandleNum ) ;		// 作成するグラフィックハンドルのハンドル値を設定する、LoadDivGraph 等の分割画像読み込み用、HandleArray に NULL を渡すと設定解除( 存在しないグラフィックハンドルの値の場合のみ有効 )
extern	int			NS_GetCreateDivGraphHandle(					int *HandleArray ) ;							// 作成するグラフィックハンドルのハンドル値を取得する、LoadDivGraph 等の分割画像読み込み用、戻り値は SetCreateDivGraphHandle の引数 HandleNum に渡した値、HandleArray を NULL にすることが可能
...
extern	int			NS_SetUseLoadDivGraphSizeCheckFlag(			int Flag ) ;									// LoadDivGraph 系の分割画像読み込み関数でサイズのチェックを行うかどうかを設定する( Flag:TRUE( チェックを行う(デフォルト) )  FALSE:チェックを行わない )
extern	int			NS_GetUseLoadDivGraphSizeCheckFlag(			void ) ;										// LoadDivGraph 系の分割画像読み込み関数でサイズのチェックを行うかどうかの設定を取得する
...
#endif // DX_COMPILE_TYPE_C_LANGUAGE
extern	const COLORDATA*	NS_GetTexColorData2(			const IMAGEFORMATDESC *Format ) ;														// フォーマットに基づいたカラーデータを得る
extern	const COLORDATA*	NS_GetTexColorData3(			int FormatIndex /* DX_GRAPHICSIMAGE_FORMAT_3D_RGB32 等 */ ) ;							// 指定のフォーマットインデックスのカラーデータを得る
...
extern	int			NS_DrawBoxSet(     const RECTDATA *RectData,   int Num ) ;																	// 矩形の集合を描画する
...
extern	int			NS_DrawCubeSet3D(   CUBEDATA *CubeDataArray, int Num, int FillFlag ) ;																				// ３Ｄの立方体の集合を描画する
...
#endif // DX_COMPILE_TYPE_C_LANGUAGE
extern	int			NS_DrawChipMap2(             int MapWidth, int MapHeight,        const int *MapData, int ChipTypeNum,                   const int *ChipGrHandle, int TransFlag, int MapDrawPointX, int MapDrawPointY, int MapDrawWidth, int MapDrawHeight, int ScreenX, int ScreenY ) ;	// チップグラフィックを使ったマップ描画
...
extern	int			NS_DrawRectGraphF2(          float DestX,  float DestY,                              float SrcX, float SrcY, float  Width, float  Height,                           int GraphHandle, int TransFlag, int ReverseXFlag DEFAULTPARAM( = FALSE ) , int ReverseYFlag DEFAULTPARAM( = FALSE ) ) ;	// 画像の指定矩形部分のみを等倍描画( 座標指定が float 版( 画像内座標の指定も float 版 ) )
...
extern	int			NS_DrawRectExtendGraphF2(    float DestX1, float DestY1, float DestX2, float DestY2, float SrcX, float SrcY, float SrcWidth, float SrcHeight,                   int GraphHandle, int TransFlag ) ;															// 画像の指定矩形部分のみを拡大描画( 座標指定が float 版( 画像内座標の指定も float 版 ) )
...
extern	int			NS_DrawBlendGraphF(          float x, float y, int GrHandle, int TransFlag,                 int BlendGraph, int BorderParam, int BorderRange ) ;									// ブレンド画像と合成して画像を等倍描画する( 座標指定が float 版 )
...
extern	int			NS_DrawCircleGauge(          int CenterX, int CenterY, double Percent, int GrHandle, double StartPercent = 0.0 , double Scale = 1.0 , int ReverseX = FALSE , int ReverseY = FALSE ) ;														// 円グラフ的な描画を行う( GrHandle の画像の上下左右の端は透過色にしておく必要があります )
extern	int			NS_DrawCircleGaugeF(         float CenterX, float CenterY, double Percent, int GrHandle, double StartPercent = 0.0 , double Scale = 1.0 , int ReverseX = FALSE , int ReverseY = FALSE ) ;										// 円グラフ的な描画を行う( GrHandle の画像の上下左右の端は透過色にしておく必要があります )( 座標指定が float 版 )
...
extern	int			NS_DrawPolygon32bitIndexed2D(               const VERTEX2D  *VertexArray, int VertexNum, const unsigned int   *IndexArray, int PolygonNum,                                                      int GrHandle, int TransFlag ) ;							// ２Ｄポリゴンを描画する( 頂点インデックスを使用 )
...
extern	int			NS_DrawPolygon32bitIndexed3D(               const VERTEX3D  *VertexArray, int VertexNum, const unsigned int   *IndexArray, int PolygonNum,                                                      int GrHandle, int TransFlag ) ;							// ３Ｄポリゴンを描画する( 頂点インデックスを使用 )
...
extern	int			NS_DrawPolygon32bitIndexed3DBase(           const VERTEX_3D *VertexArray, int VertexNum, const unsigned int   *IndexArray, int IndexNum,   int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */, int GrHandle, int TransFlag ) ;							// ３Ｄポリゴンを描画する( 頂点インデックスを使用 )( 旧バージョン用 )
...
#endif // DX_COMPILE_TYPE_C_LANGUAGE
extern	int			NS_DrawPolygon3D2(                          const VERTEX_3D *Vertex, int PolygonNum,                                                                                                    int GrHandle, int TransFlag ) ;							// ３Ｄポリゴンを描画する( 旧バージョン用 )
...
extern	int			NS_DrawPrimitive32bitIndexed2D(             const VERTEX2D  *VertexArray, int VertexNum, const unsigned int   *IndexArray, int IndexNum,   int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */, int GrHandle, int TransFlag ) ;							// ２Ｄプリミティブを描画する(頂点インデックス使用)
...
extern	int			NS_DrawPrimitive32bitIndexed3D(             const VERTEX3D  *VertexArray, int VertexNum, const unsigned int   *IndexArray, int IndexNum,   int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */, int GrHandle, int TransFlag ) ;							// ３Ｄプリミティブを描画する(頂点インデックス使用)
...
extern	int			NS_SetDrawAddColor(					int Red, int Green, int Blue ) ;								// 描画カラーに加算する色を設定する
extern	int			NS_GetDrawAddColor(					int *Red, int *Green, int *Blue ) ;								// 描画カラーに加算する色を取得する
extern	int			NS_SetWriteAlphaChannelFlag(			int Flag ) ;													// 描画先のアルファチャンネルの内容を書き換えるかを設定する( FALSE:書き換えない  TRUE:書き換える( デフォルト ) )
extern	int			NS_GetWriteAlphaChannelFlag(			void ) ;														// 描画先のアルファチャンネルの内容を書き換えるかを取得する( FALSE:書き換えない  TRUE:書き換える( デフォルト ) )
extern	int			NS_CheckSeparateAlphaBlendEnable(		void ) ;														// 描画先のアルファチャンネルの内容を書き換えないことができるかどうかを取得する( TRUE:書き換えないことができる  FALSE:書き換えないことができない )
extern	int			NS_SetIgnoreDrawGraphColor(			int EnableFlag ) ;												// 描画する画像の色成分を無視するかどうかを指定する( EnableFlag:この機能を使うかどうか( TRUE:使う  FALSE:使わない ) )
extern	int			NS_GetIgnoreDrawGraphColor(			void ) ;														// 描画する画像のＲＧＢ成分を無視するかどうかを取得する( 戻り値  この機能を使うかどうか( TRUE:使う  FALSE:使わない )
extern	int			NS_SetMaxAnisotropy(					int MaxAnisotropy ) ;											// 最大異方性の値をセットする
extern	int			NS_GetMaxAnisotropy(					void ) ;														// 最大異方性値を取得する
...
extern	int			NS_SetUseRightHandClippingProcess(		int Flag ) ;													// 右手座標系のクリッピング処理を行うかを設定する( TRUE:右手座標系のクリッピング処理を行う  FALSE:左手座標系のクリッピング処理を行う( デフォルト ) )
extern	int			NS_GetUseRightHandClippingProcess(		void ) ;														// 右手座標系のクリッピング処理を行うかを取得する( TRUE:右手座標系のクリッピング処理を行う  FALSE:左手座標系のクリッピング処理を行う( デフォルト ) )
...
extern	void *			NS_GetDesktopScreenGraphMemImage(	int x1, int y1, int x2, int y2, int *Width, int *Height, int *Stride, int ColorBitDepth = 32 ) ;	// デスクトップ画面から指定領域の画像のメモリイメージの先頭アドレスとイメージの幅・高さ・ストライドを取得する( イメージのフォーマットは ColorBitDepth = 32( バイト順で B8G8R8X8 の 32bitカラー ) ColorBitDepth = 24( バイト順で B8G8R8 の 24bitカラー ) )
extern	int				NS_GetOtherWindowGraph(				int x1, int y1, int x2, int y2, int GrHandle, int DestX DEFAULTPARAM( = 0 ) , int DestY DEFAULTPARAM( = 0 ) ) ;		// 指定のウィンドウの指定領域の画像情報をグラフィックハンドルに転送する
#endif // WINDOWS_DESKTOP_OS
...
extern	int				NS_GetUseFullScreenResolutionMode(				void ) ;																		// フルスクリーン解像度モードを取得する( GetFullScreenResolutionMode の UseResolutionMode で取得できる値を返す関数 )
...
extern	int				NS_GetVideoMemorySizeEx(						ULONGLONG *TotalSize, ULONGLONG *UseSize ) ;									// ビデオメモリの容量を得る( 64bit版 )
...
extern	int			NS_SetUseDirect3D11AdapterIndex(					int Index ) ;									// 使用するグラフィックスデバイスのアダプターのインデックスを設定する
extern	int			NS_SetUseDirect3D11BGRASupport(					int Flag ) ;									// D3D11Device 作成時に D3D11_CREATE_DEVICE_BGRA_SUPPORT を指定するかどうか( TRUE:指定する  FALSE:指定しない( デフォルト ) )
extern	int			NS_GetUseDirect3D11BGRASupport(					void ) ;									// D3D11Device 作成時に D3D11_CREATE_DEVICE_BGRA_SUPPORT を指定するかどうかを取得する
...
extern	void *		NS_GetBufferVertexBuffer(	int VertexBufHandle ) ;																	// 頂点バッファハンドルの頂点バッファのアドレスを取得する
extern	int			NS_UpdateVertexBuffer(		int VertexBufHandle, int UpdateStartIndex, int UpdateVertexNum ) ;						// 頂点バッファハンドルの頂点バッファへの変更を適用する( GetBufferVertexBuffer で取得したバッファへの変更を反映する )
...
extern	void *		NS_GetBufferIndexBuffer(	int IndexBufHandle ) ;																	// インデックスバッファハンドルのインデックスバッファのアドレスを取得する
extern	int			NS_UpdateIndexBuffer(		int IndexBufHandle, int UpdateStartIndex, int UpdateIndexNum ) ;						// インデックスバッファハンドルのインデックスバッファへの変更を適用する( GetBufferIndexBuffer で取得したバッファへの変更を反映する )
...
extern	int			NS_LoadGeometryShader(				const TCHAR *FileName                        ) ;		// ピクセルシェーダーバイナリをファイルから読み込みジオメトリシェーダーハンドルを作成する( 戻り値 -1:エラー  -1以外:シェーダーハンドル )
extern	int			NS_LoadGeometryShaderWithStrLen(	const TCHAR *FileName, size_t FileNameLength ) ;		// ピクセルシェーダーバイナリをファイルから読み込みジオメトリシェーダーハンドルを作成する( 戻り値 -1:エラー  -1以外:シェーダーハンドル )
...
extern	int			NS_LoadGeometryShaderFromMem(		const void *ImageAddress, int ImageSize ) ;				// メモリに読み込まれた頂点シェーダーバイナリからジオメトリシェーダーハンドルを作成する( 戻り値 -1:エラー  -1以外:シェーダーハンドル )
...
extern	int			NS_SetUseGeometryShader(    int ShaderHandle ) ;										// シェーダーを使用した描画に使用するジオメトリシェーダーを設定する( -1を渡すと解除 )
...
extern	int			NS_CalcPolygon32bitIndexedBinormalAndTangentsToShader( VERTEX3DSHADER *VertexArray, int VertexNum, const unsigned int   *IndexArray, int PolygonNum ) ;	// ポリゴンの頂点の接線と従法線をＵＶ座標から計算してセットする( 頂点インデックスを使用する )
...
extern	int			NS_DrawPolygon32bitIndexed2DToShader(   const VERTEX2DSHADER *VertexArray, int VertexNum, const unsigned int   *IndexArray, int PolygonNum ) ;															// シェーダーを使って２Ｄポリゴンを描画する( 頂点インデックスを使用する )
...
extern	int			NS_DrawPolygon32bitIndexed3DToShader(   const VERTEX3DSHADER *VertexArray, int VertexNum, const unsigned int   *IndexArray, int PolygonNum ) ;															// シェーダーを使って３Ｄポリゴンを描画する( 頂点インデックスを使用する )
...
extern	int			NS_SetGraphFilterBltBlendMode( int BlendMode /* DX_BLENDMODE_ALPHA など */ ) ;												// GraphFilterBlt や GraphBlendBlt の結果を転送先に転送する際のブレンドモードを設定する( 現状で対応しているのは DX_BLENDMODE_NOBLEND と DX_BLENDMODE_ALPHA のみ )
extern	int			NS_SetGraphBlendScalingFilterMode( int IsBilinearFilter ) ;																	// GraphBlend で GrHandle と BlendGrHandle のサイズが異なる場合に適用される拡大フィルターモードを設定する( IsBilinearFilter  TRUE:バイリニアフィルター(デフォルト)  FALSE:ニアレストフィルター )
...
extern	int			NS_SetUseLightAngleAttenuation( int UseFlag ) ;																// ３Ｄ描画のライティング計算で角度減衰を行うかどうかを設定する( TRUE:角度減衰を行う( デフォルト )  FALSE:角度減衰を行わない )
...
extern	int			NS_GetMaskBaseImageInfo(				BASEIMAGE *BaseImage, int MaskHandle ) ;														// マスクハンドルが持つマスクイメージのBASEIMAGEデータを取得する
...
extern	int			NS_ChangeFontFromHandle(					int FontHandle ) ;																				// デフォルトフォントハンドルとして使用するフォントハンドルを変更する
...
extern	int			NS_GetFontMaxCacheCharNumToHandle(			int FontHandle ) ;																				// フォントハンドルのキャッシュできる文字の数を取得する
...
extern	int			NS_GetDrawStringKerningPairInfo(			const TCHAR *PairChar,                        int *KernAmount ) ;								// デフォルトフォントハンドルを使用した二つの文字のペアのカーニング情報を取得する( PairChar:カーニング情報を調べるペアとなる2文字の文字列( 2文字以上あっても先頭の2文字だけ使用されます )  KernAmount:2文字目の文字を基本の位置からずらすドット数を代入するint型変数のアドレス )
extern	int			NS_GetDrawStringKerningPairInfoWithStrLen(	const TCHAR *PairChar, size_t PairCharLength, int *KernAmount ) ;								// デフォルトフォントハンドルを使用した二つの文字のペアのカーニング情報を取得する( PairChar:カーニング情報を調べるペアとなる2文字の文字列( 2文字以上あっても先頭の2文字だけ使用されます )  KernAmount:2文字目の文字を基本の位置からずらすドット数を代入するint型変数のアドレス )
...
extern	int			NS_GetDrawStringKerningPairInfoToHandle(	        const TCHAR *PairChar,                        int *KernAmount, int FontHandle ) ;		// フォントハンドルを使用した二つの文字のペアのカーニング情報を取得する( PairChar:カーニング情報を調べるペアとなる2文字の文字列( 2文字以上あっても先頭の2文字だけ使用されます )  KernAmount:2文字目の文字を基本の位置からずらすドット数を代入するint型変数のアドレス )
extern	int			NS_GetDrawStringKerningPairInfoToHandleWithStrLen(	const TCHAR *PairChar, size_t PairCharLength, int *KernAmount, int FontHandle ) ;		// フォントハンドルを使用した二つの文字のペアのカーニング情報を取得する( PairChar:カーニング情報を調べるペアとなる2文字の文字列( 2文字以上あっても先頭の2文字だけ使用されます )  KernAmount:2文字目の文字を基本の位置からずらすドット数を代入するint型変数のアドレス )
...
extern	int			NS_GetFontMaxCacheCharNum(					void ) ;																								// デフォルトフォントハンドルのキャッシュできる文字の数を取得する
...
extern	int			NS_SetFontOnlyDrawType(					int OnlyType ) ;																				// フォントの描画で縁のみ、又は本体のみ描画を行うかどうかを設定する( OnlyType  0:通常描画 1:本体のみ描画 2:縁のみ描画 )
extern	int			NS_GetFontOnlyDrawType(					void ) ;																						// フォントの描画で縁のみ、又は本体のみ描画を行うかどうかを取得する( 戻り値  0:通常描画 1:本体のみ描画 2:縁のみ描画 )
...
extern	int			NS_DrawFormatString2(                      int x, int y,                                 unsigned int Color, unsigned int EdgeColor, const TCHAR *FormatString, ... ) ;																	// デフォルトフォントハンドルを使用して書式指定文字列を描画する( 縁の色引数付き )
extern	int			NS_DrawFormatVString2(                     int x, int y,                                 unsigned int Color, unsigned int EdgeColor, const TCHAR *FormatString, ... ) ;																	// デフォルトフォントハンドルを使用して書式指定文字列を描画する( 縁の色引数付き )( 縦書き )
...
extern	int			NS_DrawExtendFormatString2(                int x, int y, double ExRateX, double ExRateY, unsigned int Color, unsigned int EdgeColor, const TCHAR *FormatString, ... ) ;																	// デフォルトフォントハンドルを使用して書式指定文字列を拡大描画する( 縁の色引数付き )
extern	int			NS_DrawExtendFormatVString2(               int x, int y, double ExRateX, double ExRateY, unsigned int Color, unsigned int EdgeColor, const TCHAR *FormatString, ... ) ;																	// デフォルトフォントハンドルを使用して書式指定文字列を拡大描画する( 縁の色引数付き )( 縦書き )
...
extern	int			NS_DrawFormatString2F(                     float x, float y,                                 unsigned int Color, unsigned int EdgeColor, const TCHAR *FormatString, ... ) ;																// デフォルトフォントハンドルを使用して書式指定文字列を描画する( 縁の色引数付き )( 座標指定が float 版 )
extern	int			NS_DrawFormatVString2F(                    float x, float y,                                 unsigned int Color, unsigned int EdgeColor, const TCHAR *FormatString, ... ) ;																// デフォルトフォントハンドルを使用して書式指定文字列を描画する( 縁の色引数付き )( 縦書き )( 座標指定が float 版 )
...
extern	int			NS_DrawExtendFormatString2F(               float x, float y, double ExRateX, double ExRateY, unsigned int Color, unsigned int EdgeColor, const TCHAR *FormatString, ... ) ;																// デフォルトフォントハンドルを使用して書式指定文字列を拡大描画する( 縁の色引数付き )( 座標指定が float 版 )
extern	int			NS_DrawExtendFormatVString2F(              float x, float y, double ExRateX, double ExRateY, unsigned int Color, unsigned int EdgeColor, const TCHAR *FormatString, ... ) ;																// デフォルトフォントハンドルを使用して書式指定文字列を拡大描画する( 縁の色引数付き )( 縦書き )( 座標指定が float 版 )
...
extern	int			NS_DrawFormatString2ToHandle(              int x, int y, unsigned int Color, unsigned int EdgeColor, int FontHandle, const TCHAR *FormatString, ... ) ;																											// フォントハンドルを使用して書式指定文字列を描画する( 縁の色引数付き )
extern	int			NS_DrawFormatVString2ToHandle(             int x, int y, unsigned int Color, unsigned int EdgeColor, int FontHandle, const TCHAR *FormatString, ... ) ;																											// フォントハンドルを使用して書式指定文字列を描画する( 縁の色引数付き )( 縦書き )
...
extern	int			NS_DrawExtendFormatString2ToHandle(        int x, int y, double ExRateX, double ExRateY, unsigned int Color, unsigned int EdgeColor, int FontHandle, const TCHAR *FormatString, ... ) ;																			// フォントハンドルを使用して書式指定文字列を拡大描画する( 縁の色引数付き )
extern	int			NS_DrawExtendFormatVString2ToHandle(       int x, int y, double ExRateX, double ExRateY, unsigned int Color, unsigned int EdgeColor, int FontHandle, const TCHAR *FormatString, ... ) ;																			// フォントハンドルを使用して書式指定文字列を拡大描画する( 縁の色引数付き )( 縦書き )
...
extern	int			NS_DrawFormatString2FToHandle(             float x, float y, unsigned int Color, unsigned int EdgeColor, int FontHandle, const TCHAR *FormatString, ... ) ;																											// フォントハンドルを使用して書式指定文字列を描画する( 縁の色引数付き )( 座標指定が float 版 )
extern	int			NS_DrawFormatVString2FToHandle(            float x, float y, unsigned int Color, unsigned int EdgeColor, int FontHandle, const TCHAR *FormatString, ... ) ;																											// フォントハンドルを使用して書式指定文字列を描画する( 縁の色引数付き )( 縦書き )( 座標指定が float 版 )
...
extern	int			NS_DrawExtendFormatString2FToHandle(       float x, float y, double ExRateX, double ExRateY, unsigned int Color, unsigned int EdgeColor, int FontHandle, const TCHAR *FormatString, ... ) ;																			// フォントハンドルを使用して書式指定文字列を拡大描画する( 縁の色引数付き )( 座標指定が float 版 )
extern	int			NS_DrawExtendFormatVString2FToHandle(      float x, float y, double ExRateX, double ExRateY, unsigned int Color, unsigned int EdgeColor, int FontHandle, const TCHAR *FormatString, ... ) ;																			// フォントハンドルを使用して書式指定文字列を拡大描画する( 縁の色引数付き )( 縦書き )( 座標指定が float 版 )
...
extern	int			NS_GetUsePremulAlphaConvertLoad(    void ) ;															// 画像ファイル読み込み時に乗算済みアルファ画像に変換するかどうかを取得する( TRUE:変換処理を行う  FALSE:変換処理を行わない( デフォルト ) )
extern	int			NS_SetUseConvertNormalFormatLoad(   int UseFlag ) ;													// 画像ファイル読み込み時に DX_BASEIMAGE_FORMAT_NORMAL 以外の形式のイメージを DX_BASEIMAGE_FORMAT_NORMAL 形式のイメージに変換するかどうかを設定する( TRUE:変換処理を行う  FALSE:変換処理を行なわない( デフォルト ) )
extern	int			NS_GetUseConvertNormalFormatLoad(   void ) ;															// 画像ファイル読み込み時に DX_BASEIMAGE_FORMAT_NORMAL 以外の形式のイメージを DX_BASEIMAGE_FORMAT_NORMAL 形式のイメージに変換するかどうかを取得する( TRUE:変換処理を行う  FALSE:変換処理を行なわない( デフォルト ) )
...
extern	int			NS_CreateRGBA8ColorBaseImage(       int SizeX, int SizeY,                             BASEIMAGE *BaseImage ) ;																					// ＲＧＢＡ８カラーの基本イメージデータを作成する
extern	int			NS_CreateABGR8ColorBaseImage(       int SizeX, int SizeY,                             BASEIMAGE *BaseImage ) ;																					// ＡＢＧＲ８カラーの基本イメージデータを作成する
extern	int			NS_CreateBGRA8ColorBaseImage(       int SizeX, int SizeY,                             BASEIMAGE *BaseImage ) ;																					// ＢＧＲＡ８カラーの基本イメージデータを作成する
...
extern	int				NS_CreateRGBA8ColorData(    COLORDATA *ColorDataBuf ) ;																			// ＲＧＢＡ８カラーのカラーフォーマットを構築する
extern	int				NS_CreateABGR8ColorData(    COLORDATA *ColorDataBuf ) ;																			// ＡＢＧＲ８カラーのカラーフォーマットを構築する
extern	int				NS_CreateBGRA8ColorData(    COLORDATA *ColorDataBuf ) ;																			// ＢＧＲＡ８カラーのカラーフォーマットを構築する
extern	int				NS_CreateBGR8ColorData(     COLORDATA *ColorDataBuf ) ;																			// ＢＧＲ８カラーのカラーフォーマットを構築する
...
extern	int			NS_MakeRGBA8ColorSoftImage(              int SizeX, int SizeY ) ;															// ソフトウエアイメージハンドルの作成( RGBA8 カラー )
extern	int			NS_MakeABGR8ColorSoftImage(              int SizeX, int SizeY ) ;															// ソフトウエアイメージハンドルの作成( ABGR8 カラー )
extern	int			NS_MakeBGRA8ColorSoftImage(              int SizeX, int SizeY ) ;															// ソフトウエアイメージハンドルの作成( BGRA8 カラー )
...
extern	int			NS_MakeColorDataSoftImage(          int SizeX, int SizeY, const COLORDATA *ColorData ) ;								// ソフトウエアイメージハンドルの作成( COLORDATA でフォーマット指定 )
...
extern	int			NS_ConvertPremulAlphaSoftImage(          int SIHandle ) ;																	// 通常のαチャンネル付き画像を乗算済みαチャンネル付き画像に変換する( ピクセルフォーマットが ARGB8 以外の場合は ARGB8 に変換されます )
extern	int			NS_ConvertInterpAlphaSoftImage(          int SIHandle ) ;																	// 乗算済みαチャンネル付き画像を通常のαチャンネル付き画像に変換する( ピクセルフォーマットが ARGB8 以外の場合は ARGB8 に変換されます )
...
extern	int			NS_AddStreamSoundMemToMem(              const void *FileImage, size_t FileImageSize, int LoopNum,  int SoundHandle, int StreamDataType, int UnionHandle = -1 ) ;		// ストリーム再生タイプのサウンドハンドルにメモリ上に展開したサウンドファイルイメージを再生対象に追加する
...
extern	int			NS_StopStreamSoundMem(                  int SoundHandle, int IsNextLoopEnd = FALSE ) ;												// ストリーム再生タイプのサウンドハンドルの再生を停止する
extern	int			NS_SetStreamSoundCurrentPosition(       LONGLONG Byte, int SoundHandle ) ;															// サウンドハンドルの再生位置をバイト単位で変更する(再生が止まっている時のみ有効)
extern	LONGLONG	NS_GetStreamSoundCurrentPosition(       int SoundHandle ) ;																		// サウンドハンドルの再生位置をバイト単位で取得する
extern	int			NS_SetStreamSoundCurrentTime(           LONGLONG Time, int SoundHandle ) ;															// サウンドハンドルの再生位置をミリ秒単位で設定する(圧縮形式の場合は正しく設定されない場合がある)
extern	LONGLONG	NS_GetStreamSoundCurrentTime(           int SoundHandle ) ;																		// サウンドハンドルの再生位置をミリ秒単位で取得する(圧縮形式の場合は正しい値が返ってこない場合がある)
...
extern	int			NS_LoadSoundMemByMemImageBase(          const void *FileImage, size_t FileImageSize, int BufferNum,      int UnionHandle = -1 ) ;				// メモリ上に展開されたサウンドファイルイメージからサウンドハンドルを作成する
extern	int			NS_LoadSoundMemByMemImage(              const void *FileImage, size_t FileImageSize, int BufferNum = 3 , int UnionHandle = -1 ) ;				// LoadSoundMemByMemImageBase の別名関数
extern	int			NS_LoadSoundMemByMemImage2(             const void *WaveImage, size_t WaveImageSize, const WAVEFORMATEX *WaveFormat, size_t WaveHeaderSize ) ;	// メモリ上に展開されたＰＣＭデータからサウンドハンドルを作成する
extern	int			NS_LoadSoundMemByMemImageToBufNumSitei( const void *FileImage, size_t FileImageSize, int BufferNum ) ;										// LoadSoundMemByMemImageBase を使用して下さい
extern	int			NS_LoadSoundMem2ByMemImage(             const void *FileImage1, size_t FileImageSize1, const void *FileImage2, size_t FileImageSize2 ) ;	// 前奏部とループ部に分かれた二つのメモリ上に展開されたサウンドファイルイメージからサウンドハンドルを作成する
...
extern	LONGLONG	NS_GetCurrentPositionSoundMem(                                                          int SoundHandle ) ;						// サウンドハンドルの再生位置をサンプル単位で取得する
extern	int			NS_SetSoundCurrentPosition(             LONGLONG Byte,                                  int SoundHandle ) ;						// サウンドハンドルの再生位置をバイト単位で設定する(再生が止まっている時のみ有効)
extern	LONGLONG	NS_GetSoundCurrentPosition(                                                             int SoundHandle ) ;						// サウンドハンドルの再生位置をバイト単位で取得する
extern	int			NS_SetSoundCurrentTime(                 LONGLONG Time,                                  int SoundHandle ) ;						// サウンドハンドルの再生位置をミリ秒単位で設定する(圧縮形式の場合は正しく設定されない場合がある)
extern	LONGLONG	NS_GetSoundCurrentTime(                                                                 int SoundHandle ) ;						// サウンドハンドルの再生位置をミリ秒単位で取得する(圧縮形式の場合は正しい値が返ってこない場合がある)
extern	LONGLONG	NS_GetSoundTotalSample(                                                                 int SoundHandle ) ;						// サウンドハンドルの音の総時間をサンプル単位で取得する
extern	LONGLONG	NS_GetSoundTotalTime(                                                                   int SoundHandle ) ;						// サウンドハンドルの音の総時間をミリ秒単位で取得する

extern	int			NS_SetLoopPosSoundMem(                  LONGLONG LoopTime,                              int SoundHandle ) ;						// SetLoopTimePosSoundMem の別名関数
extern	int			NS_SetLoopTimePosSoundMem(              LONGLONG LoopTime,                              int SoundHandle ) ;						// サウンドハンドルにループ位置を設定する(ミリ秒単位)
extern	int			NS_SetLoopSamplePosSoundMem(            LONGLONG LoopSamplePosition,                    int SoundHandle ) ;						// サウンドハンドルにループ位置を設定する(サンプル単位)

extern	int			NS_SetLoopStartTimePosSoundMem(         LONGLONG LoopStartTime,                         int SoundHandle ) ;						// サウンドハンドルにループ開始位置を設定する(ミリ秒単位)
extern	int			NS_SetLoopStartSamplePosSoundMem(       LONGLONG LoopStartSamplePosition,               int SoundHandle ) ;						// サウンドハンドルにループ開始位置を設定する(サンプル単位)

extern	int			NS_SetLoopAreaTimePosSoundMem(          LONGLONG  LoopStartTime, LONGLONG  LoopEndTime,                     int SoundHandle ) ;	// サウンドハンドルにループ範囲を設定する(ミリ秒単位)
extern	int			NS_GetLoopAreaTimePosSoundMem(          LONGLONG *LoopStartTime, LONGLONG *LoopEndTime,                     int SoundHandle ) ;	// サウンドハンドルにループ範囲を取得する(ミリ秒単位)
extern	int			NS_SetLoopAreaSamplePosSoundMem(        LONGLONG  LoopStartSamplePosition, LONGLONG  LoopEndSamplePosition, int SoundHandle ) ;	// サウンドハンドルにループ範囲を設定する(サンプル単位)
extern	int			NS_GetLoopAreaSamplePosSoundMem(        LONGLONG *LoopStartSamplePosition, LONGLONG *LoopEndSamplePosition, int SoundHandle ) ;	// サウンドハンドルにループ範囲を取得する(サンプル単位)
...
extern	int			NS_GetOggCommentNum(           const TCHAR *FileName                        ) ;																			// Oggファイルのコメント情報の数を取得する
extern	int			NS_GetOggCommentNumWithStrLen( const TCHAR *FileName, size_t FileNameLength ) ;																			// Oggファイルのコメント情報の数を取得する
extern	int			NS_GetOggComment(              const TCHAR *FileName,                        int CommentIndex, TCHAR *CommentNameBuffer, size_t CommentNameBufferBytes, TCHAR *CommentBuffer, size_t CommentBufferBytes ) ;		// Oggファイルのコメント情報を取得する
extern	int			NS_GetOggCommentWithStrLen(    const TCHAR *FileName, size_t FileNameLength, int CommentIndex, TCHAR *CommentNameBuffer, size_t CommentNameBufferBytes, TCHAR *CommentBuffer, size_t CommentBufferBytes ) ;		// Oggファイルのコメント情報を取得する
...
extern	int			NS_SetCreateSoundLoopAreaTimePos(       LONGLONG  LoopStartTime,           LONGLONG  LoopEndTime ) ;								// 作成するサウンドハンドルのループ範囲を設定する( ミリ秒単位 )
extern	int			NS_GetCreateSoundLoopAreaTimePos(       LONGLONG *LoopStartTime,           LONGLONG *LoopEndTime ) ;								// 作成するサウンドハンドルのループ範囲を取得する( ミリ秒単位 )
extern	int			NS_SetCreateSoundLoopAreaSamplePos(     LONGLONG  LoopStartSamplePosition, LONGLONG  LoopEndSamplePosition ) ;						// 作成するサウンドハンドルのループ範囲を設定する( サンプル単位 )
extern	int			NS_GetCreateSoundLoopAreaSamplePos(     LONGLONG *LoopStartSamplePosition, LONGLONG *LoopEndSamplePosition ) ;						// 作成するサウンドハンドルのループ範囲を取得する( サンプル単位 )
...
extern	int			NS_SetEnableWASAPIFlag(                 int Flag, int IsExclusive = TRUE , int DevicePeriod = -1 , int SamplePerSec = 44100 ) ;		// サウンドの再生にWASAPIを使用するかどうかを設定する( Flag  TRUE:使用する  FALSE:使用しない( デフォルト ), IsExclusive  TRUE:排他モードを使用する  FALSE:排他モードを使用しない, DevicePeriod 再生遅延時間、100ナノ秒単位( 100000 で 10ミリ秒 )、-1でデフォルト値, SamplePerSec サンプリングレート )
#ifndef DX_NON_ASIO
extern	int			NS_SetEnableASIOFlag(                   int Flag, int BufferSize = -1 , int SamplePerSec = 44100 ) ;																					// サウンドの再生にASIOを使用するかどうかを設定する( Flag  TRUE:使用する  FALSE:使用しない( デフォルト ), BufferSize 再生バッファのサイズ、小さいほど遅延が少なくなりますが、処理が間に合わずにブツブツノイズが発生する可能性も高くなります( -1 でデフォルト値 ), SamplePerSec サンプリングレート )
extern	int			NS_SetUseASIODriverIndex(               int Index ) ;																																	// サウンドの再生に使用するASIOドライバーの番号を設定する( デフォルトでは 0 )
#endif // DX_NON_ASIO
extern	int			NS_SetEnableMMEwaveOutFlag(             int Flag, int BufferSamples DEFAULTPARAM( = -1 ) , int SamplePerSec DEFAULTPARAM( = 44100 ) ) ;													// サウンドの再生にマルチメディアAPIの waveOut を使用するかどうかを設定する( Flag  TRUE:使用する  FALSE:使用しない( デフォルト ), BufferSize 再生バッファのサイズ、小さいほど遅延が少なくなりますが、処理が間に合わずにブツブツノイズが発生する可能性も高くなります( -1 でデフォルト値 ), SamplePerSec サンプリングレート )
...
extern	int			NS_SetSoundCurrentTimeType(             int Type /* DX_SOUNDCURRENTTIME_TYPE_LOW_LEVEL など */ ) ;									// GetSoundCurrentTime などを使用した場合に取得できる再生時間のタイプを設定する
extern	int			NS_GetSoundCurrentTimeType(             void ) ;																					// GetSoundCurrentTime などを使用した場合に取得できる再生時間のタイプを取得する
...
#endif // DX_COMPILE_TYPE_C_LANGUAGE
extern	int			NS_PlaySoundDX(							const TCHAR *FileName,                        int PlayType ) ;														// PlaySoundFile の旧名称
extern	int			NS_PlaySoundDXWithStrLen(				const TCHAR *FileName, size_t FileNameLength, int PlayType ) ;														// PlaySoundFile の旧名称
...
extern	int			NS_LoadSoftSoundFromMemImage(			const void *FileImage, size_t FileImageSize ) ;												// ソフトウエアで扱う波形データハンドルをメモリ上に展開されたサウンドファイルイメージから作成する
extern	int			NS_MakeSoftSound(						int UseFormat_SoftSoundHandle, LONGLONG SampleNum ) ;										// ソフトウエアで扱う空の波形データハンドルを作成する( フォーマットは引数のソフトウエアサウンドハンドルと同じものにする )
extern	int			NS_MakeSoftSound2Ch16Bit44KHz(			LONGLONG SampleNum ) ;																		// ソフトウエアで扱う空の波形データハンドルを作成する( チャンネル数:2 量子化ビット数:16bit サンプリング周波数:44.1KHz )
extern	int			NS_MakeSoftSound2Ch16Bit22KHz(			LONGLONG SampleNum ) ;																		// ソフトウエアで扱う空の波形データハンドルを作成する( チャンネル数:2 量子化ビット数:16bit サンプリング周波数:22KHz )
extern	int			NS_MakeSoftSound2Ch8Bit44KHz(			LONGLONG SampleNum ) ;																		// ソフトウエアで扱う空の波形データハンドルを作成する( チャンネル数:2 量子化ビット数: 8bit サンプリング周波数:44.1KHz )
extern	int			NS_MakeSoftSound2Ch8Bit22KHz(			LONGLONG SampleNum ) ;																		// ソフトウエアで扱う空の波形データハンドルを作成する( チャンネル数:2 量子化ビット数: 8bit サンプリング周波数:22KHz )
extern	int			NS_MakeSoftSound1Ch16Bit44KHz(			LONGLONG SampleNum ) ;																		// ソフトウエアで扱う空の波形データハンドルを作成する( チャンネル数:1 量子化ビット数:16bit サンプリング周波数:44.1KHz )
extern	int			NS_MakeSoftSound1Ch16Bit22KHz(			LONGLONG SampleNum ) ;																		// ソフトウエアで扱う空の波形データハンドルを作成する( チャンネル数:1 量子化ビット数:16bit サンプリング周波数:22KHz )
extern	int			NS_MakeSoftSound1Ch8Bit44KHz(			LONGLONG SampleNum ) ;																		// ソフトウエアで扱う空の波形データハンドルを作成する( チャンネル数:1 量子化ビット数: 8bit サンプリング周波数:44.1KHz )
extern	int			NS_MakeSoftSound1Ch8Bit22KHz(			LONGLONG SampleNum ) ;																		// ソフトウエアで扱う空の波形データハンドルを作成する( チャンネル数:1 量子化ビット数: 8bit サンプリング周波数:22KHz )
extern	int			NS_MakeSoftSoundCustom(				int ChannelNum, int BitsPerSample, int SamplesPerSec, LONGLONG SampleNum, int IsFloatType = 0 ) ;	// ソフトウエアで扱う空の波形データハンドルを作成する
...
#endif // DX_NON_SAVEFUNCTION
extern	LONGLONG	NS_GetSoftSoundSampleNum(				int SoftSoundHandle ) ;																		// ソフトウエアで扱う波形データハンドルのサンプル数を取得する
extern	int			NS_GetSoftSoundFormat(					int SoftSoundHandle, int *Channels, int *BitsPerSample, int *SamplesPerSec, int *IsFloatType = NULL ) ;				// ソフトウエアで扱う波形データハンドルのフォーマットを取得する
extern	int			NS_ReadSoftSoundData(					int SoftSoundHandle, LONGLONG SamplePosition, int   *Channel1, int   *Channel2 ) ;			// ソフトウエアで扱う波形データハンドルのサンプルを読み取る
extern	int			NS_ReadSoftSoundDataF(					int SoftSoundHandle, LONGLONG SamplePosition, float *Channel1, float *Channel2 ) ;			// ソフトウエアで扱う波形データハンドルのサンプルを読み取る( float型版 )
extern	int			NS_WriteSoftSoundData(					int SoftSoundHandle, LONGLONG SamplePosition, int    Channel1, int    Channel2 ) ;			// ソフトウエアで扱う波形データハンドルのサンプルを書き込む
extern	int			NS_WriteSoftSoundDataF(				int SoftSoundHandle, LONGLONG SamplePosition, float  Channel1, float  Channel2 ) ;			// ソフトウエアで扱う波形データハンドルのサンプルを書き込む( float型版 )
...
extern	int			NS_GetFFTVibrationSoftSound(			int SoftSoundHandle, int Channel, LONGLONG SamplePosition, int SampleNum, float *Buffer_Array, int BufferLength ) ;								// ソフトウエアで扱う波形データハンドルの指定の範囲を高速フーリエ変換を行い、各周波数域の振幅を取得する( SampleNum は 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536 の何れかである必要があります、Channel を -1 にすると二つのチャンネルを合成した結果になります )
...
extern	int			NS_AddDataSoftSoundPlayer(				int SSoundPlayerHandle, int SoftSoundHandle, LONGLONG AddSamplePosition, int AddSampleNum ) ;	// ソフトウエアで扱う波形データのプレイヤーハンドルに波形データを追加する( フォーマットが同じではない場合はエラー )
...
extern	int			NS_GetStockDataLengthSoftSoundPlayer(	int SSoundPlayerHandle, int *SoundBufferStockSamples = NULL ) ;								// ソフトウエアで扱う波形データのプレイヤーハンドルに追加した波形データでまだ再生用サウンドバッファに転送されていない波形データのサンプル数を取得する
...
extern	int			NS_LoadMusicMemByMemImage(				const void *FileImage, size_t FileImageSize ) ;														// メモリ上に展開されたＭＩＤＩファイルを読み込む
...
extern	int			NS_PlayMusicByMemImage(				const void *FileImage, size_t FileImageSize, int PlayType ) ;										// メモリ上に展開されているＭＩＤＩファイルを演奏する
...
extern	int			NS_Live2D_SetCubism4CoreDLLPath( const TCHAR *CoreDLLFilePath ) ;									// Live2DCubismCore.dll のファイルパスを設定する
extern	int			NS_Live2D_SetCubism4CoreDLLPathWithStrLen( const TCHAR *CoreDLLFilePath, size_t CoreDLLFilePathLength ) ;	// Live2DCubismCore.dll のファイルパスを設定する
extern	int			NS_Live2D_SetCubism3CoreDLLPath( const TCHAR *CoreDLLFilePath ) ;									// Live2DCubismCore.dll のファイルパスを設定する
extern	int			NS_Live2D_SetCubism3CoreDLLPathWithStrLen( const TCHAR *CoreDLLFilePath, size_t CoreDLLFilePathLength ) ;	// Live2DCubismCore.dll のファイルパスを設定する

extern	int			NS_Live2D_RenderBegin( void ) ;																			// Live2D の描画処理を開始
extern	int			NS_Live2D_RenderEnd( void ) ;																				// Live2D の描画処理を終了

extern	int			NS_Live2D_LoadModel(			const TCHAR *FilePath ) ;													// Live2D のモデルファイルを読み込む
extern	int			NS_Live2D_LoadModelWithStrLen(	const TCHAR *FilePath, size_t FilePathLength ) ;							// Live2D のモデルファイルを読み込む
extern	int			NS_Live2D_DeleteModel( int Live2DModelHandle ) ;															// Live2D のモデルを削除する
extern	int			NS_Live2D_InitModel(			void ) ;																	// すべての Live2D のモデルを削除する

extern	int			NS_Live2D_SetUserShader( int TargetShader /* DX_LIVE2D_SHADER_NORMAL_PIXEL 等 */ , int ShaderHandle DEFAULTPARAM( = -1 ) ) ;			// Live2D のモデル描画で使用するシェーダーを設定する( ShaderHandle に -1 を渡すと解除 )
extern	int			NS_Live2D_DrawCallback( void ( *Callback )( int Live2DModelHandle, int TextureIndex, void *UserData ), void *UserData ) ;			// Live2D のモデル描画の前に呼ばれるコールバック関数を設定する Callback に NULL を渡すと設定を解除 )
extern	int			NS_Live2D_SetUseAutoScaling( int UseFlag ) ;																// Live2D のモデル描画をする際に、画面サイズに応じたスケーリングを行うかを設定する( UseFlag  TRUE:スケーリングを行う( デフォルト )  FALSE:スケーリングを行わない )
extern	int			NS_Live2D_SetUseAutoCentering( int UseFlag ) ;																// Live2D のモデルを画面の中心に描画するかを設定する( UseFlag   TRUE:画面の中心に描画する( デフォルト )   FALSE:画面の中心に描画しない )
extern	int			NS_Live2D_SetUseReverseYAxis( int UseFlag ) ;																// Live2D_Model_SetTranslate で指定する平行移動値の y の向きを反転するかを設定する( UseFlag   TRUE:反転する( デフォルト )   FALSE:反転しない )

extern	int			NS_Live2D_Model_Update( int Live2DModelHandle, float DeltaTime ) ;											// Live2D のモデルの状態を更新する
extern	int			NS_Live2D_Model_SetTranslate(		int Live2DModelHandle, float x, float y ) ;								// Live2D のモデルの位置を設定する
extern	int			NS_Live2D_Model_SetExtendRate(		int Live2DModelHandle, float ExRateX, float ExRateY ) ;					// Live2D のモデルの拡大率を設定する
...
extern	int			NS_Live2D_Model_StartMotion(				int Live2DModelHandle, const TCHAR *group,						int no ) ;							// Live2D のモデルの指定のモーションを再生する
extern	int			NS_Live2D_Model_StartMotionWithStrLen(		int Live2DModelHandle, const TCHAR *group, size_t GroupLength,	int no ) ;							// Live2D のモデルの指定のモーションを再生する
...
extern	int			NS_Live2D_Model_SetExpression(				int Live2DModelHandle, const TCHAR *expressionID ) ;												// Live2D のモデルの指定の表情モーションをセットする
extern	int			NS_Live2D_Model_SetExpressionWithStrLen(	int Live2DModelHandle, const TCHAR *expressionID, size_t expressionIDLength ) ;						// Live2D のモデルの指定の表情モーションをセットする
extern	int			NS_Live2D_Model_HitTest(					int Live2DModelHandle, const TCHAR *hitAreaName,							float x, float y ) ;	// 指定の座標が Live2D のモデルの指定の当たり判定の矩形範囲内か判定する( TRUE:矩形範囲内  FALSE:矩形範囲外 )
extern	int			NS_Live2D_Model_HitTestWithStrLen(			int Live2DModelHandle, const TCHAR *hitAreaName, size_t hitAreaNameLength,	float x, float y ) ;	// 指定の座標が Live2D のモデルの指定の当たり判定の矩形範囲内か判定する( TRUE:矩形範囲内  FALSE:矩形範囲外 )

extern	int			NS_Live2D_Model_GetParameterCount(						int Live2DModelHandle ) ;																// Live2D のモデルに設定されているパラメータの数を取得する
extern	const TCHAR *NS_Live2D_Model_GetParameterId(						int Live2DModelHandle, int index ) ;													// Live2D のモデルに設定されているパラメータのIDを取得する
extern	float		NS_Live2D_Model_GetParameterValue(						int Live2DModelHandle, const TCHAR *parameterId ) ;										// Live2D のモデルに設定されているパラメータを取得する
extern	float		NS_Live2D_Model_GetParameterValueWithStrLen(			int Live2DModelHandle, const TCHAR *parameterId, size_t parameterIdLength ) ;			// Live2D のモデルに設定されているパラメータを取得する
extern	int			NS_Live2D_Model_SetParameterValue(						int Live2DModelHandle, const TCHAR *parameterId, float value ) ;						// Live2D のモデルに設定されているパラメータを設定する
extern	int			NS_Live2D_Model_SetParameterValueWithStrLen(			int Live2DModelHandle, const TCHAR *parameterId, size_t parameterIdLength, float value ) ;	// Live2D のモデルに設定されているパラメータを設定する

extern	int			NS_Live2D_Model_GetHitAreasCount(						int Live2DModelHandle ) ;																// Live2D のモデルに設定された当たり判定の数を取得する
extern	const TCHAR *NS_Live2D_Model_GetHitAreaName(						int Live2DModelHandle, int index ) ;													// Live2D のモデルの当たり判定に設定された名前を取得する
extern	const TCHAR *NS_Live2D_Model_GetPhysicsFileName(					int Live2DModelHandle ) ;																// Live2D のモデルの物理演算設定ファイルの名前を取得する
extern	const TCHAR *NS_Live2D_Model_GetPoseFileName(						int Live2DModelHandle ) ;																// Live2D のモデルのパーツ切り替え設定ファイルの名前を取得する
extern	int			NS_Live2D_Model_GetExpressionCount(					int Live2DModelHandle ) ;																// Live2D のモデルの表情設定ファイルの数を取得する
extern	const TCHAR *NS_Live2D_Model_GetExpressionName(					int Live2DModelHandle, int index ) ;													// Live2D のモデルの表情設定ファイルを識別するIDを取得する
extern	const TCHAR *NS_Live2D_Model_GetExpressionFileName(				int Live2DModelHandle, int index ) ;													// Live2D のモデルの表情設定ファイルの名前を取得する
extern	int			NS_Live2D_Model_GetMotionGroupCount(					int Live2DModelHandle ) ;																// Live2D のモデルのモーショングループの数を取得する
extern	const TCHAR *NS_Live2D_Model_GetMotionGroupName(					int Live2DModelHandle, int index ) ;													// Live2D のモデルのモーショングループの名前を取得する
extern	int			NS_Live2D_Model_GetMotionCount(						int Live2DModelHandle, const TCHAR* groupName ) ;										// Live2D のモデルのモーショングループに含まれるモーションの数を取得する
extern	int			NS_Live2D_Model_GetMotionCountWithStrLen(				int Live2DModelHandle, const TCHAR* groupName, size_t groupNameLength ) ;				// Live2D のモデルのモーショングループに含まれるモーションの数を取得する
extern	const TCHAR *NS_Live2D_Model_GetMotionFileName(					int Live2DModelHandle, const TCHAR* groupName,							int index ) ;	// Live2D のモデルのグループ名とインデックス値からモーションファイルの名前を取得する
extern	const TCHAR *NS_Live2D_Model_GetMotionFileNameWithStrLen(			int Live2DModelHandle, const TCHAR* groupName, size_t groupNameLength,	int index ) ;	// Live2D のモデルのグループ名とインデックス値からモーションファイルの名前を取得する
extern	const TCHAR *NS_Live2D_Model_GetMotionSoundFileName(				int Live2DModelHandle, const TCHAR* groupName,							int index ) ;	// Live2D のモデルのモーションに対応するサウンドファイルの名前を取得する
extern	const TCHAR *NS_Live2D_Model_GetMotionSoundFileNameWithStrLen(		int Live2DModelHandle, const TCHAR* groupName, size_t groupNameLength,	int index ) ;	// Live2D のモデルのモーションに対応するサウンドファイルの名前を取得する
extern	float		NS_Live2D_Model_GetMotionFadeInTimeValue(				int Live2DModelHandle, const TCHAR* groupName,							int index ) ;	// Live2D のモデルのモーション開始時のフェードイン処理時間を取得する
extern	float		NS_Live2D_Model_GetMotionFadeInTimeValueWithStrLen(	int Live2DModelHandle, const TCHAR* groupName, size_t groupNameLength,	int index ) ;	// Live2D のモデルのモーション開始時のフェードイン処理時間を取得する
extern	float		NS_Live2D_Model_GetMotionFadeOutTimeValue(				int Live2DModelHandle, const TCHAR* groupName,							int index ) ;	// Live2D のモデルのモーション終了時のフェードアウト処理時間を取得する
extern	float		NS_Live2D_Model_GetMotionFadeOutTimeValueWithStrLen(	int Live2DModelHandle, const TCHAR* groupName, size_t groupNameLength,	int index ) ;	// Live2D のモデルのモーション終了時のフェードアウト処理時間を取得する
extern	const TCHAR *NS_Live2D_Model_GetUserDataFile(						int Live2DModelHandle ) ;																// Live2D のモデルのユーザデータのファイル名を取得する
extern	int			NS_Live2D_Model_GetEyeBlinkParameterCount(				int Live2DModelHandle ) ;																// Live2D のモデルの目パチに関連付けられたパラメータの数を取得する
extern	const TCHAR *NS_Live2D_Model_GetEyeBlinkParameterId(				int Live2DModelHandle, int index ) ;													// Live2D のモデルの目パチに関連付けられたパラメータのIDを取得する
extern	int			NS_Live2D_Model_GetLipSyncParameterCount(				int Live2DModelHandle ) ;																// Live2D のモデルのリップシンクに関連付けられたパラメータの数を取得する
extern	const TCHAR *NS_Live2D_Model_GetLipSyncParameterId(				int Live2DModelHandle, int index ) ;													// Live2D のモデルのリップシンクに関連付けられたパラメータのIDを取得する
extern	float		NS_Live2D_Model_GetCanvasWidth(						int Live2DModelHandle ) ;																// Live2D のモデルのキャンバスの横幅を取得する
extern	float		NS_Live2D_Model_GetCanvasHeight(						int Live2DModelHandle ) ;																// Live2D のモデルのキャンバスの縦幅を取得する
