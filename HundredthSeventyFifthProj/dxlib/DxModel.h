...
struct MV1_MESH_BASE
{
	...
	int						TriangleNum ;						// このメッシュに含まれるポリゴンの数
	int						VertexNum ;							// 頂点の数
...
struct MV1_MESH
{
	...
	bool					SetupRefPolygon[ 2 ] ;				// 参照用ポリゴンリストのセットアップが完了しているかどうか( true:完了している  false:完了していない )[ 座標変換ありかどうか( 1:あり 0:なし ) ]
	MV1_REF_POLYGONLIST		*RefPolygon[ 2 ] ;					// 参照用ポリゴンリスト( [ 座標変換ありかどうか( 1:あり 0:なし ) ] )
...
struct MV1_MODEL
{
	...
	int						PrioritizePhysicsOverAnimFlag ;		// 物理演算をアニメーションより優先するかどうかのフラグ( TRUE:物理演算を優先する  FALSE:アニメーションを優先する )
...
struct MV1_MODEL_MANAGE
{
	...
	int						DrawMode ;								// 描画モード( DX_MV1_DRAWMODE_NORMAL など )
...
//	MATRIX					OrigLocalWorldMatrix ;				// もともと設定されていたＬＷ行列
	...
	void *					WorkBuffer ;						// 作業用バッファ
	size_t					WorkBufferSize ;					// 作業用バッファのサイズ

	INT4					BackupDrawAddColor ;				// モデル描画時に元の加算カラーを保存しておくための変数
...
extern	int				MV1_BeginRender( MV1_MODEL *Model ) ;													// ３Ｄモデルのレンダリングの準備を行う
extern	int				MV1_EndRender( void ) ;																	// ３Ｄモデルのレンダリングの後始末を行う
...
extern	int				MV1_EndRender_PF( void ) ;																	// ３Ｄモデルのレンダリングの後始末を行う
...
extern	int			NS_MV1SetDrawMode( int DrawMode /* DX_MV1_DRAWMODE_NORMAL 等 */ ) ;						// モデルの描画モードの設定
...
extern	int			NS_MV1SetPrioritizePhysicsOverAnimFlag(	int MHandle, int Flag ) ;											// モデルの物理演算をアニメーションより優先するかどうかを設定する( TRUE:物理演算を優先する  FALSE:アニメーションを優先する( デフォルト ) )
...
extern	int			NS_MV1GetFrameVertexNum(				int MHandle, int FrameIndex ) ;											// 指定のフレームに含まれる頂点の数を取得する
...
extern	int			NS_MV1GetMeshVertexNum( int MHandle, int MeshIndex ) ;									// 指定メッシュに含まれる頂点の数を取得する
...
// コリジョン関係
extern	int							NS_MV1SetupCollInfo(				int MHandle, int FrameIndex = -1 , int XDivNum = 32 , int YDivNum = 8 , int ZDivNum = 32 , int MeshIndex = -1 ) ;		// コリジョン情報を構築する
extern	int							NS_MV1TerminateCollInfo(			int MHandle, int FrameIndex = -1 , int MeshIndex = -1 ) ;																// コリジョン情報の後始末
extern	int							NS_MV1RefreshCollInfo(				int MHandle, int FrameIndex = -1 , int MeshIndex = -1 ) ;																// コリジョン情報を更新する
extern	MV1_COLL_RESULT_POLY		NS_MV1CollCheck_Line(				int MHandle, int FrameIndex, VECTOR PosStart, VECTOR PosEnd , int MeshIndex = -1 ) ;									// 線とモデルの当たり判定
extern	MV1_COLL_RESULT_POLY_DIM	NS_MV1CollCheck_LineDim(			int MHandle, int FrameIndex, VECTOR PosStart, VECTOR PosEnd , int MeshIndex = -1 ) ;									// 線とモデルの当たり判定( 戻り値が MV1_COLL_RESULT_POLY_DIM )
extern	MV1_COLL_RESULT_POLY_DIM	NS_MV1CollCheck_Sphere(			int MHandle, int FrameIndex, VECTOR CenterPos, float r , int MeshIndex = -1 ) ;											// 球とモデルの当たり判定
extern	MV1_COLL_RESULT_POLY_DIM	NS_MV1CollCheck_Capsule(			int MHandle, int FrameIndex, VECTOR Pos1, VECTOR Pos2, float r , int MeshIndex = -1 ) ;									// カプセルとモデルの当たり判定
extern	MV1_COLL_RESULT_POLY_DIM	NS_MV1CollCheck_Triangle(			int MHandle, int FrameIndex, VECTOR Pos1, VECTOR Pos2, VECTOR Pos3 , int MeshIndex = -1 ) ;								// 三角形とモデルの当たり判定
...
// 参照用メッシュ関係
extern	int					NS_MV1SetupReferenceMesh(		int MHandle, int FrameIndex, int IsTransform, int IsPositionOnly = FALSE , int MeshIndex = -1 ) ;						// 参照用メッシュのセットアップ
extern	int					NS_MV1TerminateReferenceMesh(	int MHandle, int FrameIndex, int IsTransform, int IsPositionOnly = FALSE , int MeshIndex = -1 ) ;						// 参照用メッシュの後始末
extern	int					NS_MV1RefreshReferenceMesh(	int MHandle, int FrameIndex, int IsTransform, int IsPositionOnly = FALSE , int MeshIndex = -1 ) ;						// 参照用メッシュの更新
extern	MV1_REF_POLYGONLIST	NS_MV1GetReferenceMesh(		int MHandle, int FrameIndex, int IsTransform, int IsPositionOnly = FALSE , int MeshIndex = -1 ) ;						// 参照用メッシュを取得する
...
#define NS_MV1GetFrameVertexNum							MV1GetFrameVertexNum
