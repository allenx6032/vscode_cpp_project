...
extern int DirectDraw7_GetVideoMemorySize( DWORD *TotalSize, DWORD *FreeSize )
{
	D_DDSCAPS2 caps ;
	...
	if( GAPIWin.DirectDraw7Object->GetAvailableVidMem( &caps, &TotalMem, &FreeMem ) != D_DD_OK )
	{
		return -1 ;
	}
...
//		if( Desc2.dwRefreshRate == 0 )
		{
			...
			ULONGLONG DeltaTime1, DeltaTime2 ;
			ULONGLONG DeltaTimeTotal, DeltaTimeSize ;
			ULONGLONG OneMilliSecondsCount ;
			int Count, LoopCount ;

			OneMilliSecondsCount = NS_ConvMilliSecondsToSysPerformanceCount( 1 ) ;
			DeltaTime1 = 0 ;
			DeltaTime2 = 0 ;
			DeltaTimeTotal = 0 ;
			Count = 0 ;
			...
			Time1 = NS_GetNowSysPerformanceCount() ;
			while( Count < 10 && LoopCount < 60 )
			{
				GAPIWin.DirectDraw7Object->WaitForVerticalBlank( D_DDWAITVB_BLOCKBEGIN, 0 ) ;
				Time2 = NS_GetNowSysPerformanceCount() ;
				DeltaTime2 = Time2 - Time1 ;
				DeltaTimeSize = DeltaTime1 > DeltaTime2 ? DeltaTime1 - DeltaTime2 : DeltaTime2 - DeltaTime1 ;
				if( DeltaTimeSize < OneMilliSecondsCount )
				{
				...
				if( Count >= 10 )
				{
					GAPIWin.DirectDraw7_VSyncWaitTime = ( LONGLONG )( DeltaTimeTotal / Count ) ;
				}
...
//	BOOL flag ;
	...
	if( GAPIWin.DirectDraw7Object == NULL )
	{
		return ;
	}
	...
	if( NS_GetNowSysPerformanceCount() - GAPIWin.DirectDraw7_VSyncTime < WaitTime )
	{
		...
		while( NS_GetNowSysPerformanceCount() - GAPIWin.DirectDraw7_VSyncTime < WaitTime ) WinAPIData.Win32Func.SleepFunc( 1 ) ;
		GAPIWin.DirectDraw7Object->WaitForVerticalBlank( D_DDWAITVB_BLOCKBEGIN, 0 ) ;
		GAPIWin.DirectDraw7_VSyncTime = NS_GetNowSysPerformanceCount() ;
	}
	else
	{
...
extern int DirectDraw7_WaitVSync( int SyncNum )
{
	...
	{
		vsync = TRUE ;
		while( vsync == TRUE && GAPIWin.DirectDraw7Object != NULL )
		{
			GAPIWin.DirectDraw7Object->GetVerticalBlankStatus( &vsync ) ;
		}

		vsync = FALSE ;
		while( vsync == FALSE && GAPIWin.DirectDraw7Object != NULL )
		{
			GAPIWin.DirectDraw7Object->GetVerticalBlankStatus( &vsync ) ;
		}
	}
...
#ifndef DX_NON_ASYNCLOAD
extern int Direct3D11_Release_GeometryShader_ASyncCallback( ASYNCLOAD_MAINTHREAD_REQUESTINFO *Info )
{
	return ( int )Direct3D11_Release_GeometryShader_ASync(
				( D_ID3D11GeometryShader * )Info->Data[ 0 ],
				FALSE ) ;
}
#endif // DX_NON_ASYNCLOAD

extern ULONG Direct3D11_Release_GeometryShader_ASync( D_ID3D11GeometryShader *pGeometryShader, int ASyncThread )
{
#ifndef DX_NON_ASYNCLOAD
	if( ASyncThread )
	{
		...
		AInfo.Function = Direct3D11_Release_GeometryShader_ASyncCallback ;
		AInfo.Data[ 0 ] = ( DWORD_PTR )pGeometryShader ;
		return ( ULONG )AddASyncLoadRequestMainThreadInfo( &AInfo ) ;
	}
#endif // DX_NON_ASYNCLOAD

	return Direct3D11_Release_GeometryShader( pGeometryShader ) ;
}
...
#ifndef DX_NON_ASYNCLOAD
extern int D3D11Device_CreateGeometryShader_ASyncCallback( ASYNCLOAD_MAINTHREAD_REQUESTINFO *Info )
{
	return D3D11Device_CreateGeometryShader_ASync(
				( const void *              )Info->Data[ 0 ],
				( SIZE_T                    )Info->Data[ 1 ],
				( D_ID3D11ClassLinkage *    )Info->Data[ 2 ],
				( D_ID3D11GeometryShader ** )Info->Data[ 3 ],
				FALSE ) ;
}
#endif // DX_NON_ASYNCLOAD

extern	HRESULT	D3D11Device_CreateGeometryShader_ASync( const void *pShaderBytecode, SIZE_T BytecodeLength, D_ID3D11ClassLinkage *pClassLinkage, D_ID3D11GeometryShader **ppGeometryShader, int ASyncThread )
{
#ifndef DX_NON_ASYNCLOAD
	if( ASyncThread )
	{
		...
		AInfo.Function = D3D11Device_CreateGeometryShader_ASyncCallback ;
		AInfo.Data[ 0 ] = ( DWORD_PTR )pShaderBytecode ;
		AInfo.Data[ 1 ] = ( DWORD_PTR )BytecodeLength ;
		AInfo.Data[ 2 ] = ( DWORD_PTR )pClassLinkage ;
		AInfo.Data[ 3 ] = ( DWORD_PTR )ppGeometryShader ;
		return AddASyncLoadRequestMainThreadInfo( &AInfo ) ;
	}
#endif // DX_NON_ASYNCLOAD

	return D3D11Device_CreateGeometryShader( pShaderBytecode, BytecodeLength, pClassLinkage, ppGeometryShader ) ;
}
...
extern	ULONG	Direct3D11_Release_GeometryShader( D_ID3D11GeometryShader *pGeometryShader )
{
	GAPIWin.D3D11GeometryShaderNum--;
	return pGeometryShader->Release() ;
}
...
extern int DXGI_LoadDLL( int OutputLog )
{
	if( OutputLog )
	{
...
extern int DXGIFactory_Create( int OutputLog )
{
	LPD_CREATEDXGIFACTORY  D_CreateDXGIFactoryFunction  = NULL ;
	LPD_CREATEDXGIFACTORY1 D_CreateDXGIFactory1Function = NULL ;
	LPD_CREATEDXGIFACTORY2 D_CreateDXGIFactory2Function = NULL ;
	...
	if( D_CreateDXGIFactory2Function == NULL )
	{
		if( OutputLog )
		{
			DXST_LOGFILE_ADDUTF16LE( "\x31\x59\x57\x65\x0a\x00\x00"/*@ L"失敗\n" @*/ ) ;
...
extern int DXGIFactory_Release( void )
{
	if( GAPIWin.DXGIFactoryObject != NULL )
	{
		...
		GAPIWin.DXGIFactory2Object = NULL ;
	}

	if( GAPIWin.DXGIFactory6Object != NULL )
	{
		Direct3D11_ObjectRelease( GAPIWin.DXGIFactory6Object ) ;
...
extern int DXGIFactory6_IsValid( void )
{
	return GAPIWin.DXGIFactory6Object != NULL ? TRUE : FALSE ;
}

extern HRESULT DXGIFactory6_EnumAdapterByGpuPreference( UINT Adapter, D_DXGI_GPU_PREFERENCE GpuPreference, REFIID riid, D_IDXGIAdapter **ppAdapter )
{
	return GAPIWin.DXGIFactory6Object->EnumAdapterByGpuPreference( Adapter, GpuPreference, riid, ( void ** )ppAdapter ) ;
}
...
extern	HRESULT	DXGIFactory2_CreateSwapChainForHwnd( D_IUnknown *pDevice, HWND hWnd, const D_DXGI_SWAP_CHAIN_DESC1 *pDesc, const D_DXGI_SWAP_CHAIN_FULLSCREEN_DESC *pFullscreenDesc, D_IDXGIOutput *pRestrictToOutput, D_IDXGISwapChain1 **ppSwapChain )
{
	return GAPIWin.DXGIFactory2Object->CreateSwapChainForHwnd( pDevice, hWnd, pDesc, pFullscreenDesc, pRestrictToOutput, ppSwapChain ) ;
}
...
extern	HRESULT DXGIAdapter_GetDesc_Default( D_DXGI_ADAPTER_DESC *pDesc )
{
	...
	return GAPIWin.DXGIAdapterObject->GetDesc( pDesc ) ;
}

extern	HRESULT	DXGIAdapter3_QueryVideoMemoryInfo_Default( UINT NodeIndex, D_DXGI_MEMORY_SEGMENT_GROUP MemorySegmentGroup, D_DXGI_QUERY_VIDEO_MEMORY_INFO *pVideoMemoryInfo )
{
	...
	return GAPIWin.DXGIAdapter3Object->QueryVideoMemoryInfo( NodeIndex, MemorySegmentGroup, pVideoMemoryInfo ) ;
}

extern	HRESULT DXGIAdapter_GetDXGIAdapter3( D_IDXGIAdapter * pAdapter, D_IDXGIAdapter3 **ppAdapter3 )
{
	return pAdapter->QueryInterface( IID_IDXGIADAPTER3, ( void ** )ppAdapter3 ) ;
}

extern	HRESULT	DXGIAdapter3_QueryVideoMemoryInfo( D_IDXGIAdapter3 * pAdapter3, UINT NodeIndex, D_DXGI_MEMORY_SEGMENT_GROUP MemorySegmentGroup, D_DXGI_QUERY_VIDEO_MEMORY_INFO *pVideoMemoryInfo )
{
	return pAdapter3->QueryVideoMemoryInfo( NodeIndex, MemorySegmentGroup, pVideoMemoryInfo ) ;
}
...
extern	HRESULT DXGIOutput1_DuplicateOutput( D_IDXGIOutput1 *pOutput1, D_IUnknown *pDevice, D_IDXGIOutputDuplication **ppOutputDuplication )
{
	return pOutput1->DuplicateOutput( pDevice, ppOutputDuplication ) ;
}
...
extern int D3D11_CreateDevice( void )
{
	...
	for( i = 0 ; i < GD3D11.Adapter.Info[ GraphicsHardDataDirect3D11.Setting.UseDXGIAdapterIndex ].OutputNum ; i ++ )
	{
		...
		if( WinData.WindowRect.left   >= OutputDesc->DesktopCoordinates.left   &&
			WinData.WindowRect.right  <= OutputDesc->DesktopCoordinates.right  &&
			WinData.WindowRect.top    >= OutputDesc->DesktopCoordinates.top    &&
			WinData.WindowRect.bottom <= OutputDesc->DesktopCoordinates.bottom )
		{
			break ;
		}
	}
	if( i == GD3D11.Adapter.Info[ GraphicsHardDataDirect3D11.Setting.UseDXGIAdapterIndex ].OutputNum )
	{
		...
		for( j = 0 ; j < GD3D11.Adapter.InfoNum ; j ++ )
		{
			...
			{
				D_DXGI_OUTPUT_DESC *OutputDesc = &GD3D11.Adapter.Info[ j ].OutputDesc[ i ] ;
				if( WinData.WindowRect.left   >= OutputDesc->DesktopCoordinates.left   &&
					WinData.WindowRect.right  <= OutputDesc->DesktopCoordinates.right  &&
					WinData.WindowRect.top    >= OutputDesc->DesktopCoordinates.top    &&
					WinData.WindowRect.bottom <= OutputDesc->DesktopCoordinates.bottom )
				{
					GraphicsHardDataDirect3D11.Setting.UseDXGIAdapterIndex = j ;
					break ;
				}
			}
			if( i != GD3D11.Adapter.Info[ j ].OutputNum )
			{
				break ;
			}
		}
		if( j == GD3D11.Adapter.InfoNum )
		{
			...
			for( j = 0 ; j < GD3D11.Adapter.InfoNum ; j ++ )
			{
				int EnableMixRect = FALSE ;
				RECT MixRect = { 0 } ;

				for( i = 0 ; i < GD3D11.Adapter.Info[ j ].OutputNum ; i ++ )
				{
					D_DXGI_OUTPUT_DESC *OutputDesc = &GD3D11.Adapter.Info[ j ].OutputDesc[ i ] ;
					if( OutputDesc->AttachedToDesktop == FALSE )
					{
						continue ;
					}
					if( EnableMixRect == FALSE )
					{
						MixRect = OutputDesc->DesktopCoordinates ;
					}
					else
					{
						if( MixRect.left   > OutputDesc->DesktopCoordinates.left   ) MixRect.left   = OutputDesc->DesktopCoordinates.left   ;
						if( MixRect.right  < OutputDesc->DesktopCoordinates.right  ) MixRect.right  = OutputDesc->DesktopCoordinates.right  ;
						if( MixRect.top    > OutputDesc->DesktopCoordinates.top    ) MixRect.top    = OutputDesc->DesktopCoordinates.top    ;
						if( MixRect.bottom < OutputDesc->DesktopCoordinates.bottom ) MixRect.bottom = OutputDesc->DesktopCoordinates.bottom ;
					}
				}

				WindowRect = WinData.WindowRect ;
				RectClipping( &WindowRect, &MixRect ) ;
				WindowPixels = ( WindowRect.right - WindowRect.left ) * ( WindowRect.bottom - WindowRect.top ) ;
				if( WindowPixels != 0 && ( MaxIndex == -1 || WindowPixels > MaxPixels ) )
				{
					MaxIndex = j ;
					MaxPixels = WindowPixels ;
				}
			}
			if( MaxIndex != -1 )
			{
				GraphicsHardDataDirect3D11.Setting.UseDXGIAdapterIndex = MaxIndex ;
			}
		}
	}
...
//			D_D3D_DRIVER_TYPE_HARDWARE,				// D_D3D_DRIVER_TYPE                DriverType,
			...
			( GD3D11.Setting.UseDirect3D11BGRASupport ? D_D3D11_CREATE_DEVICE_BGRA_SUPPORT : 0 ) |
...
ERR :
	...
	if( GAPIWin.DXGIAdapter1Object != NULL )
	{
		GAPIWin.DXGIAdapter1Object->Release() ;
		GAPIWin.DXGIAdapter1Object = NULL ;
	}

	if( GAPIWin.DXGIAdapter2Object != NULL )
	{
		GAPIWin.DXGIAdapter2Object->Release() ;
		GAPIWin.DXGIAdapter2Object = NULL ;
	}

	if( GAPIWin.DXGIAdapter3Object != NULL )
	{
		GAPIWin.DXGIAdapter3Object->Release() ;
		GAPIWin.DXGIAdapter3Object = NULL ;
	}

	if( GAPIWin.DXGIAdapter4Object != NULL )
	{
		GAPIWin.DXGIAdapter4Object->Release() ;
		GAPIWin.DXGIAdapter4Object = NULL ;
	}
...
extern HRESULT DXGISwapChain_ResizeTarget( D_IDXGISwapChain *pSwapChain, const D_DXGI_MODE_DESC *pNewTargetParameters )
{
	return pSwapChain->ResizeTarget( pNewTargetParameters ) ;
}
...
extern	HRESULT	D3D11Device_CreateGeometryShader( const void *pShaderBytecode, SIZE_T BytecodeLength, D_ID3D11ClassLinkage *pClassLinkage, D_ID3D11GeometryShader **ppGeometryShader )
{
	HRESULT hr ;

	hr = GAPIWin.D3D11DeviceObject->CreateGeometryShader( pShaderBytecode,  BytecodeLength, pClassLinkage, ppGeometryShader ) ;
	if( hr == S_OK )
	{
		GAPIWin.D3D11GeometryShaderNum ++ ;
	}
...
extern void D3D11DeviceContext_GSSetConstantBuffers( UINT StartSlot, UINT NumBuffers, D_ID3D11Buffer *const *ppConstantBuffers )
{
	GAPIWin.D3D11DeviceContext->GSSetConstantBuffers( StartSlot, NumBuffers, ppConstantBuffers ) ;
}
...
extern int Direct3D9_CheckMultiSampleParam( D_D3DFORMAT Format, D_D3DMULTISAMPLE_TYPE *Samples, DWORD *Quality, int SamplesFailedBreak )
{
	...
	for(;;)
	{
		if( Direct3D9_CheckDeviceMultiSampleType( 0, D_D3DDEVTYPE_HAL, Format, NS_GetWindowModeFlag() || NS_GetUseFullScreenResolutionMode() == DX_FSRESOLUTIONMODE_BORDERLESS_WINDOW, *Samples, &ColorBufferQuality ) == D_D3D_OK )
			break ;
	...
	for(;;)
	{
		if( Direct3D9_CheckDeviceMultiSampleType( 0, D_D3DDEVTYPE_HAL, D_D3DFMT_D16, NS_GetWindowModeFlag() || NS_GetUseFullScreenResolutionMode() == DX_FSRESOLUTIONMODE_BORDERLESS_WINDOW, *Samples, &ZBufferQuality ) == D_D3D_OK )
			break ;
...
extern int Direct3DDevice9_Present( void )
{
	...
	if( NS_GetWindowModeFlag() == TRUE || NS_GetUseFullScreenResolutionMode() == DX_FSRESOLUTIONMODE_BORDERLESS_WINDOW )
	{
		...
		else
		{
			...
			if( WinData.ToolBarUseFlag )
			{
				...
				{
					WindRect.top    = ClientHeight - _DTOL( DrawScreenHeight * ExRateY ) ;
					WindRect.bottom = ClientHeight ;
				}
				else
				{
					WindRect.top    = GetToolBarHeight() ;
					WindRect.bottom = WindRect.top + _DTOL( DrawScreenHeight * ExRateY ) ;
				}
			}
		}
	...
	else
	{
		...
		if( ( NS_GetWindowModeFlag() == TRUE || NS_GetUseFullScreenResolutionMode() == DX_FSRESOLUTIONMODE_BORDERLESS_WINDOW ) && GSYS.Screen.Emulation320x240Flag )
		{
			...
			WindRect.left     = 0 ;
			WindRect.top      = 0 ;
			WindRect.right    = _DTOL( 640 * WindowSizeExRateX ) ;
			WindRect.bottom   = _DTOL( 480 * WindowSizeExRateY ) ;
		...
		if( GD3D9.Device.Screen.SubBackBufferSurface != NULL )
		{
			...
			if( GSYS.Screen.UserScreenImage != NULL )
			{
				...
				COLORDATA *DestColorData = Graphics_D3D9_GetD3DFormatColorData( GD3D9.Device.Screen.SubBackBufferTexturePixelFormat ) ;
