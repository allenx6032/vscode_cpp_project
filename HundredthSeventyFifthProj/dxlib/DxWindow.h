...
struct WINDATA
{
	...
	int						WindowMaximizeButtonBehaviorType ;	// ウインドウの最大化ボタンが押されたときの挙動( 0=標準動作 1=標準動作は行わず、ALTキー+ENTERキーが押されたことにする )
	...
	int						WindowMaximizeButtonInputFlag ;		// 最大化ボタンが押されたかどうかのフラグ
	...
	int						BorderlessWindowFlag ;				// ボーダーレスウィンドウにするかどうかのフラグ( TRUE:ボーダーレスウィンドウ  FALSE:通常ウィンドウ )
	...
	int						EnableWindows10_WM_CHAR_CancelTime ;// Windows10_WM_CHAR_CancelTime が有効かどうか
	int						Windows10_WM_CHAR_CancelTime ;		// Windows10 で WM_CHAR で短時間に連続して同じ文字が入力された場合の無効扱いにする時間( MilliSecond のミリ秒以内に連続して同じ文字が入力された場合に無効にする )
	int						UseFPUPreserve ;					// FPU精度を落とさない設定を使用するかどうか( TRUE:使用する  FALSE:使用しない )

	int						MouseDispFlag ;						// マウスの表示フラグ
	int						MouseDispState ;					// マウスの表示状態
	int						MouseDispIgnoreMenuFlag ;			// マウスの表示状態についてメニューの状態を無視するかどうかのフラグ( TRUE:無視する  FALSE:無視しない )
	...
	LONGLONG				PerformanceTimerFrequency ;			// パフォーマンスカウンターの周波数
...
//	int						DefaultRefreshRate ;				// 起動時のデスクトップのリフレッシュレート
	...
	int						EnableWindowSizeExRate ;			// WindowSizeExRateX, WindowSizeExRateY が有効な値が設定されているかどうか
	double					WindowSizeExRateX, WindowSizeExRateY ;	// 描画画面のサイズに対するウインドウサイズの比率
	int						ChangeWindowSizeExRate ;			// ウィンドウのサイズの比率を変更中かどうか
	...
	int						UseTransparentWindowFlag ;			// SetUseBackBufferTransColorFlag( TRUE ); 又は SetUseUpdateLayerdWindowFlag( TRUE ); を設定の際にマウス関係のイベントを背後のウィンドウに透過させるかの設定( TRUE:透過する   FALSE:透過しない( デフォルト ) )
...
//	int						ReadOnlyFileAccessLockFlag ;		// ファイルアクセス構造体リストへのアクセスを禁止しているか、フラグ
	...
	HBITMAP					DesktopImageHBitmap ;				// GetDesktopScreenGraphMemImage で使用する HBITMAP
	void					*DesktopImageAddress ;				// GetDesktopScreenGraphMemImage で使用する HBITMAP のイメージアドレス
	int						DesktopImageWidth ;					// GetDesktopScreenGraphMemImage で使用する HBITMAP のイメージの幅
	int						DesktopImageHeight ;				// GetDesktopScreenGraphMemImage で使用する HBITMAP のイメージの高さ

	void					*DXGIDesktopImageAddress ;			// GetDesktopScreenGraphMemImage で使用するイメージアドレス( DXGIDuplication 用 )
	int						DXGIDesktopImageWidth ;				// GetDesktopScreenGraphMemImage で使用するイメージの幅( DXGIDuplication 用 )
	int						DXGIDesktopImageHeight ;			// GetDesktopScreenGraphMemImage で使用するイメージの高さ( DXGIDuplication 用 )
