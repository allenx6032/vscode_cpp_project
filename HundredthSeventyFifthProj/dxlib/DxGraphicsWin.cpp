...
extern int NS_LoadGraphToResourceWithStrLen2( const TCHAR *ResourceName, size_t ResourceNameLength, const TCHAR *ResourceType, size_t ResourceTypeLength )
{
	...
	Result = NS_LoadGraphToResource2( UseResourceNameBuffer, UseResourceTypeBuffer ) ;
...
extern int NS_LoadDivGraphToResourceWithStrLen2( const TCHAR *ResourceName, size_t ResourceNameLength, const TCHAR *ResourceType, size_t ResourceTypeLength, int AllNum, int XNum, int YNum, int   XSize, int   YSize, int *HandleArray )
{
	...
	Result = NS_LoadDivGraphToResource2( UseResourceNameBuffer, UseResourceTypeBuffer, AllNum, XNum, YNum, XSize, YSize, HandleArray ) ;
...
extern int NS_LoadDivGraphFToResourceWithStrLen2( const TCHAR *ResourceName, size_t ResourceNameLength, const TCHAR *ResourceType, size_t ResourceTypeLength, int AllNum, int XNum, int YNum, float XSize, float YSize, int *HandleArray )
{
	...
	Result = NS_LoadDivGraphFToResource2( UseResourceNameBuffer, UseResourceTypeBuffer, AllNum, XNum, YNum, XSize, YSize, HandleArray ) ;
...
//	int DesktopW, DesktopH ;
	...
	{
		void *Result ;
		Result = Graphics_Hardware_D3D11_GetDesktopScreenGraphMemImage_PF( x1, y1, x2, y2, Width, Height, Stride, ColorBitDepth ) ;
		if( Result != NULL )
		{
			return Result ;
		}
...
//	}
	...
	if( WinData.DesktopImageHBitmap == NULL ||
		...
		CaptureH != WinData.DesktopImageHeight )
	{
		if( WinData.DesktopImageHBitmap != NULL )
		{
		...
		WinData.DesktopImageHBitmap = WinAPIData.Win32Func.CreateDIBSectionFunc( hdc, &BitmapInfo, DIB_RGB_COLORS, &WinData.DesktopImageAddress, NULL, 0 ) ;
		WinData.DesktopImageWidth = CaptureW ;
		WinData.DesktopImageHeight = CaptureH ;
	}
	HMemDC = WinAPIData.Win32Func.CreateCompatibleDCFunc( hdc ) ;
	...
	if( Stride != NULL )
	{
		int Temp = CaptureW * ColorBitDepth / 8 ;
		Temp = ( Temp + 3 ) / 4 * 4 ;
		*Stride = Temp ;
	}
...
extern int NS_GetOtherWindowGraph( HWND Window, int x1, int y1, int x2, int y2, int GrHandle, int DestX, int DestY )
{
	...
	if( y1 < WindowRect.top )
	{
		DestY += WindowRect.top - y1 ;
		y1     = WindowRect.top ;
	}
	if( x2 > WindowRect.right )
	{
		x2 = WindowRect.right ;
	}
	if( y2 > WindowRect.bottom )
	{
...
#endif
				{
					// それでも駄目で、且つソフトウェアレンダリングモードだった場合は 32bitカラーを試す
...
#ifndef DX_NON_DIRECT3D11
	case GRAPHICS_API_DIRECT3D11_WIN32 :
		return Graphics_Hardware_D3D11_SetDrawCustomBlendMode_PF( BlendEnable, SrcBlendRGB, DestBlendRGB, BlendOpRGB, SrcBlendA, DestBlendA, BlendOpA, BlendParam ) ;
#endif // DX_NON_DIRECT3D11

#ifndef DX_NON_DIRECT3D9
	case GRAPHICS_API_DIRECT3D9_WIN32 :
		return Graphics_Hardware_D3D9_SetDrawCustomBlendMode_PF( BlendEnable, SrcBlendRGB, DestBlendRGB, BlendOpRGB, SrcBlendA, DestBlendA, BlendOpA, BlendParam ) ;
#endif // DX_NON_DIRECT3D9
...
#ifndef DX_NON_DIRECT3D11
	case GRAPHICS_API_DIRECT3D11_WIN32 :
		return Graphics_Hardware_D3D11_SetDrawAddColor_PF(  Red,  Green,  Blue ) ;
#endif // DX_NON_DIRECT3D11

#ifndef DX_NON_DIRECT3D9
	case GRAPHICS_API_DIRECT3D9_WIN32 :
		return Graphics_Hardware_D3D9_SetDrawAddColor_PF(  Red,  Green,  Blue ) ;
#endif // DX_NON_DIRECT3D9
...
extern	int		Graphics_Hardware_GetVideoMemorySizeEx_PF( ULONGLONG *TotalSize, ULONGLONG *UseSize )
{
	...
	int SuccessUseSize = FALSE ;

#ifndef DX_NON_DIRECT3D11
	if( Graphics_Hardware_D3D11_GetVideoMemorySizeEx_PF( TotalSize, UseSize ) == 0 )
	{
		return 0 ;
	}

	{
		...
		if( EnumDXGIAdapterInfo() == 0 )
		{
			...
			{
				for( j = 0 ; j < GD3D11.Adapter.Info[ i ].OutputNum ; j ++ )
				{
					if( CL_strcmp( DX_CHARCODEFORMAT_UTF16LE, ( char * )GD3D11.Adapter.Info[ i ].OutputDesc[ j ].DeviceName, ( char * )GSYS.Screen.DisplayInfo[ GSYS.Screen.UseDisplayIndex ].Name ) == 0 )
					{
						D_IDXGIAdapter *DXGIAdapter ;
						D_IDXGIAdapter3 *DXGIAdapter3 ;
						HRESULT hr ;

						if( DXGIFactory6_IsValid() )
						{
							hr = DXGIFactory6_EnumAdapterByGpuPreference( ( UINT )i, D_DXGI_GPU_PREFERENCE_HIGH_PERFORMANCE, IID_IDXGIADAPTER, &DXGIAdapter ) ;
						}
						else
						{
							hr = DXGIFactory_EnumAdapters( ( UINT )i, &DXGIAdapter ) ;
						}
						...
						if( hr == S_OK )
						{
							...
							if( DXGIAdapter_GetDesc( DXGIAdapter, &Desc ) == S_OK )
							{
								if( TotalSize != NULL )
								{
									*TotalSize = Desc.DedicatedVideoMemory ;
								}
								SuccessTotalSize = TRUE ;
							}

							if( DXGIAdapter_GetDXGIAdapter3( DXGIAdapter, &DXGIAdapter3 ) == S_OK )
							{
								D_DXGI_QUERY_VIDEO_MEMORY_INFO MemoryInfo ;

								if( DXGIAdapter3_QueryVideoMemoryInfo( DXGIAdapter3, 0, D_DXGI_MEMORY_SEGMENT_GROUP_LOCAL, &MemoryInfo ) == S_OK )
								{
									if( UseSize != NULL )
									{
								...
								Direct3D9_ObjectRelease( DXGIAdapter3 ) ;
							}

							Direct3D9_ObjectRelease( DXGIAdapter ) ;
						}
					}

					if( SuccessUseSize && SuccessTotalSize )
					{
						break ;
					}
				}
...
#ifndef DX_NON_DIRECT3D9
	{
		DWORD AllSize, FreeSize ;
		if( Graphics_Hardware_D3D9_GetVideoMemorySize_PF( &AllSize, &FreeSize ) < 0 )
		{
			return -1 ;
		}

		if( SuccessTotalSize == FALSE && TotalSize != NULL )
		{
			*TotalSize = AllSize ;
		}
		if( SuccessUseSize == FALSE && UseSize != NULL )
		{
...
)
{
	...
	if( GSYS.Screen.DisplayInfo != NULL )
	{
		...
		DisplayInfo->MonitorHandle = hMonitor ;
		if( WinAPIData.Win32Func.GetDpiForMonitorFunc != NULL )
		{
			...
			DXST_LOGFILEFMT_ADDUTF16LE(( ""/*@ L"No.%d  モニター名：%s　%dx%d　%dbit　%dHz  %dx%ddpi" @*/, GSYS.Screen.DisplayNum, DisplayInfo->Name, DisplayInfo->DesktopPixelSizeX, DisplayInfo->DesktopPixelSizeY, DisplayInfo->DesktopColorBitDepth, DisplayInfo->DesktopRefreshRate, DisplayInfo->XDpi, DisplayInfo->YDpi )) ;
		}
		else
		{
...
extern int Graphics_SetupDisplayInfo_PF( void )
{
	...
	DWORD                    j ;
	DWORD                    k ;
	D_DISPLAY_DEVICEW        DisplayDevice ;
	GRAPHICSSYS_DISPLAYINFO *DisplayInfo ;
	wchar_t *                DeviceName ;
		...
		OsVersionInfo.dwMajorVersion >= 5 )
	{
		...
		DisplayDeviceNum = 0 ;
		while( EnumDisplayDevicesWFunc( NULL, DisplayDeviceNum, &DisplayDevice, 0 ) != 0 )
		{
			DisplayDeviceNum ++ ;
		}
	}
	...
	else
	{
		...
		DXST_LOGFILEFMT_ADDUTF16LE(( ""/*@ L"No.%d  モニター名：%s　%dx%d　%dbit　%dHz" @*/, 0, DisplayInfo->Name, DisplayInfo->DesktopPixelSizeX, DisplayInfo->DesktopPixelSizeY, DisplayInfo->DesktopColorBitDepth, DisplayInfo->DesktopRefreshRate )) ;
	}
	...
	for( j = 0 ; j < DisplayDeviceNum ; j ++ )
	{
		...
		if( OsVersionInfo.dwPlatformId >= VER_PLATFORM_WIN32_NT &&
			OsVersionInfo.dwMajorVersion >= 5 )
		{
			EnumDisplayDevicesWFunc( NULL, j, &DisplayDevice, 0 ) ;
...
//		DXST_LOGFILEFMT_ADDUTF16LE(( "\x4e\x00\x6f\x00\x2e\x00\x25\x00\x64\x00\x20\x00\x20\x00\xc7\x30\xa3\x30\xb9\x30\xd7\x30\xec\x30\xa4\x30\xc7\x30\xd0\x30\xa4\x30\xb9\x30\x0d\x54\x1a\xff\x25\x00\x73\x00\x00"/*@ L"No.%d  ディスプレイデバイス名：%s" @*/, j, DeviceName == NULL ? L"None" : DeviceName )) ;
		...
		{
			_MEMSET( &DevMode, 0, sizeof( DevMode ) ) ;
			if( EnumDisplaySettingsWFunc( DeviceName, ModeNum, &DevMode ) == 0 )
			{
				break ;
			}
		...
		else
		{
			...
			if( GSYS.Screen.UserScreenImage != NULL )
			{
				...
				case DX_USER_SCREEN_PIXEL_FORMAT_R5G6B5 :
					...
					( ( DWORD * )&pBmpInfo->bmiColors )[ 2 ] = 0x001f ;
					break ;
				...
				case DX_USER_SCREEN_PIXEL_FORMAT_R5G5B5X1 :
					...
					pBmpInfo->bmiHeader.biCompression        = BI_BITFIELDS ;
					( ( DWORD * )&pBmpInfo->bmiColors )[ 0 ] = 0xf800 ;
					( ( DWORD * )&pBmpInfo->bmiColors )[ 1 ] = 0x07c0 ;
					( ( DWORD * )&pBmpInfo->bmiColors )[ 2 ] = 0x003e ;
					break ;

				case DX_USER_SCREEN_PIXEL_FORMAT_X1R5G5B5 :
					pBmpInfo->bmiHeader.biBitCount           = 16 ;
					pBmpInfo->bmiHeader.biCompression        = BI_BITFIELDS ;
					( ( DWORD * )&pBmpInfo->bmiColors )[ 0 ] = 0x7c00 ;
					( ( DWORD * )&pBmpInfo->bmiColors )[ 1 ] = 0x03e0 ;
					( ( DWORD * )&pBmpInfo->bmiColors )[ 2 ] = 0x001f ;
					break ;
				...
				case DX_USER_SCREEN_PIXEL_FORMAT_X8B8G8R8 :
					...
					pBmpInfo->bmiHeader.biCompression        = BI_BITFIELDS ;
					( ( DWORD * )&pBmpInfo->bmiColors )[ 0 ] = 0x000000ff ;
					( ( DWORD * )&pBmpInfo->bmiColors )[ 1 ] = 0x0000ff00 ;
					( ( DWORD * )&pBmpInfo->bmiColors )[ 2 ] = 0x00ff0000 ;
					break ;

				case DX_USER_SCREEN_PIXEL_FORMAT_X8R8G8B8 :
					pBmpInfo->bmiHeader.biBitCount           = 32 ;
					pBmpInfo->bmiHeader.biCompression        = BI_BITFIELDS ;
					( ( DWORD * )&pBmpInfo->bmiColors )[ 0 ] = 0x00ff0000 ;
					( ( DWORD * )&pBmpInfo->bmiColors )[ 1 ] = 0x0000ff00 ;
					( ( DWORD * )&pBmpInfo->bmiColors )[ 2 ] = 0x000000ff ;
					break ;
				}
			}
			else
			...
			if( Dc )
			{
				...
				if( ( NS_GetWindowModeFlag() || NS_GetUseFullScreenResolutionMode() == DX_FSRESOLUTIONMODE_BORDERLESS_WINDOW ) && GSYS.Screen.Emulation320x240Flag )
				{
					WinAPIData.Win32Func.StretchDIBitsFunc(
						...
						320,
						240,
						GSYS.Screen.UserScreenImage != NULL ? GSYS.Screen.UserScreenImage : UseScreenMemImg->UseImage,
						pBmpInfo, DIB_RGB_COLORS, SRCCOPY );
				}
				else
				{
...
//							)) ;
					...
					else
					{
						...
						{
							int DestX, DestY ;
							DestX = ( WinData.WindowRect.right  - WinData.WindowRect.left - WindowSizeX ) / 2 ;
							DestY = ( WinData.WindowRect.bottom - WinData.WindowRect.top  - WindowSizeY ) / 2 ;
							WinAPIData.Win32Func.StretchDIBitsFunc(
								Dc,
								DestX, DestY,
...
#ifndef DX_NON_DIRECT3D11
	case GRAPHICS_API_DIRECT3D11_WIN32 :
		return Graphics_Hardware_D3D11_Light_SetNoAngleAttenuation_PF( NoAngleAttenuation ) ;
#endif // DX_NON_DIRECT3D11

#ifndef DX_NON_DIRECT3D9
	case GRAPHICS_API_DIRECT3D9_WIN32 :
		return Graphics_Hardware_D3D9_Light_SetNoAngleAttenuation_PF( NoAngleAttenuation ) ;
#endif // DX_NON_DIRECT3D9
...
#ifndef DX_NON_DIRECT3D11
	case GRAPHICS_API_DIRECT3D11_WIN32 :
		return Graphics_Hardware_D3D11_Shader_Create_PF(  ShaderHandle,  ShaderType, Image,  ImageSize,  ImageAfterFree,  ASyncThread ) ;
#endif // DX_NON_DIRECT3D11

#ifndef DX_NON_DIRECT3D9
	case GRAPHICS_API_DIRECT3D9_WIN32 :
		return Graphics_Hardware_D3D9_Shader_Create_PF(  ShaderHandle,  ShaderType, Image,  ImageSize,  ImageAfterFree,  ASyncThread ) ;
#endif // DX_NON_DIRECT3D9
...
#ifndef DX_NON_DIRECT3D11
	case GRAPHICS_API_DIRECT3D11_WIN32 :
		return Graphics_D3D11_DrawBoxSet( RectData,  Num ) ;
#endif // DX_NON_DIRECT3D11

#ifndef DX_NON_DIRECT3D9
	case GRAPHICS_API_DIRECT3D9_WIN32 :
		return Graphics_D3D9_DrawBoxSet( RectData,  Num ) ;
#endif // DX_NON_DIRECT3D9
...
#ifndef DX_NON_DIRECT3D11
	case GRAPHICS_API_DIRECT3D11_WIN32 :
		return Graphics_D3D11_Draw32bitIndexedPrimitive( Vertex,  VertexNum, Indices, IndexNum,  PrimitiveType, Image,  TransFlag ) ;
#endif // DX_NON_DIRECT3D11

#ifndef DX_NON_DIRECT3D9
	case GRAPHICS_API_DIRECT3D9_WIN32 :
		return Graphics_D3D9_Draw32bitIndexedPrimitive( Vertex,  VertexNum, Indices, IndexNum,  PrimitiveType, Image,  TransFlag ) ;
#endif // DX_NON_DIRECT3D9
...
#ifndef DX_NON_DIRECT3D11
	case GRAPHICS_API_DIRECT3D11_WIN32 :
		return Graphics_D3D11_Draw32bitIndexedPrimitiveLight(                 Vertex, VertexNum, Indices,  IndexNum,  PrimitiveType, Image,  TransFlag ) ;
#endif // DX_NON_DIRECT3D11

#ifndef DX_NON_DIRECT3D9
	case GRAPHICS_API_DIRECT3D9_WIN32 :
		return Graphics_D3D9_Draw32bitIndexedPrimitiveLight(                 Vertex, VertexNum, Indices,  IndexNum,  PrimitiveType, Image,  TransFlag ) ;
#endif // DX_NON_DIRECT3D9
...
#ifndef DX_NON_DIRECT3D11
	case GRAPHICS_API_DIRECT3D11_WIN32 :
		return Graphics_D3D11_Draw32bitIndexedPrimitive2DUser(                Vertex,  VertexNum, Indices,  IndexNum,  PrimitiveType, Image,  TransFlag ) ;
#endif // DX_NON_DIRECT3D11

#ifndef DX_NON_DIRECT3D9
	case GRAPHICS_API_DIRECT3D9_WIN32 :
		return Graphics_D3D9_Draw32bitIndexedPrimitive2DUser(                Vertex,  VertexNum, Indices,  IndexNum,  PrimitiveType, Image,  TransFlag ) ;
#endif // DX_NON_DIRECT3D9
...
#ifndef DX_NON_DIRECT3D11
	case GRAPHICS_API_DIRECT3D11_WIN32 :
		return Graphics_D3D11_DrawPrimitive32bitIndexed2DToShader( Vertex,  VertexNum, Indices,  IndexNum,  PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */ ) ;
#endif // DX_NON_DIRECT3D11

#ifndef DX_NON_DIRECT3D9
	case GRAPHICS_API_DIRECT3D9_WIN32 :
		return Graphics_D3D9_DrawPrimitive32bitIndexed2DToShader( Vertex,  VertexNum, Indices,  IndexNum,  PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */ ) ;
#endif // DX_NON_DIRECT3D9
...
#ifndef DX_NON_DIRECT3D11
	case GRAPHICS_API_DIRECT3D11_WIN32 :
		return Graphics_D3D11_DrawPrimitive32bitIndexed3DToShader( Vertex,  VertexNum, Indices,  IndexNum,  PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */ ) ;
#endif // DX_NON_DIRECT3D11
