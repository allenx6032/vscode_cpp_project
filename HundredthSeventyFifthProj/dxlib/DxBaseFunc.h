...
#define CHAR_TO_WCHAR_T_STRING_SETUP( str, err, CHARCODEFORMAT )										\
	if( str != NULL )																					\
	{																									\
		if( CHARCODEFORMAT == WCHAR_T_CHARCODEFORMAT )													\
		{																								\
			Use##str##Buffer = ( wchar_t * )str ;														\
		}																								\
		else																							\
		{																								\
			str##Length = CL_strlen( CHARCODEFORMAT, ( char	* )str ) ;									\
...
#define WCHAR_T_TO_CHAR_TEMPSTRINGLENGTH				(512)


#define WCHAR_T_TO_CHAR_STRING_BEGIN( str )													\
	BYTE    str##Buffer[ sizeof( char ) * WCHAR_T_TO_CHAR_TEMPSTRINGLENGTH ] ;				\
	void    *Alloc##str##Buffer = NULL ;													\
	char    *Use##str##Buffer = NULL ;														\
	size_t   str##Length ;																	\
	size_t   wchar_##str##Length ;															\


#define WCHAR_T_TO_CHAR_STRING_SETUP( str, err, CHARCODEFORMAT )										\
	if( str != NULL )																					\
	{																									\
		wchar_##str##Length = CL_strlen( WCHAR_T_CHARCODEFORMAT, ( char * )str ) ;						\
		if( wchar_##str##Length * 6 + 16 > WCHAR_T_TO_CHAR_TEMPSTRINGLENGTH )							\
		{																								\
			Alloc##str##Buffer = DXALLOC( sizeof( char ) * ( wchar_##str##Length * 6 + 16 ) ) ;			\
			if( Alloc##str##Buffer == NULL )															\
			{																							\
				err ;																					\
			}																							\
			Use##str##Buffer = ( char * )Alloc##str##Buffer ;											\
		}																								\
		else																							\
		{																								\
			Use##str##Buffer = ( char * )str##Buffer ;													\
		}																								\
																										\
		str##Length = ConvString( ( const char * )str, ( int )wchar_##str##Length, WCHAR_T_CHARCODEFORMAT, ( char * )Use##str##Buffer, sizeof( char ) * ( wchar_##str##Length * 6 + 16 ), CHARCODEFORMAT ) / GetCharCodeFormatUnitSize( CHARCODEFORMAT ) ;\
		if( str##Length > 0 )																			\
		{																								\
			str##Length -- ;																			\
		}																								\
...
#define WCHAR_T_TO_CHAR_STRING_END( str )			\
	if( Alloc##str##Buffer != NULL )				\
	{												\
		DXFREE( Alloc##str##Buffer ) ;				\
		Alloc##str##Buffer = NULL ;					\
...
#define WCHAR_T_TO_CHAR_STRING_ONE_BEGIN( str, err, CHARCODEFORMAT )		\
	WCHAR_T_TO_CHAR_STRING_BEGIN( str )										\
	WCHAR_T_TO_CHAR_STRING_SETUP( str, err, CHARCODEFORMAT )
...
#define TCHAR_TO_CHAR_TEMPSTRINGBYTES				(512)


#define TCHAR_TO_CHAR_STRING_BEGIN( str )													\
	BYTE    str##Buffer[ sizeof( char ) * TCHAR_TO_CHAR_TEMPSTRINGBYTES ] ;					\
	void    *Alloc##str##Buffer = NULL ;													\
	char    *Use##str##Buffer = NULL ;														\
	size_t   str##Length ;																	\
	size_t   char_##str##Length ;															\


#define TCHAR_TO_CHAR_STRING_SETUP( str, err, CHARCODEFORMAT )											\
	if( str != NULL )																					\
	{																									\
		if( _TCHARCODEFORMAT != CHARCODEFORMAT )														\
		{																								\
			char_##str##Length = CL_strlen( _TCHARCODEFORMAT, ( char * )str ) ;							\
			if( char_##str##Length * 6 + 16 > TCHAR_TO_CHAR_TEMPSTRINGBYTES )							\
			{																							\
				Alloc##str##Buffer = DXALLOC( sizeof( char ) * ( char_##str##Length * 6 + 16 ) ) ;		\
				if( Alloc##str##Buffer == NULL )														\
				{																						\
					err ;																				\
				}																						\
			...
			{																							\
				Use##str##Buffer = ( char * )str##Buffer ;												\
																										...
																										\
			str##Length = ConvString( ( const char * )str, ( int )char_##str##Length, _TCHARCODEFORMAT, ( char * )Use##str##Buffer, sizeof( char ) * ( char_##str##Length * 6 + 16 ), CHARCODEFORMAT ) / GetCharCodeFormatUnitSize( CHARCODEFORMAT ) ;\
			if( str##Length > 0 )																		\
			{																							\
				str##Length -- ;																		\
			}																							\
		...
		{																								\
			Use##str##Buffer = ( char * )str ;															\
		}																								\
	}


#define TCHAR_TO_CHAR_STRING_END( str )				\
	if( Alloc##str##Buffer != NULL )				\
	{												\
		DXFREE( Alloc##str##Buffer ) ;				\
		Alloc##str##Buffer = NULL ;					\
...
#define TCHAR_TO_CHAR_STRING_ONE_BEGIN( str, err, CHARCODEFORMAT )			\
	TCHAR_TO_CHAR_STRING_BEGIN( str )										\
	TCHAR_TO_CHAR_STRING_SETUP( str, err, CHARCODEFORMAT )
...
__inline LONGLONG _ABS64( LONGLONG Number )
{
	return Number < 0 ? -Number : Number ;
}
