...
struct LOADSOUND_GPARAM
{
	...
	LONGLONG					CreateSoundLoopStartTimePosition ;		// 作成するサウンドデータのループ範囲の先端( ミリ秒 )
	LONGLONG					CreateSoundLoopStartSamplePosition ;	// 作成するサウンドデータのループ範囲の先端( サンプル )
	LONGLONG					CreateSoundLoopEndTimePosition ;		// 作成するサウンドデータのループ範囲の終端( ミリ秒 )
	LONGLONG					CreateSoundLoopEndSamplePosition ;		// 作成するサウンドデータのループ範囲の終端( サンプル )
...
struct SOUNDBUFFER
{
	...
	volatile int				PlayPos ;					// 再生位置
	volatile int				CompPos ;					// 転送が完了した位置
	volatile double				CompPosF ;					// 転送が完了した位置( SetFrequency で再生周波集を変更されたとき用 )
	volatile int				EnableComPosF ;				// CompPosF が有効かどうか( TRUE:有効  FALSE:無効 )
	volatile int				Loop ;						// ループするかどうか( TRUE:ループする  FALSE:ループしない )
	volatile float				CalcPan ;					// キャプチャ用に計算した後のパン( 256:左100%右0%  0:左右100%  -256:右100% )
	volatile float				CalcVolume ;				// キャプチャ用に計算した後のボリューム( 256:100%  0:0% )
...
struct STREAMFILEDATA
{
	...
	ULONGLONG					FileImageSize ;				// メモリ上に展開したファイルイメージのサイズ( DX_SOUNDDATATYPE_FILE 以外で使用 )
	STREAMDATA					MemStream ;					// メモリストリーム処理用データ( DX_SOUNDDATATYPE_FILE 以外で使用 )
	LONGLONG					LoopSamplePosition ;		// ループ後の再生位置(サンプル単位、OGG か 無圧縮WAV のみ有効 )
	LONGLONG					LoopStartSamplePosition ;	// ループを開始する再生位置(サンプル単位、OGG か 無圧縮WAV のみ有効 )
...
struct STREAMPLAYDATA
{
	...
	LONGLONG					FileCompCopyLength ;									// 現在アクティブなストリームデータ中のサウンドバッファへの転送が完了したバイト数
	int							FileUseFile ;											// メモリイメージではなくディスクなどの補助記憶装置上にあるファイルを使用しているかどうか
	LONGLONG					TotalSample ;											// 全ストリームのサンプル総数

	ULONGLONG					CompPlayWaveLength ;									// 再生し終わった波形データの長さ
...
//	char						SoundName[ 256 ] ;										// サウンドファイルネーム
	...
	volatile int				BufferPlayStateBackupFlagValid[ MAX_SOUNDBUFFER_NUM ] ;	// BufferPlayStateBackupFlag が有効かどうかのフラグ( TRUE:有効  FALSE:無効 )
	volatile int				BufferPlayStateBackupFlag[ MAX_SOUNDBUFFER_NUM ] ;		// サウンドバッファを一時停止するときの再生状態フラグ
...
{
	LONGLONG					BufferSampleNum ;										// バッファのサイズ( サンプル数 )
	void *						Buffer ;												// データバッファ
} ;
...
struct SOUNDSYSTEMDATA
{
	...
	SIMPLELIST					PlaySoundBufferListFirst ;				// 再生中サウンドバッファリストの先頭
	...
	WAVEFORMATEX				SelfMixingFormat ;				// 自前ミキシングのフォーマット
	int							SelfMixingFormatValidBitsPerSample ;	// 自前ミキシングのフォーマットの有効ビット数
	int							SelfMixingFormatIsFloat ;		// 自前ミキシングのフォーマットが float型か
	int							SelfMixingFormatIsMSB ;			// 自前ミキシングのフォーマットがビッグエンディアンか
	volatile ULONGLONG			SelfMixingTotalWriteSamples ;	// 自前ミキシングで出力したサンプル数
//	volatile int				SelfMixingInitlizeTotalPlayTimeBaseCountFlag ;	// 自前ミキシングの総再生時間の基準となるカウント値が初期化済みかどうか
//	volatile ULONGLONG			SelfMixingTotalPlayTimeBaseCount ;				// 自前ミキシングの総再生時間の基準となるカウント値
	int							SelfMixingBufferIsFloat ;		// 自前ミキシング処理用のバッファが float型かどうか
	int							SelfMixingBufferSamples ;		// 自前ミキシング処理用のバッファのサンプル数
	void *						SelfMixingBuffer ;				// 自前ミキシング処理用のバッファ
	int							SelfMixingOutputDelaySamplesUnit ;	// 一回の再生辺りに加算する遅延サンプル数の数
	int							SelfMixingOutputDelaySamples ;	// 自前ミキシングで次の再生開始サウンドバッファに対して割り当てるディレイサンプル数
	int							SelfMixingScalingFlag ;			// 自前ミキシングでスケーリングを行っているかどうかのフラグ
	float						SelfMixingScalingStartParam ;	// 自前ミキシングでスケーリングを行う処理の初期値
	int							SelfMixingScalingTotalSamples ;	// 自前ミキシングでスケーリングを行う時間
	...
	LONGLONG					SoundCaptureSample ;			// キャプチャしたサンプルの数(44.1KHz換算)
	...
	short						Bit8To16Table[ 256 ] ;				// 8bit波形を16bit波形に変換するためのテーブル
...
struct MIDIHANDLEDATA
{
	...
	size_t						DataSize ;						// ＭＩＤＩファイルのデータサイズ( DSOUND.SoundMode が DX_MIDIMODE_MCI の時のみ有効 )
...
extern	int		SetupSelfMixingWorkBuffer( int IsFloat, int Samples ) ;													// 自前ミキシング処理用のバッファをセットアップする
extern	int		WriteSelfMixingSample( BYTE *Buffer0, BYTE *Buffer1, DWORD Stride, DWORD SampleNum ) ;					// 指定のバッファに自前ミキシングのデータを書き込む
...
extern	int		LoadSoundMem2ByMemImage_UseGParam(       LOADSOUND_GPARAM *GParam, const void *FileImageBuffer1, size_t ImageSize1, const void *FileImageBuffer2, size_t ImageSize2, int ASyncLoadFlag = FALSE ) ;														// LoadSoundMem2ByMemImage のグローバル変数にアクセスしないバージョン
...
extern	int		AddStreamSoundMemToMem_UseGParam(  LOADSOUND_GPARAM *GParam, void *FileImageBuffer, size_t ImageSize, int LoopNum,  int SoundHandle, int StreamDataType, int UnionHandle = -1, int ASyncThread = FALSE ) ;												// AddStreamSoundMemToMem のグローバル変数にアクセスしないバージョン
...
extern	int		SetLoopAreaTimePosSoundMem_UseGParam(    LONGLONG LoopStartTime,           LONGLONG LoopEndTime,           int SoundHandle, int ASyncThread = FALSE ) ;																									// SetLoopAreaTimePosSoundMem のグローバル変数にアクセスしないバージョン
extern	int		SetLoopAreaSamplePosSoundMem_UseGParam(  LONGLONG LoopStartSamplePosition, LONGLONG LoopEndSamplePosition, int SoundHandle, int ASyncThread = FALSE ) ;																									// SetLoopAreaSamplePosSoundMem のグローバル変数にアクセスしないバージョン
extern	int		SetLoopTimePosSoundMem_UseGParam(        LONGLONG LoopTime,                                                int SoundHandle, int ASyncThread = FALSE ) ;																									// SetLoopTimePosSoundMem のグローバル変数にアクセスしないバージョン
extern	int		SetLoopSamplePosSoundMem_UseGParam(      LONGLONG LoopSamplePosition,                                      int SoundHandle, int ASyncThread = FALSE ) ;																									// SetLoopSamplePosSoundMem のグローバル変数にアクセスしないバージョン
extern	int		SetLoopStartTimePosSoundMem_UseGParam(   LONGLONG LoopStartTime,                                           int SoundHandle, int ASyncThread = FALSE ) ;																									// SetLoopStartTimePosSoundMem のグローバル変数にアクセスしないバージョン
extern	int		SetLoopStartSamplePosSoundMem_UseGParam( LONGLONG LoopStartSamplePosition,                                 int SoundHandle, int ASyncThread = FALSE ) ;																									// SetLoopStartTimePosSoundMem のグローバル変数にアクセスしないバージョン

extern	LONGLONG GetSoundTotalSample_UseGParam( int SoundHandle, int ASyncThread = FALSE ) ;																																												// GetSoundTotalSample のグローバル変数にアクセスしないバージョン
extern	int		SetStreamSoundCurrentPosition_UseGParam( LONGLONG Byte, int SoundHandle, int ASyncThread = FALSE ) ;																																					// SetStreamSoundCurrentPosition のグローバル変数にアクセスしないバージョン
...
extern	int		SetupSoftSoundHandle( int SoftSoundHandle, int IsPlayer, int Channels, int BitsPerSample, int SamplesPerSec, int IsFloatType, LONGLONG SampleNum ) ;	// ソフトサウンドハンドルをセットアップ
...
extern	int		LoadSoftSoundBase_UseGParam( LOADSOUND_GPARAM *GParam, const wchar_t *FileName, const void *FileImage, size_t FileImageSize, int ASyncLoadFlag = FALSE ) ;		// ソフトウエアで扱う波形データをファイルまたはメモリ上に展開されたファイルイメージから作成する
extern	int		MakeSoftSoundBase_UseGParam( int IsPlayer, int Channels, int BitsPerSample, int SamplesPerSec, LONGLONG SampleNum, int IsFloatType, int UseFormat_SoftSoundHandle = -1, int ASyncLoadFlag = FALSE ) ;	// ソフトウエアで制御するサウンドデータハンドルの作成
...
extern	int		LoadMusicMemByMemImage_UseGParam( void *FileImage, size_t FileImageSize, int ASyncLoadFlag = FALSE ) ;	// LoadMusicMemByMemImage のグローバル変数にアクセスしないバージョン
...
extern	int		LoadMusicMemByMemImage_Static( int MusicHandle, const void *FileImage, size_t FileImageSize, int ASyncThread ) ;	// LoadMusicMemByMemImage の実処理関数
...
extern	int		ProcessSoundSystem_PF( void ) ;														// サウンドシステムで周期的に行う処理用の関数の環境依存処理を行う関数

extern	int		CheckSoundSystem_Initialize_PF( void ) ;											// サウンドシステムの初期化チェックの環境依存処理を行う関数( TRUE:初期化されている  FALSE:初期化されていない )
extern	int		GetSoundSystemTotalPlaySamples_PF( ULONGLONG *TotalPlaySamples, ULONGLONG *Frequency ) ;	// サウンドシステムの総再生時間を取得する
