...
static int GraphFilterUseWorkScreen_Table[DX_GRAPH_FILTER_NUM] =
{
		...
		0,		// DX_GRAPH_FILTER_SSAO
		0,		// DX_GRAPH_FILTER_FLOAT_COLOR_SCALE
		...
		0,		// DX_GRAPH_FILTER_PMA_REPLACEMENT
};
...
static int  Direct3D9_MemLoadShaderCode( const char *ShaderName, int ShaderType ) ;	// 指定名の事前用意シェーダーを読み込む
...
extern void Direct3D9_GraphFilter_PushBaseDrawParam( GRAPHFILTER_DRAWPARAMTEMP_DIRECT3D9 *DrawParamTemp, int BltBlendMode )
{
	...
	NS_SetDrawBlendMode( BltBlendMode == DX_BLENDMODE_NOBLEND ? DX_BLENDMODE_SRCCOLOR : BltBlendMode, 255 ) ;
...
extern int	GraphFilter_D3D9_Down_Scale_PF(  GRAPHFILTER_INFO *Info, int DivNum )
{
	...
	{
		GraphFilterShaderHandle.DownScalePS[ UseShader ] = Direct3D9_MemLoadShaderCode( PsoFileName[ UseShader ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.DownScalePS[ UseShader ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
...
extern int	GraphFilter_D3D9_Bright_Clip_PF( GRAPHFILTER_INFO *Info, int CmpType, float CmpParam, int ClipFillFlag, COLOR_F *ClipFillColor, int IsPMA )
{
	...
	{
		GraphFilterShaderHandle.BrightClipPS[ UseShader ][ UseFill ][ IsPMA ] = Direct3D9_MemLoadShaderCode( PsoFileName[ UseShader ][ UseFill ][ IsPMA ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.BrightClipPS[ UseShader ][ UseFill ][ IsPMA ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
...
extern int	GraphFilter_D3D9_HSB_PF(         GRAPHFILTER_INFO *Info, int HueType, float Hue, float Saturation, float Bright, int IsPMA )
{
	...
	{
		GraphFilterShaderHandle.HsbPS[ UseShader ][ IsPMA ] = Direct3D9_MemLoadShaderCode( PsoFileName[ UseShader ][ IsPMA ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.HsbPS[ UseShader ][ IsPMA ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
...
extern int	GraphFilter_D3D9_GradientMap_PF( GRAPHFILTER_INFO *Info, int MapGrHandle, int Reverse, int IsPMA )
{
	...
	{
		GraphFilterShaderHandle.GradientMapPS[ UseShader ][ IsPMA ] = Direct3D9_MemLoadShaderCode( PsoFileName[ UseShader ][ IsPMA ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.GradientMapPS[ UseShader ][ IsPMA ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
...
extern int	GraphFilter_D3D9_Replacement_PF(   GRAPHFILTER_INFO *Info, COLOR_U8 TargetColor, COLOR_U8 NextColor, int IsPMA )
{
	static const char *PsoFileName[ 2 ] =
	{
		"Replacement.pso",
	...
	ParamF4[ 0 ].x = ( float )TargetColor.r / 255.0f ;
	ParamF4[ 0 ].y = ( float )TargetColor.g / 255.0f ;
	ParamF4[ 0 ].z = ( float )TargetColor.b / 255.0f ;
	ParamF4[ 0 ].w = ( float )TargetColor.a / 255.0f ;
	ParamF4[ 1 ].x = ( float )NextColor.r / 255.0f ;
	ParamF4[ 1 ].y = ( float )NextColor.g / 255.0f ;
	ParamF4[ 1 ].z = ( float )NextColor.b / 255.0f ;
	ParamF4[ 1 ].w = ( float )NextColor.a / 255.0f ;
	Graphics_D3D9_ShaderConstant_InfoSet_SetParam( &GD3D9.Device.Shader.ShaderConstantInfo, DX_SHADERCONSTANTTYPE_PS_FLOAT, DX_SHADERCONSTANTSET_LIB_SUB, 0, ParamF4, 2, FALSE ) ;

	NS_SetUsePixelShader( GraphFilterShaderHandle.ReplacementPS[ IsPMA ] ) ;
...
extern int	GraphFilter_D3D9_PremulAlpha_PF( GRAPHFILTER_INFO *Info )
{
	...
	{
		GraphFilterShaderHandle.PreMulAlphaPS = Direct3D9_MemLoadShaderCode( PsoFileName[ UseShader ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.PreMulAlphaPS < 0 )
		{
			char PathUTF16LE[ 128 ] ;
...
extern int	GraphFilter_D3D9_InterpAlpha_PF( GRAPHFILTER_INFO *Info )
{
	...
	{
		GraphFilterShaderHandle.InterpAlphaPS = Direct3D9_MemLoadShaderCode( PsoFileName[ UseShader ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.InterpAlphaPS < 0 )
		{
			char PathUTF16LE[ 128 ] ;
...
extern int	GraphFilter_D3D9_YUVtoRGB_PF(     GRAPHFILTER_INFO *Info, int UVGrHandle )
{
	...
	{
		GraphFilterShaderHandle.YUVtoRGBPS[ UseShader ] = Direct3D9_MemLoadShaderCode( PsoFileName[ UseShader ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.YUVtoRGBPS[ UseShader ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
...
extern int	GraphFilter_D3D9_BicubicScale_PF( GRAPHFILTER_INFO *Info, int DestSizeX, int DestSizeY )
{
	...
	{
		GraphFilterShaderHandle.BicubicPS = Direct3D9_MemLoadShaderCode( PsoFileName[ UseShader ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.BicubicPS < 0 )
		{
			char PathUTF16LE[ 128 ] ;
...
extern int	GraphFilter_D3D9_Lanczos3Scale_PF( GRAPHFILTER_INFO *Info, int DestSizeX, int DestSizeY )
{
	...
	{
		GraphFilterShaderHandle.Lanczos3PS = Direct3D9_MemLoadShaderCode( PsoFileName[ UseShader ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.Lanczos3PS < 0 )
		{
			char PathUTF16LE[ 128 ] ;
...
//	NS_SetDrawMode( DX_DRAWMODE_NEAREST ) ;
	...
	{
		GraphFilterShaderHandle.BasicBlendPS[ UseShader ] = Direct3D9_MemLoadShaderCode( PsoFileName[ UseShader ], DX_SHADERTYPE_PIXEL ) ;
		if( GraphFilterShaderHandle.BasicBlendPS[ UseShader ] < 0 )
		{
			char PathUTF16LE[ 128 ] ;
...
extern int	GraphBlend_D3D9_RGBA_Select_Mix_PF( GRAPHFILTER_INFO *Info, int SelectR, int SelectG, int SelectB, int SelectA, int IsPMA )
{
	...
	if( SelectR == SelectG && SelectR == SelectB && SelectR != DX_RGBA_SELECT_BLEND_A && SelectR != DX_RGBA_SELECT_SRC_A )
	{
		...
		if( *PixelShaderHandle < 0 )
		{
			...
			*PixelShaderHandle = Direct3D9_MemLoadShaderCode( FileName, DX_SHADERTYPE_PIXEL ) ;
			if( *PixelShaderHandle < 0 )
			{
...
USE_BASE_SHADER:
		...
		if( *PixelShaderHandle < 0 )
		{
			*PixelShaderHandle = Direct3D9_MemLoadShaderCode( PsoFileName[ IsPMA ], DX_SHADERTYPE_PIXEL ) ;
			if( *PixelShaderHandle < 0 )
			{
				ConvString( PsoFileName[ IsPMA ], -1, DX_CHARCODEFORMAT_ASCII, ( char * )PathUTF16LE, sizeof( PathUTF16LE ), DX_CHARCODEFORMAT_UTF16LE ) ;
				DXST_LOGFILEFMT_ADDUTF16LE(( "\xd5\x30\xa3\x30\xeb\x30\xbf\x30\xfc\x30\x28\x75\xb7\x30\xa7\x30\xfc\x30\xc0\x30\xfc\x30\x6e\x30\x5c\x4f\x10\x62\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x20\x00\x25\x00\x73\x00\x00"/*@ L"フィルター用シェーダーの作成に失敗しました %s" @*/, PathUTF16LE )) ;
...
static int Direct3D9_BasePolygonDraw( GRAPHFILTER_INFO *Info, int ScaleDivNum, VERTEX_TEX8_2D *Texcoord8Vertex )
{
	...
	if( Info->IsBlend )
	{
		...
		if( Texcoord8Vertex == NULL )
		{
			...
			{
				BlendX1TexF = ( float )( BlendImage->Hard.Draw[ 0 ].UsePosXI + Info->BlendX                 ) / BlendImage->Orig->Hard.Tex[ 0 ].TexWidth ;
				BlendY1TexF = ( float )( BlendImage->Hard.Draw[ 0 ].UsePosYI + Info->BlendY                 ) / BlendImage->Orig->Hard.Tex[ 0 ].TexHeight ;
				if( Info->BlendPos2Enable )
				{
					BlendX2TexF = ( float )( BlendImage->Hard.Draw[ 0 ].UsePosXI + Info->BlendX2            ) / BlendImage->Orig->Hard.Tex[ 0 ].TexWidth ;
					BlendY2TexF = ( float )( BlendImage->Hard.Draw[ 0 ].UsePosYI + Info->BlendY2            ) / BlendImage->Orig->Hard.Tex[ 0 ].TexHeight ;
				}
				else
				{
					BlendX2TexF = ( float )( BlendImage->Hard.Draw[ 0 ].UsePosXI + Info->BlendX + SrcWidth  ) / BlendImage->Orig->Hard.Tex[ 0 ].TexWidth ;
					BlendY2TexF = ( float )( BlendImage->Hard.Draw[ 0 ].UsePosYI + Info->BlendY + SrcHeight ) / BlendImage->Orig->Hard.Tex[ 0 ].TexHeight ;
				}
			}
			else
			{
...
extern int GraphFilter_D3D9_DestGraphSetup_PF( GRAPHFILTER_INFO *Info, int *UseSrcGrHandle, int *UseDestGrHandle )
{
	...
	if( Info->DestGrHandle == DX_SCREEN_BACK )
	{
		...
		DestTexChannelBitDepth = GSYS.Screen.MainScreenColorBitDepth ;
	}
	else
	if( !GRAPHCHK( Info->DestGrHandle, DestImage ) )
	{
