...
//				if( WinData.NotWindowVisibleFlag == TRUE ) AddExStyle |= WS_EX_TRANSPARENT ;
				...
				else
				{
					...
					// ウィンドウサイズが明示的に設定されておらず、且つ
					// DPI が従来のデフォルトである 96 ではなかったら拡大率を変更する
		...
		//		Rect.bottom += + 3;
				...
				DXST_LOGFILEFMT_ADDUTF16LE(( "\xa6\x30\xa4\x30\xf3\x30\xc9\x30\xa6\x30\xe9\x77\x62\x5f\x20\x00\x28\x00\x20\x00\x25\x00\x64\x00\x2c\x00\x20\x00\x25\x00\x64\x00\x20\x00\x29\x00\x2d\x00\x28\x00\x20\x00\x25\x00\x64\x00\x2c\x00\x20\x00\x25\x00\x64\x00\x20\x00\x29\x00\x00"/*@ L"ウインドウ矩形 ( %d, %d )-( %d, %d )" @*/, Rect.left, Rect.top, Rect.right, Rect.bottom )) ;
...
*/
		...
		// ドラッグ＆ドロップ機能を有効にするフラグが立っていたらここで有効にする
...
extern int TerminateWindow( void )
{
	...
	DXST_LOGFILE_ADDUTF16LE( "\xa6\x30\xa3\x30\xf3\x30\xc9\x30\xa6\x30\xa2\x95\x23\x90\x6e\x30\x42\x7d\x86\x4e\xe6\x51\x06\x74\x2e\x00\x2e\x00\x2e\x00\x20\x00\x00"/*@ L"ウィンドウ関連の終了処理... " @*/ ) ;
...
//						WinData.GetMessageHookHandle = NULL ;
	...
	*/
	...
	// GetDesktopScreenGraphMemImage で使用していた HBITMAP があったらここで削除する
	...
	// GetDesktopScreenGraphMemImage で使用していたバッファがあったらここで削除する
...
MESNAMEEND:
	...
	case WM_DEVICECHANGE :
		// デバイスの追加や削除が行われた場合、パッドが追加されたり削除された可能性もあるのでジョイパッドを再セットアップする
	...
	case WM_CHAR :
		{
			...
			// Windows10 でIME有効状態では文字入力を無効化、IME文字入力中の場合は WM_CHAR のメッセージ自体を無視する
			...
			// SetWindows10_WM_CHAR_CancelTime が使用されていたら設定された値を使用する
	...
	case WM_KEYDOWN:
		{
			...
			// コントロール文字コードに対応するキーが
			// 押されていたらバッファに格納する
			// ( Windows10 でIME文字入力中の場合はコントロールキーを無視する )
...
extern int SetWindowStyle( int CenterPosition )
{
	...
	else
	{
		...
		if( WinData.WindowModeFlag == TRUE )
		{
			...
			// ウィンドウの拡大率が等倍かどうかを判定( DPI が 96 以外の場合は 96 換算で等倍かどうかも判定 )
			...
			// ウィンドウの拡大率を変更中では無く、ウィンドウモード変更中でもなく、且つウィンドウの拡大率が等倍かどうかで処理を分岐
		...
		//		Rect.bottom += + 3;
			...
			if( CenterPosition )
			{
				...
				// タスクバーの分だけセンタリング対象の領域を狭めつつセンタリングする
				...
				if( TrayWindow == NULL )
				{
					...
					// タスクバーのウィンドウハンドルが見つからなかったら通常のセンタリング処理
					Rect.left   += ( DesktopW - WindowSizeX ) / 2 + DesktopX ;
					Rect.top    += ( DesktopH - WindowSizeY ) / 2 + DesktopY ;
					Rect.right  += ( DesktopW - WindowSizeX ) / 2 + DesktopX ;
					Rect.bottom += ( DesktopH - WindowSizeY ) / 2 + DesktopY ;
...
extern int NS_SetWindowSizeExtendRate( double ExRateX, double ExRateY )
{
	...
	if( WinData.WindowModeFlag == TRUE || NS_GetUseFullScreenResolutionMode() == DX_FSRESOLUTIONMODE_BORDERLESS_WINDOW )
	{
		// ウィンドウのサイズの比率を変更中フラグを立てる
		WinData.ChangeWindowSizeExRate = TRUE ;

		SetWindowStyle() ;

		// ウィンドウのサイズの比率を変更中フラグを倒す
...
// SetUseUpdateLayerdWindowFlag( TRUE ); の設定の際にマウス関係のイベントを背後のウィンドウに透過させるかを設定する( TRUE:透過する   FALSE:透過しない( デフォルト ) )
...
int WM_SIZEProcess( void )
{
	...
	// クライアント領域に画面をフィットさせる場合はツールバーの高さを加算する
	if( WinData.ScreenNotFitWindowSize == FALSE && WinData.WindowSizeValid == FALSE )
	{
...
//		DXST_LOGFILE_ADDW( L"アクティブになりました\n" ) ;
		...
		if( ( WinData.WindowModeFlag == FALSE && NS_GetUseFullScreenResolutionMode() != DX_FSRESOLUTIONMODE_BORDERLESS_WINDOW ) && /*WinData.StopFlag == TRUE &&*/ DxSysData.DxLib_InitializeFlag == TRUE )
		{
			...
			// Direct3D 11 の場合はフルスクリーン設定を元に戻すだけ
		...
		else
		{
			...
			// ボーダーレスウィンドウの場合のみウィンドウ最前面にセットする
			if( WinData.WindowModeFlag == FALSE && NS_GetUseFullScreenResolutionMode() == DX_FSRESOLUTIONMODE_BORDERLESS_WINDOW )
			{
...
//		DXST_LOGFILE_ADDW( L"非アクティブになりました\n" ) ;
...
#ifndef DX_NON_GRAPHICS
#ifndef DX_NON_DIRECT3D11
				// Direct3D 11 の場合はフルスクリーン設定を解除するだけ
				if( GRAWIN.Setting.UseGraphicsAPI == GRAPHICS_API_DIRECT3D11_WIN32 )
				{
		...
		// ボーダーレスウィンドウの場合のみウィンドウ最前面固定を解除する
		if( WinData.WindowModeFlag == FALSE && NS_GetUseFullScreenResolutionMode() == DX_FSRESOLUTIONMODE_BORDERLESS_WINDOW )
		{
			WinAPIData.Win32Func.SetWindowPosFunc(
				WinData.MainWindow,
