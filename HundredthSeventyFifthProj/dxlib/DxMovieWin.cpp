...
//		BOOL IsContiguous ;
//		hr = p2DBuffer->IsContiguousFormat( &IsContiguous ) ;
//		IsContiguous = IsContiguous ;
		...
		// バッファにデータをコピーする
		{
			...
			// ystride が出力先の stride と等しい場合は処理を分岐
			...
			else
			{
				...
				BYTE *dst ;
				BYTE *y_ ;
				DWORD dwnum = Movie->PF.MFYWidth / 4 ;
				DWORD btnum = Movie->PF.MFYWidth - dwnum * 4 ;

				dst = ( BYTE * )Movie->PF.MFYBuffer ;
				y_ = yimage ;
				for( n = 0; n < ( DWORD )Movie->PF.MFYHeight ; n ++, dst += Movie->PF.MFYStride, y_ += ystride )
				{
					BYTE *y_t ;
					BYTE *dst_t ;
					...
					dst_t = dst ;
					y_t = y_ ;
					for( m = 0 ; m < dwnum ; m ++, dst_t += 4, y_t += 4 )
					{
						*( ( DWORD * )dst_t ) = *( ( DWORD * )y_t ) ;
					}
					for( m = 0 ; m < btnum ; m ++, dst_t ++, y_t ++ )
					{
						*dst_t = *y_t ;
					}
				}
...
)
{
	...
	// アーカイブファイル内のファイルだった場合はテンポラリファイルに書き出す
	if( DX_FISDXA( fp ) == 1 )
	{
...
#else
		AnalysisFileNameAndDirPathW_( FileName, lFileName, sizeof( lFileName ), lDirPath, sizeof( lDirPath ) ) ;
#endif
	...
	if( Movie->PF.pMovieImage->ImageType == 8 )
	{
		NS_CreateR5G6B5ColorData( &Movie->NowImage.ColorData ) ;
	}
	else
	if( Movie->PF.pMovieImage->ImageType == 9 )
	{
		NS_CreateX1R5G5B5ColorData( &Movie->NowImage.ColorData ) ;
	}
	else
	if( Movie->PF.pMovieImage->ImageType == 10 )
	{
		int i ;
		NS_CreatePal8ColorData( &Movie->NowImage.ColorData, FALSE ) ;
		for( i = 0 ; i < 256 ; i ++ )
		{
			Movie->NowImage.ColorData.Palette[ i ].Alpha = 255 ;
			Movie->NowImage.ColorData.Palette[ i ].Red   = Movie->PF.pMovieImage->Palette[ i ].rgbRed ;
			Movie->NowImage.ColorData.Palette[ i ].Green = Movie->PF.pMovieImage->Palette[ i ].rgbGreen ;
			Movie->NowImage.ColorData.Palette[ i ].Blue  = Movie->PF.pMovieImage->Palette[ i ].rgbBlue ;
		}
	}
	else
	{
...
extern int TerminateMovieHandle_PF( HANDLEINFO *HandleInfo )
{
	...
	{
		NS_DeleteSoundMem( Movie->PF.MFSoundHandle, FALSE ) ;
...
extern int SeekMovie_PF( MOVIEGRAPH * Movie, int Time )
{
	...
	if( Movie->PF.pMFReader != NULL )
	{
		...
		Result = Movie->PF.pMFReader->SetCurrentPosition( _GUID_NULL, variant ) ;
		if( Result != S_OK )
		{
			return -1 ;
		}
		...
		if( Movie->PlayFlag )
		{
			NS_PlaySoundMem( Movie->PF.MFSoundHandle, DX_PLAYTYPE_BACK, FALSE ) ;
		}
#endif // DX_NON_SOUND
