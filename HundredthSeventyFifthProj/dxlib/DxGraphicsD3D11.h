...
//	D_ID3D11RenderTargetView *		TextureRTV[ CUBEMAP_SURFACE_NUM ] ;		// テクスチャ用レンダリングターゲットビュー
	...
	D_ID3D11ShaderResourceView *	DepthBufferSRV ;						// 深度バッファ用シェーダーリソースビュー
} ;
...
struct SHADERHANDLEDATA_DIRECT3D11
{
	union
	{
		...
		D_ID3D11GeometryShader		*GeometryShader ;						// ID3D11GeometryShader のポインタ
	} ;
} ;
...
{
	// モデル描画用法線・深度描画用のピクセルシェーダー( リストは配列の左から )
	// [ バンプマップの有無( 0:無し 1:有り ) ]
...
struct GRAPHICS_HARDDATA_DIRECT3D11_DEVICE_STATE
{
	...
	D_ID3D11BlendState *					DefaultBlendStateArray[ DX_BLENDMODE_NUM ][ 2 ] ;	// よく使う設定のブレンディングステートの配列
	...
	D_ID3D11Buffer *						SetGeometryShaderConstantBuffer[ D_D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT ] ;	// 現在 ID3D11DeviceContext にジオメトリシェーダー用にセットしてある定数バッファ
	...
	D_ID3D11GeometryShader *				SetGeometryShader ;								// 現在 ID3D11DeviceContext にセットしてあるジオメトリシェーダー
...
struct GRAPHICS_HARDDATA_DIRECT3D11_DRAWINFO
{
	...
	D_ID3D11Buffer *			CommonIndexBuffer[ 2 ] ;						// 汎用描画用インデックスバッファ( 0:16bit  1:32bit )
	int							CommonIndexBufferSize[ 2 ] ;					// 汎用描画用インデックスバッファのサイズ( 0:16bit  1:32bit )
} ;
...
struct GRAPHICS_HARDDATA_DIRECT3D11_OUTPUTWINDOWINFO
{
	...
	HWND							TargetWindow ;							// 対象のウィンドウ
	int								IsMainTarget ;							// メインターゲットかどうか
	int								IsFullscreen ;							// フルスクリーンモードかどうか
	...
	D_ID3D11Texture2D *				DXGIDuplicationTexture2D ;				// DXGIOutputDuplication の D3D11Texture2D
	D_ID3D11ShaderResourceView *	DXGIDuplicationTextureSRV ;				// DXGIOutputDuplication の D3D11ShaderResourceView
	D_D3D11_TEXTURE2D_DESC			DXGIDuplicationTexture2DDesc ;			// DXGIOutputDuplication の D3D11Texture2D の情報
	void *							DXGIDuplicationImageAddress ;			// GetDesktopScreenGraphMemImage で使用するイメージアドレス
	int								DXGIDuplicationImageWidth ;				// GetDesktopScreenGraphMemImage で使用するイメージの幅
	int								DXGIDuplicationImageHeight ;			// GetDesktopScreenGraphMemImage で使用するイメージの高さ
...
//	D_D3D11_TEXTURE2D_DESC			BackBufferTexture2DDesc ;
	...
	D_ID3D11Texture2D *				SubBackBufferMSTexture2D ;				// ScreenCopy や GetDrawScreen を実現するために使用する描画可能テクスチャ( マルチサンプル用 )
	...
	D_DXGI_FORMAT					SubBackBufferFormat ;					// サブバックバッファのテクスチャフォーマット
...
struct GRAPHICS_HARDDATA_DIRECT3D11_DEVICE_CAPS
{
	...
	D_DXGI_FORMAT					DepthBufferTextureSRVFormat[ ZBUFFER_FORMAT_NUM ] ;		// 深度バッファ用Textureをシェーダーリソースとして使用する場合のフォーマット
...
#ifndef DX_NON_MODEL
	// モデル描画用の頂点単位ライティングあり頂点シェーダーバイナリ
	// [ メッシュタイプ( 0:剛体メッシュ 1:4ボーン内スキニングメッシュ  2:8ボーン内スキニングメッシュ ) ]
	// [ バンプマップの有無( 0:無し 1:有り ) ]
...
struct GRAPHICS_HARDDATA_DIRECT3D11_SETTING
{
	...
	int								UseDirect3D11BGRASupport ;					// D3D11Device 作成時に D3D11_CREATE_DEVICE_BGRA_SUPPORT を指定するかどうか( TRUE:指定する  FALSE:指定しない( デフォルト ) )
	int								EnableUseDXGIAdapterIndex ;					// UseDXGIAdapterIndex が有効化どうか
	int								UseDXGIAdapterIndex ;						// 使用するDXGIアダプターのインデックス
...
extern	int		Graphics_D3D11_CompileShader( const char *FileName, const char *EntryPoint, const char *Profile, const D_D3DXMACRO *MacroTable, D_ID3DBlob **ppShader ) ;							// Direct3D11 のシェーダーソースをコンパイルする
extern	int		Graphics_D3D11_CompileVertexShader( const char *FileName, const char *EntryPoint, const D_D3DXMACRO *MacroTable, D_ID3D11VertexShader **VertexShader, D_ID3DBlob **pCode = NULL ) ;	// Direct3D11 のシェーダーソースをコンパイルして頂点シェーダーを作成する
extern	int		Graphics_D3D11_CompilePixelShader( const char *FileName, const char *EntryPoint, const D_D3DXMACRO *MacroTable, D_ID3D11PixelShader **PixelShader, D_ID3DBlob **pCode = NULL ) ;	// Direct3D11 のシェーダーソースをコンパイルしてピクセルシェーダーを作成する

#ifndef DX_NON_MODEL
extern	int		CreateD3D11MV1ToonOutLineShadowMapVertexShader( int MeshType, int BumpMap ) ;																										// モデル描画用のシャドウマップへのトゥーンレンダリング輪郭線描画用頂点シェーダーのコンパイルを行う
extern	int		CreateD3D11MV1NormalDepthPixelShader( int BumpMap ) ;																																// モデル描画用の法線と深度描画用のピクセルシェーダーのコンパイルを行う
extern	int		CreateD3D11MV1PixelLightingVertexShader( int ShadowMap, int MeshType, int BumpMap, int FogType ) ;																					// モデル描画用ピクセル単位ライティングタイプの頂点シェーダーのコンパイルを行う
extern	int		CreateD3D11MV1PixelLightingToonType1PixelShader( int ShadowMap, int ToonDiffuseBlendOP, int ToonSpecularBlendOP, int BumpMap, int EnableLightNumMinusOne, int AlphaTestMode ) ;		// モデル描画用ピクセル単位ライティングタイプのトゥーンタイプ１用ピクセルシェーダーのコンパイルを行う
extern	int		CreateD3D11MV1PixelLightingToonType2PixelShader( int ShadowMap, int SphereMapBlendType, int EnableLightNumMinusOne, int AlphaTestMode ) ;											// モデル描画用ピクセル単位ライティングタイプのトゥーンタイプ２用ピクセルシェーダーのコンパイルを行う
extern	int		CreateD3D11MV1PixelLightingNormalPixelShader( int ShadowMap, int MultiTexMode, int BumpMap, int EnableLightNumMinusOne, int AlphaTestMode ) ;										// モデル描画用ピクセル単位ライティングタイプのピクセルシェーダーのコンパイルを行う
extern	int		CreateD3D11MV1NoLightingVertexShader( int MeshType, int BumpMap, int FogType ) ;																									// モデル描画用のライティングなし頂点シェーダーのコンパイルを行う
extern	int		CreateD3D11MV1VertexLightingVertexShader( int ShadowMap, int MeshType, int BumpMap, int FogType, int LightIndex ) ;																	// モデル描画用のライティングあり頂点シェーダーのコンパイルを行う
extern	int		CreateD3D11MV1ShadowMapVertexShader( int MeshType, int BumpMap ) ;																													// モデル描画用のシャドウマップへのレンダリング用頂点シェーダーのコンパイルを行う
extern	int		CreateD3D11MV1ToonOutLineVertexShader( int MeshType, int BumpMap, int FogType ) ;																									// モデル描画用のトゥーンレンダリングの輪郭線描画用頂点シェーダーのコンパイルを行う
extern	int		CreateD3D11MV1MaterialTypePixelShader( int MaterialType, int AlphaTestMode ) ;																										// モデル描画用のマテリアル要素描画用ピクセルシェーダーのコンパイルを行う
extern	int		CreateD3D11MV1NoLightingToonPixelShader( int ToonType, int ToonDiffuseBlendOP, int AlphaTestMode ) ;																				// モデル描画用のライティングなしトゥーン用ピクセルシェーダーのコンパイルを行う
extern	int		CreateD3D11MV1VertexLightingToonType1PixelShader( int ShadowMap, int DiffuseGradBlendType, int SpecularGradBlendType, int BumpMap, int LightIndex, int AlphaTestMode ) ;			// モデル描画用のライティングありトゥーンタイプ１用ピクセルシェーダーのコンパイルを行う
extern	int		CreateD3D11MV1VertexLightingToonType2PixelShader( int ShadowMap, int SphereMapBlendType, int LightIndex, int AlphaTestMode ) ;														// モデル描画用のライティングありトゥーンタイプ１用ピクセルシェーダーのコンパイルを行う
extern	int		CreateD3D11MV1NoLightingNormalPixelShader( int MultiTexBlendMode, int AlphaTestMode ) ;																								// モデル描画用のライティングなしピクセルシェーダーのコンパイルを行う
extern	int		CreateD3D11MV1VertexLightingNormalPixelShader( int ShadowMap, int MultiTexBlendMode, int BumpMap, int LightIndex, int AlphaTestMode ) ;												// モデル描画用のライティングありピクセルシェーダーのコンパイルを行う
extern	int		CreateD3D11MV1ShadowMapToonPixelShader( int ToonType, int ToonSphereOP, int ToonDiffuseBlendOP, int AlphaTestMode ) ;																// モデル描画用のシャドウマップへのトゥーンレンダリング用ピクセルシェーダーのコンパイルを行う
extern	int		CreateD3D11MV1ShadowMapNormalPixelShader( int AlphaTestMode ) ;																														// モデル描画用のシャドウマップへのレンダリング用ピクセルシェーダーのコンパイルを行う
#endif // DX_NON_MODEL
...
extern	int		Graphics_D3D11_OutputWindow_ResizeTarget( int Index, int NewSizeX, int NewSizeY, int NewRefreshRate ) ;	// 指定の出力先ウインドウのサイズを変更する
extern	int		Graphics_D3D11_OutputWindow_ReCreate( int Index ) ;														// 指定の出力先ウインドウの SwapChain を作成しなおす
...
extern	int		Graphics_D3D11_TerminateSubBackBuffer( void ) ;															// サブバックバッファの後始末をする
extern	int		Graphics_D3D11_CreateDepthBuffer( void ) ;																// 深度バッファオブジェクトの作成( 0:成功  -1:失敗 )
extern	int		Graphics_D3D11_FullscreenSetup( int IsFullscreen ) ;													// フルスクリーン設定を行う
...
extern	int		Graphics_D3D11_DeviceState_SetBlendMode( int BlendMode, int BlendEnable, int BlendRGBSrc, int BlendRGBDest, int BlendRGBOp, int BlendASrc, int BlendADest, int BlendAOp, int NotWriteAlphaChannelFlag ) ;			// 描画ブレンドモードのセット
...
extern	int		Graphics_D3D11_DeviceState_SetGeometryShaderToHandle( int ShaderHandle ) ;							// 指定のジオメトリシェーダーをデバイスにセットする
extern	int		Graphics_D3D11_DeviceState_SetGeometryShader( D_ID3D11GeometryShader *GeometryShader ) ;			// 使用するジオメトリシェーダーを変更する
extern	int		Graphics_D3D11_DeviceState_ResetGeometryShader( void ) ;											// ジオメトリシェーダーの使用を止める
extern	int		Graphics_D3D11_DeviceState_SetGeometryShaderConstantBuffers( UINT StartSlot, UINT NumBuffers, D_ID3D11Buffer * const *pConstantBuffers ) ;	// ジオメトリシェーダーで使用する定数バッファーを設定する
...
// 描画設定関係関数
extern	int		Graphics_D3D11_DrawSetting_SetDrawBlendMode( int BlendMode, int BlendEnable, int BlendRGBSrc, int BlendRGBDest, int BlendRGBOp, int BlendASrc, int BlendADest, int BlendAOp, int AlphaTestValidFlag, int AlphaChannelValidFlag ) ;	// 描画ブレンドモードの設定
...
extern	int		Graphics_D3D11_DrawSetting_SetWriteAlphaChannelFlag( int NotFlag ) ;							// 描画先のアルファチャンネルの内容を書き換えるかを設定する
...
extern	int		Graphics_D3D11_DrawSetting_SetLightNoAngleAttenuation( int NoAngleAttenuation ) ;					// ライティング計算の角度減衰を行わないかを設定する
...
extern	int		Graphics_D3D11_ConstantBuffer_GSSet( UINT StartSlot, UINT NumBuffers, CONSTANTBUFFER_DIRECT3D11 * const *pConstantBuffers ) ;	// 指定の定数バッファをジオメトリシェーダーに設定する
...
extern	int		Graphics_D3D11_CommonIndexBuffer_Setup( int Size, D_DXGI_FORMAT IndexFormat ) ;	// 指定サイズの汎用描画用インデックスバッファの準備を行う
extern	int		Graphics_D3D11_CommonVertexBuffer_Terminate( void ) ;							// 汎用描画用頂点バッファの後始末を行う
extern	int		Graphics_D3D11_CommonIndexBuffer_Terminate( D_DXGI_FORMAT IndexFormat ) ;		// 汎用描画用インデックスバッファの後始末を行う
...
extern	int		Graphics_D3D11_DrawBoxSet( const RECTDATA *RectData, int Num ) ;																						// ハードウエアアクセラレータ使用版 DrawBoxSet
...
extern	int		Graphics_D3D11_DrawPrimitive32bitIndexed2DToShader( const VERTEX2DSHADER *Vertex, int VertexNum, const unsigned int   *Indices, int IndexNum, int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */ ) ;		// シェーダーを使って２Ｄプリミティブを描画する( 頂点インデックスを使用する )
...
extern	int		Graphics_D3D11_DrawPrimitive32bitIndexed3DToShader( const VERTEX3DSHADER *Vertex, int VertexNum, const unsigned int   *Indices, int IndexNum, int PrimitiveType /* DX_PRIMTYPE_TRIANGLELIST 等 */ ) ;		// シェーダーを使って３Ｄプリミティブを描画する( 頂点インデックスを使用する )
...
extern	int		Graphics_Hardware_D3D11_SetDrawCustomBlendMode_PF( int BlendEnable, int SrcBlendRGB, int DestBlendRGB, int BlendOpRGB, int SrcBlendA, int DestBlendA, int BlendOpA, int BlendParam ) ; // カスタムブレンドモードを設定する
...
extern	int		Graphics_Hardware_D3D11_SetDrawAddColor_PF( int Red, int Green, int Blue ) ;								// 描画色に加算する色をセット
...
extern	int		Graphics_Hardware_D3D11_GetVideoMemorySizeEx_PF( ULONGLONG *TotalSize, ULONGLONG *UseSize ) ;									// ビデオメモリの容量を得る( 64bit版 )
...
extern	int		Graphics_Hardware_D3D11_Light_SetNoAngleAttenuation_PF( int NoAngleAttenuation ) ;								// ライトの計算で角度減衰を行わないようにするかどうかを設定する
...
// 環境依存シェーダー関係
extern	int		Graphics_Hardware_D3D11_Shader_Create_PF( int ShaderHandle, int ShaderType, void *Image, int ImageSize, int ImageAfterFree, int ASyncThread ) ;		// シェーダーハンドルを初期化する
