...
struct SHADERHANDLEDATA
{
	...
	int						ShaderType ;						// シェーダータイプ( DX_SHADERTYPE_VERTEX 等 )
...
struct INDEXBUFFERHANDLEDATA
{
	...
	int						Type ;								// インデックスデータのタイプ( DX_INDEX_TYPE_16BIT 等 )
...
struct SETUP_GRAPHHANDLE_GPARAM
{
	...
	int						NotInitGraphDelete ;					// InitGraph で削除しないかどうかのフラグ( TRUE:InitGraphでは削除しない  FALSE:InitGraphで削除する )
	int						NotInitGraphDeleteUser ;				// InitGraph で削除しないかどうかのフラグ、ユーザー用( TRUE:InitGraphでは削除しない  FALSE:InitGraphで削除する )
	int						CreateGraphHandle ;						// 作成するグラフィックハンドルとして使用するグラフィックハンドル値
...
struct GRAPHICSSYS_USERRENDERINFO
{
	...
	int						SetGeometryShaderHandle ;				// ユーザー設定で使用するジオメトリシェーダー
...
//	int						NotUseBasicGraphDraw3DDeviceMethodFlag ;// 単純図形の描画に３Ｄデバイスの機能を使用しないかどうかのフラグ
	...
	int						UseRightHandClippingProcess ;			// 右手座標系のクリッピング処理を行うかどうか( TRUE:右手座標系  FALSE:左手座標系 )
...
//	int						SystemMemImageCreateFlag ;				// システムメモリを使用する画像作成指定フラグ( 標準サーフェスのみ )
	...
	int						NotUseAlphaImageLoadFlag ;				// _a が付いたアルファチャンネル用の画像ファイルを追加で読み込む処理を行わないかどうか( TRUE:行わない  FALSE:行う )
	int						NotUsePaletteGraphFlag ;				// パレット画像が使用できる場合もパレット画像を使用しないかどうか( TRUE:使用しない  FALSE:使用する )
	int						NotUseLoadDivGraphSizeCheck ;			// LoadDivGraph系の関数でサイズチェックを行わないかどうか( TRUE:行わない  FALSE:行う )
} ;
...
struct GRAPHICSSYS_SCREENDATA
{
	...
	void *					UserScreenImage ;						// ScreenFlip でＤＸライブラリ側のメインスクリーンの代わりに画面に転送されるユーザーイメージのアドレス
	int						UserScreenImagePixelFormat ;			// ScreenFlip でＤＸライブラリ側のメインスクリーンの代わりに画面に転送されるユーザーイメージのピクセルフォーマット( DX_USER_SCREEN_PIXEL_FORMAT_R5G6B5 等 )
	int						UserScreenImagePixelFormatMatchSoftRenderMode ;	// UserScreenImagePixelFormat のピクセルフォーマットがソフトウェアレンダリングモードに対応しているかどうか( TRUE:対応している  FALSE：対応していない )
...
//	int						PreSetWaitVSyncFlag ;					// DxLib_Init が呼ばれる前に SetWaitVSyncFlag( TRUE ) ; が実行されたかどうかのフラグ( TRUE:実行された  FALSE:実行されていない )
	...
	int						Graphics_Screen_Change_BackScreenW ;	// 画面モード変更前の横解像度
	int						Graphics_Screen_Change_BackScreenH ;	// 画面モード変更前の縦解像度
...
//	DISPLAYMODEDATA			*DisplayMode ;							// ディスプレイモードリスト
	...
	int						PrimaryDisplayIndex ;					// プライマリディスプレイのインデックス
...
struct GRAPHICSSYS_HARDWAREINFO
{
	...
	int						SeparateAlphaBlendEnable ;				// RGBのブレンド設定とは別にアルファチャンネルのブレンド設定が行えるかどうか( TRUE:別に設定できる  FALSE:別に設定できない )
} ;
...
struct GRAPHICSSYS_PERFORMANCEINFO
{
	...
	ULONGLONG				ScreenFlipTime[ 2 ] ;					// 前回、前々回 ScreenFlip が呼ばれた時間
} ;
...
extern	int		Graphics_Screen_ChangeMode( int ScreenSizeX, int ScreenSizeY, int ColorBitDepth, int ChangeWindowFlag, int RefreshRate, int AlwaysRunFlag ) ;				// 画面モードの変更２
...
extern	COLORDATA *Graphics_Screen_UserScreenPixelFormatColorData( int PixelFormat /* DX_USER_SCREEN_PIXEL_FORMAT_R5G6B5 等 */ ) ;	// SetUserScreenImage で指定するピクセルフォーマットの COLORDATA を取得する
...
extern	int		Graphics_Image_AddHandle( int GrHandle, int ASyncThread ) ;																	// 新しいグラフィックハンドルを確保する
...
extern	int		Graphics_Image_DumpInfoHandle( HANDLEINFO *HandleInfo ) ;																// グラフィックハンドルの情報出力
...
extern	int		Graphics_Shader_CreateHandle_UseGParam( int ShaderType /* DX_SHADERTYPE_VERTEX 等 */, void *Image, int ImageSize, int ImageAfterFree, int ASyncLoadFlag = FALSE, int ASyncThread = FALSE ) ;		// シェーダーハンドルを作成する
extern	int		Graphics_Shader_LoadShader_UseGParam( int ShaderType /* DX_SHADERTYPE_VERTEX 等 */, const wchar_t *FileName, int ASyncLoadFlag = FALSE ) ;	// シェーダーバイナリをファイルから読み込む
...
extern	int		Graphics_Hardware_GetVideoMemorySizeEx_PF( ULONGLONG *TotalSize, ULONGLONG *UseSize ) ;	// ビデオメモリの容量を得る( 64bit版 )
...
// 環境依存シェーダー関係
extern	int		Graphics_Hardware_Shader_Create_PF( int ShaderHandle, int ShaderType /* DX_SHADERTYPE_VERTEX 等 */, void *Image, int ImageSize, int ImageAfterFree, int ASyncThread ) ;		// シェーダーハンドルを初期化する
...
extern	int		Graphics_Hardware_DrawBoxSet_PF(           const RECTDATA  *RectData,  int Num ) ;																				// ハードウエアアクセラレータ使用版 DrawBoxSet
