...
static	int      SetSampleTimeSoundConvert_WAVE(    SOUNDCONV *SoundConv, LONGLONG SampleTime ) ;		// 変換処理の位置を変更する( サンプル単位 )
static	LONGLONG GetSoundConvertDestSize_Fast_WAVE( SOUNDCONV *SoundConv ) ;							// 変換後の大凡のデータサイズを得る
static	int      GetSoundConvertLoopAreaInfo_WAVE(  SOUNDCONV *SoundConv, LONGLONG *LoopStartPos, LONGLONG *LoopEndPos ) ;	// ループ情報を取得( [戻] -1:エラー )
