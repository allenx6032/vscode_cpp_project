...
extern HANDLE CreateTemporaryFile( wchar_t *TempFileNameBuffer, size_t BufferBytes, wchar_t *ExeName )
{
	...
	else
	{
		...
		{
			char ExeNameA[ 64 ] ;
			ConvString( ( char * )ExeName, -1, WCHAR_T_CHARCODEFORMAT, ExeNameA, sizeof( ExeNameA ), CHAR_CHARCODEFORMAT ) ;
...
//		WinAPIData.Win32Func.WaitForSingleObjectFunc( dat->FuncEvent, INFINITE ) ;
		...
		case FILEACCESSTHREAD_FUNCTION_OPEN :
			dat->Handle = CreateFileWFunc( dat->FilePath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL ) ;
...
extern	size_t ReadOnlyFileAccessRead_PF( void *Buffer, size_t BlockSize, size_t DataNum, FILEACCESS *FileAccess )
{
	...
	{
		size_t ReadSize = BlockSize * DataNum ;
		...
		if( ReadSize > 0xffffffff )
		{
			...
			{
				size_t lTempReadSize ;

				lTempReadSize = ReadSize - TotalReadSize ;
				if( lTempReadSize > 0xffffffff )
				{
					lTempReadSize = 0xffffffff ;
				}
				WinAPIData.Win32Func.ReadFileFunc( FileAccess->PF.Handle, ( BYTE * )Buffer + TotalReadSize, ( DWORD )lTempReadSize, &BytesRead, NULL ) ;

				TotalReadSize += BytesRead ;

				if( BytesRead < lTempReadSize )
				{
					break ;
				}
			}
...
extern int ReadOnlyFileAccessChDir_PF( const wchar_t *Path )
{
	...
	return SetCurrentDirectoryWFunc( Path ) ;
}
...
extern int ReadOnlyFileAccessGetDirS_PF( wchar_t *Buffer, size_t BufferBytes )
{
	...
	return ( int )GetCurrentDirectoryWFunc( ( DWORD )( BufferBytes / sizeof( wchar_t ) ), Buffer ) ;
}
...
extern int ReadOnlyFileAccessFindFirst_PF( FINDINFO *FindInfo, const wchar_t *FilePath, FILEINFOW *Buffer )
{
	...
	FindInfo->PF.FindHandle = FindFirstFileWFunc( FilePath, &FindData ) ;
...
extern int ReadOnlyFileAccessFindNext_PF( FINDINFO *FindInfo, FILEINFOW *Buffer )
{
	...
	if( FindNextFileWFunc( FindInfo->PF.FindHandle, &FindData ) == 0 )
	{
		return -1 ;
	}
...
extern	DWORD_PTR	WriteOnlyFileAccessOpen_PF(  const wchar_t *Path )
{
	...
	DeleteFileWFunc( Path ) ;
	HANDLE Result = CreateFileWFunc( Path, GENERIC_WRITE, 0, NULL, CREATE_NEW, FILE_ATTRIBUTE_NORMAL, NULL ) ;
...
extern	int			WriteOnlyFileAccessWrite_PF( DWORD_PTR Handle, void *Buffer, size_t WriteSize, size_t *GetWriteSize )
{
	...
	else
	{
		...
		while( TotalWriteSize < WriteSize )
		{
			size_t lTempWriteSize ;

			lTempWriteSize = WriteSize - TotalWriteSize ;
			if( lTempWriteSize > 0xffffffff )
			{
				lTempWriteSize = 0xffffffff ;
			}
			WinAPIData.Win32Func.WriteFileFunc( ( HANDLE )Handle, ( BYTE * )Buffer + TotalWriteSize, ( DWORD )lTempWriteSize, &lWriteSize, NULL ) ;

			TotalWriteSize += lWriteSize ;

			if( lWriteSize < lTempWriteSize )
			{
				break ;
			}
		}
