...
// ビープ音関係
#define SOUND_BEEP_SAMPLEPERSEC				(44100)										// ビープ音用のサウンドバッファの再生周波数
#define SOUND_BEEP_SAMPLEPERBYTES			(2)											// ビープ音用のサウンドバッファの１サンプルあたりのバイト数
#define SOUND_BEEP_SOUNDBUFFER_SAMPLECOUNT	(SOUND_BEEP_SAMPLEPERSEC / 2)				// ビープ音用のサウンドバッファの長さ
#define SOUND_BEEP_ONE_WRITE_SAMPLES		(SOUND_BEEP_SOUNDBUFFER_SAMPLECOUNT / 4)	// ビープ音用のサウンドで一度に書き込むサンプル数
#define SOUND_DEFAULT_BEEP_FREQUENCY		(1500)										// デフォルトのビープ音周波数
#define SOUND_BEEP_PLAY_VOLUME_DELAY		(16)										// ビープ音を再生してからサウンドバッファの音量を上げるまでの時間
#define SOUND_BEEP_STOP_DELAY				(16)										// ビープ音を停止してから実際にサウンドバッファの再生を停止するまでの時間

// 自前ミキシング関係
#define SELF_MIXING_ONE_OUTPUT_DELAY_DIV	(60 * 8)									// 自前ミキシングで１音辺りにずらす時間
#define SELF_MIXING_SCALING_TIME			(2)											// 自前ミキシングで音割れ防止用スケーリングを元に戻す時間
...
extern	int CreateWaveFileImage( 	void **DestBufferP, size_t *DestBufferSizeP,
									...
									const void *WaveData, size_t WaveDataSize ) ;										// 音声フォーマットと波形イメージからＷＡＶＥファイルイメージを作成する
static	int FileFullRead( const wchar_t *FileName, void **BufferP, size_t *SizeP, int OutputFileOpenErrorLog = TRUE ) ;	// ファイルを丸まるメモリに読み込む
extern	int StreamFullRead( STREAMDATA *Stream, void **BufferP, size_t *SizeP ) ;										// ストリームデータを丸まるメモリに読み込む
static	int SoundTypeChangeToStream( int SoundHandle ) ;																// サウンドハンドルのタイプがストリームハンドルではない場合にストリームハンドルに変更する
static	LONGLONG MilliSecPositionToSamplePosition( int SamplesPerSec, LONGLONG MilliSecTime ) ;							// ミリ秒単位の数値をサンプル単位の数値に変換する
static	LONGLONG SamplePositionToMilliSecPosition( int SamplesPerSec, LONGLONG SampleTime ) ;							// サンプル単位の数値をミリ秒単位の数値に変換する
...
extern int InitializeSoundSystem( void )
{
	...
	// 8bit波形を16bit波形に変換するためのテーブルを初期化
	{
		...
		for( i = 0 ; i < 256 ; i ++ )
		{
			SoundSysData.Bit8To16Table[ i ] = ( short )( ( i * 65535 ) / 255 - 32768 ) ;
		}
	}

	// 自前ミキシングの総再生サンプル数を初期化
...
extern int TerminateSoundSystem( void )
{
	...
	if( SoundSysData.InitializeFlag )
	{
		...
		CriticalSection_Delete( &SoundSysData.PlaySoundListCriticalSection ) ;
	...
	// 自前ミキシングを行っていた場合は作業用バッファのメモリの解放
...
extern int TerminateSoundHandle( HANDLEINFO *HandleInfo )
{
	...
	{
		if( Sound->Buffer[0].Valid ) NS_StopSoundMem( HandleInfo->Handle, FALSE ) ;
	}
...
extern int DumpInfoSoundHandle( HANDLEINFO *HandleInfo )
{
	...
	if( Sound->Type == DX_SOUNDTYPE_STREAMSTYLE )
	{
		DXST_LOGFILEFMT_ADDW(( L"Handle:0x%08x Type:Stream SamplePerSec:%d BitsPerSample:%d Channels:%d", HandleInfo->Handle, Sound->BufferFormat.nSamplesPerSec, Sound->BufferFormat.wBitsPerSample, Sound->BufferFormat.nChannels )) ;
	}
	else
	{
...
)
{
	...
	case DX_SOUNDDATATYPE_MEMNOPRESS :
		// メモリ上に直接再生可能なデータを保持する場合
		{
			...
			// ＰＣＭ形式に変換
			{
				...
				res = SoundConvertFast( &ConvData, &Format, &PlayData->FileImage, &DestSize ) ;
				PlayData->FileImageSize = DestSize ;
	...
	case DX_SOUNDDATATYPE_MEMPRESS :
		// データはメモリ上に存在して、圧縮されている場合
		{
			...
			{
				LONGLONG LoopStartPos, LoopEndPos ;
...
)
{
	...
	if( fp == 0 )
	{
		DXST_LOGFILEFMT_ADDW(( L"Sound File Open Error : %s" , WaveFile )) ;
		goto ERR ;
	}
	...
	{
		DX_FCLOSE( fp ) ;
		DXST_LOGFILEFMT_ADDW(( L"Sound File Setup Error : %s" , WaveFile )) ;
		goto ERR ;
	}
...
extern int NS_StopStreamSoundMem( int SoundHandle, int IsNextLoopEnd )
{
	...
	// 次回ループ終了時に音を止める指定の場合は、フラグを立てて終了する
	if( IsNextLoopEnd == TRUE )
	{
		// フラグを立てる
		Sound->Stream.IsNextLoopEndStop = TRUE ;

		// 再生タイプが DX_PLAYTYPE_LOOP だった場合は DX_PLAYTYPE_BACK に変更する
...
LOOPSTART:
	...
	for( List = SoundSysData.PlaySoundListFirst.Next ; List->Next != NULL ; List = List->Next )
	{
		if( NS_CheckSoundMem( List->Handle ) == FALSE )
		{
			Sound = ( SOUND * )List->Data ;
			Sound->AddPlaySoundList = FALSE ;
			SubHandleList( &Sound->PlaySoundList ) ;
...
static void LoadSoundMem2ByMemImage_ASync( ASYNCLOADDATA_COMMON *AParam )
{
	...
	ImageSize1 = GetASyncLoadParamSize_t( AParam->Data, &Addr ) ;
...
)
{
	...
	if( ASyncLoadFlag )
	{
		...
		AddASyncLoadParamSize_t( NULL, &Addr, ImageSize1 ) ; 
		...
		AddASyncLoadParamSize_t( AParam->Data, &Addr, ImageSize1 ) ; 
...
static int SetupSoundPitchRateTimeStretchRateChangeHandle( int SoundHandle, LOADSOUND_GPARAM *GParam, int SSSHandleI, int BufferNum, int ASyncThread )
{
	...
	if( GParam->CreateSoundPitchRateEnable )
	{
		...
		if( PitchRate > 2.0f )
		{
			DSSSampleNum = _DTOL64( NS_GetSoftSoundSampleNum( SSSHandle ) * 2.0 ) ;
		...
		DSSSampleNum = _DTOL64( NS_GetSoftSoundSampleNum( SSSHandle ) * PitchRate ) ;
		DSSHandle = NS_MakeSoftSound( SSSHandle, DSSSampleNum ) ;
		if( DSSHandle < 0 )
		{
			ErrorPos = -3 ;
	...
	if( GParam->CreateSoundTimeStretchRateEnable )
	{
		...
		DSSSampleNum = _DTOL64( NS_GetSoftSoundSampleNum( SSSHandle ) * GParam->CreateSoundTimeStretchRate ) ;
		DSSHandle = NS_MakeSoftSound( SSSHandle, DSSSampleNum ) ;
		if( DSSHandle < 0 )
		{
			ErrorPos = -4 ;
	...
	if( CreateWaveFileImage(
			...
			sizeof( WAVEFORMATEX ),
			SSSound->Wave.Buffer,
			( size_t )( SSSound->Wave.BufferSampleNum * SSSound->BufferFormat.nBlockAlign ) ) < 0 )
	{
		ErrorPos = -6 ;
		goto ERR ;
	}
...
static	int	BeepSound_Initialize( void )
{
	// BEEP音用のサウンドバッファを作成する
	{
		...
		wfmtx.nBlockAlign		= ( WORD )( wfmtx.wBitsPerSample * wfmtx.nChannels / 8 ) ;
		...
		for( i = 0 ; i < SOUND_BEEPSOUNDBUFFER_NUM ; i ++ )
		{
			if( SoundBuffer_Initialize(
					...
					( DWORD )( SOUND_BEEP_SOUNDBUFFER_SAMPLECOUNT * wfmtx.nBlockAlign ),
					&wfmtx,
					NULL,
					TRUE,
					FALSE,
...
)
{
	...
	if( SetLoopAreaCancel == FALSE )
	{
		...
		if( GParam->CreateSoundLoopStartSamplePosition != GParam->CreateSoundLoopEndSamplePosition && GParam->CreateSoundIgnoreLoopAreaInfo == FALSE )
		{
...
//		Sound->PitchRate       = GParam->CreateSoundPitchRate ;
		...
		Sound->Normal.WaveSize = ( int )SoundSize ;	// データサイズを保存
...
static void LoadSoundMemByMemImageBase_ASync( ASYNCLOADDATA_COMMON *AParam )
{
	...
	ImageSize = GetASyncLoadParamSize_t( AParam->Data, &Addr ) ;
	BufferNum = GetASyncLoadParamInt( AParam->Data, &Addr ) ;
	UnionHandle = GetASyncLoadParamInt( AParam->Data, &Addr ) ;
...
)
{
	...
	if( ASyncLoadFlag && ASyncThread == FALSE )
	{
		...
		AddASyncLoadParamSize_t( NULL, &Addr, ImageSize ) ;
		AddASyncLoadParamInt( NULL, &Addr, BufferNum ) ;
		AddASyncLoadParamInt( NULL, &Addr, UnionHandle ) ;
		...
		AddASyncLoadParamSize_t( AParam->Data, &Addr, ImageSize ) ;
		AddASyncLoadParamInt( AParam->Data, &Addr, BufferNum ) ;
		AddASyncLoadParamInt( AParam->Data, &Addr, UnionHandle ) ;
...
extern int NS_StopSoundMem( int SoundHandle, int IsNextLoopEnd )
{
	...
	// ループ終了時に止める指定の場合はループフラグを倒す( セルフミキシングの場合のみ )
	if( IsNextLoopEnd )
	{
		...
		else
		{
			for( i = 0 ; i < Sound->ValidBufferNum ; i ++ )
			{
				...
				{
					SoundBuffer_Stop( &Sound->Buffer[ i ] ) ;
					SoundBuffer_Play( &Sound->Buffer[ i ], FALSE ) ;
				}
			}
		}
	}
...
//	return _DTOL( (double)time * 1000 / ( sd->BufferFormat.nSamplesPerSec * sd->BufferFormat.nBlockAlign ) ) ;
	return SamplePositionToMilliSecPosition( ( int )sd->BufferFormat.nSamplesPerSec, ( LONGLONG )( time / sd->BufferFormat.nBlockAlign ) ) ;
}
...
extern int SetLoopStartSamplePosSoundMem_UseGParam( LONGLONG LoopStartSamplePosition, int SoundHandle, int ASyncThread )
{
	...
	{
		RefreshStreamSoundPlayCompLength( SoundHandle ) ;
		SetStreamSoundCurrentPosition_UseGParam( ( LONGLONG )sd->Stream.CompPlayWaveLength, SoundHandle, ASyncThread ) ;
	}
...
extern int SoundBuffer_Play( SOUNDBUFFER *Buffer, int Loop )
{
	...
	if( SoundSysData.EnableSoundCaptureFlag || SoundSysData.EnableSelfMixingFlag )
	{
		...
		// 自前ミキシングを行っている場合は再生中のサウンドリストに追加する
...
extern int SoundBuffer_Stop( SOUNDBUFFER *Buffer, int EffectStop )
{
	...
	if( SoundSysData.EnableSoundCaptureFlag || SoundSysData.EnableSelfMixingFlag )
	{
		...
		// 自前ミキシングを行っている場合は再生中のサウンドリストから外す
		if( Buffer->AddPlaySoundBufferList )
		{
			Buffer->AddPlaySoundBufferList = FALSE ;

			SubSimpleList( ( SIMPLELIST * )&Buffer->PlaySoundBufferList ) ;
		}
...
extern int SoundBuffer_SetFrequency( SOUNDBUFFER *Buffer, DWORD Frequency )
{
	...
	{
		Buffer->Frequency = ( int )Frequency ;
	}

	if( Buffer->Frequency > 0 && ( DWORD )Buffer->Frequency != Buffer->Format.nSamplesPerSec )
	{
		Buffer->EnableComPosF = TRUE ;
		Buffer->CompPosF = ( double )Buffer->CompPos ;
	}
	else
	if( _DABS( Buffer->CompPosF - ( double )Buffer->CompPos ) < 0.0001 )
	{
		Buffer->EnableComPosF = FALSE ;
	}
...
//				TotalPlayTime = NS_GetNowSysPerformanceCount() - SoundSysData.SelfMixingTotalPlayTimeBaseCount ;
//				TotalPlaySamples = _DTOL64( ( double )TotalPlayTime * Buffer->Format.nSamplesPerSec / NS_GetSysPerformanceFrequency() ) ;
				...
				if( GetSoundSystemTotalPlaySamples_PF( &TotalPlaySamples, &Frequency ) >= 0 )
				{
					...
					{
						ULONGLONG PlaySamples = BufferFormatTotalPlaySamples - Buffer->TopPos_TotalWriteSamples ;

						if( PlaySamples >= ( ULONGLONG )Buffer->SampleNum )
						{
							if( Buffer->Loop )
							{
								*PlayPos = ( DWORD )( PlaySamples % ( ULONGLONG )Buffer->SampleNum ) * Buffer->Format.nBlockAlign ;
							}
							else
							{
								*PlayPos = ( DWORD )Buffer->SampleNum * Buffer->Format.nBlockAlign ;
							}
						}
						else
						{
							*PlayPos = ( DWORD )PlaySamples * Buffer->Format.nBlockAlign ;
						}
					}
				}
				else
				{
					*PlayPos = ( DWORD )Buffer->PlayPos * Buffer->Format.nBlockAlign ;
				}
			}
			else
			{
		...
		if( WritePos )
		{
			*WritePos = ( DWORD )Buffer->PlayPos * Buffer->Format.nBlockAlign ;
		}
	}
	else
	{
		...
		if( Result == 2 )
		{
			if( PlayPos  ) *PlayPos  = ( DWORD )Buffer->PlayPos * Buffer->Format.nBlockAlign ;
			if( WritePos ) *WritePos = ( DWORD )Buffer->PlayPos * Buffer->Format.nBlockAlign ;
		}
...
extern int SoundBuffer_FrameProcess( SOUNDBUFFER *Buffer, int Sample, short *DestBuf )
{
	...
	if( SoundSysData.EnableSoundCaptureFlag )
	{
		...
		vol  = Buffer->Volume[ 0 ] ;
		pan  = Buffer->Pan ;
		s    = ( short * )( ( BYTE * )Buffer->Wave->Buffer + Buffer->PlayPos * Buffer->Format.nBlockAlign ) ;
		sb   = (BYTE *)s ;
		...
		if( Buffer->PlayPos + Sample >= Buffer->SampleNum )
		{
			if( Buffer->Loop == TRUE )
			{
				Buffer->PlayPos = ( Buffer->PlayPos + Sample ) - Buffer->SampleNum ;
			}
			else
			{
				Buffer->PlayPos = Buffer->SampleNum ;
				Buffer->State = FALSE ;
			}
		}
		else
		{
			Buffer->PlayPos += Sample ;
		}
		Buffer->CompPos = Buffer->PlayPos ;
	}
	else
...
{
	int   SrcPos ;
	float SrcPosF = *pSrcPosF ;
	DWORD OutputSamples = 0 ;
	...
	*pIsEndSrc = FALSE ;
	if( Channels == 2 )
	{
		if( SrcBitsPerSample == 8 )
		{
			...
			if( DestBitsPerSample == 8 )
			{
				BYTE *Dest = ( BYTE * )DestBuffer ;

				while( OutputSamples < ( DWORD )DestSamples )
				{
					SrcPos = _FTOL( SrcPosF ) ;
					BYTE Src1Ch1, Src1Ch2 ;
					BYTE Src2Ch1, Src2Ch2 ;
					DWORD Rate ;
					...
					if( SrcPos >= SrcSamples )
					{
						if( Loop )
						{
							SrcPosF -= ( float )SrcSamples ;
							SrcPos = _FTOL( SrcPosF ) ;
						}
						else
						{
							*pSrcPosF = ( float )SrcSamples ;
							*pSrcPos = SrcSamples ;
							*pOutputSamples = OutputSamples ;
							*pIsEndSrc = TRUE ;
							return 0 ;
						}
					}

					Rate = ( DWORD )_FTOL( ( SrcPosF - ( float )SrcPos ) * ( float )( 1 << 15 ) ) ;

					Src1 = ( BYTE * )SrcBuffer + SrcPos * 2 ;
					Src1Ch1 = Src1[ 0 ] ;
					Src1Ch2 = Src1[ 1 ] ;
					if( SrcPos == SrcSamples - 1 )
					{
						if( Loop )
						{
							Src2 = ( BYTE * )SrcBuffer ;
					...
					Dest[ 0 ] = ( BYTE )( ( ( Src2Ch1 - Src1Ch1 ) * Rate + ( Src1Ch1 << 15 ) ) >> 15 ) ;
					Dest[ 1 ] = ( BYTE )( ( ( Src2Ch2 - Src1Ch2 ) * Rate + ( Src1Ch2 << 15 ) ) >> 15 ) ;
			...
			{
				short *Dest = ( short * )DestBuffer ;

				while( OutputSamples < ( DWORD )DestSamples )
				{
					SrcPos = _FTOL( SrcPosF ) ;
					short Src1Ch1, Src1Ch2 ;
					short Src2Ch1, Src2Ch2 ;
					int Rate ;
					...
					Rate = _FTOL( ( SrcPosF - ( float )SrcPos ) * ( float )( 1 << 15 ) ) ;
					...
					if( SrcPos == SrcSamples - 1 )
					{
						...
						{
							Src2 = ( BYTE * )SrcBuffer ;
							Src2Ch1 = SoundSysData.Bit8To16Table[ Src2[ 0 ] ] ;
							Src2Ch2 = SoundSysData.Bit8To16Table[ Src2[ 1 ] ] ;
						}
						else
						{
							Src2Ch1 = 0 ;
					...
					Src1Ch1 = SoundSysData.Bit8To16Table[ Src1[ 0 ] ] ;
					Src1Ch2 = SoundSysData.Bit8To16Table[ Src1[ 1 ] ] ;
					Dest[ 0 ] = ( short )( ( ( Src2Ch1 - Src1Ch1 ) * Rate + ( Src1Ch1 << 15 ) ) >> 15 ) ;
					Dest[ 1 ] = ( short )( ( ( Src2Ch2 - Src1Ch2 ) * Rate + ( Src1Ch2 << 15 ) ) >> 15 ) ;
		...
		else
		{
			...
			if( DestBitsPerSample == 8 )
			{
				...
				while( OutputSamples < ( DWORD )DestSamples )
				{
					...
					Src1 = ( short * )SrcBuffer + SrcPos * 2 ;
					if( SrcPos == SrcSamples - 1 )
					{
						if( Loop )
						{
							Src2 = ( short * )SrcBuffer ;
							Src2Ch1 = ( BYTE )( ( ( WORD )Src2[ 0 ] + 32768 ) >> 8 ) ;
							Src2Ch2 = ( BYTE )( ( ( WORD )Src2[ 1 ] + 32768 ) >> 8 ) ;
						}
						else
						{
							Src2Ch1 = 0 ;
					...
					Src1Ch1 = ( BYTE )( ( ( WORD )Src1[ 0 ] + 32768 ) >> 8 ) ;
					Src1Ch2 = ( BYTE )( ( ( WORD )Src1[ 1 ] + 32768 ) >> 8 ) ;
					Dest[ 0 ] = ( BYTE )( ( ( Src2Ch1 - Src1Ch1 ) * Rate + ( Src1Ch1 << 15 ) ) >> 15 ) ;
					Dest[ 1 ] = ( BYTE )( ( ( Src2Ch2 - Src1Ch2 ) * Rate + ( Src1Ch2 << 15 ) ) >> 15 ) ;
...
extern int SetupSelfMixingWorkBuffer( int IsFloat, int Samples )
{
	...
	if( SoundSysData.SelfMixingBuffer == NULL )
	{
		DXST_LOGFILE_ADDUTF16LE( "\xea\x81\x4d\x52\xdf\x30\xad\x30\xb7\x30\xf3\x30\xb0\x30\xe6\x51\x06\x74\x28\x75\x6e\x30\xe1\x30\xe2\x30\xea\x30\x6e\x30\xba\x78\xdd\x4f\x6b\x30\x31\x59\x57\x65\x57\x30\x7e\x30\x57\x30\x5f\x30\x0a\x00\x00"/*@ L"自前ミキシング処理用のメモリの確保に失敗しました\n" @*/ ) ;
		return -1 ;
	}
...
#define DEST_FLOAT_HZ_CHANGE_1CH_ADD_PARTS\
	Output0F = ( float )( ( Src1_N - Src1_P ) * Ratio + Src1_P ) ;\
	DestBufF[ 0 ] += Output0F * Scale0 ;\
	DestBufF[ 1 ] += Output0F * Scale1 ;\
	DestBufF += 2 ;

#define DEST_FLOAT_HZ_CHANGE_2CH_ADD_PARTS\
	DestBufF[ 0 ] += ( float )( ( Src1_N - Src1_P ) * Ratio + Src1_P ) * Scale0 ;\
	DestBufF[ 1 ] += ( float )( ( Src2_N - Src2_P ) * Ratio + Src2_P ) * Scale1 ;\
	DestBufF += 2 ;

#define DEST_INT_HZ_CHANGE_1CH_ADD_SCALE_PARTS\
	Output0F = ( float )( ( Src1_N - Src1_P ) * Ratio + Src1_P ) ;\
	DestBufI[ 0 ] += _DTOL( Output0F * Scale0 ) ;\
	DestBufI[ 1 ] += _DTOL( Output0F * Scale1 ) ;\
	DestBufI += 2 ;

#define DEST_INT_HZ_CHANGE_1CH_ADD_PARTS\
	Output0 = _DTOL( ( Src1_N - Src1_P ) * Ratio + Src1_P ) ;\
	DestBufI[ 0 ] += Output0 ;\
	DestBufI[ 1 ] += Output0 ;\
	DestBufI += 2 ;

#define DEST_INT_HZ_CHANGE_2CH_ADD_SCALE_PARTS\
	DestBufI[ 0 ] += _DTOL( ( ( Src1_N - Src1_P ) * Ratio + Src1_P ) * Scale0 ) ;\
	DestBufI[ 1 ] += _DTOL( ( ( Src2_N - Src2_P ) * Ratio + Src2_P ) * Scale1 ) ;\
	DestBufI += 2 ;

#define DEST_INT_HZ_CHANGE_2CH_ADD_PARTS\
	DestBufI[ 0 ] += _DTOL( ( Src1_N - Src1_P ) * Ratio + Src1_P ) ;\
	DestBufI[ 1 ] += _DTOL( ( Src2_N - Src2_P ) * Ratio + Src2_P ) ;\
	DestBufI += 2 ;

#define HZ_CHANGE_SRC_1CH_U8_PARTS0\
	{\
		BYTE *SrcBuf = ( BYTE * )SoundBuf->Wave->Buffer + CompPos ;\
\
		...
		{\
			Src1_N = SoundSysData.Bit8To16Table[ SrcBuf[ 1 ] ] ;\
		}\
\
		while( OutputSampleNum > 0 )\
		{

#define HZ_CHANGE_SRC_1CH_U8_PARTS1\
			OutputSampleNum -- ;\
\
			Ratio += SampleAdd ;\
			while( Ratio >= 1.0 )\
...
#define HZ_CHANGE_SRC_2CH_U8_PARTS0\
	{\
		BYTE *SrcBuf = ( BYTE * )SoundBuf->Wave->Buffer + CompPos * 2 ;\
\
		Src1_P = SoundSysData.Bit8To16Table[ SrcBuf[ 0 ] ] ;\
		...
		{\
			Src1_N = SoundSysData.Bit8To16Table[ SrcBuf[ 2 ] ] ;\
			Src2_N = SoundSysData.Bit8To16Table[ SrcBuf[ 3 ] ] ;\
		}\
\
		while( OutputSampleNum > 0 )\
		{

#define HZ_CHANGE_SRC_2CH_U8_PARTS1\
			OutputSampleNum -- ;\
\
			Ratio += SampleAdd ;\
			while( Ratio >= 1.0 )\
...
#define HZ_CHANGE_SRC_1CH_S16_PARTS1\
			OutputSampleNum -- ;\
\
			Ratio += SampleAdd ;\
			while( Ratio >= 1.0 )\
...
#define HZ_CHANGE_SRC_2CH_S16_PARTS0\
	{\
		short *SrcBuf = ( short * )SoundBuf->Wave->Buffer + CompPos * 2 ;\
\
		Src1_P = SrcBuf[ 0 ] ;\
		Src2_P = SrcBuf[ 1 ] ;\
		if( CompPos == SoundBuf->SampleNum - 1 )\
...
#define HZ_CHANGE_SRC_2CH_S16_PARTS1\
			OutputSampleNum -- ;\
\
			Ratio += SampleAdd ;\
			while( Ratio >= 1.0 )\
...
#define DESTADD_FLOAT_2CH\
	DestBufF[ 0 ] += ( float )Output0 * Scale0 ;\
	DestBufF[ 1 ] += ( float )Output1 * Scale1 ;\
	DestBufF += 2 ;

#define DESTADD_INT_1CH_SCALE\
	DestBufI[ 0 ] += _FTOL( Output0 * Scale0 ) ;\
	DestBufI[ 1 ] += _FTOL( Output0 * Scale1 ) ;\
	DestBufI += 2 ;

#define DESTADD_INT_1CH\
	DestBufI[ 0 ] += Output0 ;\
	DestBufI[ 1 ] += Output0 ;\
	DestBufI += 2 ;

#define DESTADD_INT_2CH_SCALE\
	DestBufI[ 0 ] += _FTOL( Output0 * Scale0 ) ;\
	DestBufI[ 1 ] += _FTOL( Output1 * Scale1 ) ;\
	DestBufI += 2 ;

#define DESTADD_INT_2CH\
	DestBufI[ 0 ] += Output0 ;\
	DestBufI[ 1 ] += Output1 ;\
	DestBufI += 2 ;

#define INT_CONVCODE( INT_SRC_PARTS0, TYPE, CH_NUM, DEST_INT_ADD_SCALE, DEST_INT_ADD, DEST_FLOAT_ADD )\
	if( SoundSysData.SelfMixingBufferIsFloat )\
	{\
		INT_SRC_PARTS0\
		DEST_FLOAT_ADD\
...
#define INT_SRC_2CH_U8_PARTS0\
	{\
		BYTE *SrcBuf = ( BYTE * )SoundBuf->Wave->Buffer + ( CompPos << 1 ) ;\
\
		while( OutputSampleNum > 0 )\
		{\
			Output0 = SoundSysData.Bit8To16Table[ SrcBuf[ 0 ] ] ;\
...
#define INT_SRC_2CH_S16_PARTS0\
	{\
		short *SrcBuf = ( short * )SoundBuf->Wave->Buffer + ( CompPos << 1 ) ;\
\
		while( OutputSampleNum > 0 )\
		{\
			Output0 = SrcBuf[ 0 ] ;\
...
\
			...
			{\
				...
				{\
					CompPos = 0 ;\
					SrcBuf = ( TYPE * )SoundBuf->Wave->Buffer ;\
				}\
				else\
				{\
					SoundBuf->CompPos = SoundBuf->SampleNum ;\
...
extern int WriteSelfMixingSample( BYTE *Buffer0, BYTE *Buffer1, DWORD Stride, DWORD SampleNum )
{
	DWORD i ;
	SIMPLELIST *List ;
	SIMPLELIST *ListNext ;
	...
	for( List = SoundSysData.PlaySoundBufferListFirst.Next ; List->Next != NULL ; List = ListNext )
	{
		...
		float Scale1 ;
		int IsScale ;
		float *DestBufF = ( float * )SoundSysData.SelfMixingBuffer ;
		int *DestBufI = ( int * )SoundSysData.SelfMixingBuffer ;

		ListNext = List->Next ;

		OutputSampleNum = SampleNum ;

		SoundBuf = ( SOUNDBUFFER * )List->Data ;
		CompPos = SoundBuf->CompPos ;
		Volume = SoundBuf->CalcVolume ;
		Pan = SoundBuf->CalcPan ;

		// 再生遅延サンプルがある場合はその分再生を遅らせる
		while( OutputSampleNum > 0 && SoundBuf->OutputDelaySamples > 0 )
		{
			DestBufF += 2 ;
			DestBufI += 2 ;
			SoundBuf->OutputDelaySamples -- ;
			OutputSampleNum -- ;
		}
		...
		if( SoundBuf->EnableComPosF == FALSE && SoundBuf->Format.nSamplesPerSec != SoundSysData.SelfMixingFormat.nSamplesPerSec )
		{
			SoundBuf->EnableComPosF = TRUE ;
			SoundBuf->CompPosF = ( double )SoundBuf->CompPos ;
		}

		if( SoundBuf->EnableComPosF )
		{
			double Ratio = SoundBuf->CompPosF - ( double )CompPos ;
			double SampleAdd ;
			int Src1_P ;
			int Src1_N ;
			int Src2_P ;
			...
			if( SoundBuf->Frequency <= 0 )
			{
				SampleAdd = ( double )SoundBuf->Format.nSamplesPerSec / SoundSysData.SelfMixingFormat.nSamplesPerSec ;
			}
			else
			{
				SampleAdd = ( ( double )SoundBuf->Format.nSamplesPerSec / SoundSysData.SelfMixingFormat.nSamplesPerSec ) * ( SoundBuf->Frequency / ( double )SoundBuf->Format.nSamplesPerSec ) ;
			}

			if( SoundBuf->Format.nChannels == 2 )
			{
			...
			SoundBuf->CompPos = CompPos ;
			SoundBuf->PlayPos = CompPos ;
			SoundBuf->CompPosF = ( double )CompPos + Ratio ;
		}
		else
		{
			if( SoundBuf->Format.nChannels == 2 )
	...
	// 音割れ防止処理をした後、本バッファに出力
	{
		...
		if( SoundSysData.SelfMixingBufferIsFloat )
		{
			...
			BufferF = ( float * )SoundSysData.SelfMixingBuffer ;
			for( i = 0 ; i < SampleNum ; i ++, BufferF += 2 )
			{
				float NowSample0 ;
				float NowSample1 ;
				float Scale = 1.0f ;
				...
				if( ScalingFlag )
				{
					Scale = ( 1.0f - ScalingStartParam ) * ScalingSampleCount / SoundSysData.SelfMixingScalingTotalSamples + ScalingStartParam ;
					NowSample0 = Scale * BufferF[ 0 ] ;
					NowSample1 = Scale * BufferF[ 1 ] ;
				}
				else
				{
					NowSample0 = BufferF[ 0 ] ;
					NowSample1 = BufferF[ 1 ] ;
				}

				if( NowSample0 > FloatMax )
				{
					ScalingFlag = TRUE ;
					ScalingStartParam = FloatMax / BufferF[ 0 ] ;
					ScalingSampleCount = 0 ;
					Scale = ScalingStartParam ;

					NowSample0 = FloatMax ;
					NowSample1 = Scale * BufferF[ 1 ] ;
				}
				else
				if( NowSample0 < FloatMin )
				{
					ScalingFlag = TRUE ;
					ScalingStartParam = FloatMin / BufferF[ 0 ] ;
					ScalingSampleCount = 0 ;
					Scale = ScalingStartParam ;
				...
				if( NowSample1 > FloatMax )
				{
					ScalingFlag = TRUE ;
					ScalingStartParam = FloatMax / BufferF[ 1 ] ;
					ScalingSampleCount = 0 ;
					Scale = ScalingStartParam ;
				...
				if( NowSample1 < FloatMin )
				{
					ScalingFlag = TRUE ;
					ScalingStartParam = FloatMin / BufferF[ 1 ] ;
					ScalingSampleCount = 0 ;
					Scale = ScalingStartParam ;
				...
				if( ScalingFlag )
				{
					BufferF[ 0 ] = BufferF[ 0 ] * Scale ;
					BufferF[ 1 ] = BufferF[ 1 ] * Scale ;

					ScalingSampleCount ++ ;
					if( ScalingSampleCount >= SoundSysData.SelfMixingScalingTotalSamples )
					{
						ScalingFlag = FALSE ;
						ScalingSampleCount = 0 ;
					}
			...
			if( SoundSysData.SelfMixingFormatIsFloat )
			{
				if( SoundSysData.SelfMixingFormat.nChannels == 1 )
				{
					for( i = 0 ; i < SampleNum ; i ++, BufferF += 2 )
					{
						*( ( float * )Buffer0 ) = ( BufferF[ 0 ] + BufferF[ 1 ] ) * 0.5f ;
						Buffer0 += Stride ;
					}
				}
				else
				{
					for( i = 0 ; i < SampleNum ; i ++, BufferF += 2 )
					{
						*( ( float * )Buffer0 ) = BufferF[ 0 ] ;
						*( ( float * )Buffer1 ) = BufferF[ 1 ] ;
						Buffer0 += Stride ;
						Buffer1 += Stride ;
					}
				}
			...
			{
 				switch( SoundSysData.SelfMixingFormatValidBitsPerSample )
 				{
 				case 16:
					if( SoundSysData.SelfMixingFormat.nChannels == 1 )
					{
						for( i = 0 ; i < SampleNum ; i ++, BufferF += 2 )
						{
							*( ( short * )Buffer0 ) = ( short )_FTOL( ( BufferF[ 0 ] + BufferF[ 1 ] ) * 0.5f ) ;
							Buffer0 += Stride ;
						}
					}
					else
					{
						for( i = 0 ; i < SampleNum ; i ++, BufferF += 2 )
						{
							*( ( short * )Buffer0 ) = ( short )_FTOL( BufferF[ 0 ] ) ;
							*( ( short * )Buffer1 ) = ( short )_FTOL( BufferF[ 1 ] ) ;
							Buffer0 += Stride ;
							Buffer1 += Stride ;
						}
					}
 				...
 				case 24:
 					if( SoundSysData.SelfMixingFormat.wBitsPerSample == 32 )
 					{
						if( SoundSysData.SelfMixingFormat.nChannels == 1 )
						{
							for( i = 0 ; i < SampleNum ; i ++, BufferF += 2 )
							{
								*( ( int * )Buffer0 ) = ( ( short )_FTOL( ( BufferF[ 0 ] + BufferF[ 1 ] ) * 0.5f ) << 8 ) & 0xffffff ;
								Buffer0 += Stride ;
							}
						}
						else
						{
							for( i = 0 ; i < SampleNum ; i ++, BufferF += 2 )
							{
								*( ( int * )Buffer0 ) = ( ( short )_FTOL( BufferF[ 0 ] ) << 8 ) & 0xffffff ;
								*( ( int * )Buffer1 ) = ( ( short )_FTOL( BufferF[ 1 ] ) << 8 ) & 0xffffff ;
								Buffer0 += Stride ;
								Buffer1 += Stride ;
							}
						}
 					...
 					else
 					{
						if( SoundSysData.SelfMixingFormat.nChannels == 1 )
						{
							...
							{
			 					DWORD Dest0D ;
			 					Dest0D = ( DWORD )_FTOL( ( BufferF[ 0 ] + BufferF[ 1 ] ) * 0.5f ) ;
						...
						else
						{
							for( i = 0 ; i < SampleNum ; i ++, BufferF += 2 )
							{
			 					DWORD Dest0D ;
			 					DWORD Dest1D ;
			 					Dest0D = ( DWORD )_FTOL( BufferF[ 0 ] ) ;
			 					Dest1D = ( DWORD )_FTOL( BufferF[ 1 ] ) ;
			 					Buffer0[ 0 ] = ( BYTE )Dest0D ;
			 					Buffer0[ 1 ] = ( BYTE )( Dest0D >> 8  ) ;
			 					Buffer0[ 2 ] = ( BYTE )( Dest0D >> 16 ) ;
 				...
 				case 32:
					if( SoundSysData.SelfMixingFormat.nChannels == 1 )
					{
						for( i = 0 ; i < SampleNum ; i ++, BufferF += 2 )
						{
							*( ( int * )Buffer0 ) = ( short )_FTOL( ( BufferF[ 0 ] + BufferF[ 1 ] ) * 0.5f ) << 16 ;
							Buffer0 += Stride ;
						}
					}
					else
					{
						for( i = 0 ; i < SampleNum ; i ++, BufferF += 2 )
						{
							*( ( int * )Buffer0 ) = ( short )_FTOL( BufferF[ 0 ] ) << 16 ;
							*( ( int * )Buffer1 ) = ( short )_FTOL( BufferF[ 1 ] ) << 16 ;
							Buffer0 += Stride ;
							Buffer1 += Stride ;
						}
					}
		...
		else
		{
			...
			BufferI = ( int * )SoundSysData.SelfMixingBuffer ;
			for( i = 0 ; i < SampleNum ; i ++, BufferI += 2 )
			{
				int NowSample0 ;
				int NowSample1 ;
				float Scale = 1.0f ;
				...
				{
					Scale = ( 1.0f - ScalingStartParam ) * ScalingSampleCount / SoundSysData.SelfMixingScalingTotalSamples + ScalingStartParam ;
					NowSample0 = _FTOL( Scale * BufferI[ 0 ] ) ;
					NowSample1 = _FTOL( Scale * BufferI[ 1 ] ) ;
				}
				else
				{
					NowSample0 = BufferI[ 0 ] ;
					NowSample1 = BufferI[ 1 ] ;
				}

				if( NowSample0 > 32767 )
				{
					ScalingFlag = TRUE ;
					ScalingStartParam = 32767.0f / BufferI[ 0 ] ;
					ScalingSampleCount = 0 ;
					Scale = ScalingStartParam ;
				...
				{
					ScalingFlag = TRUE ;
					ScalingStartParam = -32768.0f / BufferI[ 0 ] ;
					ScalingSampleCount = 0 ;
					Scale = ScalingStartParam ;
				...
				{
					ScalingFlag = TRUE ;
					ScalingStartParam = 32767.0f / BufferI[ 1 ] ;
					ScalingSampleCount = 0 ;
					Scale = ScalingStartParam ;
				...
				{
					ScalingFlag = TRUE ;
					ScalingStartParam = -32768.0f / BufferI[ 1 ] ;
					ScalingSampleCount = 0 ;
					Scale = ScalingStartParam ;
				...
				if( ScalingFlag )
				{
					BufferI[ 0 ] = _FTOL( BufferI[ 0 ] * Scale ) ;
					BufferI[ 1 ] = _FTOL( BufferI[ 1 ] * Scale ) ;
			...
			if( SoundSysData.SelfMixingFormatIsFloat )
			{
				if( SoundSysData.SelfMixingFormat.nChannels == 1 )
				{
					for( i = 0 ; i < SampleNum ; i ++, BufferI += 2 )
					{
						*( ( float * )Buffer0 ) = ( ( BufferI[ 0 ] + BufferI[ 1 ] ) >> 1 ) * ( 1.0f / 32768.0f ) ;
						Buffer0 += Stride ;
					}
				}
				else
				{
					for( i = 0 ; i < SampleNum ; i ++, BufferI += 2 )
					{
						*( ( float * )Buffer0 ) = BufferI[ 0 ] * ( 1.0f / 32768.0f ) ;
						*( ( float * )Buffer1 ) = BufferI[ 1 ] * ( 1.0f / 32768.0f ) ;
						Buffer0 += Stride ;
						Buffer1 += Stride ;
					}
				}
			...
			else
			{
 				...
 				case 16:
					if( SoundSysData.SelfMixingFormat.nChannels == 1 )
					{
						for( i = 0 ; i < SampleNum ; i ++, BufferI += 2 )
						{
							*( ( short * )Buffer0 ) = ( short )( ( BufferI[ 0 ] + BufferI[ 1 ] ) >> 1 ) ;
							Buffer0 += Stride ;
						}
					}
					else
					{
						for( i = 0 ; i < SampleNum ; i ++, BufferI += 2 )
						{
							*( ( short * )Buffer0 ) = ( short )BufferI[ 0 ] ;
							*( ( short * )Buffer1 ) = ( short )BufferI[ 1 ] ;
							Buffer0 += Stride ;
							Buffer1 += Stride ;
						}
					}
 				...
 				case 24:
 					if( SoundSysData.SelfMixingFormat.wBitsPerSample == 32 )
 					{
						if( SoundSysData.SelfMixingFormat.nChannels == 1 )
						{
							for( i = 0 ; i < SampleNum ; i ++, BufferI += 2 )
							{
								*( ( int * )Buffer0 ) = ( ( short )( ( BufferI[ 0 ] + BufferI[ 1 ] ) >> 1 ) << 8 ) & 0xffffff ;
								Buffer0 += Stride ;
							}
						}
						else
						{
							for( i = 0 ; i < SampleNum ; i ++, BufferI += 2 )
							{
								*( ( int * )Buffer0 ) = ( ( short )BufferI[ 0 ] << 8 ) & 0xffffff ;
								*( ( int * )Buffer1 ) = ( ( short )BufferI[ 1 ] << 8 ) & 0xffffff ;
								Buffer0 += Stride ;
								Buffer1 += Stride ;
							}
						}
 					...
 					else
 					{
						if( SoundSysData.SelfMixingFormat.nChannels == 1 )
						{
							...
							{
			 					DWORD Dest0D ;
			 					Dest0D = ( DWORD )( ( BufferI[ 0 ] + BufferI[ 1 ] ) >> 1 ) ;
			 					Buffer0[ 0 ] = ( BYTE )0 ;
 				...
 				case 32:
					if( SoundSysData.SelfMixingFormat.nChannels == 1 )
					{
						for( i = 0 ; i < SampleNum ; i ++, BufferI += 2 )
						{
							*( ( int * )Buffer0 ) = ( short )( ( BufferI[ 0 ] + BufferI[ 1 ] ) >> 1 ) << 16 ;
							Buffer0 += Stride ;
						}
					}
					else
					{
						for( i = 0 ; i < SampleNum ; i ++, BufferI += 2 )
						{
							*( ( int * )Buffer0 ) = ( short )BufferI[ 0 ] << 16 ;
							*( ( int * )Buffer1 ) = ( short )BufferI[ 1 ] << 16 ;
							Buffer0 += Stride ;
							Buffer1 += Stride ;
						}
					}
		...
		SoundSysData.SelfMixingScalingFlag = ScalingFlag ;
		SoundSysData.SelfMixingScalingStartParam = ScalingStartParam ;
...
static void MakeSoftSoundBase_ASync( ASYNCLOADDATA_COMMON *AParam )
{
	...
	SampleNum = GetASyncLoadParamLONGLONG( AParam->Data, &Addr ) ;
...
)
{
	...
	if( ASyncLoadFlag )
	{
		...
		AddASyncLoadParamLONGLONG( NULL, &Addr, SampleNum ) ;
		...
		AddASyncLoadParamLONGLONG( AParam->Data, &Addr, SampleNum ) ;
...
extern int SaveSoftSound_WCHAR_T( int SoftSoundHandle, const wchar_t *FileName )
{
	...
	if( SSound->Wave.BufferSampleNum * SSound->BufferFormat.nBlockAlign + NORMALWAVE_HEADERSIZE > 0xffffffff )
	{
		...
		*( ( DWORD * )p ) = 0xffffffff ;																							p += 4 ;
		...
		_MEMCPY( p, "ds64", 4 ) ;																									p += 4 ;
		*( ( DWORD * ) p ) = 28 ;																									p += 4 ;
		*( ( ULONGLONG * ) p ) = ( ULONGLONG )( SSound->Wave.BufferSampleNum * format->nBlockAlign + RF64WAVE_HEADERSIZE - 8 ) ;	p += 8 ;
		*( ( ULONGLONG * ) p ) = SSound->Wave.BufferSampleNum * format->nBlockAlign ;												p += 8 ;
		*( ( ULONGLONG * ) p ) = SSound->Wave.BufferSampleNum ;																		p += 8 ;
		*( ( DWORD * ) p ) = 0 ;																									p += 4 ;
		...
		*( ( DWORD * )p ) = 0xffffffff ;																							p += 4 ;
		...
		WriteOnlyFileAccessWrite( fp, Header, RF64WAVE_HEADERSIZE ) ;
...
extern int NS_WriteTimeStretchSoftSoundData( int SrcSoftSoundHandle, int DestSoftSoundHandle )
{
	...
	if( SSSound->Wave.BufferSampleNum < DSSound->Wave.BufferSampleNum )
	{
		...
		LONGLONG p, q = 0 ;
		int n, m, template_size, pmin, pmax ;
		double rate ;
		int rateI ;
		LONGLONG SSampleNum ;
		LONGLONG DSampleNum ;
...
extern int NS_WritePitchShiftSoftSoundData( int SrcSoftSoundHandle, int DestSoftSoundHandle )
{
	...
	case WAVE_FORMAT_PCM :
#if 1
		{
			...
			TempFloatSSSHandle = NS_MakeSoftSoundCustom( SSSound->BufferFormat.nChannels, 32, ( int )SSSound->BufferFormat.nSamplesPerSec, SSSound->Wave.BufferSampleNum, TRUE ) ;
	...
	case WAVE_FORMAT_IEEE_FLOAT :
		{
			...
			_MEMSET( DBuffer, 0, ( size_t )( sizeof( float ) * SSSound->BufferFormat.nChannels * DSampleNum ) ) ;
			...
			case 1 :
				for( n = 0; n < DSampleNum ; n++ )
				{
					...
					for( m = Start; m <= End; m++, T_M_I -= 128 )
					{
						AngleI = ( int )( ( int )( DX_PI_F * 128.0f ) * T_M_I ) ;
						if( AngleI == 0 )
						{
							SinCI = 16384 ;
						}
									...
									const void *WaveData, size_t WaveDataSize )
{
	...
	if( WaveDataSize + FormatSize + 28 > 0xffffffff )
	{
		...
		BufferSize = FormatSize + WaveDataSize
						+ 12/*"RF64" + ファイルサイズ + "WAVE"*/
						+ 36/*"ds64" + RF64チャンクサイズ + dataサイズ + サンプル数 + テーブルエントリー */
		...
		*( ( DWORD * ) p ) = 0xffffffff ;								p += 4 ;
		_MEMCPY( p, "WAVE", 4 ) ;										p += 4 ;

		_MEMCPY( p, "ds64", 4 ) ;										p += 4 ;
		*( ( DWORD * ) p ) = 28 ;										p += 4 ;
		*( ( ULONGLONG * ) p ) = BufferSize - 8 ;						p += 8 ;
		*( ( ULONGLONG * ) p ) = WaveDataSize ;							p += 8 ;
		*( ( ULONGLONG * ) p ) = WaveDataSize / Format->nBlockAlign ;	p += 8 ;
		*( ( DWORD * ) p ) = 0 ;										p += 4 ;
...
static int SoundTypeChangeToStream( int SoundHandle )
{
	...
	// ストリーム再生形式のハンドルとして作り直す
	{
		...
		InitLoadSoundGParam( &GParam ) ;
		GParam.Create3DSoundFlag = Is3DSound ;
...
static	LONGLONG MilliSecPositionToSamplePosition( int SamplesPerSec, LONGLONG MilliSecTime )
{
	...
	TempValue = SamplesPerSec ;
	_MUL128_1( ( DWORD * )&TempValue, ( DWORD * )&MilliSecTime, ( DWORD * )Temp ) ;
	TempValue = 1000 ;
	_DIV128_1( ( DWORD * )Temp, ( DWORD * )&TempValue, ( DWORD * )&TempValue ) ;
	return TempValue ;
}
...
static	LONGLONG SamplePositionToMilliSecPosition( int SamplesPerSec, LONGLONG SampleTime )
{
	...
	TempValue = 1000 ;
	_MUL128_1( ( DWORD * )&SampleTime, ( DWORD * )&TempValue, ( DWORD * )Temp ) ;
	TempValue = SamplesPerSec ;
	_DIV128_1( ( DWORD * )Temp, ( DWORD * )&TempValue, ( DWORD * )&TempValue ) ;
	return TempValue ;
}
