...
{
	D_IWbemLocator*           pIWbemLocator = NULL;
	D_IEnumWbemClassObject*   pEnumDevices = NULL;
	D_IWbemClassObject*       pDevices[20] = { NULL };
	D_IWbemServices*          pIWbemServices = NULL;
    ...
    // CoInit if needed
    HRESULT hr = WinAPIData.Win32Func.CoInitializeExFunc( NULL, 0 ) ;
    BOOL bCleanupCOM = SUCCEEDED( hr ) ;

    // So we can call VariantClear() later, even if we never had a successful IWbemClassObject::Get().
    VARIANT var = {};
    WinAPIData.Win32Func.VariantInitFunc(&var);

    // Create WMI
    hr = WinAPIData.Win32Func.CoCreateInstanceFunc( CLSID_WBEMLOCATOR, NULL, CLSCTX_INPROC_SERVER, IID_IWBEMLOCATOR, ( LPVOID * )&pIWbemLocator ) ;
	if( FAILED( hr ) || pIWbemLocator == NULL )
        goto LCleanup ;

    bstrNamespace = WinAPIData.Win32Func.SysAllocStringFunc( L"\\\\.\\root\\cimv2" ) ;
	if( bstrNamespace == NULL )
		goto LCleanup ;

    bstrClassName = WinAPIData.Win32Func.SysAllocStringFunc( L"Win32_PNPEntity" ) ;
	if( bstrClassName == NULL )
		goto LCleanup ;

    bstrDeviceID  = WinAPIData.Win32Func.SysAllocStringFunc( L"DeviceID" ) ;
    ...
    hr = WinAPIData.Win32Func.CoSetProxyBlanketFunc( pIWbemServices, RPC_C_AUTHN_WINNT, RPC_C_AUTHZ_NONE, NULL, RPC_C_AUTHN_LEVEL_CALL, RPC_C_IMP_LEVEL_IMPERSONATE, NULL, D_EOAC_NONE ) ;
    if( FAILED( hr ) )
        goto LCleanup ;
    ...
    {
        ULONG uReturned = 0;
        hr = pEnumDevices->Next( 10000, sizeof( pDevices ) / sizeof( D_IWbemClassObject * ), pDevices, &uReturned ) ;
        if( FAILED( hr ) )
            goto LCleanup ;
        ...
        for( size_t iDevice = 0; iDevice < uReturned; ++iDevice )
        {
            ...
            if( SUCCEEDED( hr ) && var.vt == VT_BSTR && var.bstrVal != NULL )
            {
                ...
                if( _WCSSTR( var.bstrVal, L"IG_" ) )
                {
                    // If it does, then get the VID/PID from var.bstrVal
                    DWORD dwPid = 0, dwVid = 0;

                    const WCHAR* strVid = _WCSSTR( var.bstrVal, L"VID_" ) ;
					if( strVid && CL_sscanf( WCHAR_T_CHARCODEFORMAT, TRUE, CHAR_CHARCODEFORMAT, WCHAR_T_CHARCODEFORMAT, ( char * )strVid, ( char * )L"VID_%4X", &dwVid ) != 1 )
					{
						dwVid = 0 ;
					}

                    const WCHAR* strPid = _WCSSTR( var.bstrVal, L"PID_" ) ;
					if( strPid && CL_sscanf( WCHAR_T_CHARCODEFORMAT, TRUE, CHAR_CHARCODEFORMAT, WCHAR_T_CHARCODEFORMAT, ( char * )strPid, ( char * )L"PID_%4X", &dwPid ) != 1 )
					{
						dwPid = 0;
					}
            ...
            WinAPIData.Win32Func.VariantClearFunc( &var ) ;
...
LCleanup:
	...
	for( size_t iDevice = 0; iDevice < sizeof( pDevices ) / sizeof( D_IWbemClassObject * ) ; ++iDevice )
	{
        if( pDevices[ iDevice ] != NULL )
		{
			pDevices[ iDevice ]->Release() ;
	...
	if( bstrNamespace != NULL )
	{
		WinAPIData.Win32Func.SysFreeStringFunc( bstrNamespace ) ;
		bstrNamespace = NULL ;
	}

	if( bstrClassName != NULL )
	{
		WinAPIData.Win32Func.SysFreeStringFunc( bstrClassName ) ;
		bstrClassName = NULL ;
	}

	if( bstrDeviceID != NULL )
	{
		WinAPIData.Win32Func.SysFreeStringFunc( bstrDeviceID ) ;
		bstrDeviceID = NULL ;
	}
...
// BOOL FAR PASCAL EnumJoypadCheckCallback( const D_DIDEVICEINSTANCEW *pdinst , LPVOID pvRef )
// {
// 	// Instance GUID の異なる物が検出されたら列挙ストップ
...
//	InputSysData.PF.PadInstanceGUIDNum ++ ;
	...
	DXST_LOGFILEFMT_ADDUTF16LE(( "\x44\x00\x69\x00\x72\x00\x65\x00\x63\x00\x74\x00\x49\x00\x6e\x00\x70\x00\x75\x00\x74\x00\x20\x00\xfe\x5b\xdc\x5f\x65\x51\x9b\x52\xc5\x88\x6e\x7f\x92\x30\x1c\x69\xfa\x51\x57\x30\x7e\x30\x57\x30\x5f\x30\x00"/*@ L"DirectInput 対応入力装置を検出しました" @*/ )) ;
	DXST_LOGFILE_TABADD ;

	DXST_LOGFILE_TABADD ;
	DXST_LOGFILEFMT_ADDW(( L"Device Product GUID  : %08x,%04x,%04x,%02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x",
		pdinst->guidProduct.Data1, pdinst->guidProduct.Data2, pdinst->guidProduct.Data3,
		pdinst->guidProduct.Data4[ 0 ], pdinst->guidProduct.Data4[ 1 ],
		pdinst->guidProduct.Data4[ 2 ], pdinst->guidProduct.Data4[ 3 ],
		pdinst->guidProduct.Data4[ 4 ], pdinst->guidProduct.Data4[ 5 ],
		pdinst->guidProduct.Data4[ 6 ], pdinst->guidProduct.Data4[ 7 ] )) ;

	DXST_LOGFILEFMT_ADDW(( L"Device Instance GUID : %08x,%04x,%04x,%02x,%02x,%02x,%02x,%02x,%02x,%02x,%02x",
		pdinst->guidInstance.Data1, pdinst->guidInstance.Data2, pdinst->guidInstance.Data3,
		pdinst->guidInstance.Data4[ 0 ], pdinst->guidInstance.Data4[ 1 ],
		pdinst->guidInstance.Data4[ 2 ], pdinst->guidInstance.Data4[ 3 ],
		pdinst->guidInstance.Data4[ 4 ], pdinst->guidInstance.Data4[ 5 ],
	...
	{
		DXST_LOGFILE_ADDUTF16LE( "\x44\x00\x69\x00\x72\x00\x65\x00\x63\x00\x74\x00\x49\x00\x6e\x00\x70\x00\x75\x00\x74\x00\x44\x00\x65\x00\x76\x00\x69\x00\x63\x00\x65\x00\x38\x00\x20\x00\x6e\x30\x5c\x4f\x10\x62\x2e\x00\x2e\x00\x2e\x00\x20\x00\x00"/*@ L"DirectInputDevice8 の作成... " @*/ ) ;
		hr = InputSysData.PF.DirectInputObject->CreateDevice( pdinst->guidInstance, ( D_IDirectInputDevice ** )&Joystick , NULL ) ;
		if( hr != D_DI_OK )
		{
			DXST_LOGFILEFMT_ADDUTF16LE(( "\x31\x59\x57\x65\x20\x00\x45\x00\x72\x00\x72\x00\x6f\x00\x72\x00\x20\x00\x43\x00\x6f\x00\x64\x00\x65\x00\x20\x00\x3a\x00\x30\x00\x78\x00\x25\x00\x30\x00\x38\x00\x78\x00\x00"/*@ L"失敗 Error Code :0x%08x" @*/, hr )) ;
			DXST_LOGFILE_TABSUB ;
			DXST_LOGFILEFMT_ADDUTF16LE(( ""/*@ L"検出した DirectInput 対応入力装置の初期化を中断します" @*/ )) ;
			return D_DIENUM_CONTINUE ;
		}
	...
	{
		DXST_LOGFILE_ADDUTF16LE( "\x44\x00\x69\x00\x72\x00\x65\x00\x63\x00\x74\x00\x49\x00\x6e\x00\x70\x00\x75\x00\x74\x00\x44\x00\x65\x00\x76\x00\x69\x00\x63\x00\x65\x00\x37\x00\x20\x00\x6e\x30\x5c\x4f\x10\x62\x2e\x00\x2e\x00\x2e\x00\x20\x00\x00"/*@ L"DirectInputDevice7 の作成... " @*/ ) ;
	...
	// 情報表示
	{
		...
		DXST_LOGFILE_ADDUTF16LE( "\x44\x00\x69\x00\x72\x00\x65\x00\x63\x00\x74\x00\x49\x00\x6e\x00\x70\x00\x75\x00\x74\x00\x44\x00\x65\x00\x76\x00\x69\x00\x63\x00\x65\x00\x20\x00\x6e\x30\xc5\x60\x31\x58\x92\x30\xd6\x53\x97\x5f\x57\x30\x7e\x30\x59\x30\x2e\x00\x2e\x00\x2e\x00\x20\x00\x00"/*@ L"DirectInputDevice の情報を取得します... " @*/ ) ;
		hr = Joystick->GetDeviceInfo( &State ) ;
		if( hr != D_DI_OK )
		{
			DXST_LOGFILEFMT_ADDUTF16LE(( "\x31\x59\x57\x65\x20\x00\x45\x00\x72\x00\x72\x00\x6f\x00\x72\x00\x43\x00\x6f\x00\x64\x00\x65\x00\x20\x00\x3a\x00\x25\x00\x30\x00\x38\x00\x78\x00\x00"/*@ L"失敗 ErrorCode :%08x" @*/, hr )) ;
			DXST_LOGFILE_TABSUB ;
			DXST_LOGFILEFMT_ADDUTF16LE(( ""/*@ L"検出した DirectInput 対応入力装置の初期化を中断します" @*/ )) ;
			Joystick->Release() ;
			Joystick = NULL ;
		...
		if( InputSysData.PF.UseXboxControllerDirectInputFlag == FALSE )
		{
			...
			// Xbox360 コントローラ又は XboxOne コントローラか調べる
			DXST_LOGFILE_ADDUTF16LE( ""/*@ L"検出した DirectInput 対応入力装置が Xbox360コントローラーか確認... " @*/ ) ;
			if( _WCSSTR( tszInstanceNameUPR, L"XBOX 360 FOR WINDOWS" ) != NULL )
			{
				DXST_LOGFILE_ADDUTF16LE( "\x00\x4e\xf4\x81\x0a\x00\x00"/*@ L"一致\n" @*/ ) ;
				Flag = TRUE ;
				goto XINPUT_CHECK_END ;
			}
			DXST_LOGFILE_ADDUTF16LE( "\x0d\x4e\x00\x4e\xf4\x81\x0a\x00\x00"/*@ L"不一致\n" @*/ ) ;

			DXST_LOGFILE_ADDUTF16LE( ""/*@ L"検出した DirectInput 対応入力装置が XboxOneコントローラーか確認... " @*/ ) ;
	...
	// ジョイスパッドのデータ形式を設定
	DXST_LOGFILE_ADDUTF16LE( ""/*@ L"DirectInputDevice のデータ形式に JOYSTICK 形式を設定... " @*/ ) ;
...
//		return D_DIENUM_CONTINUE ;
	...
	DXST_LOGFILE_ADDUTF16LE( ""/*@ L"DirectInputDevice から取得する X軸 の値の範囲を設定... " @*/ ) ;
...
//		return D_DIENUM_CONTINUE ;
	...
	DXST_LOGFILE_ADDUTF16LE( ""/*@ L"DirectInputDevice から取得する Y軸 の値の範囲を設定... " @*/ ) ;
	hr = Joystick->SetProperty( D_DIPROP_RANGE , &diprg.diph ) ;
	if( hr != D_DI_OK )
	{
//		Joystick->Release() ;
...
//		return D_DIENUM_CONTINUE ;
	...
	DXST_LOGFILE_ADDUTF16LE( ""/*@ L"DirectInputDevice から取得する Z軸 の値の範囲を設定... " @*/ ) ;
	...
	DXST_LOGFILE_ADDUTF16LE( ""/*@ L"DirectInputDevice から取得する Rx軸 の値の範囲を設定... " @*/ ) ;
	Joystick->SetProperty( D_DIPROP_RANGE , &diprg.diph ) ;
	if( hr != D_DI_OK )
	{
		DXST_LOGFILEFMT_ADDUTF16LE(( "\x31\x59\x57\x65\x20\x00\x45\x00\x72\x00\x72\x00\x6f\x00\x72\x00\x20\x00\x43\x00\x6f\x00\x64\x00\x65\x00\x20\x00\x3a\x00\x30\x00\x78\x00\x25\x00\x30\x00\x38\x00\x78\x00\x00"/*@ L"失敗 Error Code :0x%08x" @*/, hr )) ;
	...
	DXST_LOGFILE_ADDUTF16LE( ""/*@ L"DirectInputDevice から取得する Ry軸 の値の範囲を設定... " @*/ ) ;
	Joystick->SetProperty( D_DIPROP_RANGE , &diprg.diph ) ;
	if( hr != D_DI_OK )
	{
		DXST_LOGFILEFMT_ADDUTF16LE(( "\x31\x59\x57\x65\x20\x00\x45\x00\x72\x00\x72\x00\x6f\x00\x72\x00\x20\x00\x43\x00\x6f\x00\x64\x00\x65\x00\x20\x00\x3a\x00\x30\x00\x78\x00\x25\x00\x30\x00\x38\x00\x78\x00\x00"/*@ L"失敗 Error Code :0x%08x" @*/, hr )) ;
	...
	DXST_LOGFILE_ADDUTF16LE( ""/*@ L"DirectInputDevice から取得する Rz軸 の値の範囲を設定... " @*/ ) ;
	Joystick->SetProperty( D_DIPROP_RANGE , &diprg.diph ) ;
	if( hr != D_DI_OK )
	{
		DXST_LOGFILEFMT_ADDUTF16LE(( "\x31\x59\x57\x65\x20\x00\x45\x00\x72\x00\x72\x00\x6f\x00\x72\x00\x20\x00\x43\x00\x6f\x00\x64\x00\x65\x00\x20\x00\x3a\x00\x30\x00\x78\x00\x25\x00\x30\x00\x38\x00\x78\x00\x00"/*@ L"失敗 Error Code :0x%08x" @*/, hr )) ;
	...
	DXST_LOGFILE_ADDUTF16LE( "\x44\x00\x69\x00\x72\x00\x65\x00\x63\x00\x74\x00\x49\x00\x6e\x00\x70\x00\x75\x00\x74\x00\x44\x00\x65\x00\x76\x00\x69\x00\x63\x00\x65\x00\x20\x00\x6e\x30\xdc\x30\xbf\x30\xf3\x30\x6e\x30\x70\x65\x92\x30\xd6\x53\x97\x5f\x2e\x00\x2e\x00\x2e\x00\x20\x00\x00"/*@ L"DirectInputDevice のボタンの数を取得... " @*/ ) ;
	...
	DXST_LOGFILEFMT_ADDUTF16LE(( "\x10\x62\x9f\x52\x00\x30\xdc\x30\xbf\x30\xf3\x30\x6e\x30\x70\x65\x6f\x30\x20\x00\x25\x00\x64\x00\x20\x00\x0b\x50\x67\x30\x59\x30\x00"/*@ L"成功　ボタンの数は %d 個です" @*/, pad->Buttons )) ;
	...
	DXST_LOGFILE_ADDUTF16LE( ""/*@ L"DirectInputDevice の X軸 のデッドゾーンを設定... " @*/ ) ;
	...
	DXST_LOGFILE_ADDUTF16LE( ""/*@ L"DirectInputDevice の Y軸 のデッドゾーンを設定... " @*/ ) ;
	hr = Joystick->SetProperty( D_DIPROP_DEADZONE , &dipdw.diph ) ;
	if( hr != D_DI_OK )
	{
		DXST_LOGFILEFMT_ADDUTF16LE(( "\x31\x59\x57\x65\x20\x00\x45\x00\x72\x00\x72\x00\x6f\x00\x72\x00\x20\x00\x43\x00\x6f\x00\x64\x00\x65\x00\x20\x00\x3a\x00\x30\x00\x78\x00\x25\x00\x30\x00\x38\x00\x78\x00\x00"/*@ L"失敗 Error Code :0x%08x" @*/, hr )) ;
	...
	DXST_LOGFILE_ADDUTF16LE( ""/*@ L"DirectInputDevice の Z軸 のデッドゾーンを設定... " @*/ ) ;
	...
	DXST_LOGFILE_ADDUTF16LE( ""/*@ L"DirectInputDevice の Rx軸 のデッドゾーンを設定... " @*/ ) ;
	Joystick->SetProperty( D_DIPROP_DEADZONE , &dipdw.diph ) ;
	if( hr != D_DI_OK )
	{
		DXST_LOGFILEFMT_ADDUTF16LE(( "\x31\x59\x57\x65\x20\x00\x45\x00\x72\x00\x72\x00\x6f\x00\x72\x00\x20\x00\x43\x00\x6f\x00\x64\x00\x65\x00\x20\x00\x3a\x00\x30\x00\x78\x00\x25\x00\x30\x00\x38\x00\x78\x00\x00"/*@ L"失敗 Error Code :0x%08x" @*/, hr )) ;
	...
	DXST_LOGFILE_ADDUTF16LE( ""/*@ L"DirectInputDevice の Ry軸 のデッドゾーンを設定... " @*/ ) ;
	Joystick->SetProperty( D_DIPROP_DEADZONE , &dipdw.diph ) ;
	if( hr != D_DI_OK )
	{
		DXST_LOGFILEFMT_ADDUTF16LE(( "\x31\x59\x57\x65\x20\x00\x45\x00\x72\x00\x72\x00\x6f\x00\x72\x00\x20\x00\x43\x00\x6f\x00\x64\x00\x65\x00\x20\x00\x3a\x00\x30\x00\x78\x00\x25\x00\x30\x00\x38\x00\x78\x00\x00"/*@ L"失敗 Error Code :0x%08x" @*/, hr )) ;
	...
	DXST_LOGFILE_ADDUTF16LE( ""/*@ L"DirectInputDevice の Rz軸 のデッドゾーンを設定... " @*/ ) ;
	Joystick->SetProperty( D_DIPROP_DEADZONE , &dipdw.diph ) ;
	if( hr != D_DI_OK )
	{
		DXST_LOGFILEFMT_ADDUTF16LE(( "\x31\x59\x57\x65\x20\x00\x45\x00\x72\x00\x72\x00\x6f\x00\x72\x00\x20\x00\x43\x00\x6f\x00\x64\x00\x65\x00\x20\x00\x3a\x00\x30\x00\x78\x00\x25\x00\x30\x00\x38\x00\x78\x00\x00"/*@ L"失敗 Error Code :0x%08x" @*/, hr )) ;
...
//		pad->EffectPlayStateRight	= FALSE ;
	...
	DXST_LOGFILE_ADDUTF16LE( ""/*@ L"DirectInputDevice の通知処理用の Event を DirectInputDevice に設定... " @*/ ) ;
	...
	// デバイスのアクセス権を取得する
	DXST_LOGFILE_ADDUTF16LE(  "\x44\x00\x69\x00\x72\x00\x65\x00\x63\x00\x74\x00\x49\x00\x6e\x00\x70\x00\x75\x00\x74\x00\x44\x00\x65\x00\x76\x00\x69\x00\x63\x00\x65\x00\x20\x00\x6e\x30\xa2\x30\xaf\x30\xbb\x30\xb9\x30\x29\x6a\x92\x30\xd6\x53\x97\x5f\x2e\x00\x2e\x00\x2e\x00\x20\x00\x00"/*@ L"DirectInputDevice のアクセス権を取得... " @*/ ) ;
	hr = Joystick->Acquire() ;
	if( hr != D_DI_OK )
	{
		DXST_LOGFILEFMT_ADDUTF16LE(( "\x31\x59\x57\x65\x20\x00\x45\x00\x72\x00\x72\x00\x6f\x00\x72\x00\x20\x00\x43\x00\x6f\x00\x64\x00\x65\x00\x20\x00\x3a\x00\x30\x00\x78\x00\x25\x00\x30\x00\x38\x00\x78\x00\x00"/*@ L"失敗 Error Code :0x%08x" @*/, hr )) ;
		...
		DXST_LOGFILE_ADDUTF16LE(  ""/*@ L"DirectInputDevice の協調レベルを DISCL_BACKGROUND | DISCL_NONEXCLUSIVE に変更... " @*/ ) ;
		hr = Joystick->SetCooperativeLevel( NS_GetMainWindowHandle(), D_DISCL_BACKGROUND | D_DISCL_NONEXCLUSIVE ) ;
		if( hr != D_DI_OK )
		{
			DXST_LOGFILEFMT_ADDUTF16LE(( "\x31\x59\x57\x65\x20\x00\x45\x00\x72\x00\x72\x00\x6f\x00\x72\x00\x20\x00\x43\x00\x6f\x00\x64\x00\x65\x00\x20\x00\x3a\x00\x30\x00\x78\x00\x25\x00\x30\x00\x38\x00\x78\x00\x00"/*@ L"失敗 Error Code :0x%08x" @*/, hr )) ;
	...
	DXST_LOGFILEFMT_ADDUTF16LE(( ""/*@ L"検出した DirectInput 対応入力装置を入力ナンバー %d で登録します" @*/, InputSysData.PadNum )) ;
...
extern int InitializeInputSystem_PF_Timing0( void )
{
	...
	int    UseXboxControllerDirectInputFlag				= InputSysData.PF.UseXboxControllerDirectInputFlag ;
	int    DirectInputMouseMode							= InputSysData.PF.DirectInputMouseMode ;
	...
	InputSysData.PF.UseXboxControllerDirectInputFlag	= UseXboxControllerDirectInputFlag ;
	InputSysData.PF.DirectInputMouseMode				= DirectInputMouseMode ;
...
KEYBOARDDEVICEINITEND:
	...
	else
	{
		...
		for( i = 0 ; i < MAX_JOYPAD_NUM ; i ++ )
		{
			...
			_MEMSET( &InputSysData.Pad[ InputSysData.PadNum ], 0, sizeof( InputSysData.Pad[ InputSysData.PadNum ] ) ) ;
			InputSysData.Pad[ InputSysData.PadNum ].PF.XInputDeviceNo = -1 ;
			InputSysData.Pad[ InputSysData.PadNum ].PF.RightStickFlag = ( joycaps.wCaps & JOYCAPS_HASZ ) && ( joycaps.wCaps & JOYCAPS_HASR ) ;
			InputSysData.Pad[ InputSysData.PadNum ].PF.MultimediaAPICaps = joycaps.wCaps ;
			InputSysData.Pad[ InputSysData.PadNum ].DeadZone = InputSysData.PadDefaultDeadZone ;
			InputSysData.Pad[ InputSysData.PadNum ].DeadZoneD = InputSysData.PadDefaultDeadZoneD ;
			InputSysData.Pad[ InputSysData.PadNum ].Buttons = joycaps.wNumButtons ;
			InputSysData.PadNum ++ ;
		}
...
extern int SetupJoypad_PF( void )
{
	...
	DXST_LOGFILEFMT_ADDUTF16LE(( "\x58\x00\x49\x00\x6e\x00\x70\x00\x75\x00\x74\x00\x20\x00\xfe\x5b\xdc\x5f\xb3\x30\xf3\x30\xc8\x30\xed\x30\xfc\x30\xe9\x30\xfc\x30\x6e\x30\xc1\x30\xa7\x30\xc3\x30\xaf\x30\x92\x30\x8b\x95\xcb\x59\x57\x30\x7e\x30\x59\x30\x00"/*@ L"XInput 対応コントローラーのチェックを開始します" @*/ )) ;
	DXST_LOGFILE_TABADD ;
...
//			pad->EffectPlayStateRight	= FALSE ;
			...
			DXST_LOGFILEFMT_ADDUTF16LE(( ""/*@ L"XInput 対応コントローラー No.%d を入力ナンバー %d で登録します" @*/, i, InputSysData.PadNum )) ;

			pad ++ ;
			InputSysData.PadNum ++ ;
		}
		InputSysData.PF.XInputPadNum = InputSysData.PadNum ;
	}

	DXST_LOGFILE_TABSUB ;
	DXST_LOGFILEFMT_ADDUTF16LE(( "\x58\x00\x49\x00\x6e\x00\x70\x00\x75\x00\x74\x00\x20\x00\xfe\x5b\xdc\x5f\xb3\x30\xf3\x30\xc8\x30\xed\x30\xfc\x30\xe9\x30\xfc\x30\x6e\x30\xc1\x30\xa7\x30\xc3\x30\xaf\x30\x8c\x5b\x86\x4e\x00"/*@ L"XInput 対応コントローラーのチェック完了" @*/ )) ;

	// ＤｉｒｅｃｔＩｎｐｕｔ を使用する場合は ＤｉｒｅｃｔＩｎｐｕｔ のジョイパッドも検出
	if( InputSysData.PF.NoUseDirectInputFlag == FALSE )
	{
		DXST_LOGFILEFMT_ADDUTF16LE(( "\x44\x00\x69\x00\x72\x00\x65\x00\x63\x00\x74\x00\x49\x00\x6e\x00\x70\x00\x75\x00\x74\x00\x20\x00\xfe\x5b\xdc\x5f\xb8\x30\xe7\x30\xa4\x30\xd1\x30\xc3\x30\xc9\x30\x6e\x30\x17\x52\x19\x63\x92\x30\x8b\x95\xcb\x59\x57\x30\x7e\x30\x59\x30\x00"/*@ L"DirectInput 対応ジョイパッドの列挙を開始します" @*/ )) ;
		DXST_LOGFILE_TABADD ;
	...
	//	InputSysData.PF.PadInstanceGUIDNum = 0 ;
		...
		DXST_LOGFILE_TABSUB ;
		DXST_LOGFILEFMT_ADDUTF16LE(( "\x44\x00\x69\x00\x72\x00\x65\x00\x63\x00\x74\x00\x49\x00\x6e\x00\x70\x00\x75\x00\x74\x00\x20\x00\xfe\x5b\xdc\x5f\xb8\x30\xe7\x30\xa4\x30\xd1\x30\xc3\x30\xc9\x30\x6e\x30\x17\x52\x19\x63\x42\x7d\x86\x4e\x00"/*@ L"DirectInput 対応ジョイパッドの列挙終了" @*/ )) ;
		...
		if( pad->PF.Device != NULL )
		{
			...
			for( j = 0 ; j < DINPUTPAD_MOTOR_NUM ; j ++ )
			{
				if( pad->Effect[ j ].PF.DIEffect != NULL )
				{
					...
					pad->Effect[ j ].PF.DIEffect->Release() ;
					pad->Effect[ j ].PF.DIEffect = NULL ;
				}
			}
...
extern int RefreshEffectPlayState_PF( void )
{
	...
	int                 AlwaysRunFlag = NS_GetAlwaysRunFlag() ;
	...
	for( i = 0 ; i < num ; i ++, Pad ++ )
	{
		...
		if( Pad->PF.XInputDeviceNo >= 0 )
		{
			...
			// ソフトが非アクティブでも動作する設定になっているか、ソフトのアクティブ状態の場合は振動を行う
			if( NS_GetActiveFlag() == TRUE || AlwaysRunFlag )
			{
				...
				for( j = 0 ; j < DINPUTPAD_MOTOR_NUM ; j ++ )
				{
					...
					if( Pad->Effect[ j ].SetPower != SetPower ||
						time - Pad->Effect[ 0 ].PrevSetTime > 1000 )
					{
						Flag = 1 ;
				...
				if( InputSysData.PF.XInputSetStateFunc != NULL && ( Flag == 1 || AlwaysRunFlag ) )
				{
					InputSysData.PF.XInputSetStateFunc( ( DWORD )Pad->PF.XInputDeviceNo, &VibState ) ;
					Pad->Effect[ 0 ].PrevSetTime = time ;
				}
			}
			else
			{
			...
			if( NS_GetActiveFlag() == TRUE )
			{
				for( j = 0 ; j < DINPUTPAD_MOTOR_NUM ; j ++ )
				{
					...
					if( Pad->Effect[ j ].SetPower  != SetPower ||
						...
						time - Pad->Effect[ j ].PrevSetTime > 1000 )
					{
...
extern int GetJoypadType_PF( int InputType )
{
	...
	int PadIndex = ( InputType & ~DX_INPUT_KEY ) - 1 ;

	if( InputSysData.PadNum <= PadIndex )
		return -1 ;

	if( InputSysData.Pad[ PadIndex ].PF.XInputDeviceNo >= 0 )
		return DX_PADTYPE_XBOX_ONE ;

	if( InputSysData.Pad[ PadIndex ].PF.Device == NULL )
		return -1 ;

	_MEMSET( &inst, 0, sizeof( inst ) ) ;
	inst.dwSize = sizeof( inst ) ;
	InputSysData.Pad[ PadIndex ].PF.Device->GetDeviceInfo( &inst ) ;
	if( _MEMCMP( &inst.guidProduct, &GUID_XBOX_360_CONTROLLER, sizeof( GUID ) ) == 0 )
	{
		return DX_PADTYPE_XBOX_360 ;
	}
	if( _MEMCMP( &inst.guidProduct, &GUID_XBOX_ONE_CONTROLLER, sizeof( GUID ) ) == 0 )
	{
		return DX_PADTYPE_XBOX_ONE ;
	}
	if( _MEMCMP( &inst.guidProduct, &GUID_DUALSHOCK3, sizeof( GUID ) ) == 0 )
	{
		return DX_PADTYPE_DUAL_SHOCK_3 ;
	}
	if( _MEMCMP( &inst.guidProduct, &GUID_DUALSHOCK4, sizeof( GUID ) ) == 0 )
	{
		return DX_PADTYPE_DUAL_SHOCK_4 ;
	}
	if( _MEMCMP( &inst.guidProduct, &GUID_DUALSHOCK4_2, sizeof( GUID ) ) == 0 )
	{
		return DX_PADTYPE_DUAL_SHOCK_4 ;
	}
	if( _MEMCMP( &inst.guidProduct, &GUID_DUALSENSE, sizeof( GUID ) ) == 0 )
	{
		return DX_PADTYPE_DUAL_SENSE ;
	}
	if( _MEMCMP( &inst.guidProduct, &GUID_SWITCH_JOY_CON_L, sizeof( GUID ) ) == 0 )
	{
		return DX_PADTYPE_SWITCH_JOY_CON_L ;
	}
	if( _MEMCMP( &inst.guidProduct, &GUID_SWITCH_JOY_CON_R, sizeof( GUID ) ) == 0 )
	{
		return DX_PADTYPE_SWITCH_JOY_CON_R ;
	}
	if( _MEMCMP( &inst.guidProduct, &GUID_SWITCH_PRO_CONTROLLER, sizeof( GUID ) ) == 0 )
	{
