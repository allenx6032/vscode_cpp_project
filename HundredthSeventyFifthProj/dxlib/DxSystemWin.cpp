...
//			DXST_LOGFILEFMT_ADDUTF16LE(( L"%dx%d %dbit color", Mode.Width, Mode.Height, Mode.ColorBitDepth ) ) ;
		...
		if( i == Num )
		{
			...
			{
				// ソフトウェアレンダリングモードの場合はチェックする対象のみを 32bitにする
...
//						( hr2 = ( PeekMessageWFunc( &msg2, NULL, 0, 0, PM_NOREMOVE ) != 0 && msg2.message == WM_SYSCOMMAND && msg2.wParam == SC_SCREENSAVE ) ) ||
						( hr2 = ( PeekMessageWFunc( &msg2,       NULL, 0, 0, PM_NOREMOVE ) != 0 /*&& msg2.message == WM_SYSCOMMAND && msg2.wParam == SC_SCREENSAVE*/ ) ) != 0 || // ←ATOKの場合スクリーンセーバーメッセージに限定すると半角／全角キーで日本語入力モードに移行できなかったので、無条件に。何か問題があったらそのとき考えよう・・・
...
extern ULONGLONG NS_GetNowSysPerformanceCount( void )
{
	...
	else
	{
		...
		// パフォーマンスカウンタが使用できない場合はメディアタイマーの値を返す
