...
namespace NE {
	...
	void CreateBitmap(BitmapFile& file, int w, int h, int colorBitCount, uint8_t* pixels, uint32_t* pals = nullptr) {
		...
		file.infoHeader.biBitCount = (uint16_t)colorBitCount;
	...
	uint8_t MixAlpha(uint8_t color, uint8_t alpha)
	{
		...
		return (uint8_t)(res > 0xff ?(res % 0xff) : res);
	}
	...
	bool WDF::LoadSpriteData(Sprite* sprite, std::vector<PalSchemePart>* patMatrix)
	{
		...
		// if (mIdToPos.count(id) == 0) return nullprt;
		if (mIdToPos.count(id) == 0) return false;

		// if (m_SpritesLoading[id]) return nullprt;
		if (m_SpritesLoading[id]) return false;
