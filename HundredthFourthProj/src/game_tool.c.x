...
// 定义split函数，按指定分隔符分割字符串
static char** split(const char *str, const char *delimiter, int *count) {
    // 计算字符串中子字符串的数量
    int num_tokens = 0;
    const char *temp = str;
    ...
    while (1) {
        ...
        if (end == NULL) {
            // 如果没有找到分隔符，处理剩余部分
            result[index] = (char *)malloc(strlen(start) + 1);
        ...
        strncpy(result[index], start, end - start);
						...
						int n_files )
{
	...
	snprintf(dir_name, 24, "res-%s", data_file);
	/* 生成所需命令行，以创建文件目录 */
	...
	for(i=0; i<n_files; ++i) {
		...
		sprintf( filepath, "%s/%s", dir_name, list[i].filepath );
		/* 写入至输出文件 */
...
int main(int argc, char** argv)
{
	...
	if( argc >= 2 ) {
		...
		ResourceTool_UnPackDataFile(
			"main.data", MAIN_RES, main_file_info, MAIN_RES_NUM
		);
		ResourceTool_UnPackDataFile(
