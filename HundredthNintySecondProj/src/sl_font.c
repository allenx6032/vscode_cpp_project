...
#endif
#include "sl_drawtext.h"
#include "sl_drawtext_impl.h"
...
void dtx_use_interpolation(int interpolation)
{
	dtx_interpolation = interpolation;
	//gmap->tex = 0;
...
{
	struct dtx_glyphmap *gmap = dtx_get_font_glyphmap(dtx_font, dtx_font_sz, *str);
	...
	while(*str) {
		...
		// get the next character in the string and then advance to the next string character
		code = *str;//dtx_utf8_char_code(str);
