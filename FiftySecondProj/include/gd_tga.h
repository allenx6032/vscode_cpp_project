...
{
	obyte  identsize;	// size of ID field that follows 18 byte header (0 usually)
	obyte  colormaptype;	// type of colour map 0=none, 1=has palette					[IGNORED] Adrian requested no support
	obyte  imagetype;	// type of image 0=none,1=indexed,2=rgb,3=grey,+8=rle packed
	...
	obyte  colormapbits;	// number of bits per palette entry 15,16,24,32				[IGNORED] Adrian requested no support

	int xstart;			// image x origin
	int ystart;			// image y origin
	int width;			// image width in pixels
	int height;			// image height in pixels
	obyte  bits;			// image bits per pixel 8,16,24,32
	obyte alphabits;		// alpha bits (low 4bits of header 17)
	obyte fliph;			// horizontal or vertical
	obyte flipv;			// flip
