	...
	void onDownMessage(TcpServer* server, int32_t /*index*/, TcpConnectionPtr conn)
	{
		...
		if (pipe->m_downState == kWaitHandshaking) {
			...
			case RELAY_FORWARD:
			{
				...
				if (m_enable_statistics) {
					m_up_total += forwardMsg.size;
					m_up_statistics.push(forwardMsg.size);
				}
...
private:
	...
	void onServerMessage(TcpClientPtr client, TcpConnectionPtr conn)
	{
		...
		while (!ringBuf.empty()) {
			...
			Packet packet;
			packet.build_from_memory(RELAY_PACKET_HEADSIZE, RelayForwardMsg::ID, (uint16_t)(sizeof(RelayForwardMsg) + buf_round_size), nullptr);
...
public:
	RelayServer(bool encryptMode, bool enableStatistics)
