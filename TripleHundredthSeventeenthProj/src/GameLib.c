...
void GameLib_ProcLoop(void *data) {
	...
	do {
		...
		for (i = 0; i < g_NumEntities; i++) {
			...
			if (!(ent->flags & EntityFlag_Collision) || ent->body->mass < 0.0f) {
				continue;
			}
			if (ent->body->vel[0] <= 0.0f && ent->body->vel[0] >= -0.0f && ent->body->vel[1] <= 0.0f && ent->body->vel[1] >= -0.0f) {
				continue;
			}
			TBucket bucket;
		...
		for (i = 0; i < g_NumEntities; i++) {
			...
			for (j = 0; j < bucket.count; j++) {
				...
				if (Entity_CheckCollision(ent, entCollision, NULL)) {
					vec2_set(ent->body->vel, 0, 0);
					Entity_CalcBBox(ent);
					vec2_set(entCollision->body->vel, 0, 0);
	...
	for (i = 0; i < g_NumEntities; i++) {
		Entity ent = g_Entity[i];
		if (!(ent->flags & EntityFlag_Overlap) || ent->body->mass < 0.0f) {
			continue;
		}
		TBucket bucket;
		BucketGrid_Query(&g_BucketGrid, &ent->bbox, &bucket);
	...
	do {
		...
		for (i = 1; i < n; i++) {
			...
			if (ent1->zorder > ent2->zorder) {
				...
				// Same level
				const float y1 = ent1->body->pos[1] + ent1->sortYOffset;
				const float y2 = ent2->body->pos[1] + ent2->sortYOffset;
				if (y1 == y2) {
					if (ent1->body->pos[0] < ent2->body->pos[0]) {
						swap = 1;
					}
				} else if (y1 > y2) {
					swap = 1;
...
int GameLib_EntityCustomCheckCollision(Entity ent, vec2 vel) {
	...
	vec2_copy(originalVel, ent->body->vel);
	vec2_copy(ent->body->vel, vel);
	Entity_CalcBBox(ent);
	...
	vec2_copy(ent->body->vel, originalVel);
	Entity_CalcBBox(ent);
