    ...
    for i=1, WorldWidth do
        for j=1, WorldHeight do
            ...
            if party then
                ...
                rnd:copyTo(font:renderText(rnd, content, table.unpack(color)), (i-1)*50, (j-1)*50)
            end
        end
    end
...
local function drawHint()
    ...
    if hint then
        pcall(function()
            local t = font:renderText(rnd, hint, 255, 255, 255, 0)
    ...
    if playerParty == 1 then
        local t = font:renderText(rnd, "玩家阵营: Red", 255, 0, 0, 0)
        rnd:copyTo(t, 0, nexty)
        local w, h = t:getSize()
        nexty = nexty + h
    elseif playerParty == 2 then
        local t = font:renderText(rnd, "玩家阵营: Blue", 0, 0, 255, 0)
        rnd:copyTo(t, 0, nexty)
        local w, h = t:getSize()
        nexty = nexty + h
    end
    ...
    if clickBlocked then
        local t = font:renderText(rnd, "现在是对手的回合", 255, 255, 255, 0)
        rnd:copyTo(t, 0, nexty)
        local w, h = t:getSize()
        nexty = nexty + h
    else
        local t = font:renderText(rnd, "现在是你的回合", 255, 255, 0, 0)
        rnd:copyTo(t, 0, nexty)
        local w, h = t:getSize()
        nexty = nexty + h
    end
    ...
    local t = font:renderText(rnd, string.format("红方: %d 蓝方: %d", cred, cblue), 255, 255, 255, 0)
    rnd:copyTo(t, 0, nexty)
end
...
local function server_game_main(client)
    rnd:clear()
    rnd:copyTo(font:renderText(rnd, "正在同步地图到客户端...", 255, 255, 255, 0), 0, 0)
    rnd:update()
    local tdata = {}
...
local function server_main()
    rnd:clear()
    rnd:copyTo(font:renderText(rnd, "正在创建服务器...", 255, 255, 255, 0), 0, 0)
    rnd:update()
    ...
    rnd:clear()
    rnd:copyTo(font:renderText(rnd, "等待客户端连接...", 255, 255, 255, 0), 0, 0)
    rnd:update()
...
local function client_main()
    rnd:clear()
    rnd:copyTo(font:renderText(rnd, "连接到服务器...", 255, 255, 255, 0), 0, 0)
    rnd:update()
...
while true do
    rnd:clear()
    local t1 = font:renderText(rnd, "按下 S 键开启一场对局", 255, 255, 255, 0)
    local t2 = font:renderText(rnd, "按下 C 键加入一场对局", 255, 255, 255, 0)
    rnd:copyTo(t1, 0, 0)
