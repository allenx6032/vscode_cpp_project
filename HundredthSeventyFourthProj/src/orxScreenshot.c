...
static orxINLINE orxSTATUS orxScreenshot_ComputeIndex()
{
  ...
  if(orxFile_Exists(zDirectory) != orxFALSE)
  {
    do
    {
      ...
      orxString_NPrint(sstScreenshot.acScreenshotBuffer, sizeof(sstScreenshot.acScreenshotBuffer), "%s/%s%0*u.%s", zDirectory, zBaseName, u32Digits, sstScreenshot.u32ScreenshotIndex + 1, zExtension);
