...
#include "modplug_stdafx.h"
#include "modplug_sndfile.h"
#ifndef PATH_MAX
#define PATH_MAX 256
#endif
	...
	if( pat_readpat_attr(gm-1, &hw, 0) ) {
		...
		if( p ) {
			if( hw.modes & PAT_16BIT ) {
				...
				cs->ReadSample(q, (hw.modes&PAT_UNSIGNED)?RS_PCM16U:RS_PCM16S, (LPWSTR)p, hw.wave_size);
			}
			else {
				dec_pat_Decompress8Bit((short int *)p, hw.wave_size, gm - 1);
				cs->ReadSample(q, (hw.modes&PAT_UNSIGNED)?RS_PCM16U:RS_PCM16S, (LPWSTR)p, hw.wave_size * sizeof(short int));
			}
			free(p);
		}
	}
		...
		if( p ) {
			...
			cs->ReadSample(q, RS_PCM16S, (LPWSTR)p, q->nLength*2);
			free(p);
		}
	}
}
...
BOOL CSoundFile::ReadPAT(const BYTE *lpStream, DWORD dwMemLength)
{
	...
	for(t=1; t<(int)m_nSamples; t++) { // xmms modplug doesn't use slot zero
		...
		if( p ) {
			...
			if( hw.modes & PAT_16BIT ) {
				ReadSample(q, (hw.modes&PAT_UNSIGNED)?RS_PCM16U:RS_PCM16S, (LPWSTR)p, hw.wave_size);
			}
			else {
				pat_blowup_to16bit((short int *)p, hw.wave_size);
				ReadSample(q, (hw.modes&PAT_UNSIGNED)?RS_PCM16U:RS_PCM16S, (LPWSTR)p, hw.wave_size * sizeof(short int));
			}
			free(p);
		}
	}
