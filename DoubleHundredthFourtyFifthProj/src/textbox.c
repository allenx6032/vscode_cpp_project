...
void textbox_init_background(DARNIT_TILEMAP *tc, int w, int h) {
	...
	wt = w / my_ws.camera.tile_w;
	ht = h / my_ws.camera.tile_h;
...
void textbox_init(int w, int h, int x, int y, int pad_x, int pad_y, int pad_x2, int pad_y2) {
	...
	tb->face_ts = NULL;
	w -= (w % my_ws.camera.tile_w);
	x += (w % my_ws.camera.tile_w) / 2;
	h -= (h % my_ws.camera.tile_h);
	y += (h % my_ws.camera.tile_h);

	tb->surface_w = (w - pad_x - pad_x2);
	tb->text = d_text_surface_color_new(my_ws.font, 2048, tb->surface_w, x + pad_x, y + pad_y);
	tb->rows = (h - pad_y - pad_y2) / d_font_glyph_hs(my_ws.font);
	tb->tc = d_tilemap_new(0xFF, my_ws.camera.sys, 0xFFF, w / my_ws.camera.tile_w, h / my_ws.camera.tile_h);
...
void textbox_update_pointers(struct textbox *tb) {
	...
	y = d_font_glyph_hs(my_ws.font);
	y *= tb->selection;
	y += tb->y_selection;
	y -= my_ws.camera.tile_h / 2;
	y += (d_font_glyph_hs(my_ws.font) / 2);

	d_render_tile_move(tb->pointer, 0, my_ws.camera.screen_w - my_ws.camera.tile_w  * tb->qt->w, y);
	d_render_tile_move(tb->pointer, 1, my_ws.camera.screen_w - my_ws.camera.tile_w, y);
	...
	for (; tb->dt > tb->ms_per_char && tb->message[tb->char_pos]; tb->dt -= tb->ms_per_char) {
		...
		if (tb->message[tb->char_pos]) {
			if (tb->message[tb->char_pos] == ' ') {
				...
				if (d_text_surface_pos(tb->text) + d_font_word_w(my_ws.font, 
...
void textbox_add_message(const char *message, const char *question, int face, int pingback) {
	struct textbox *tb = my_ws.textbox;
	struct char_gfx *cg;
	struct aicomm_struct ac;
	...
	if (!question)
		question = "";
	w = 0;
	h = d_font_string_geometrics_o(my_ws.font, question, my_ws.camera.screen_w / 2, &w);
	w += my_ws.camera.tile_w * 2 + 5;
	h += 16;
	wt = w / my_ws.camera.tile_w;
	if (w % my_ws.camera.tile_w)
		wt++;
	ht = h / my_ws.camera.tile_h;
	if (h % my_ws.camera.tile_h)
		ht++;

	tb->pointer = d_render_tile_new(2, my_ws.camera.sys);
	d_render_tile_set(tb->pointer, 0, 12);
	d_render_tile_set(tb->pointer, 1, 13);

	tb->qt = d_tilemap_new(0xFFF, my_ws.camera.sys, 0xFFF, wt, ht);
	textbox_init_background(tb->qt, wt * my_ws.camera.tile_w, ht * my_ws.camera.tile_h);
	d_tilemap_camera_move(tb->qt, -(my_ws.camera.screen_w - wt * my_ws.camera.tile_w), -(my_ws.camera.screen_h - (ht + tb->tc->h) * my_ws.camera.tile_h));

	x = y = 0;
	x = my_ws.camera.screen_w - my_ws.camera.tile_w * wt;
	x += (wt * my_ws.camera.tile_w - w) / 2 + my_ws.camera.tile_w;
	y = my_ws.camera.screen_h - ht * my_ws.camera.tile_h;
	y -= (tb->tc->h * my_ws.camera.tile_h);
	y += (my_ws.camera.tile_h * ht - h) / 2;
	tb->qts = d_text_surface_new(my_ws.font, strlen(question), w - 2 * my_ws.camera.tile_w, x, y);
