...
#ifdef _GLFW_DMITRI_WINDOWS_TOUCH_ENABLED		// Dmitri: Touch
		case WM_TOUCH:
		{
			UINT cInputs, Input;
			PTOUCHINPUT pInputs;
			...
			pInputs = (TOUCHINPUT *)malloc(sizeof(TOUCHINPUT) * cInputs);
			if (NULL != pInputs)
			{
				if (GetTouchInputInfo((HTOUCHINPUT)lParam, cInputs, pInputs, sizeof(TOUCHINPUT)))
				{
					// process pInputs
					//printf("Got %d touch points.\n", cInputs);

					for (Input = 0; Input < cInputs; ++Input)
					{
						if (pInputs[Input].dwFlags & TOUCHEVENTF_PALM)
						{
							printf(" Palm %d touch event.\n", Input);
							continue;
						}

						ptInput.x = TOUCH_COORD_TO_PIXEL(pInputs[Input].x);
						ptInput.y = TOUCH_COORD_TO_PIXEL(pInputs[Input].y);
						ScreenToClient(hWnd, &ptInput);

						if (TOUCHEVENTF_PRIMARY != (pInputs[Input].dwFlags & TOUCHEVENTF_PRIMARY))
						{
							//printf(" Touch event #%d flags %d: %d, %d.\n", Input, pInputs[Input].dwFlags, ptInput.x, ptInput.y);
							//printf(" Touch event #%d ID %d.\n", Input, pInputs[Input].dwID);

							if (TOUCHEVENTF_DOWN == (pInputs[Input].dwFlags & TOUCHEVENTF_DOWN))
							{
								if( _glfwWin.touchCallback )
								{
									//_glfwWin.touchButtonCallback( pInputs[Input].dwID, GLFW_PRESS );
									_glfwWin.touchCallback( pInputs[Input].dwID, 0, ptInput.x, ptInput.y );
								}
							}
							else if (TOUCHEVENTF_UP == (pInputs[Input].dwFlags & TOUCHEVENTF_UP))
							{
								if( _glfwWin.touchCallback )
								{
									//_glfwWin.touchButtonCallback( pInputs[Input].dwID, GLFW_RELEASE );
									_glfwWin.touchCallback( pInputs[Input].dwID, 2, ptInput.x, ptInput.y );
								}
							}
							else //if (TOUCHEVENTF_MOVE == (pInputs[Input].dwFlags & TOUCHEVENTF_MOVE))
							{
								if( _glfwWin.touchCallback )
								{
									//_glfwWin.touchPosCallback( ptInput.x, ptInput.y, (TOUCHEVENTF_DOWN == (pInputs[Input].dwFlags & TOUCHEVENTF_DOWN)) ? 1 : 0 );
									_glfwWin.touchCallback( pInputs[Input].dwID, 1, ptInput.x, ptInput.y );
								}
							}
...
#ifdef _GLFW_DMITRI_WINDOWS_TOUCH_ENABLED		// Dmitri: Touch
	RegisterTouchWindow(_glfwWin.window, 0);
	/*{BOOL Result = RegisterPointerInputTarget(_glfwWin.window, PT_TOUCH);
	printf("RegisterPointerInputTarget %d\n", Result);}*/
...
{
#ifdef _GLFW_DMITRI_WINDOWS_TOUCH_ENABLED		// Dmitri: Touch
	UnregisterTouchWindow(_glfwWin.window);
	//UnregisterPointerInputTarget(_glfwWin.window, PT_TOUCH);
#endif // _GLFW_DMITRI_WINDOWS_TOUCH_ENABLED
