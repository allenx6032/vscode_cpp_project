...
void savedata_load(const char *fname) {
	...
	d_file_read_ints(&sh, sizeof(sh) / 4, f);
	if (sh.ints > my_ws.savedata.is || sh.bytes > my_ws.savedata.bs) {
	...
	d_util_decompress(data, sh.intz, (void **) &my_ws.savedata.i);
	d_util_endian_convert(my_ws.savedata.i, my_ws.savedata.is);

	data = realloc(data, sh.bytez);
	d_file_read(data, sh.bytez, f);
	d_util_decompress(data, sh.bytez, (void **) &my_ws.savedata.b);

	my_ws.active_world = sh.map;
	my_ws.state = WORLD_STATE_MAPSTATE;
...
void savedata_save(const char *file) {
	...
	sh.ints = my_ws.savedata.is;
	sh.bytes = my_ws.savedata.bs;
	sh.info = 0;
	sh.infoz = 0;
	sh.map = my_ws.active_world;

	d_util_endian_convert(my_ws.savedata.i, my_ws.savedata.is);

	if (my_ws.savedata.is) {
		sh.intz = d_util_compress(my_ws.savedata.i, my_ws.savedata.is * 4, &intz);
	} else
		sh.intz = 0;
	if (my_ws.savedata.bs) {
		sh.bytez = d_util_compress(my_ws.savedata.b, my_ws.savedata.bs, &bytez);
	} else
		sh.bytez = 0;
