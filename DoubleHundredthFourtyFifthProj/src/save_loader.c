...
void dungeon_init() {
	my_ws.dm = calloc(sizeof(*my_ws.dm), 1);
...
void dungeon_load(int ns) {
	...
	DARNIT_FILE *f;
	off_t size;
	struct dungeon_map *dm = my_ws.dm;
	...
	if (n != 4) {
		if (n > 4)
			n -= 1;
		if (my_ws.dm->grid[4].neighbours[n] == -1)
			return;
	} else
		n = my_ws.active_world;
...
void dungeon_do_shift(int op[24]) {
	...
	for (i = 0; i < 6; i++) {
		my_ws.dm->grid[op[4 + (i << 1)]] = my_ws.dm->grid[op[3 + (i << 1)]];
		my_ws.dm->grid[op[3 + (1 << 1)]].layers = 0;
		my_ws.dm->grid[op[3 + (1 << 1)]].layer = NULL;
	}
	for (i = 0; i < 3; i++)
