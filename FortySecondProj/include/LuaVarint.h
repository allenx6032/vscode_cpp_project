...
class CLuaVariant
{
	typedef std::variant<int,char,short,long,__int64,unsigned int,unsigned char,unsigned short,unsigned long,unsigned __int64,float,double,bool,std::wstring,ILuaElementCollection*,ILuaApi*,LuaVarArray,std::vector<char>> var_t;
public:
	enum VAR_ENUM
	{
		VAR_EMPTY = -1,
	...
	CLuaVariant(const CLuaVariant& other)
	{
		...
		if(!other.IsEmpty())
		{
			...
			if(vType == VAR_ILUAEC)
			{
				ILuaElementCollection* pColl = std::get<ILuaElementCollection*>(m_var);
				if(pColl)
					pColl->AddRef();
			}
		}
	...
	std::wstring getValue() const
	{
		if(!m_bEmpty && m_var.type() == typeid(std::wstring))
			return std::get<std::wstring>(m_var);
		else
		{
			assert(0);
		}
