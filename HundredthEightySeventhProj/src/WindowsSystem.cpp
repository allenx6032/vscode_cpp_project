...
HWND WindowsSystem::Initialize(const DGL_SysInitInfo* pSysInitInfo)
{
    ...
    wcex.lpszClassName = (LPCWSTR)cWindowClassName;
    ...
    mWindowHandle = CreateWindowExW(
        (DWORD)0, 
        (LPCWSTR)cWindowClassName,
        (LPCWSTR)pSysInitInfo->mWindowTitle,
        (DWORD)pSysInitInfo->mWindowStyle,
        (int)CW_USEDEFAULT, 
        (int)CW_USEDEFAULT,
        (int)pSysInitInfo->mWindowWidth,
        (int)pSysInitInfo->mWindowHeight,
        NULL,
        NULL,
        (HINSTANCE )pSysInitInfo->mAppInstance,
        NULL
    );
...
void WindowsSystem::ShutDown()
{
    ...
    UnregisterClass((LPCTSTR )cWindowClassName, (HINSTANCE )mInstance);
}
