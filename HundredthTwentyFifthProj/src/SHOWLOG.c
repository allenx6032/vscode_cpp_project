...
** calcNumObjects
**
** Calculate the number of object names contained in the OBJECT file
**************************************************************************/
int calcNumObjects()
{
   FILE *objFile;
   byte numObjHi, numObjLo;
   ...
   numObjLo = fgetc(objFile) ^ AVIS_DURGAN[0];
   numObjHi = fgetc(objFile) ^ AVIS_DURGAN[1];

   fclose(objFile);

   return ((numObjHi * 256 + numObjLo) / 3);
}
...
void loadObjectNames()
{
   ...
   //objFile = fopen("TEMP", "wb");
   //fwrite(objData, 1, fileLen, objFile);
   //fclose(objFile);
...
void loadMessages(byte *fileData)
{
   ...
   //startPos = fileData[0] + 256*fileData[1] - 2;
	    ...
	    */
	    ...
	    switch (cmd) {
		  ...
		  if (cmd > 18) {
			...
			//sprintf(tempString, "%d", getData(data));
			//addToLine(tempString);
		     }
		  }
		  addToLine(")");
...
{
   FILE *logicFile, *comFile, *dummyFile;
   byte *fileData, *data, *endOfData, cmd, param, value;
   long progLength;
   int counter=1, oldCount, lineNum, i, opt, result, disp;
   char tempString[500], ch;
   boolean STOP = FALSE, HALF = FALSE, BROWSE = FALSE;

   if (argc < 2) {
      printf("SHOWLOG version 1.0b\n\n");
      printf("Usage:   showlog [options] filename");
      printf("\n-3   decode version 3 script.\n");
...
**************************************************************************/
   ...
   while (!feof(dumpFile)) {
	 ...
	 //printf("%-4d:%s", lineArray[lineNum], tempString);
	 if (BROWSE)
	    fprintf(dummyFile, "%s", tempString);
	 else
	    printf("%s", tempString);
	 lineNum++;
	 if (STOP && (counter++ % 10) == 0) ch = getch();
	 if ((ch == 'q') || (ch == 27)) break;
      }
   }
