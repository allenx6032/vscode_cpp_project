...
{
    LOG_INFO << ("Graphics Manager : Initializing OpenGL");
    ...
    {
        LOG_ERROR << ("Graphics Manager : No display found");
        return;
    }
    if (!eglInitialize(mDisplay, NULL, NULL))
    {
        LOG_ERROR << ("Graphics Manager : Could not initialize display");
        return;
    }
    if (!eglChooseConfig(mDisplay, lAttributes, &lConfig, 1, &lNumConfigs) || (lNumConfigs <= 0))
    {
        LOG_ERROR << ("Graphics Manager : No display config");
        return;
    }
    if (!eglGetConfigAttrib(mDisplay, lConfig, EGL_NATIVE_VISUAL_ID, &lFormat))
    {
        LOG_ERROR << ("Graphics Manager : No config attributes");
        return;
    }
    ...
    {
        LOG_ERROR << ("Graphics Manager : Could not create surface");
        return;
    }
    EGLint contextAttrs[] = {
    ...
    {
        LOG_ERROR << ("Graphics Manager : Could not create context");
        return;
    }
    EGLint sX(0), sY(0);
    if (!eglMakeCurrent(mDisplay, mSurface, mSurface, mContext)
        ...
        || (sX <= 0) || (sY <= 0))
    {
        LOG_ERROR << ("Graphics Manager : Could not activate display");
        return;
    }
...
void GraphicsManager::destroy()
{
    ...
    if (mDisplay != EGL_NO_DISPLAY)
    {
        ...
        LOG_INFO << ("Graphics Manager : Destroyed");
    }
}
#endif
