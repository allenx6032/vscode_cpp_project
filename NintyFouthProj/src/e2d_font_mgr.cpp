...
void FontManager::initialize(const String& path)
{
    ...
    if (error)
    {
        LOG_ERROR << ("FontManager::setPath FT_Init_FreeType failed! code: ") << error;
    }
...
const SPtr<Font> FontManager::loadFont(const String& name, const String& fontName, uint32 size /* = 12 */)
{
    ...
    {
        LOG_ERROR << ("FontManager::loadFont font file not exist! name: ") << fontName;
        return nullptr;
    }
    ...
    if (dit == mFontDatas.end())
    {
        ...
        {
            LOG_ERROR << ("FontManager::loadFont loadAsset failed!");
            return FALSE;
        }
    ...
    if (!font->load())
    {
        LOG_ERROR << ("FontManager::loadFont font load failed! name: ") << fontName;
        return nullptr;
    }
