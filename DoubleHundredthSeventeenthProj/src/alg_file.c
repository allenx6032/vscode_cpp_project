...
int _save_commander_file (char *path)
{
	FILE *fp;
...
int _load_commander_file (char *path, struct commander *saved_cmdr)
{
	FILE *fp;
	unsigned char block[256];
	int i;
...
void set_commander_name (char *fname, char *cname)
{
	int i;
	
	for (i = 0; i < MAX_CMDR_NAME_LENGTH; i++)
	{
		if (!isalnum(*fname))
			break;
...
{
	char *result;
	result = (char *)malloc(strlen(patha) + strlen(PATH_SEPARATOR) + strlen(pathb) + 1);
	sprintf(result, "%s%s%s", patha, PATH_SEPARATOR, pathb);
	return result;
}
...
int load_commander_file (char *filename, struct commander *saved_cmdr)
{
	...
	path = concat_paths(save_game_dir, filename);

    rv = _load_commander_file(path, saved_cmdr);

	if (!rv)
	{
		set_commander_name(filename, saved_cmdr->name);
	}
