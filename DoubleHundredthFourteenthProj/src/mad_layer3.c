...
// printf("frame size: %u  si.main_data_begin %u\n", stream->next_frame - stream->this_frame ,si.main_data_begin);

	if(si.main_data_begin == 0) // no need for data from bit reservoir
	{
		ptr = stream->ptr;
		stream->md_len = 0;
		frame_used = md_len;
	...
	{
		if (si.main_data_begin > stream->md_len) // we don't have enough data in bit reservoire
		{
			...
			while(need)
			{
				...
				memcpy(*stream->main_data + si.main_data_begin - stream->md_len - data_length,
  ...
  else
  {
    ...
    else
	{
      ...
      if (md_len > si.main_data_begin)
	  {
		...
		memcpy(*stream->main_data + stream->md_len, mad_bit_nextbyte(&stream->ptr), frame_used = md_len - si.main_data_begin);
		stream->md_len += frame_used;
      }
    }
  }
