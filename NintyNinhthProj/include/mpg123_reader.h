...
struct bufferchain
{
	...
	mpg123_ssize_t size;        /* Aggregated size of all buffies. */
	/* These positions are relative to buffer chain beginning. */
	mpg123_ssize_t pos;         /* Position in whole chain. */
	mpg123_ssize_t firstpos;    /* The point of return on non-forget() */
...
struct reader_data
{
	...
	int   flags;
	long timeout_sec;
	mpg123_ssize_t (*fdread) (mpg123_handle *, void *, size_t);
	/* User can replace the read and lseek functions. The r_* are the stored replacement functions or NULL. */
	mpg123_ssize_t (*r_read) (int fd, void *buf, size_t count);
	...
	mpg123_ssize_t (*r_read_handle) (void *handle, void *buf, size_t count);
	...
	mpg123_ssize_t (*read) (int fd, void *buf, size_t count);
	off_t   (*lseek)(int fd, off_t offset, int whence);
	/* Buffered readers want that abstracted, set internally. */
	mpg123_ssize_t (*fullread)(mpg123_handle *, unsigned char *, mpg123_ssize_t);
#ifndef NO_FEEDER
