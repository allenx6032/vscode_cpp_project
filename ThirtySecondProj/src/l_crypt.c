...
ldesencode(lua_State *L) {
	...
	if(top<1 || top>2) {
		return luaL_error(L, "Invalid args,must be (string key,string text)");
	}
	uint32_t SK[32];
	des_key(L, SK);
	
	size_t textsz = 0;
	const uint8_t * text = (const uint8_t *)luaL_checklstring(L, top, &textsz);
