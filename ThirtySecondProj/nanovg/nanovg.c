...
static void nvg__dashStroke(NVGcontext* ctx, float scale, float strokeWidth)
{
  ...
  for (i = 0; i < npaths; ++i) {
    ...
    nvg__addPath(ctx);
    nvg__addPoint(ctx, cur.x, cur.y, NVG_PT_CORNER);  // initial state is dash (not gap)
    ...
    for (j = 1; j < jlim; ) {
      ...
      if (totalDist + dist > dashLen) {
        ...
        nvg__addPoint(ctx, x, y, NVG_PT_CORNER);
        ...
        cur = points[k];
        if (dashState)
          nvg__addPoint(ctx, cur.x, cur.y, NVG_PT_CORNER);
        j++;
      }
    }
  }
...
void nvgStroke(NVGcontext* ctx)
{
	...
	/* dash line add start  */
	  ...
	  npaths0 = ctx->cache->npaths;
	  npoints0 = ctx->cache->npoints;
	  paths0 = ctx->cache->paths;
	  if(state->dashArray && state->dashArray[0] >= 0) {
	    nvg__dashStroke(ctx, scale, strokeWidth);
	    paths0 = ctx->cache->paths;  // maybe have been realloced in nvg__dashStroke
	     // value of npaths before nvg__dashStroke call is offset into cache->paths of dashed pieces
	    ctx->cache->paths += npaths0;
	    ctx->cache->npaths -= npaths0;
	  }
	 /* dash line add end  */
    ...
    /* dash line add start  */
	ctx->cache->npaths = npaths0;
	ctx->cache->npoints = npoints0;
	ctx->cache->paths = paths0;
    /* dash line add end  */
