...
#include "mpg123_compat.h"
#include "mpg123_debug.h"

static int default_init(mpg123_handle *fr);
static off_t get_fileinfo(mpg123_handle *);
static mpg123_ssize_t posix_read(int fd, void *buf, size_t count){ return read(fd, buf, count); }
...
static mpg123_ssize_t plain_fullread(mpg123_handle *fr,unsigned char *buf, mpg123_ssize_t count);
...
static mpg123_ssize_t io_read(struct reader_data *rdat, void *buf, size_t count);

#ifndef NO_FEEDER
/* Bufferchain methods. */
static void bc_init(struct bufferchain *bc);
static void bc_reset(struct bufferchain *bc);
static int bc_append(struct bufferchain *bc, mpg123_ssize_t size);
#if 0
static void bc_drop(struct bufferchain *bc);
#endif
static int bc_add(struct bufferchain *bc, const unsigned char *data, mpg123_ssize_t size);
static mpg123_ssize_t bc_give(struct bufferchain *bc, unsigned char *out, mpg123_ssize_t size);
static mpg123_ssize_t bc_skip(struct bufferchain *bc, mpg123_ssize_t count);
static mpg123_ssize_t bc_seekback(struct bufferchain *bc, mpg123_ssize_t count);
static void bc_forget(struct bufferchain *bc);
#endif

/* A normal read and a read with timeout. */
static mpg123_ssize_t plain_read(mpg123_handle *fr, void *buf, size_t count)
{
	mpg123_ssize_t ret = io_read(&fr->rdat, buf, count);
...
static mpg123_ssize_t timeout_read(mpg123_handle *fr, void *buf, size_t count)
{
	struct timeval tv;
...
static mpg123_ssize_t icy_fullread(mpg123_handle *fr, unsigned char *buf, mpg123_ssize_t count)
{
	mpg123_ssize_t ret,cnt;
	cnt = 0;
...
static off_t stream_skip_bytes(mpg123_handle *fr,off_t len)
{
	...
	else if(len >= 0)
	{
		...
		while (len > 0)
		{
			mpg123_ssize_t num = len < (off_t)sizeof(buf) ? (ssize_t)len : (ssize_t)sizeof(buf);
...
static mpg123_ssize_t bc_need_more(struct bufferchain *bc)
{
...
static mpg123_ssize_t bc_give(struct bufferchain *bc, unsigned char *out, mpg123_ssize_t size)
{
	...
	mpg123_ssize_t gotcount = 0;
	mpg123_ssize_t offset = 0;
	...
	{
		mpg123_ssize_t loff = bc->pos - offset;
		mpg123_ssize_t chunk = size - gotcount; /* amount of bytes to get from here... */
...
static mpg123_ssize_t feed_read(mpg123_handle *fr, unsigned char *out, mpg123_ssize_t count)
{
	mpg123_ssize_t gotcount = bc_give(&fr->rdat.buffer, out, count);
...
static off_t feed_skip_bytes(mpg123_handle *fr,off_t len)
{
	...
	off_t res = bc_skip(&fr->rdat.buffer, (mpg123_ssize_t)len);
	if(res < 0) return res;
...
static mpg123_ssize_t buffered_fullread(mpg123_handle *fr, unsigned char *out, mpg123_ssize_t count)
{
	struct bufferchain *bc = &fr->rdat.buffer;
	mpg123_ssize_t gotcount;
	if(bc->size - bc->pos < count)
	{ /* Add more stuff to buffer. If hitting end of file, adjust count. */
		unsigned char readbuf[4096];
		mpg123_ssize_t need = count - (bc->size-bc->pos);
		while(need>0)
		{
			int ret;
			mpg123_ssize_t got = fr->rdat.fullread(fr, readbuf, sizeof(readbuf));
			if(got < 0)
			{
...
static mpg123_ssize_t bad_fullread(mpg123_handle *mh, unsigned char *data, mpg123_ssize_t count) bugger_off
