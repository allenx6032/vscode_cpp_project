...
int loadDemoData(NVGcontext* vg, DemoData* data)
{
	...
	for (i = 0; i < 12; i++) {
		char file[128];
		snprintf(file, 128, "/usr/share/camera/images/image%d.jpg", i+1);
...
//			printf("Could not load %s.\n", file);
	...
	data->fontIcons = nvgCreateFont(vg, "icons", "/usr/share/camera/data/entypo.ttf");
	if (data->fontIcons == -1) {
//		printf("Could not add font icons.\n");
		return -1;
	}
	data->fontNormal = nvgCreateFont(vg, "sans", "/usr/share/camera/data/Roboto-Regular.ttf");
	if (data->fontNormal == -1) {
//		printf("Could not add font italic.\n");
		return -1;
	}
	data->fontBold = nvgCreateFont(vg, "sans-bold", "/usr/share/camera/data/Roboto-Bold.ttf");
	if (data->fontBold == -1) {
//		printf("Could not add font bold.\n");
		return -1;
	}
	data->fontEmoji = nvgCreateFont(vg, "emoji", "/usr/share/camera/data/NotoEmoji-Regular.ttf");
	if (data->fontEmoji == -1) {
...
void drawParagraph(NVGcontext* vg, float x, float y, float width, float height, float mx, float my)
{
	...
	const char* text = "This is longer chunk of text.\n  \n  Would have used lorem ipsum but she    was busy jumping over the lazy dog with the fox and all the men who came to the aid of the party.�沁�";
	const char* start;
	const char* end;
