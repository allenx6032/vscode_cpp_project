#include "sdlms_Input.h"

#include "sdlms_ECSSystem.h"
...
{
	return pressed_keys_momentary.contains(key);
}

bool Input::is_key_released(SDL_Keycode key)
{
	return released_keys_momentary.contains(key);
}
...
{
	return pressed_mouse_momentary.contains(button);
}

bool Input::is_mouse_button_released(MouseButton button)
{
	return released_mouse_momentary.contains(button);
}
