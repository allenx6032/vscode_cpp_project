...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/11/23 10:54:16 by ███████           #+#    #+#             */
/*   Updated: 2022/05/10 11:07:17 by ███████          ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	// static char	buffer[sysconf(_SC_OPEN_MAX)][BUFFER_SIZE +1];
	char		*line;

	if (read(fd, buffer, 0) < 0 || fd > OPEN_MAX)
	// if (read(fd, buffer, 0) < 0 || fd > sysconf(_SC_OPEN_MAX))
		return (NULL);
