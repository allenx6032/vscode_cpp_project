...
#include "FallingSand_profiler.hpp"
#include "FallingSand_engine.hpp"

// #define VE_FONTCACHE_DEBUGPRINT
#include "FallingSand_fontcache.hpp"
...
ME_PRIVATE(GLint) ME_font_compile_shader(const std::string vs, const std::string fs) {
...
ME_PRIVATE(void) compile_vbo(GLuint &dest_vb, GLuint &dest_ib, const ve_fontcache_vertex *verts, int nverts, const uint32_t *indices, int nindices) {
    if (dest_vb == 0 || dest_ib == 0) {
        ...
        glBufferData(GL_ARRAY_BUFFER, nverts * sizeof(ve_fontcache_vertex), verts, GL_DYNAMIC_DRAW);
...
void test_font( ve_font_id id )
{
    ...
    ve_fontcache_draw_text( &cache, id, s, 0.1f, 0.8f, 1.0f / this->screen_w,  1.0f / this->screen_h );
}
...
void test_font2( ve_font_id id )
{
    ...
    for( int i= 0; i < 2; i++ ) {
        ve_fontcache_draw_text( &cache, id, s, 0.12f, 0.05f + i * 0.033f, 1.0f / this->screen_w,  1.0f / this->screen_h );
    }
}
#endif
...
void fontcache::push(const std::string &text, const font_index font, const MEvec2 pos) {
    ...
    //     // ve_fontcache_draw_text(&cache, logo_font, U8_STR("草"), 0.4f, current_scroll + 0.0f, 1.0f / this->screen_w, 1.0f / this->screen_h);
    //     // ve_fontcache_draw_text(&cache, title_font, U8_STR("基于 FontCache 的字体渲染"), 0.2f, current_scroll - 0.1f, 1.0f / this->screen_w, 1.0f / this->screen_h);
    //     // ve_fontcache_draw_text(&cache, print_font, intro, 0.2f, current_scroll - 0.14f, 1.0f / this->screen_w, 1.0f / this->screen_h);
    // }
    ...
    if (current_scroll > section_start && current_scroll < section_end) {
        ...
        ve_fontcache_draw_text(&cache, title_font, u8"How it works", 0.2f, current_scroll - (section_start + 0.06f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, print_font, how_it_works, 0.2f, current_scroll - (section_start + 0.1f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, mono_font, caching_strategy, 0.28f, current_scroll - (section_start + 0.32f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, print_font, how_it_works2, 0.2f, current_scroll - (section_start + 0.82f), 1.0f / this->screen_w, 1.0f / this->screen_h);
    }
    ...
    if (current_scroll > section_start && current_scroll < section_end) {
        ...
        ve_fontcache_draw_text(&cache, title_font, u8"Showcase", 0.2f, current_scroll - (section_start + 0.2f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, print_font, u8"This is a showcase demonstrating different hb_font categories and languages.", 0.2f, current_scroll - (section_start + 0.24f),
                               1.0f / this->screen_w, 1.0f / this->screen_h);

        ve_fontcache_draw_text(&cache, print_font, u8"Sans serif", 0.2f, current_scroll - (section_start + 0.28f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, demo_sans_font, font_family_test, 0.3f, current_scroll - (section_start + 0.28f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, print_font, u8"Serif", 0.2f, current_scroll - (section_start + 0.36f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, demo_serif_font, font_family_test, 0.3f, current_scroll - (section_start + 0.36f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, print_font, u8"Script", 0.2f, current_scroll - (section_start + 0.44f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, demo_script_font, font_family_test, 0.3f, current_scroll - (section_start + 0.44f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, print_font, u8"Monospace", 0.2f, current_scroll - (section_start + 0.52f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, demo_mono_font, font_family_test, 0.3f, current_scroll - (section_start + 0.52f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, print_font, u8"Small", 0.2f, current_scroll - (section_start + 0.60f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, small_font, font_family_test, 0.3f, current_scroll - (section_start + 0.60f), 1.0f / this->screen_w, 1.0f / this->screen_h);

        ve_fontcache_draw_text(&cache, print_font, u8"Greek", 0.2f, current_scroll - (section_start + 0.72f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, demo_sans_font, u8"Ήταν απλώς θέμα χρόνου.", 0.3f, current_scroll - (section_start + 0.72f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, print_font, u8"Vietnamnese", 0.2f, current_scroll - (section_start + 0.76f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, demo_sans_font, u8"Bầu trời trong xanh thăm thẳm, không một gợn mây.", 0.3f, current_scroll - (section_start + 0.76f), 1.0f / this->screen_w,
                               1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, print_font, u8"Thai", 0.2f, current_scroll - (section_start + 0.80f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, demo_thai_font, u8"การเดินทางขากลับคงจะเหงา", 0.3f, current_scroll - (section_start + 0.80f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, print_font, u8"Chinese", 0.2f, current_scroll - (section_start + 0.84f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, demo_chinese_font, u8"床前明月光 疑是地上霜 举头望明月 低头思故乡", 0.3f, current_scroll - (section_start + 0.84f), 1.0f / this->screen_w,
                               1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, print_font, u8"Japanese", 0.2f, current_scroll - (section_start + 0.88f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, demo_japanese_font, u8"ぎょしょうとナレズシの研究 モンスーン・アジアの食事文化", 0.3f, current_scroll - (section_start + 0.88f), 1.0f / this->screen_w,
                               1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, print_font, u8"Korean", 0.2f, current_scroll - (section_start + 0.92f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, demo_korean_font, u8"그들의 장비와 기구는 모두 살아 있다.", 0.3f, current_scroll - (section_start + 0.92f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, print_font, u8"Arabic", 0.2f, current_scroll - (section_start + 0.96f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, demo_arabic_font, u8"حب السماء لا تمطر غير الأحلام. This one needs HarfBuzz to work!", 0.3f, current_scroll - (section_start + 0.96f), 1.0f / this->screen_w,
                               1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, print_font, u8"Hebrew", 0.2f, current_scroll - (section_start + 1.0f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        ve_fontcache_draw_text(&cache, demo_hebrew_font, u8"אז הגיע הלילה של כוכב השביט הראשון. This one needs HarfBuzz to work!", 0.3f, current_scroll - (section_start + 1.0f),
                               1.0f / this->screen_w, 1.0f / this->screen_h);
    }
    ...
    if (current_scroll > section_start && current_scroll < section_end) {
        ...
        // Draw grid.
        ve_fontcache_draw_text(&cache, title_font, u8"Raincode demo", 0.2f, current_scroll - (section_start + 0.2f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        for (int y = 0; y < GRID_H; y++) {
            for (int x = 0; x < GRID_W; x++) {
                ...
                ve_fontcache_draw_text(&cache, demo_raincode_font, codes[grid[y * GRID_W + x]], posx, posy, 1.0f / this->screen_w, 1.0f / this->screen_h);
            }
        }
    ...
    if (current_scroll > section_start && current_scroll < section_end) {
        ...
        // Draw grid.
        ve_fontcache_draw_text(&cache, title_font, u8"Cache pressure test", 0.2f, current_scroll - (section_start + 0.2f), 1.0f / this->screen_w, 1.0f / this->screen_h);
        for (int y = 0; y < GRID_H; y++) {
            for (int x = 0; x < GRID_W; x++) {
                float posx = 0.2f + x * 0.02f, posy = current_scroll - (section_start + 0.24f + y * 0.025f);
                char c[5] = {'\0', '\0', '\0', '\0', '\0'};
                codepoint_to_utf8(c, grid[y * GRID_W + x]);
                ve_fontcache_draw_text(&cache, demo_chinese_font, c, posx, posy, 1.0f / this->screen_w, 1.0f / this->screen_h);
            }
        }
        for (int y = 0; y < GRID2_H; y++) {
            for (int x = 0; x < GRID2_W; x++) {
                ...
                ve_fontcache_draw_text(&cache, demo_grid2_font, c, posx, posy, 1.0f / this->screen_w, 1.0f / this->screen_h);
            }
        }
        for (int y = 0; y < GRID3_H; y++) {
            for (int x = 0; x < GRID3_W; x++) {
                ...
                ve_fontcache_draw_text(&cache, demo_grid3_font, c, posx, posy, 1.0f / this->screen_w, 1.0f / this->screen_h);
            }
        }
    }
...
void test_plist() {
    ...
    for (int repeat = 0; repeat < 128; repeat++) {
        ...
        assert(plist.size == 3);
        auto v = ve_fontcache_poollist_pop_back(plist);
        assert(v == 31337);
        v = ve_fontcache_poollist_pop_back(plist);
        assert(v == 31338);
        v = ve_fontcache_poollist_pop_back(plist);
        assert(v == 31339);
        assert(plist.size == 0);
