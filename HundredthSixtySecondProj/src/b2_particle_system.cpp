...
static const uint32 relativeTagBottomLeft = (uint32)((1 << yShift) + ((~uint32(0)) << xShift));
...
public:
    b2ParticleBodyContactRemovePredicate(b2ParticleSystem* system, int32* discarded) : m_system(system), m_lastIndex(-1), m_currentContacts(0), m_discarded(discarded) {}
...
public:
    ...
    // lifetimes.
    ExpirationTimeComparator(const int32* const expirationTimes) : m_expirationTimes(expirationTimes) {}
    // Empty destructor.
    ...
    bool operator()(const int32 particleIndexA, const int32 particleIndexB) const {
...
struct LightweightPair {
    ...
    static bool Compare(const LightweightPair& left, const LightweightPair& right) { return left.first < right.first && left.second < right.second; }
};
...
public:
    ...
    TypedFixedSetAllocator(b2StackAllocator* allocator) : FixedSetAllocator(allocator) {}
    ...
    int32 GetIndex(const T* item) const {
        if (item) {
            b2Assert(item >= GetBuffer() && item < GetBuffer() + GetCount());
            const int32 index = (int32)(((uint8*)item - (uint8*)GetBuffer()) / sizeof(*item));
...
class FixtureParticleSet : public TypedFixedSetAllocator<FixtureParticle> {
public:
    // Initialize members of this class.
    FixtureParticleSet(b2StackAllocator* allocator) : TypedFixedSetAllocator<FixtureParticle>(allocator) {}

    // Initialize from a set of particle / body contacts for particles
    // that have the b2_fixtureContactListenerParticle flag set.
    void Initialize(const b2ParticleBodyContact* const bodyContacts, const int32 numBodyContacts, const uint32* const particleFlagsBuffer);
...
public:
    // Initialize members of this class.
    b2ParticlePairSet(b2StackAllocator* allocator) : TypedFixedSetAllocator<ParticlePair>(allocator) {}

    // Initialize from a set of particle contacts.
    void Initialize(const b2ParticleContact* const contacts, const int32 numContacts, const uint32* const particleFlagsBuffer);
...
static inline uint32 computeTag(float32 x, float32 y) { return ((uint32)(y + yOffset) << yShift) + (uint32)(xScale * x + xOffset); }

static inline uint32 computeRelativeTag(uint32 tag, int32 x, int32 y) { return tag + (y << yShift) + (x << xShift); }

b2ParticleSystem::InsideBoundsEnumerator::InsideBoundsEnumerator(uint32 lower, uint32 upper, const Proxy* first, const Proxy* last) {
...
template <typename T>
void b2ParticleSystem::FreeBuffer(T** b, int capacity) {
    if (*b == NULL) return;
...
template <typename T>
void b2ParticleSystem::FreeUserOverridableBuffer(UserOverridableBuffer<T>* b) {
...
// Reallocate a buffer
template <typename T>
T* b2ParticleSystem::ReallocateBuffer(T* oldBuffer, int32 oldCapacity, int32 newCapacity) {
...
// Reallocate a buffer
template <typename T>
T* b2ParticleSystem::ReallocateBuffer(T* buffer, int32 userSuppliedCapacity, int32 oldCapacity, int32 newCapacity, bool deferred) {
    b2Assert(newCapacity > oldCapacity);
...
// Reallocate a buffer
template <typename T>
T* b2ParticleSystem::ReallocateBuffer(UserOverridableBuffer<T>* buffer, int32 oldCapacity, int32 newCapacity, bool deferred) {
    b2Assert(newCapacity > oldCapacity);
...
void b2ParticleSystem::ReallocateHandleBuffers(int32 newCapacity) {
    ...
    m_handleIndexBuffer.data = ReallocateBuffer(&m_handleIndexBuffer, m_internalAllocatedCapacity, newCapacity, true);
    // Set the size of the next handle allocation.
    m_handleAllocator.SetItemsPerSlab(newCapacity - m_internalAllocatedCapacity);
}

template <typename T>
T* b2ParticleSystem::RequestBuffer(T* buffer) {
    if (!buffer) {
        if (m_internalAllocatedCapacity == 0) {
            ReallocateInternalAllocatedBuffers(b2_minParticleSystemBufferCapacity);
        }
...
static int32 LimitCapacity(int32 capacity, int32 maxCount) { return maxCount && capacity > maxCount ? maxCount : capacity; }
...
void b2ParticleSystem::ReallocateInternalAllocatedBuffers(int32 capacity) {
    ...
    if (m_internalAllocatedCapacity < capacity) {
        ...
        m_lastBodyContactStepBuffer.data = ReallocateBuffer(&m_lastBodyContactStepBuffer, m_internalAllocatedCapacity, capacity, stuck);
        m_bodyContactCountBuffer.data = ReallocateBuffer(&m_bodyContactCountBuffer, m_internalAllocatedCapacity, capacity, stuck);
        m_consecutiveContactStepsBuffer.data = ReallocateBuffer(&m_consecutiveContactStepsBuffer, m_internalAllocatedCapacity, capacity, stuck);
        ...
        m_forceBuffer = ReallocateBuffer(m_forceBuffer, 0, m_internalAllocatedCapacity, capacity, false);
        ...
        m_expirationTimeBuffer.data = ReallocateBuffer(&m_expirationTimeBuffer, m_internalAllocatedCapacity, capacity, true);
        m_indexByExpirationTimeBuffer.data = ReallocateBuffer(&m_indexByExpirationTimeBuffer, m_internalAllocatedCapacity, capacity, true);
...
int32 b2ParticleSystem::CreateParticle(const b2ParticleDef& def) {
    ...
    if (m_count >= m_internalAllocatedCapacity) {
        ...
        int32 capacity = m_count ? 2 * m_count : b2_minParticleSystemBufferCapacity;
...
const b2ParticleHandle* b2ParticleSystem::GetParticleHandleFromIndex(const int32 index) {
    b2Assert(index >= 0 && index < GetParticleCount() && index != b2_invalidParticleIndex);
...
void b2ParticleSystem::DestroyParticle(int32 index, bool callDestructionListener) {
...
void b2ParticleSystem::DestroyOldestParticle(const int32 index, const bool callDestructionListener) {
    ...
    const int32 oldestFiniteLifetimeParticle = m_indexByExpirationTimeBuffer.data[particleCount - (index + 1)];
    const int32 oldestInfiniteLifetimeParticle = m_indexByExpirationTimeBuffer.data[index];
    DestroyParticle(m_expirationTimeBuffer.data[oldestFiniteLifetimeParticle] > 0.0f ? oldestFiniteLifetimeParticle : oldestInfiniteLifetimeParticle, callDestructionListener);
}

int32 b2ParticleSystem::DestroyParticlesInShape(const b2Shape& shape, const b2Transform& xf, bool callDestructionListener) {
    b2Assert(m_world->IsLocked() == false);
    if (m_world->IsLocked()) {
        return 0;
    }
    ...
    public:
        DestroyParticlesInShapeCallback(b2ParticleSystem* system, const b2Shape& shape, const b2Transform& xf, bool callDestructionListener) {
            m_system = system;
            m_shape = &shape;
            m_xf = xf;
        ...
        bool ReportParticle(const b2ParticleSystem* particleSystem, int32 index) {
            if (particleSystem != m_system) return false;
...
int32 b2ParticleSystem::CreateParticleForGroup(const b2ParticleGroupDef& groupDef, const b2Transform& xf, const b2Vec2& p) {
    b2ParticleDef particleDef;
    particleDef.flags = groupDef.flags;
    particleDef.position = b2Mul(xf, p);
    particleDef.velocity = groupDef.linearVelocity + b2Cross(groupDef.angularVelocity, particleDef.position - groupDef.position);
...
void b2ParticleSystem::CreateParticlesStrokeShapeForGroup(const b2Shape* shape, const b2ParticleGroupDef& groupDef, const b2Transform& xf) {
...
void b2ParticleSystem::CreateParticlesFillShapeForGroup(const b2Shape* shape, const b2ParticleGroupDef& groupDef, const b2Transform& xf) {
    float32 stride = groupDef.stride;
    if (stride == 0) {
        stride = GetParticleStride();
    }
...
void b2ParticleSystem::CreateParticlesWithShapeForGroup(const b2Shape* shape, const b2ParticleGroupDef& groupDef, const b2Transform& xf) {
...
void b2ParticleSystem::CreateParticlesWithShapesForGroup(const b2Shape* const* shapes, int32 shapeCount, const b2ParticleGroupDef& groupDef, const b2Transform& xf) {
    ...
    public:
        ...
        void ComputeAABB(b2AABB* aabb, const b2Transform& xf) const {
...
b2ParticleGroup* b2ParticleSystem::CreateParticleGroup(const b2ParticleGroupDef& groupDef) {
    ...
    if (groupDef.shapes) {
        CreateParticlesWithShapesForGroup(groupDef.shapes, groupDef.shapeCount, groupDef, transform);
    }
...
void b2ParticleSystem::JoinParticleGroups(b2ParticleGroup* groupA, b2ParticleGroup* groupB) {
    ...
    class JoinParticleGroupsFilter : public ConnectionFilter {
        bool ShouldCreatePair(int32 a, int32 b) const { return (a < m_threshold && m_threshold <= b) || (b < m_threshold && m_threshold <= a); }
        bool ShouldCreateTriad(int32 a, int32 b, int32 c) const { return (a < m_threshold || b < m_threshold || c < m_threshold) && (m_threshold <= a || m_threshold <= b || m_threshold <= c); }
        int32 m_threshold;

    public:
        JoinParticleGroupsFilter(int32 threshold) { m_threshold = threshold; }
...
void b2ParticleSystem::InitializeParticleLists(const b2ParticleGroup* group, ParticleListNode* nodeBuffer) {
...
void b2ParticleSystem::MergeParticleListsInContact(const b2ParticleGroup* group, ParticleListNode* nodeBuffer) const {
    int32 bufferIndex = group->GetBufferIndex();
...
void b2ParticleSystem::MergeParticleLists(ParticleListNode* listA, ParticleListNode* listB) {
...
b2ParticleSystem::ParticleListNode* b2ParticleSystem::FindLongestParticleList(const b2ParticleGroup* group, ParticleListNode* nodeBuffer) {
    int32 particleCount = group->GetParticleCount();
...
void b2ParticleSystem::MergeZombieParticleListNodes(const b2ParticleGroup* group, ParticleListNode* nodeBuffer, ParticleListNode* survivingList) const {
    int32 particleCount = group->GetParticleCount();
    for (int32 i = 0; i < particleCount; i++) {
        ParticleListNode* node = &nodeBuffer[i];
        if (node != survivingList && (m_flagsBuffer.data[node->index] & b2_zombieParticle)) {
            MergeParticleListAndNode(survivingList, node);
        }
    }
}

void b2ParticleSystem::MergeParticleListAndNode(ParticleListNode* list, ParticleListNode* node) {
...
void b2ParticleSystem::CreateParticleGroupsFromParticleList(const b2ParticleGroup* group, ParticleListNode* nodeBuffer, const ParticleListNode* survivingList) {
    int32 particleCount = group->GetParticleCount();
...
void b2ParticleSystem::UpdatePairsAndTriadsWithParticleList(const b2ParticleGroup* group, const ParticleListNode* nodeBuffer) {
    int32 bufferIndex = group->GetBufferIndex();
...
int32 b2ParticleSystem::CloneParticle(int32 oldIndex, b2ParticleGroup* group) {
    ...
    if (m_lastBodyContactStepBuffer.data) {
        m_lastBodyContactStepBuffer.data[newIndex] = m_lastBodyContactStepBuffer.data[oldIndex];
    }
    if (m_bodyContactCountBuffer.data) {
        m_bodyContactCountBuffer.data[newIndex] = m_bodyContactCountBuffer.data[oldIndex];
    }
    if (m_consecutiveContactStepsBuffer.data) {
        m_consecutiveContactStepsBuffer.data[newIndex] = m_consecutiveContactStepsBuffer.data[oldIndex];
    }
    if (m_hasForce) {
...
void b2ParticleSystem::UpdatePairsAndTriadsWithReactiveParticles() {
    class ReactiveFilter : public ConnectionFilter {
        bool IsNecessary(int32 index) const { return (m_flagsBuffer[index] & b2_reactiveParticle) != 0; }
        const uint32* m_flagsBuffer;

    public:
        ReactiveFilter(uint32* flagsBuffer) { m_flagsBuffer = flagsBuffer; }
...
static bool ParticleCanBeConnected(uint32 flags, b2ParticleGroup* group) {
    return (flags & (b2_wallParticle | b2_springParticle | b2_elasticParticle)) || (group && group->GetGroupFlags() & b2_rigidParticleGroup);
}

void b2ParticleSystem::UpdatePairsAndTriads(int32 firstIndex, int32 lastIndex, const ConnectionFilter& filter) {
    ...
    if (particleFlags & k_pairFlags) {
        for (int32 k = 0; k < m_contactBuffer.GetCount(); k++) {
            ...
            if (a >= firstIndex && a < lastIndex && b >= firstIndex && b < lastIndex && !((af | bf) & b2_zombieParticle) && ((af | bf) & k_pairFlags) &&
                (filter.IsNecessary(a) || filter.IsNecessary(b)) && ParticleCanBeConnected(af, groupA) && ParticleCanBeConnected(bf, groupB) && filter.ShouldCreatePair(a, b)) {
    ...
    if (particleFlags & k_triadFlags) {
        b2VoronoiDiagram diagram(&m_world->m_stackAllocator, lastIndex - firstIndex);
        for (int32 i = firstIndex; i < lastIndex; i++) {
            uint32 flags = m_flagsBuffer.data[i];
            b2ParticleGroup* group = m_groupBuffer[i];
            if (!(flags & b2_zombieParticle) && ParticleCanBeConnected(flags, group)) {
                diagram.AddGenerator(m_positionBuffer.data[i], i, filter.IsNecessary(i));
            }
        }
        ...
        class UpdateTriadsCallback : public b2VoronoiDiagram::NodeCallback {
            void operator()(int32 a, int32 b, int32 c) {
                ...
                if (((af | bf | cf) & k_triadFlags) && m_filter->ShouldCreateTriad(a, b, c)) {
                    ...
                    b2Vec2 dbc = pb - pc;
                    b2Vec2 dca = pc - pa;
                    float32 maxDistanceSquared = b2_maxTriadDistanceSquared * m_system->m_squaredDiameter;
                    if (b2Dot(dab, dab) > maxDistanceSquared || b2Dot(dbc, dbc) > maxDistanceSquared || b2Dot(dca, dca) > maxDistanceSquared) {
                        return;
                    }
        ...
        public:
            UpdateTriadsCallback(b2ParticleSystem* system, const ConnectionFilter* filter) {
                m_system = system;
                m_filter = filter;
            }
...
bool b2ParticleSystem::ComparePairIndices(const b2ParticlePair& a, const b2ParticlePair& b) {
    int32 diffA = a.indexA - b.indexA;
    if (diffA != 0) return diffA < 0;
    return a.indexB < b.indexB;
}

bool b2ParticleSystem::MatchPairIndices(const b2ParticlePair& a, const b2ParticlePair& b) { return a.indexA == b.indexA && a.indexB == b.indexB; }

bool b2ParticleSystem::CompareTriadIndices(const b2ParticleTriad& a, const b2ParticleTriad& b) {
    int32 diffA = a.indexA - b.indexA;
    if (diffA != 0) return diffA < 0;
...
bool b2ParticleSystem::MatchTriadIndices(const b2ParticleTriad& a, const b2ParticleTriad& b) { return a.indexA == b.indexA && a.indexB == b.indexB && a.indexC == b.indexC; }
...
void b2ParticleSystem::ComputeDepth() {
    b2ParticleContact* contactGroups = (b2ParticleContact*)m_world->m_stackAllocator.Allocate(sizeof(b2ParticleContact) * m_contactBuffer.GetCount());
    ...
    b2ParticleGroup** groupsToUpdate = (b2ParticleGroup**)m_world->m_stackAllocator.Allocate(sizeof(b2ParticleGroup*) * m_groupCount);
...
b2ParticleSystem::InsideBoundsEnumerator b2ParticleSystem::GetInsideBoundsEnumerator(const b2AABB& aabb) const {
    uint32 lowerTag = computeTag(m_inverseDiameter * aabb.lowerBound.x - 1, m_inverseDiameter * aabb.lowerBound.y - 1);
    uint32 upperTag = computeTag(m_inverseDiameter * aabb.upperBound.x + 1, m_inverseDiameter * aabb.upperBound.y + 1);
...
inline void b2ParticleSystem::AddContact(int32 a, int32 b, b2GrowableBuffer<b2ParticleContact>& contacts) const {
...
void b2ParticleSystem::FindContacts(b2GrowableBuffer<b2ParticleContact>& contacts) const {
    const Proxy* beginProxy = m_proxyBuffer.Begin();
    const Proxy* endProxy = m_proxyBuffer.End();
...
static inline bool b2ParticleContactIsZombie(const b2ParticleContact& contact) { return (contact.GetFlags() & b2_zombieParticle) == b2_zombieParticle; }

// Get the world's contact filter if any particles with the
// b2_particleContactFilterParticle flag are present in the system.
inline b2ContactFilter* b2ParticleSystem::GetParticleContactFilter() const { return (m_allParticleFlags & b2_particleContactFilterParticle) ? m_world->m_contactManager.m_contactFilter : NULL; }
...
inline b2ContactListener* b2ParticleSystem::GetParticleContactListener() const {
    return (m_allParticleFlags & b2_particleContactListenerParticle) ? m_world->m_contactManager.m_contactListener : NULL;
}
...
// static
bool b2ParticleSystem::ProxyBufferHasIndex(int32 index, const Proxy* const a, int count) {
    for (int j = 0; j < count; ++j) {
        if (a[j].index == index) return true;
    }
    return false;
}

// static
int b2ParticleSystem::NumProxiesWithSameTag(const Proxy* const a, const Proxy* const b, int count) {
    const uint32 tag = a[0].tag;
    for (int num = 0; num < count; ++num) {
        if (a[num].tag != tag || b[num].tag != tag) return num;
    }
    return count;
}
...
// sorted by index.
// static
bool b2ParticleSystem::AreProxyBuffersTheSame(const b2GrowableBuffer<Proxy>& a, const b2GrowableBuffer<Proxy>& b) {
    ...
    for (int i = 0; i < a.GetCount();) {
        const int numWithSameTag = NumProxiesWithSameTag(&a[i], &b[i], a.GetCount() - i);
        if (numWithSameTag == 0) return false;

        for (int j = 0; j < numWithSameTag; ++j) {
            const bool hasIndex = ProxyBufferHasIndex(a[i + j].index, &b[i], numWithSameTag);
...
void b2ParticleSystem::SortProxies(b2GrowableBuffer<Proxy>& proxies) const { std::sort(proxies.Begin(), proxies.End()); }

class b2ParticleContactRemovePredicate {
public:
    b2ParticleContactRemovePredicate(b2ParticleSystem* system, b2ContactFilter* contactFilter) : m_system(system), m_contactFilter(contactFilter) {}

    bool operator()(const b2ParticleContact& contact) {
        return (contact.GetFlags() & b2_particleContactFilterParticle) && !m_contactFilter->ShouldCollide(m_system, contact.GetIndexA(), contact.GetIndexB());
    }
...
void b2ParticleSystem::FilterContacts(b2GrowableBuffer<b2ParticleContact>& contacts) {
...
void b2ParticleSystem::NotifyContactListenerPreContact(b2ParticlePairSet* particlePairs) const {
...
void b2ParticleSystem::NotifyContactListenerPostContact(b2ParticlePairSet& particlePairs) {
    b2ContactListener* const contactListener = GetParticleContactListener();
    if (contactListener == NULL) return;
    ...
    for (int32 i = 0; i < pairCount; ++i) {
        if (valid[i]) {
            contactListener->EndContact(this, pairs[i].first, pairs[i].second);
        }
    }
}
...
inline b2ContactListener* b2ParticleSystem::GetFixtureContactListener() const { return (m_allParticleFlags & b2_fixtureContactListenerParticle) ? m_world->m_contactManager.m_contactListener : NULL; }

// Get the world's contact filter if any particles with the
// b2_fixtureContactFilterParticle flag are present in the system.
inline b2ContactFilter* b2ParticleSystem::GetFixtureContactFilter() const { return (m_allParticleFlags & b2_fixtureContactFilterParticle) ? m_world->m_contactManager.m_contactFilter : NULL; }
...
// Associate a memory allocator with this object.
FixedSetAllocator::FixedSetAllocator(b2StackAllocator* allocator) : m_buffer(NULL), m_valid(NULL), m_count(0), m_allocator(allocator) { b2Assert(allocator); }

// Allocate internal storage for this object.
int32 FixedSetAllocator::Allocate(const int32 itemSize, const int32 count) {
    Clear();
    if (count) {
        m_buffer = m_allocator->Allocate((itemSize + sizeof(*m_valid)) * count);
        b2Assert(m_buffer);
...
// otherwise.
template <typename T>
static int32 FindItemIndexInFixedSet(const TypedFixedSetAllocator<T>& set, const T& item) {
    if (set.GetCount()) {
...
// Initialize from a set of particle / body contacts for particles
// that have the b2_fixtureContactListenerParticle flag set.
void FixtureParticleSet::Initialize(const b2ParticleBodyContact* const bodyContacts, const int32 numBodyContacts, const uint32* const particleFlagsBuffer) {
    Clear();
    if (Allocate(numBodyContacts)) {
        ...
        for (int32 i = 0; i < numBodyContacts; ++i) {
            ...
            if (bodyContact.index == b2_invalidParticleIndex || !(particleFlagsBuffer[bodyContact.index] & b2_fixtureContactListenerParticle)) {
                continue;
            }
...
// present.
int32 FixtureParticleSet::Find(const FixtureParticle& fixtureParticle) const { return FindItemIndexInFixedSet(*this, fixtureParticle); }

// Initialize from a set of particle contacts.
void b2ParticlePairSet::Initialize(const b2ParticleContact* const contacts, const int32 numContacts, const uint32* const particleFlagsBuffer) {
    Clear();
    if (Allocate(numContacts)) {
        ...
        for (int32 i = 0; i < numContacts; ++i) {
            ...
            if (contact.GetIndexA() == b2_invalidParticleIndex || contact.GetIndexB() == b2_invalidParticleIndex ||
                !((particleFlagsBuffer[contact.GetIndexA()] | particleFlagsBuffer[contact.GetIndexB()]) & b2_particleContactListenerParticle)) {
                continue;
            }
            pair->first = contact.GetIndexA();
            pair->second = contact.GetIndexB();
...
public:
    explicit b2FixtureParticleQueryCallback(b2ParticleSystem* system) { m_system = system; }
...
private:
    ...
    bool ReportFixture(b2Fixture* fixture) {
        ...
        b2ParticleSystem::InsideBoundsEnumerator enumerator = m_system->GetInsideBoundsEnumerator(aabb);
        int32 index;
    ...
    virtual void ReportFixtureAndParticle(b2Fixture* fixture, int32 index) = 0;

protected:
    b2ParticleSystem* m_system;
};

void b2ParticleSystem::NotifyBodyContactListenerPreContact(FixtureParticleSet* fixtureSet) const {
...
void b2ParticleSystem::NotifyBodyContactListenerPostContact(FixtureParticleSet& fixtureSet) {
    b2ContactListener* const contactListener = GetFixtureContactListener();
    if (contactListener == NULL) return;
    ...
    for (int32 i = 0; i < fixtureParticleCount; ++i) {
        if (fixtureParticlesValid[i]) {
            ...
            contactListener->EndContact(fixtureParticle->first, this, fixtureParticle->second);
        }
    }
}
...
void b2ParticleSystem::UpdateBodyContacts() {
    ...
    class UpdateBodyContactsCallback : public b2FixtureParticleQueryCallback {
        ...
        void ReportFixtureAndParticle(b2Fixture* fixture, int32 a) {
            ...
            if (d < m_system->m_particleDiameter && ShouldCollide(fixture, a)) {
                ...
                float32 invAm = m_system->m_flagsBuffer.data[a] & b2_wallParticle ? 0 : m_system->GetParticleInvMass();
                b2Vec2 rp = ap - bp;
    ...
    public:
        UpdateBodyContactsCallback(b2ParticleSystem* system, b2ContactFilter* contactFilter) : b2FixtureParticleQueryCallback(system) { m_contactFilter = contactFilter; }
...
void b2ParticleSystem::RemoveSpuriousBodyContacts() {
    ...
    int32 discarded = 0;
    (void)std::remove_if(m_bodyContactBuffer.Begin(), m_bodyContactBuffer.End(), b2ParticleBodyContactRemovePredicate(this, &discarded));

    m_bodyContactBuffer.SetCount(m_bodyContactBuffer.GetCount() - discarded);
}

bool b2ParticleSystem::BodyContactCompare(const b2ParticleBodyContact& lhs, const b2ParticleBodyContact& rhs) {
...
void b2ParticleSystem::SolveCollision(const b2TimeStep& step) {
    ...
    public:
        SolveCollisionCallback(b2ParticleSystem* system, const b2TimeStep& step, b2ContactFilter* contactFilter) : b2FixtureParticleQueryCallback(system) {
            m_step = step;
...
void b2ParticleSystem::SolveBarrier(const b2TimeStep& step) {
    ...
    for (int32 i = 0; i < m_count; i++) {
        uint32 flags = m_flagsBuffer.data[i];
        static const uint32 k_barrierWallFlags = b2_barrierParticle | b2_wallParticle;
...
void b2ParticleSystem::Solve(const b2TimeStep& step) {
    ...
    for (m_iterationIndex = 0; m_iterationIndex < step.particleIterations; m_iterationIndex++) {
        ++m_timestamp;
    ...
    for (int32 t = 0; t < m_def.staticPressureIterations; t++) {
        ...
        for (int32 i = 0; i < m_count; i++) {
            ...
            if (m_flagsBuffer.data[i] & b2_staticPressureParticle) {
                ...
                float32 h = (wh + pressurePerWeight * (w - b2_minParticleWeight)) / (w + relaxation);
...
inline bool b2ParticleSystem::IsRigidGroup(b2ParticleGroup* group) const { return group && (group->m_groupFlags & b2_rigidParticleGroup); }

inline b2Vec2 b2ParticleSystem::GetLinearVelocity(b2ParticleGroup* group, int32 particleIndex, const b2Vec2& point) const {
...
inline void b2ParticleSystem::InitDampingParameter(float32* invMass, float32* invInertia, float32* tangentDistance, float32 mass, float32 inertia, const b2Vec2& center, const b2Vec2& point,
...
inline void b2ParticleSystem::InitDampingParameterWithRigidGroupOrParticle(float32* invMass, float32* invInertia, float32* tangentDistance, bool isRigidGroup, b2ParticleGroup* group,
                                                                           int32 particleIndex, const b2Vec2& point, const b2Vec2& normal) const {
    if (isRigidGroup) {
        InitDampingParameter(invMass, invInertia, tangentDistance, group->GetMass(), group->GetInertia(), group->GetCenter(), point, normal);
    } else {
        uint32 flags = m_flagsBuffer.data[particleIndex];
        InitDampingParameter(invMass, invInertia, tangentDistance, flags & b2_wallParticle ? 0 : GetParticleMass(), 0, point, point, normal);
    }
}

inline float32 b2ParticleSystem::ComputeDampingImpulse(float32 invMassA, float32 invInertiaA, float32 tangentDistanceA, float32 invMassB, float32 invInertiaB, float32 tangentDistanceB,
                                                       float32 normalVelocity) const {
    float32 invMass = invMassA + invInertiaA * tangentDistanceA * tangentDistanceA + invMassB + invInertiaB * tangentDistanceB * tangentDistanceB;
    return invMass > 0 ? normalVelocity / invMass : 0;
}

inline void b2ParticleSystem::ApplyDamping(float32 invMass, float32 invInertia, float32 tangentDistance, bool isRigidGroup, b2ParticleGroup* group, int32 particleIndex, float32 impulse,
                                           const b2Vec2& normal) {
    if (isRigidGroup) {
...
void b2ParticleSystem::SolveRigidDamping() {
    ...
    for (int32 k = 0; k < m_bodyContactBuffer.GetCount(); k++) {
        ...
        if (IsRigidGroup(aGroup)) {
            ...
            // the particle into the body.
            {
                ...
                InitDampingParameterWithRigidGroupOrParticle(&invMassA, &invInertiaA, &tangentDistanceA, true, aGroup, a, p, n);
                InitDampingParameter(&invMassB, &invInertiaB, &tangentDistanceB, b->GetMass(),
                                     // Calculate b->m_I from public functions of b2Body.
                                     b->GetInertia() - b->GetMass() * b->GetLocalCenter().LengthSquared(), b->GetWorldCenter(), p, n);
                float32 f = damping * b2Min(w, 1.0f) * ComputeDampingImpulse(invMassA, invInertiaA, tangentDistanceA, invMassB, invInertiaB, tangentDistanceB, vn);
                ApplyDamping(invMassA, invInertiaA, tangentDistanceA, true, aGroup, a, f, n);
    ...
    for (int32 k = 0; k < m_contactBuffer.GetCount(); k++) {
        ...
        if (aGroup != bGroup && (aRigid || bRigid)) {
            b2Vec2 p = 0.5f * (m_positionBuffer.data[a] + m_positionBuffer.data[b]);
            b2Vec2 v = GetLinearVelocity(bGroup, b, p) - GetLinearVelocity(aGroup, a, p);
            float32 vn = b2Dot(v, n);
            if (vn < 0) {
                float32 invMassA, invInertiaA, tangentDistanceA;
                float32 invMassB, invInertiaB, tangentDistanceB;
                InitDampingParameterWithRigidGroupOrParticle(&invMassA, &invInertiaA, &tangentDistanceA, aRigid, aGroup, a, p, n);
                InitDampingParameterWithRigidGroupOrParticle(&invMassB, &invInertiaB, &tangentDistanceB, bRigid, bGroup, b, p, n);
                float32 f = damping * w * ComputeDampingImpulse(invMassA, invInertiaA, tangentDistanceA, invMassB, invInertiaB, tangentDistanceB, vn);
                ApplyDamping(invMassA, invInertiaA, tangentDistanceA, aRigid, aGroup, a, f, n);
                ApplyDamping(invMassB, invInertiaB, tangentDistanceB, bRigid, bGroup, b, -f, n);
            }
        }
    }
}
...
void b2ParticleSystem::SolveTensile(const b2TimeStep& step) {
    ...
    for (int32 k = 0; k < m_contactBuffer.GetCount(); k++) {
        ...
        if (contact.GetFlags() & b2_tensileParticle) {
            ...
            float32 fn = b2Min(pressureStrength * (h - 2) + normalStrength * b2Dot(s, n), maxVelocityVariation) * w;
            b2Vec2 f = fn * n;
            m_velocityBuffer.data[a] -= f;
            m_velocityBuffer.data[b] += f;
        }
...
void b2ParticleSystem::SolveRepulsive(const b2TimeStep& step) {
    float32 repulsiveStrength = m_def.repulsiveStrength * GetCriticalVelocity(step);
    for (int32 k = 0; k < m_contactBuffer.GetCount(); k++) {
        const b2ParticleContact& contact = m_contactBuffer[k];
...
void b2ParticleSystem::SolveZombie() {
    ...
    for (int32 i = 0; i < m_count; i++) {
        ...
        if (flags & b2_zombieParticle) {
            b2DestructionListener* const destructionListener = m_world->m_destructionListener;
            if ((flags & b2_destructionListenerParticle) && destructionListener) {
            ...
            if (i != newCount) {
                ...
                if (m_lastBodyContactStepBuffer.data) {
                    m_lastBodyContactStepBuffer.data[newCount] = m_lastBodyContactStepBuffer.data[i];
                }
                if (m_bodyContactCountBuffer.data) {
                    m_bodyContactCountBuffer.data[newCount] = m_bodyContactCountBuffer.data[i];
                }
                if (m_consecutiveContactStepsBuffer.data) {
                    m_consecutiveContactStepsBuffer.data[newCount] = m_consecutiveContactStepsBuffer.data[i];
                }
                ...
                if (m_expirationTimeBuffer.data) {
                    m_expirationTimeBuffer.data[newCount] = m_expirationTimeBuffer.data[i];
                }
            }
            newCount++;
    ...
    // predicate functions
    struct Test {
        static bool IsProxyInvalid(const Proxy& proxy) { return proxy.index < 0; }
        static bool IsContactInvalid(const b2ParticleContact& contact) { return contact.GetIndexA() < 0 || contact.GetIndexB() < 0; }
        static bool IsBodyContactInvalid(const b2ParticleBodyContact& contact) { return contact.index < 0; }
        static bool IsPairInvalid(const b2ParticlePair& pair) { return pair.indexA < 0 || pair.indexB < 0; }
        static bool IsTriadInvalid(const b2ParticleTriad& triad) { return triad.indexA < 0 || triad.indexB < 0 || triad.indexC < 0; }
    };
    ...
    for (int32 k = 0; k < m_contactBuffer.GetCount(); k++) {
        ...
        contact.SetIndices(newIndices[contact.GetIndexA()], newIndices[contact.GetIndexB()]);
    }
    ...
    if (m_indexByExpirationTimeBuffer.data) {
        ...
        for (int32 readOffset = 0; readOffset < m_count; readOffset++) {
            const int32 newIndex = newIndices[m_indexByExpirationTimeBuffer.data[readOffset]];
...
void b2ParticleSystem::RotateBuffer(int32 start, int32 mid, int32 end) {
    ...
    if (m_expirationTimeBuffer.data) {
        ...
        const int32 particleCount = GetParticleCount();
        int32* const indexByExpirationTime = m_indexByExpirationTimeBuffer.data;
        for (int32 i = 0; i < particleCount; ++i) {
...
/// time.
void b2ParticleSystem::SetParticleLifetime(const int32 index, const float32 lifetime) {
    b2Assert(ValidateParticleIndex(index));
    const bool initializeExpirationTimes = m_indexByExpirationTimeBuffer.data == NULL;
    m_expirationTimeBuffer.data = RequestBuffer(m_expirationTimeBuffer.data);
    m_indexByExpirationTimeBuffer.data = RequestBuffer(m_indexByExpirationTimeBuffer.data);
    ...
    const int32 newExpirationTime = quantizedLifetime > 0 ? GetQuantizedTimeElapsed() + quantizedLifetime : quantizedLifetime;
...
float32 b2ParticleSystem::ExpirationTimeToLifetime(const int32 expirationTime) const {
    return (float32)(expirationTime > 0 ? expirationTime - GetQuantizedTimeElapsed() : expirationTime) * m_def.lifetimeGranularity;
}
...
int32 b2ParticleSystem::GetQuantizedTimeElapsed() const { return (int32)(m_timeElapsed >> 32); }

/// Convert a lifetime in seconds to an expiration time.
int64 b2ParticleSystem::LifetimeToExpirationTime(const float32 lifetime) const { return m_timeElapsed + (int64)((lifetime / m_def.lifetimeGranularity) * (float32)(1LL << 32)); }

template <typename T>
void b2ParticleSystem::SetUserOverridableBuffer(UserOverridableBuffer<T>* buffer, T* newData, int32 newCapacity) {
...
void b2ParticleSystem::SetFlagsBuffer(uint32* buffer, int32 capacity) { SetUserOverridableBuffer(&m_flagsBuffer, buffer, capacity); }

void b2ParticleSystem::SetPositionBuffer(b2Vec2* buffer, int32 capacity) { SetUserOverridableBuffer(&m_positionBuffer, buffer, capacity); }

void b2ParticleSystem::SetVelocityBuffer(b2Vec2* buffer, int32 capacity) { SetUserOverridableBuffer(&m_velocityBuffer, buffer, capacity); }

void b2ParticleSystem::SetColorBuffer(b2ParticleColor* buffer, int32 capacity) { SetUserOverridableBuffer(&m_colorBuffer, buffer, capacity); }

void b2ParticleSystem::SetUserDataBuffer(void** buffer, int32 capacity) { SetUserOverridableBuffer(&m_userDataBuffer, buffer, capacity); }
...
void b2ParticleSystem::SetParticleFlags(int32 index, uint32 newFlags) {
    ...
    if (~m_allParticleFlags & newFlags) {
        ...
        if (newFlags & b2_tensileParticle) {
            m_accumulation2Buffer = RequestBuffer(m_accumulation2Buffer);
        }
...
void b2ParticleSystem::SetGroupFlags(b2ParticleGroup* group, uint32 newFlags) {
...
static inline bool IsSignificantForce(const b2Vec2& force) { return force.x != 0 || force.y != 0; }

inline bool b2ParticleSystem::ForceCanBeApplied(uint32 flags) const { return !(flags & b2_wallParticle); }
...
void b2ParticleSystem::ApplyForce(int32 firstIndex, int32 lastIndex, const b2Vec2& force) {
...
void b2ParticleSystem::ParticleApplyForce(int32 index, const b2Vec2& force) {
    if (IsSignificantForce(force) && ForceCanBeApplied(m_flagsBuffer.data[index])) {
        PrepareForceBuffer();
        m_forceBuffer[index] += force;
    }
}

void b2ParticleSystem::ApplyLinearImpulse(int32 firstIndex, int32 lastIndex, const b2Vec2& impulse) {
...
void b2ParticleSystem::QueryAABB(b2QueryCallback* callback, const b2AABB& aabb) const {
    ...
    const Proxy* beginProxy = m_proxyBuffer.Begin();
    const Proxy* endProxy = m_proxyBuffer.End();
    const Proxy* firstProxy = std::lower_bound(beginProxy, endProxy, computeTag(m_inverseDiameter * aabb.lowerBound.x, m_inverseDiameter * aabb.lowerBound.y));
    const Proxy* lastProxy = std::upper_bound(firstProxy, endProxy, computeTag(m_inverseDiameter * aabb.upperBound.x, m_inverseDiameter * aabb.upperBound.y));
...
void b2ParticleSystem::QueryShapeAABB(b2QueryCallback* callback, const b2Shape& shape, const b2Transform& xf) const {
    b2AABB aabb;
    shape.ComputeAABB(&aabb, xf);
...
b2ParticleSystem::b2ExceptionType b2ParticleSystem::IsBufCopyValid(int startIndex, int numParticles, int copySize, int bufSize) const {
    ...
    if (startIndex < 0 || startIndex >= maxNumParticles || numParticles < 0 || numParticles + startIndex > maxNumParticles) {
