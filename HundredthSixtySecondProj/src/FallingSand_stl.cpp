...
std::string RemoveQuotes(std::string line) {
    if (line.empty() == false && (line[0] == '"' || line[0] == '\'')) line = line.substr(1, line.size() - 1);
...
bool IsNumericString(const std::string &what) { return (what.empty() == false && what.find_first_not_of("0123456789.+-") == what.npos); }
...
std::string RemoveBeginning(const std::string &beginning, const std::string &line) { return line.substr(beginning.size()); }
