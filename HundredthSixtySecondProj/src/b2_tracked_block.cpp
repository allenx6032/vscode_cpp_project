...
b2TrackedBlock::b2TrackedBlock() {
    b2TrackedBlock** pointerToThis = (b2TrackedBlock**)((uint8*)GetMemory() - sizeof(b2TrackedBlock**));
    *pointerToThis = this;
}
...
void* b2TrackedBlock::GetMemory() const {
    ...
    static const uint32 kSizeOfThisWithNoPadding = sizeof(*this) - sizeof(m_padding) + sizeof(b2TrackedBlock**);
    ...
    uint8* const aligned = (uint8*)(((uintptr_t)this + kSizeOfThisWithNoPadding + b2_mallocAlignment - 1) & ~((uintptr_t)b2_mallocAlignment - 1));
...
void* b2TrackedBlock::Allocate(uint32 size) {
    void* memory = (b2TrackedBlock*)b2Alloc(sizeof(b2TrackedBlock) + size);
    if (!memory) {
        return NULL;
    }
...
b2TrackedBlock* b2TrackedBlock::GetFromMemory(void* memory) {
    ...
    b2Assert(memory);
    b2TrackedBlock** blockPtr = (b2TrackedBlock**)(aligned - sizeof(b2TrackedBlock**));
    b2Assert(*blockPtr);
    return *blockPtr;
}

/// Free a block of memory returned by b2TrackedBlock::Allocate()
void b2TrackedBlock::Free(void* memory) { Free(GetFromMemory(memory)); }
...
void b2TrackedBlockAllocator::Free(void* memory) { b2TrackedBlock::Free(memory); }
