...
int Audio::PlaySounds(const std::string &strSoundName, const MEvec3 &vPosition, float fVolumedB) {
...
FMOD_VECTOR Audio::VectorToFmod(const MEvec3 &vPosition) {
    FMOD_VECTOR fVec;
    fVec.x = vPosition.x;
    fVec.y = vPosition.y;
    fVec.z = vPosition.z;
...
int Audio::ErrorCheck(FMOD_RESULT result) {
    if (result != FMOD_OK) {
        METADOT_ERROR("[Audio] FMOD Error: ", result);
