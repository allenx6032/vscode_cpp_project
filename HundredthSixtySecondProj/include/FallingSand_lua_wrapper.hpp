...
#include "FallingSand_name.hpp"
#include "FallingSand_type.hpp"
#include "lua.hpp"
...
int CallOnStructCreate(lua_State *L) {
    // #1 = target lua struct
    auto struct_name = ME::Type_of<T>.GetName();
...
decltype(auto) GetStructMember(TT &&x) {
    ...
    if constexpr (N >= GetAmountOfPrivateDataFields<typename ClassTraits<T>::BaseClass>::value) {
        return StructApply(static_cast<typename std::conditional<std::is_const<TT>::value, const PrivateData &, PrivateData &>::type>(x),
                           [](auto &&...args) -> decltype(auto) { return std::get<N - GetAmountOfPrivateDataFields<BaseClass>::value>(std::tie(args...)); });
    } else {
...
auto MetaIndexImpl2(lua_State *L, T &&arg) {
    if constexpr (std::disjunction<typename std::negation<IsCompleteType<typename std::remove_cv<T>::type>>::type, std::is_aggregate<typename std::remove_cv<T>::type>,
...
public:
    std::type_index GetTypeID() const override { return typeid(typename std::remove_cv<T>::type); }
    int NewScalar(lua_State *L) const override {
...
template <class T, class Base, bool = std::conjunction<IsCompleteType<typename std::remove_cv<T>::type>, std::is_aggregate<typename std::remove_cv<T>::type>>::value,
...
public:
    ...
    int MetaIndex(lua_State *L) const override {
        if (lua_type(L, 2) == LUA_TSTRING) {
            if (!strcmp(lua_tostring(L, 2), "self")) return this->Copy(L);
            detail::TypeInterfaceStringIndexReplace(this, L, "struct/", ME::Type_of<typename std::decay<T>::type>.GetName());
        }
        void *value = lua_touserdata(L, 1);
        int N = lua_tointeger(L, 2);
        T *object = reinterpret_cast<T *>(value);
        bool success = StructApply(*object, [L, N](auto &&...args) { return detail::MetaIndexImpl(L, N, std::make_index_sequence<sizeof...(args)>(), std::forward<decltype(args)>(args)...); });
        return success ? 1 : 0;
    }
    int MetaNewIndex(lua_State *L) const override {
        ...
        int N = lua_tointeger(L, 2);
        T *object = reinterpret_cast<T *>(value);
        bool success = StructApply(*object, [L, N](auto &&...args) { return detail::MetaNewIndexImpl(L, N, std::make_index_sequence<sizeof...(args)>(), std::forward<decltype(args)>(args)...); });
        if (!success) {
            std::string type(this->GetTypeName());
...
public:
    ...
    int MetaIndexImpl(lua_State *L, T &objref, std::size_t N, std::index_sequence<I...>) const {
        static constexpr void (*meta_switch[sizeof...(I)])(lua_State * L, T & objref) = {+[](lua_State *L, T &objref) { detail::MetaIndexImpl2(L, GetStructMember<I>(objref)); }...};
    ...
    int MetaNewIndexImpl(lua_State *L, T &objref, std::size_t N, std::index_sequence<I...>) const {
        static constexpr void (*meta_switch[sizeof...(I)])(lua_State * L, T & objref) = {+[](lua_State *L, T &objref) { Get(L, -1, GetStructMember<I>(objref)); }...};
        if (N < 1 || N > sizeof...(I)) return 0;
        meta_switch[N - 1](L, objref);
        return 1;
    }
    int MetaIndex(lua_State *L) const override {
        ...
        detail::TypeInterfaceStringIndexReplace(this, L, "class/", ME::Type_of<typename std::decay<T>::type>.GetName());
        void *value = lua_touserdata(L, 1);
        int N = lua_tointeger(L, 2);
    ...
    int MetaNewIndex(lua_State *L) const override {
        ...
        int N = lua_tointeger(L, 2);
        T *object = reinterpret_cast<T *>(value);
        if (T *object = reinterpret_cast<T *>(value)) return MetaNewIndexImpl(L, *object, N, std::make_index_sequence<MemberCount>());
        return 0;
    }
};
...
class TypeInterfaceDecoratorCall<T, Base, true> : public Base {
    ...
    int MetaCallImpl2(lua_State *L, std::index_sequence<I...>, TypeIdentity<std::function<Ret(Args...)>>) const {
        ...
        return (!std::is_void_v<Ret> + ... +
                PushIfLvRef(L, std::get<I>(args), std::bool_constant<(std::is_lvalue_reference<Args>::value && !std::is_const<typename std::remove_reference<Args>::type>::value)>()));
    }
    ...
    int MetaCall(lua_State *L) const override { return MetaCallImpl(L, TypeIdentity<decltype(std::function(std::declval<T>()))>()); }
};

template <class T>
class TypeInterface
    : public TypeInterfaceDecoratorCommon<
              T, TypeInterfaceDecoratorIndex<T, TypeInterfaceDecoratorComplete<T, TypeInterfaceDecoratorCopy<T, TypeInterfaceDecoratorCall<T, TypeInterfaceDecoratorToString<T, ITypeInterface>>>>>> {
public:
    TypeInterface();
};
...
inline void PushString(lua_State *L, std::string_view str) { lua_pushlstring(L, str.data(), str.size()); }
...
auto PushStruct(lua_State *L, T &&v) -> typename std::enable_if<std::is_aggregate<typename std::decay<T>::type>::value>::type {
    StructApply(v, [L](auto &&...args) { detail::PushStructImpl(L, std::make_index_sequence<sizeof...(args)>(), std::forward<decltype(args)>(args)...); });
...
auto GetStruct(lua_State *L, int N, T &v) -> typename std::enable_if<std::is_aggregate<typename std::decay<T>::type>::value>::type {
    lua_pushvalue(L, N);
    // #1 = struct
    StructApply(v, [L](auto &...args) { detail::GetStructImpl(L, std::make_index_sequence<sizeof...(args)>(), args...); });
    lua_pop(L, 1);
    // #0
}
...
// If you want use registered class by lua_wrapper between multiple shared library,
...
public:
    typedef typename std::conditional<std::is_array<U>::value, typename std::remove_extent<U>::type *,
                                      typename std::conditional<is_function<U>::value, typename std::add_pointer<U>::type, typename std::remove_cv<U>::type>::type>::type type;
};
...
inline int lua_rawgetp_rtype(lua_State *L, int idx, const void *ptr) { return lua_rawgetp(L, idx, ptr); }
inline int lua_rawget_rtype(lua_State *L, int idx) { return lua_rawget(L, idx); }
inline int lua_getfield_rtype(lua_State *L, int idx, const char *k) { return lua_getfield(L, idx, k); }
...
template <typename T>
struct FunctionSignature<T, typename std::enable_if<has_operator_fn<T>::value>::type> : public FunctorSignature<decltype(&T::operator())> {};
#endif
...
template <std::size_t remain, class Arg, class... Args>
struct TypeIndexGet<remain, TypeTuple<Arg, Args...>, false> : TypeIndexGet<remain - 1, TypeTuple<Args...>> {};
...
public:
    ...
    /// @param state
    explicit ScopedSavedStack(lua_State *state) : state_(state), saved_top_index_(state_ ? lua_gettop(state_) : 0) {}

    /// @brief save stack count
    /// @param state
    /// @param count stack count
    explicit ScopedSavedStack(lua_State *state, int count) : state_(state), saved_top_index_(count) {}
...
struct ObjectSharedPointerWrapper : ObjectWrapperBase {
    template <typename T>
    ObjectSharedPointerWrapper(const ref<T> &sptr)
        : object_(std::const_pointer_cast<typename std::remove_const<T>::type>(sptr)),
          type_(metatableType<T>()),
          shared_ptr_type_(metatableType<ref<typename traits::decay<T>::type>>()),
          const_value_(traits::is_const<T>::value) {}

    template <typename T>
    ObjectSharedPointerWrapper(ref<T> &&sptr)
    ...
    ref<void> object() const { return const_value_ ? ref<void>() : object_; }
    ref<const void> const_object() const { return object_; }
    const std::type_info &shared_ptr_type() const { return shared_ptr_type_; }

    virtual const std::type_info &native_type() { return metatableType<ref<void>>(); }
...
struct PointerConverter {
    ...
    template <typename T, typename F>
    static ref<void> base_shared_pointer_cast(const ref<void> &from) {
        return ref<T>(std::static_pointer_cast<F>(from));
    }

    typedef void *(*convert_function_type)(void *);
    typedef ref<void> (*shared_ptr_convert_function_type)(const ref<void> &);
    ...
    void add_type_conversion() {
        ...
        add_function(metatableType<ref<ToType>>(), metatableType<ref<FromType>>(), &base_shared_pointer_cast<ToType, FromType>);
    }
    ...
    template <typename TO>
    ref<TO> get_shared_pointer(ObjectSharedPointerWrapper *from) const {
        const std::type_info &to_type = metatableType<ref<typename traits::decay<TO>::type>>();
        // unreachable
        //          if (to_type == from->type())
        //          {
        //              return
    ...
    template <class T>
    ref<T> get_pointer(ObjectWrapperBase *from, types::typetag<ref<T>>) {
...
private:
    void add_function(const std::type_info &to_type, const std::type_info &from_type, convert_function_type f) {
        ...
        for (std::map<convert_map_key, std::vector<convert_function_type>>::iterator it = function_map_.begin(); it != function_map_.end(); ++it) {
    ...
    void add_function(const std::type_info &to_type, const std::type_info &from_type, shared_ptr_convert_function_type f) {
        ...
        for (std::map<convert_map_key, std::vector<shared_ptr_convert_function_type>>::iterator it = shared_ptr_function_map_.begin(); it != shared_ptr_function_map_.end(); ++it) {
            if (it->first.second == to_type.name()) {
    ...
    ref<void> pcvt_list_apply(ref<void> ptr, const std::vector<shared_ptr_convert_function_type> &flist) const {
...
template <class T>
ref<T> get_shared_pointer(lua_State *l, int index, types::typetag<T>) {
    ...
    if (ptr) {
        ...
        const std::type_info &to_type = metatableType<ref<typename traits::decay<T>::type>>();
#if ME_LUAWRAPPER_NAME_BASED_TYPE_CHECK
        ...
        if (from_type == to_type) {
            ...
            if (std::is_const<T>::value) {
                return std::static_pointer_cast<T>(std::const_pointer_cast<void>(ptr->const_object()));
            } else {
...
inline ref<void> get_shared_pointer(lua_State *l, int index, types::typetag<void>) {
    ObjectSharedPointerWrapper *ptr = dynamic_cast<ObjectSharedPointerWrapper *>(object_wrapper(l, index));
    if (ptr) {
        return ptr->object();
    }
...
inline ref<const void> get_shared_pointer(lua_State *l, int index, types::typetag<const void>) {
    ObjectSharedPointerWrapper *ptr = dynamic_cast<ObjectSharedPointerWrapper *>(object_wrapper(l, index));
    if (ptr) {
        return ptr->const_object();
    }
    return ref<const void>();
}
...
bool checkType(lua_State *l, int index) {
    return lua_type_traits<From>::checkType(l, index) || checkType<To, Remain...>(l, index);
}
template <class To>
...
bool strictCheckType(lua_State *l, int index) {
    return lua_type_traits<From>::strictCheckType(l, index) || strictCheckType<To, Remain...>(l, index);
    ;
}
...
struct index_range<step, last, index_tuple<indexes...>, false> : index_range<step + 1, last, index_tuple<indexes..., step>> {};
...
template <typename T>
struct lua_type_traits<T, typename traits::enable_if<traits::is_const_reference<T>::value>::type> : lua_type_traits<typename traits::remove_const_reference<T>::type> {};

/// @ingroup lua_type_traits
/// @brief lua_type_traits for lvalue reference type
template <typename REF>
struct lua_type_traits<REF, typename traits::enable_if<traits::is_lvalue_reference<REF>::value && !traits::is_const<typename traits::remove_reference<REF>::type>::value>::type> {
    typedef void Registerable;
    ...
    static bool strictCheckType(lua_State *l, int index) { return object_wrapper<T>(l, index, false) != 0; }
    static bool checkType(lua_State *l, int index) {
...
struct lua_type_traits<PTR, typename traits::enable_if<traits::is_pointer<typename traits::remove_const_reference<PTR>::type>::value &&
...
template <typename T>
typename traits::enable_if<!has_optional_get<T>::value, optional<T>>::type opt_helper(lua_State *state, int index) {
    try {
...
template <typename T>
struct lua_type_traits<ref<T>> {
    typedef const ref<T> &push_type;
    typedef ref<T> get_type;
    ...
    static bool strictCheckType(lua_State *l, int index) {
        ...
        const std::type_info &type = metatableType<ref<typename traits::decay<T>::type>>();
#if ME_LUAWRAPPER_NAME_BASED_TYPE_CHECK
    ...
    static bool checkType(lua_State *l, int index) { return get_shared_pointer(l, index, types::typetag<T>()) || lua_isnil(l, index); }
    static get_type get(lua_State *l, int index) {
...
struct lua_type_traits<std::unique_ptr<T, Deleter>> {
    ...
    static bool strictCheckType(lua_State *l, int index) { return object_wrapper<type>(l, index, false) != 0; }
    static bool checkType(lua_State *l, int index) { return object_wrapper<type>(l, index) != 0 || lua_isnil(l, index); }
    static get_type get(lua_State *l, int index) {
...
struct lua_type_traits<ObjectWrapperBase *> {
    ...
    static bool strictCheckType(lua_State *l, int index) { return object_wrapper(l, index) != 0; }
...
struct lua_type_traits<T, typename traits::enable_if<traits::is_enum<T>::value>::type> {
    ...
    static get_type get(lua_State *l, int index) { return static_cast<get_type>(lua_type_traits<luaInt>::get(l, index)); }
    static int push(lua_State *l, push_type s) { return util::push_args(l, static_cast<typename lua_type_traits<int64_t>::push_type>(s)); }
};
...
private:
    ...
    int pushStackIndex_(lua_State *state) const { return static_cast<const Derived *>(this)->pushStackIndex(state); }
    template <typename T>
...
protected:
    ...
    int stack_index_;
    mutable bool pop_;
    StackRef(StackRef &&src) : state_(src.state_), stack_index_(src.stack_index_), pop_(src.pop_) { src.pop_ = false; }
...
public:
    ...
    struct RefHolder {
        ...
        RefHolder(RefHolder &&src) throw() : state_(src.state_), ref_(src.ref_) { src.ref_ = LUA_REFNIL; }
        RefHolder &operator=(RefHolder &&src) throw() {
            swap(src);
            return *this;
        }
...
private:
    lua_State *state_() const { return static_cast<const Derived *>(this)->state(); }
    int pushStackIndex_(lua_State *state) const { return static_cast<const Derived *>(this)->pushStackIndex(state); }
    int push_(lua_State *state) const { return static_cast<const Derived *>(this)->push(state); }
...
struct table_proxy {
    ...
    static void get(lua_State *state, int table_index, const char *key) { lua_getfield(state, table_index, key); }
    static void get(lua_State *state, int table_index, const std::string &key) { lua_getfield(state, table_index, key.c_str()); }
    static void get(lua_State *state, int table_index, luaInt key) { lua_geti(state, table_index, key); }
    ...
    static void rawget(lua_State *state, int table_index, luaInt key) { lua_rawgeti(state, table_index, key); }
};
...
public:
    ...
    // deprecated. use get<lua_wrapper::optional<T> >() instead;
    template <typename T>
...
inline std::tuple<> get_result_tuple_impl(lua_State *, int, types::typetag<std::tuple<>>) { return std::tuple<>(); }
template <typename T, typename... TYPES>
inline std::tuple<T, TYPES...> get_result_tuple_impl(lua_State *l, int index, types::typetag<std::tuple<T, TYPES...>>) {
    return std::tuple_cat(std::tuple<T>(lua_type_traits<T>::get(l, index)), get_result_tuple_impl(l, index + 1, types::typetag<std::tuple<TYPES...>>()));
}
template <typename... TYPES>
inline std::tuple<TYPES...> get_result_impl(lua_State *l, int startindex, types::typetag<std::tuple<TYPES...>> tag) {
...
class LuaUserData : public Ref::RegistoryRef, public detail::LuaUserDataImpl<LuaUserData>, public detail::LuaTableOrUserDataImpl<LuaUserData>, public detail::LuaBasicTypeFunctions<LuaUserData> {
...
public:
    ...
    LuaUserData(lua_State *state, StackTop) : Ref::RegistoryRef(state, StackTop()) { typecheck(); }
    template <typename TYPE>
    LuaUserData(lua_State *state, const TYPE &table) : Ref::RegistoryRef(state, table) {
        typecheck();
    }
    explicit LuaUserData(lua_State *state) : Ref::RegistoryRef(state, NilValue()) { typecheck(); }
...
struct lua_type_traits<LuaUserData> {
    ...
    static bool strictCheckType(lua_State *l, int index) { return lua_type(l, index) == LUA_TUSERDATA; }
    static bool checkType(lua_State *l, int index) { return lua_type(l, index) == LUA_TUSERDATA || lua_isnil(l, index); }
...
class LuaTable : public Ref::RegistoryRef, public detail::LuaTableImpl<LuaTable>, public detail::LuaTableOrUserDataImpl<LuaTable>, public detail::LuaBasicTypeFunctions<LuaTable> {
...
public:
    ...
    LuaTable(lua_State *state, const NewTable &table) : Ref::RegistoryRef(state, table) { typecheck(); }
...
struct lua_type_traits<LuaTable> {
    ...
    static bool checkType(lua_State *l, int index) { return lua_istable(l, index) || lua_isnil(l, index); }
...
class LuaFunction : public Ref::RegistoryRef, public detail::LuaFunctionImpl<LuaFunction>, public detail::LuaBasicTypeFunctions<LuaFunction> {
    void typecheck() {
        int t = type();
...
public:
    ...
    /// lua_wrapper::function(function_ptr),lua_wrapper::overload(function_ptr)
    template <typename F>
    ...
    /// @param state pointer to lua_State
    LuaFunction(lua_State *state, StackTop) : Ref::RegistoryRef(state, StackTop()) { typecheck(); }
    ...
    static LuaFunction loadstring(lua_State *state, const std::string &luacode) { return loadstring(state, luacode.c_str()); }
    /// @brief load lua code .
    /// @param state pointer to lua_State
    /// @param luacode string
    ...
    static LuaFunction loadfile(lua_State *state, const std::string &file) { return loadfile(state, file.c_str()); }
...
class LuaThread : public Ref::RegistoryRef, public detail::LuaThreadImpl<LuaThread>, public detail::LuaBasicTypeFunctions<LuaThread> {
    void typecheck() {
        int t = type();
...
public:
    /// @brief construct with stack top value.
    LuaThread(lua_State *state, StackTop) : Ref::RegistoryRef(state, StackTop()) { typecheck(); }
...
struct _wcheckeval {
    ...
    int index;
    bool opt_arg;
    operator bool() { return (opt_arg && lua_isnoneornil(state, index)) || lua_type_traits<T>::checkType(state, index); }
};
...
struct _scheckeval {
    ...
    int index;
    bool opt_arg;
    operator bool() { return (opt_arg && lua_isnoneornil(state, index)) || lua_type_traits<T>::strictCheckType(state, index); }
};
...
struct ConstructorFunctor<util::FunctionSignatureType<ClassType, Args...>> {
    ...
    bool checkArgTypes(lua_State *L, int opt_count = 0) const { return _ctype_apply(L, get_index(), typename signature_type::argument_type_tuple(), opt_count); }
...
struct PolymorphicInvoker {
    typedef ref<FunctionImpl> holder_type;
...
typename lua_type_traits<typename util::ArgumentType<INDEX, F>::type>::get_type getArgument(lua_State *state) {
...
template <class MemType, class T>
typename traits::enable_if<traits::is_object<MemType>::value, int>::type call(lua_State *state, MemType T::*mptr) {
    ...
    if (lua_gettop(state) == 1) {
        if (!this_) {
            ...
            if (is_usertype<MemType>::value && !traits::is_pointer<MemType>::value) {
                return util::push_args(state, std::reference_wrapper<const MemType>(this_.*mptr));
            } else {
                return util::push_args(state, this_.*mptr);
            }
        } else {
            if (is_usertype<MemType>::value && !traits::is_pointer<MemType>::value) {
                return util::push_args(state, std::reference_wrapper<MemType>(this_->*mptr));
            } else {
                return util::push_args(state, this_->*mptr);
            }
        }
...
template <class MemType, class T>
typename traits::enable_if<traits::is_object<MemType>::value, std::string>::type argTypesName(MemType T::*) {
    return util::pretty_name(typeid(T *)) + ",[OPT] " + util::pretty_name(typeid(MemType));
}
template <class MemType, class T>
typename traits::enable_if<traits::is_object<MemType>::value, int>::type minArgCount(MemType T::*) {
    return 1;
}
template <class MemType, class T>
typename traits::enable_if<traits::is_object<MemType>::value, int>::type maxArgCount(MemType T::*) {
    return 2;
}
...
inline bool checkArgTypes(lua_State *state, const PolymorphicInvoker &f) { return f.checkArgTypes(state); }
inline bool strictCheckArgTypes(lua_State *state, const PolymorphicInvoker &f) { return f.strictCheckArgTypes(state); }
...
inline int call(lua_State *state, const PolymorphicMemberInvoker &f) { return f.invoke(state); }
inline int call(lua_State *state, PolymorphicMemberInvoker &f) { return f.invoke(state); }
inline bool checkArgTypes(lua_State *state, const PolymorphicMemberInvoker &f) { return f.checkArgTypes(state); }
inline bool strictCheckArgTypes(lua_State *state, const PolymorphicMemberInvoker &f) { return f.strictCheckArgTypes(state); }
inline std::string argTypesName(const PolymorphicMemberInvoker &f) { return f.argTypesName(); }
...
public:
    ...
    struct iterator {
        ...
        reference operator[](difference_type offset) const { return reference(state_, stack_index_ + offset); }
        /**
         * @name relational operators
         * @brief
         */
        //@{
        bool operator==(const iterator &other) const { return state_ == other.state_ && stack_index_ == other.stack_index_; }
        bool operator!=(const iterator &other) const { return !(*this == other); }
        bool operator<(const iterator &other) const { return stack_index_ < other.stack_index_; }
    ...
    reference operator[](size_t index) const { return reference(state_, startIndex_ + static_cast<int>(index)); }
...
inline VariadicArgType::iterator operator+(int n, const VariadicArgType::iterator &i) { return i + n; }
...
void function_match_scoring(lua_State *state, uint8_t *score_array, int current_index, const Fn &fn) {
    score_array[current_index] = nativefunction::compute_function_matching_score(state, fn);
}
template <typename Fn, typename... Functions>
void function_match_scoring(lua_State *state, uint8_t *score_array, int current_index, const Fn &fn, const Functions &...fns) {
    score_array[current_index] = nativefunction::compute_function_matching_score(state, fn);
...
template <typename Fn, typename... Functions>
int invoke_index(lua_State *state, int index, int current_index, Fn &&fn, Functions &&...fns) {
...
int invoke_tuple_impl(lua_State *state, TupleType &&tuple, nativefunction::index_tuple<S...>) {
...
template <typename TupleType, std::size_t... S>
void push_arg_typename_tuple_impl(lua_State *state, TupleType &&tuple, nativefunction::index_tuple<S...>) {
...
inline FunctionInvokerType<fntuple::tuple<std::function<FTYPE>>> function(T f) {
    return FunctionInvokerType<fntuple::tuple<std::function<FTYPE>>>(fntuple::tuple<std::function<FTYPE>>(std::function<FTYPE>(f)));
}
...
template <typename T>
struct lua_type_traits<T, typename traits::enable_if<traits::is_function<typename traits::remove_pointer<T>::type>::value>::type> {
    static int push(lua_State *l, T f) { return util::one_push(l, lua_wrapper::function(f)); }
};
/// @ingroup lua_type_traits
...
struct lua_type_traits<luacfunction> {
    ...
    static bool strictCheckType(lua_State *l, int index) { return lua_iscfunction(l, index) != 0; }
...
struct lua_type_traits<std::function<T>> {
    ...
    static bool strictCheckType(lua_State *l, int index) { return lua_type(l, index) == LUA_TFUNCTION; }
    ...
    static int push(lua_State *l, push_type v) { return util::one_push(l, lua_wrapper::function(v)); }
};

template <typename F, typename Ret = typename util::FunctionSignature<F>::type::result_type>
struct OverloadFunctionImpl : lua_wrapper::FunctionImpl {
    typedef Ret result_type;
    ...
    virtual bool checkArgTypes(lua_State *state) const { return lua_wrapper::nativefunction::checkArgTypes(state, c_function_type(0), maxArgCount() - minArgCount()); }
    virtual bool strictCheckArgTypes(lua_State *state) const { return lua_wrapper::nativefunction::strictCheckArgTypes(state, c_function_type(0), maxArgCount() - minArgCount()); }
};

template <typename F>
struct OverloadFunctionImpl<F, void> : lua_wrapper::FunctionImpl {
...
class FunctionResults : public Ref::StackRef, public detail::LuaVariantImpl<FunctionResults> {
    FunctionResults(lua_State *state, int return_status, int startIndex)
        : Ref::StackRef(state, startIndex, true), state_(state), resultStatus_(return_status), resultCount_(lua_gettop(state) + 1 - startIndex) {}
    FunctionResults(lua_State *state, int return_status, int startIndex, int endIndex)
        : Ref::StackRef(state, startIndex, true), state_(state), resultStatus_(return_status), resultCount_(endIndex - startIndex) {}
...
public:
    ...
    FunctionResults(FunctionResults &&src) : Ref::StackRef(std::move(src)), state_(src.state_), resultStatus_(src.resultStatus_), resultCount_(src.resultCount_) { src.state_ = 0; }
    ...
    struct iterator_base {
        ...
        template <typename U>
        iterator_base(const iterator_base<U> &other) : state(other.state), stack_index(other.stack_index) {}
         ...
         */
        //@{
        bool operator==(const iterator_base &other) const { return state == other.state && stack_index == other.stack_index; }
...
struct lua_type_traits<LuaFunction> {
    ...
    static bool checkType(lua_State *l, int index) { return lua_isfunction(l, index) || lua_isnil(l, index); }
...
public:
    ...
    bool pushCreateMetatable(lua_State *state) const {
        ...
        if (!traits::is_same<base_class_type, void>::value || !property_map_.empty())  // if base class has property and derived class
    ...
    UserdataMetatable &addFunction(const char *name, Fun f) {
        if (has_key(name)) {
            throw KaguyaException(std::string(name) +
...
public:
    ...
    void setFunction(T f) {
        detail::table_proxy::set(state_, table_index_, key_, lua_wrapper::function(f));
    }
    ...
    ///!constructs the reference. Accessible only to lua_wrapper::LuaRef itself
    TableKeyReferenceProxy(const TableKeyReferenceProxy &src) : state_(src.state_), stack_top_(src.stack_top_), table_index_(src.table_index_), key_(src.key_) { src.state_ = 0; }
...
struct lua_type_traits<TableKeyReferenceProxy<KEY>> {
    static int push(lua_State *l, const TableKeyReferenceProxy<KEY> &ref) { return ref.push(l); }
};
...
struct lua_type_traits<std::array<T, S>> {
    ...
    static bool checkType(lua_State *l, int index) {
        ...
        bool valid = true;
        table.foreach_table_breakable<LuaStackRef, LuaStackRef>([&](const LuaStackRef &k, const LuaStackRef &v) {
...
public:
    ...
    void setErrorHandler(std::function<void(int statuscode, const char *message)> errorfunction) {
        if (!state_) {
            return;
        }
        util::ScopedSavedStack save(state_);
    ...
    /// @param file  file path of lua script
    /// @return reference of lua function
    LuaFunction loadfile(const std::string &file) { return LuaFunction::loadfile(state_, file); }
    /// @brief If there are no errors,compiled file as a Lua function and return.
    ///  Otherwise send error message to error handler and return nil reference
    /// @param file  file path of lua script
    /// @return reference of lua function
    ...
    /// @return reference of lua function
    LuaFunction loadstream(std::istream &stream, const char *chunkname = 0) { return LuaFunction::loadstream(state_, stream, chunkname); }
    ...
    /// @param str lua code
    /// @return reference of lua function
    LuaFunction loadstring(const std::string &str) { return LuaFunction::loadstring(state_, str); }
    /// @brief If there are no errors,compiled string as a Lua function and
    /// return.
    ///  Otherwise send error message to error handler and return nil reference
    /// @param str lua code
    ...
    /// @param env execute env table
    /// @return If there are no errors, returns true.Otherwise return false
    bool dofile(const std::string &file, const LuaTable &env = LuaTable()) { return dofile(file.c_str(), env); }
    ...
    /// @param env execute env table
    /// @return If there are no errors, returns true.Otherwise return false
    bool dostring(const std::string &str, const LuaTable &env = LuaTable()) { return dostring(str.c_str(), env); }
    ...
    /// @return Lua table reference.
    LuaTable newTable(int reserve_array, int reserve_record) { return LuaTable(state_, NewTable(reserve_array, reserve_record)); }
...
struct ref_tuple {
    ...
    void operator=(const FunctionResults &fres) { tref = fres.get_result(types::typetag<GetTuple>()); }
    template <class T>
...
struct scope {
    ...
    TableKeyReferenceProxy<std::string> attr(const std::string &name) { return scope_table_.operator[]<std::string>(name); }
...
void function(const char *name, F f) {
    LuaTable scope = detail::scope_stack::instance().current_scope();
    if (scope) {
        scope[name] = lua_wrapper::function(f);
    }
}
template <typename F>
...
struct Binding {
    ...
    static void push(lua_State *L, const ref<T> &sp) {
        ...
        void *ud = lua_newuserdata(L, sizeof(ref<T>));
    ...
    static void setMembers(lua_State *L, std::true_type) { luaL_setfuncs(L, B::members(), 0); }
    ...
    static int construct(lua_State *L, std::false_type) { return luaL_error(L, "Can not create an instance of %s.", B::class_name); }
    ...
    static const ref<T> &fromStack(lua_State *L, int index) {
...
protected:
    ...
    LuaRefBase(lua_State *L, FromStack) : m_L(L) { m_ref = luaL_ref(m_L, LUA_REGISTRYINDEX); }
...
public:
    ...
    LuaRef(LuaRef &&other) noexcept : LuaRefBase(other.m_L, other.m_ref) { other.m_ref = LUA_REFNIL; }
