...
struct MEcolor {
    ...
    MEcolor() : r(255), g(255), b(255), a(255) {}
    MEcolor(u8 R, u8 G, u8 B, u8 A) : r(R), g(G), b(B), a(A) {}
    MEcolor(float R, float G, float B) : r(R * 255), g(G * 255), b(B * 255), a(255) {}
    MEcolor(MEvec3 &v3) : r(v3.r), g(v3.g), b(v3.b), a(255) {}
    MEcolor(MEvec3 &v3, u8 A) : r(v3.r), g(v3.g), b(v3.b), a(A) {}
 ...
 */
typedef enum { R_NEVER = 0x0200, R_LESS = 0x0201, R_EQUAL = 0x0202, R_LEQUAL = 0x0203, R_GREATER = 0x0204, R_NOTEQUAL = 0x0205, R_GEQUAL = 0x0206, R_ALWAYS = 0x0207 } R_ComparisonEnum;
 ...
 * Values chosen for direct OpenGL compatibility.
 */
typedef enum { R_EQ_ADD = 0x8006, R_EQ_SUBTRACT = 0x800A, R_EQ_REVERSE_SUBTRACT = 0x800B } R_BlendEqEnum;
 ...
 */
typedef enum { R_FILTER_NEAREST = 0, R_FILTER_LINEAR = 1, R_FILTER_LINEAR_MIPMAP = 2 } R_FilterEnum;
 ...
 * \see R_GetSnapMode()
 */
typedef enum { R_SNAP_NONE = 0, R_SNAP_POSITION = 1, R_SNAP_DIMENSIONS = 2, R_SNAP_POSITION_AND_DIMENSIONS = 3 } R_SnapEnum;

/*! \ingroup ImageControls
 * Image wrapping options.  These affect how images handle src_rect coordinates beyond their dimensions when blitted.
 * \see R_SetWrapMode()
 */
typedef enum { R_WRAP_NONE = 0, R_WRAP_REPEAT = 1, R_WRAP_MIRRORED = 2 } R_WrapEnum;
...
typedef enum { R_FILE_AUTO = 0, R_FILE_PNG, R_FILE_BMP, R_FILE_TGA } R_FileFormatEnum;
 ...
 * the<engine>().eng()->ing context data.  Only R_Targets which represent windows will store this. */
...
/*! \ingroup TargetControls
 * the<engine>().eng()-> target object for use as a blitting destination.
...
typedef enum { R_VERTEX_SHADER = 0, R_FRAGMENT_SHADER = 1, R_PIXEL_SHADER = 1, R_GEOMETRY_SHADER = 2 } R_ShaderEnum;

/*! \ingroup ShaderInterface
 * Type enumeration for the shader language used by the renderer.
 */
typedef enum { R_LANGUAGE_NONE = 0, R_LANGUAGE_ARB_ASSEMBLY = 1, R_LANGUAGE_GLSL = 2, R_LANGUAGE_GLSLES = 3, R_LANGUAGE_HLSL = 4, R_LANGUAGE_CG = 5 } R_ShaderLanguageEnum;
...
/*! Gets the current renderer ID order for initialization copied into the 'order' array and the number of renderer IDs into 'order_size'.  Pass NULL for 'order' to just get the size of the renderer
 * order array. */
...
/*! Sets the coordinate mode for this renderer.  Target and image coordinates will be either "inverted" (0,0 is the upper left corner, y increases downward) or "mathematical" (0,0 is the bottom-left
 * corner, y increases upward). The default is inverted (0), as this is traditional for 2D graphics. \param inverted 0 is for inverted coordinates, 1 is for mathematical coordinates */
...
/*! Creates a new render target from the given image.  It can then be accessed from image->target.  This does not increment the internal refcount of the target, so it will be invalidated when the
 * image is freed. */
...
/*! Change the logical size of the given target.  the<engine>().eng()->ing to this target will be scaled as if the dimensions were actually the ones given. */
...
/*! Change the logical size of the given image.  the<engine>().eng()->ing this image will scaled it as if the dimensions were actually the ones given. */
...
/*! the<engine>().eng()->s triangles from the given set of vertices.  This lets you render arbitrary geometry.  It is a direct path to the GPU, so the format is different than typical SDL_gpu calls.
...
/*! the<engine>().eng()->s primitives from the given set of vertices.  This lets you render arbitrary geometry.  It is a direct path to the GPU, so the format is different than typical SDL_gpu calls.
...
// End of the<engine>().eng()->ing
/*! @} */

/*! \ingroup Shapes
 *  @{ */

/*! the<engine>().eng()->s a colored point.
 * \param target The destination render target
 * \param x x-coord of the point
 * \param y y-coord of the point
 * \param color The color of the shape to render
 */
void R_Pixel(R_Target *target, float x, float y, MEcolor color);

/*! the<engine>().eng()->s a colored line.
 * \param target The destination render target
...
/*! the<engine>().eng()->s a colored arc curve (circle segment).
 * \param target The destination render target
...
/*! the<engine>().eng()->s a colored filled arc (circle segment / pie piece).
 * \param target The destination render target
 * \param x x-coord of center point
 * \param y y-coord of center point
 * \param radius The radius of the circle / distance from the center point that rendering will occur
...
/*! the<engine>().eng()->s a colored circle outline.
 * \param target The destination render target
 * \param x x-coord of center point
 * \param y y-coord of center point
 * \param radius The radius of the circle / distance from the center point that rendering will occur
...
/*! the<engine>().eng()->s a colored filled circle.
 * \param target The destination render target
 * \param x x-coord of center point
 * \param y y-coord of center point
 * \param radius The radius of the circle / distance from the center point that rendering will occur
...
/*! the<engine>().eng()->s a colored ellipse outline.
 * \param target The destination render target
 * \param x x-coord of center point
 * \param y y-coord of center point
...
/*! the<engine>().eng()->s a colored filled ellipse.
 * \param target The destination render target
 * \param x x-coord of center point
 * \param y y-coord of center point
 * \param rx x-radius of ellipse
...
/*! the<engine>().eng()->s a colored annular sector outline (ring segment).
 * \param target The destination render target
 * \param x x-coord of center point
 * \param y y-coord of center point
...
/*! the<engine>().eng()->s a colored filled annular sector (ring segment).
 * \param target The destination render target
 * \param x x-coord of center point
 * \param y y-coord of center point
 * \param inner_radius The inner radius of the ring
...
/*! the<engine>().eng()->s a colored triangle outline.
 * \param target The destination render target
...
/*! the<engine>().eng()->s a colored filled triangle.
 * \param target The destination render target
 * \param x1 x-coord of first point
 * \param y1 y-coord of first point
 * \param x2 x-coord of second point
...
/*! the<engine>().eng()->s a colored rectangle outline.
 * \param target The destination render target
...
/*! the<engine>().eng()->s a colored filled rectangle.
 * \param target The destination render target
 * \param x1 x-coord of top-left corner
 * \param y1 y-coord of top-left corner
 * \param x2 x-coord of bottom-right corner
...
/*! the<engine>().eng()->s a colored rounded (filleted) rectangle outline.
 * \param target The destination render target
 * \param x1 x-coord of top-left corner
 * \param y1 y-coord of top-left corner
 * \param x2 x-coord of bottom-right corner
...
/*! the<engine>().eng()->s a colored filled rounded (filleted) rectangle.
 * \param target The destination render target
 * \param x1 x-coord of top-left corner
 * \param y1 y-coord of top-left corner
 * \param x2 x-coord of bottom-right corner
...
/*! the<engine>().eng()->s a colored polygon outline.  The vertices are expected to define a convex polygon.
 * \param target The destination render target
...
/*! the<engine>().eng()->s a colored sequence of line segments.
 * \param target The destination render target
 * \param num_vertices Number of vertices (x and y pairs)
 * \param vertices An array of vertex positions stored as interlaced x and y coords, e.g. {x1, y1, x2, y2, ...}
 * \param color The color of the shape to render
...
/*! the<engine>().eng()->s a colored filled polygon.  The vertices are expected to define a convex polygon.
 * \param target The destination render target
 * \param num_vertices Number of vertices (x and y pairs)
 * \param vertices An array of vertex positions stored as interlaced x and y coords, e.g. {x1, y1, x2, y2, ...}
 * \param color The color of the shape to render
...
typedef struct R_RendererImpl {
    ...
    /*! \see R_GetPixel() */
    MEcolor (*GetPixel)(R_Renderer *renderer, R_Target *target, i16 x, i16 y);
