...
public:
    ...
    uint32 GetLength() const {
        ...
        for (const b2IntrusiveListNode* node = GetNext(); node != terminator; node = node->GetNext()) {
            length++;
        }
        return length;
    }
    ...
    b2IntrusiveListNode* GetNext() const { return m_next; }

    /// Get the previous node in the list.
    b2IntrusiveListNode* GetPrevious() const { return m_prev; }
    ...
    bool ValidateList() const {
        ...
        const b2IntrusiveListNode* const terminator = GetTerminator();
        for (b2IntrusiveListNode* node = GetNext(); node != terminator; node = node->GetNext()) {
...
#define B2_INTRUSIVE_LIST_NODE_GET_CLASS_ACCESSOR(Class, NodeMemberName, FunctionName)    \
    static Class* FunctionName(b2IntrusiveListNode* node) {                               \
        ...
        return reinterpret_cast<Class*>((uint8*)(node) - (uint8*)(&cls->NodeMemberName)); \
    }                                                                                     \
                                                                                          \
    static const Class* FunctionName(const b2IntrusiveListNode* node) { return FunctionName(const_cast<b2IntrusiveListNode*>(node)); }
...
#define B2_INTRUSIVE_LIST_NODE_GET_CLASS(Class, NodeMemberName) B2_INTRUSIVE_LIST_NODE_GET_CLASS_ACCESSOR(Class, NodeMemberName, GetInstanceFromListNode)
...
public:
    ...
    T* GetNext() const { return GetInstanceFromListNode(GetListNode()->GetNext()); }

    /// Get the previous object in the list.
    /// Check against GetTerminator() before deferencing the object.
    T* GetPrevious() const { return GetInstanceFromListNode(GetListNode()->GetPrevious()); }

    /// Get the terminator of the list.
    /// This should not be dereferenced as it is a pointer to
    /// b2TypedIntrusiveListNode<T> *not* T.
    T* GetTerminator() const { return (T*)GetListNode(); }
    ...
    /// Calculate the length of the list.
    uint32 GetLength() const { return GetListNode()->GetLength(); }
