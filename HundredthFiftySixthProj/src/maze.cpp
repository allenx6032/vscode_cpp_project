...
const wchar_t g_wblocks[][4] = { L"━", L"┃", L"┏", L"┛", L"┗", L"┓", L"┫",
							  L"┻", L"┣", L"┳", L"╋"  };
...
void Map::printMap()
{
	...
	float fontHeight = SCR_HEIGHT / (float)m_height;
	float fontWidth = SCR_WIDTH / (float)m_width;
	fnt.lfHeight = fontHeight;
	fnt.lfWidth = fontWidth;
	printf("width = %d, height = %d\n", m_width, m_height);
	printf("font width = %d, font height = %d\n", fnt.lfWidth, fnt.lfHeight);
	...
	for (int i = 0; i != m_height; ++i)
	{
		for (int j = 0; j != m_width; ++j)
			xyprintf(j * fontWidth, i * fontHeight, "%s", g_cblocks[m_map[i * m_width + j]]);
	}
	setfontbkcolor(RED);
	fnt.lfWidth = 12;
	fnt.lfHeight = 20;
