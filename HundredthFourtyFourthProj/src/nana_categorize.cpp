...
#include <nana_float_listbox.hpp>
#include <nana_gadget.hpp>
#include <nana_tree_cont.hpp>
#include <stdexcept>
	...
	namespace drawerbase
	{
		namespace categorize
		{
			...
			public:
				...
				{
					window_ = wd;
					API::background(window_, 0xFFFFFF);
					graph_ = graph;
				}
				...
				void _m_show_list()
				{
					...
					if(ui_el_.what == ui_el_.item_arrow)
					{
						...
						if(i)
						{
							...
							{
								nana::shared_ptr<float_listbox::item_interface> p(new item(child->value.first));
								style_.module.items.push_back(p);
							}
						}
					...
					else if(ui_el_.item_root == ui_el_.what)
					{
						...
						if(treebase_.seq(0, v))
						{
							...
							{
								nana::shared_ptr<float_listbox::item_interface> p(new item((*i)->value.first));
								style_.module.items.push_back(p);
							}
						}
						r = style_.active_item_rectangle;
