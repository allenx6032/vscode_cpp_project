...
namespace gui
{
	...
	namespace element
	{
		...
		public:
			...
			bool draw(graph_reference, nana::color_t bgcolor, nana::color_t fgcolor, const nana::rectangle&, element_state::t);
		private:
		...
		public:
			...
			void states(const std::vector<element_state::t> &);	///< Set the state sequence of the background picture.
			void reset_states();

			void join(element_state::t target, element_state::t joiner);

			void stretch_parts(unsigned left, unsigned top, unsigned right, unsigned bottom);

			//Implement the methods of element_interface.
			virtual bool draw(graph_reference, nana::color_t bgcolor, nana::color_t fgcolor, const nana::rectangle&, element_state::t);
		private:
			struct draw_method;
			struct draw_image;
			struct draw_graph;
			...
			std::vector<element_state::t> states_;
			std::map<element_state::t, element_state::t> join_;
			bool	stretch_all_;
