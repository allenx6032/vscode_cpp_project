...
#include "nana_handle_manager.hpp"
#include "nana_window_layout.hpp"
#include "nana_eventinfo.hpp"
		...
		void close(core_window_t *wd)
		{
			...
			if(wd->other.category == category::flags::root)
			{
				...
				if(false == ei.unload.cancel)
				{
					...
					bedrock_type::instance().evt_manager.umake(reinterpret_cast<window>(wd), true);
					wd->drawer.detached();
