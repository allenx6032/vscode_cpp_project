    ...
    while (ndata)   /* Process all tables in the segment */
    {
...
#if JD_FASTDECODE == 2
        {
            ...
            if (cls)
            {
                ...
                memset(tbl_ac, 0xFF, HUFF_LEN * sizeof(
                           uint16_t));      /* Default value (0xFFFF: may be long code) */
            }
            else
            {
...
//     JDEC *jd,           /* Pointer to the decompressor object */
//     int (*outfunc)(JDEC *, void *, JRECT *, gui_obj_t *), /* RGB output function */
//     unsigned int x,     /* MCU location in the image */
//     unsigned int y,     /* MCU location in the image */
//     gui_obj_t *obj
// )
// {
//     const int CVACC = (sizeof(int) > 2) ? 1024 :
//                       128;   /* Adaptive accuracy for both 16-/32-bit systems */
...
//     return outfunc(jd, jd->workbuf, &rect, obj) ? JDR_OK : JDR_INTR;
// }
...
//             if (rc != JDR_OK) { return rc; }
//             rc = mcu_output(jd, outfunc, x, y, obj);    /* Output the MCU (YCbCr to RGB, scaling and output) */
//             if (rc != JDR_OK) { return rc; }
//         }
//     }
//     return rc;
