...
void drawing(void)
{
	...
	ipaint_raster(paint, pts, picture, NULL, 0xffffffff, 0);

	ipaint_set_color(paint, 0xa000aaaa);

	for (i = 0; i < 4; i++) {
		double x2 = pts[(i + 1) & 3].x;
		double y2 = pts[(i + 1) & 3].y;
		ipaint_draw_line(paint, pts[i].x, pts[i].y, x2, y2);
	}
	...
	for (i = 0; i < 4; i++) {
		...
		ipaint_set_color(paint, 0xa0ff0000);
		if (mouse_on == i || mouse_drag == i) r = 6;
		if (mouse_drag == i) ipaint_set_color(paint, 0xa08000ff);
		ipaint_draw_circle(paint, pts[i].x, pts[i].y, r);

		ipaint_text_color(paint, 0xa0333333);
		ipaint_text_background(paint, 0);
		ipaint_cprintf(paint, pts[i].x - 4, pts[i].y - 16, "%d", i + 1);
	}
	...
	ipaint_cprintf(paint, 1,  0, "USE MOUSE TO DRAG THE FOUR CORNERS OF THE PICTURE");
	ipaint_cprintf(paint, 1, 16, "CLICK RIGHT MOUSE BUTTON TO RESET");
}
