...
void drawing(void)
{
	...
	ipaint_raster_draw(paint, 480, 320, picture, NULL,
		w / 2, h / 2, 1.0, 1.0, 15, cc);

	ipaint_raster_draw(paint, x, 250 + zaka->h, zaka, NULL, 0, 0, 1.0, 1.0, 0, 0xffffffff);
	ipaint_draw(paint, x, 250, zaka, NULL, 0xffffffff, 0);

	ipaint_text_background(paint, 0);
	ipaint_text_color(paint, 0xaf00cccc);
	ipaint_sprintf(paint, 1, 260, "TRADITION INTEGER POSITION DRAW");
	ipaint_sprintf(paint, 1, 320, "MORDEN FLOAT POINT POSITION DRAW");
	ipaint_sprintf(paint, 400, 460, "COLOR SPACE TRANSFORM");

	ipaint_raster_draw(paint, 150, 120, picture, NULL,
		w / 2, h / 2, 1.0, 1.0, theta, 0xffffffff);

	ipaint_raster_draw_3d(paint, 400, 120, 0, picture, NULL,
		w / 2, h / 2, 1.5, 1.5, theta * 10, 30, 0, 0xffffffff);
	theta += 0.5;
	x -= 0.1;

	ipaint_sprintf(paint, 1, 0, "2D ROTATION");
	ipaint_sprintf(paint, 321, 0, "3D ROTATION");
