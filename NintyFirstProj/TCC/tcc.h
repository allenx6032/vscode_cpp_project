...
#pragma warning( \
    disable : 4996) // The POSIX name for this item is deprecated. Instead, use the ISO C and C++ conformant name
...
#define CONFIG_TCC_SYSINCLUDEPATHS \
  "{B}/include" \
  ":" ALSO_TRIPLET(CONFIG_SYSROOT "/usr/local/include") ":" ALSO_TRIPLET(CONFIG_SYSROOT "/usr/include")
#endif
#endif
...
#define CONFIG_TCC_LIBPATHS \
  ...
  ":" ALSO_TRIPLET(CONFIG_SYSROOT "/" CONFIG_LDDIR) ":" ALSO_TRIPLET(CONFIG_SYSROOT "/usr/local/" CONFIG_LDDIR)
#endif
#endif
...
ST_FUNC void
asm_gen_code(ASMOperand* operands, int nb_operands, int nb_outputs, int is_output, uint8_t* clobber_regs, int out_reg);
