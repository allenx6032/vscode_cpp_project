...
static  void EnterCriticalSectionT(CRITICAL_SECTION_T *cs)
{
    int ret;
    ret = pthread_mutex_lock(cs);
    assert(ret == 0);
}
static  void LeaveCriticalSectionT(CRITICAL_SECTION_T *cs)
{
    int ret;
    ret = pthread_mutex_unlock(cs);
    assert(ret == 0);
}
static  void InitializeCriticalSectionT(CRITICAL_SECTION_T *cs)
{
...
static  void DeleteCriticalSectionT(CRITICAL_SECTION_T *cs)
{
    int ret;
...
static inline ALuint timeGetTimeT(void)
{
    int ret;
#if _POSIX_TIMERS > 0
    struct timespec ts;
...
static inline void SleepT(ALuint t)
{
