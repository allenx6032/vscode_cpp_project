...
static Source *checkSource(lua_State *ls, int idx) {
  Source **p = luaL_checkudata(ls, idx, CLASS_NAME);
  return *p;
}

static Source *newSource(lua_State *ls) {
  ...
  Source **p = lua_newuserdata(ls, sizeof(self));
  luaL_setmetatable(ls, CLASS_NAME);
  *p = self;
  return self;
}
...
void source_process(Source *self, int len) {
  ...
  if (self->callbackRef != LUA_NOREF) {
    lockLua();
    lua_State *ls = luaState;
    /* Get _pcall function */
    lua_getglobal(ls, "juno");
    if (!lua_isnil(ls, -1)) {
      lua_getfield(ls, -1, "_pcall");
      if (!lua_isnil(ls, -1)) {
        int i, n;
        /* Get callback function */
        lua_rawgeti(ls, LUA_REGISTRYINDEX, self->callbackRef);
        /* Create a table for our PCM if we don't already have one */
        if (self->tableRef == LUA_NOREF) {
          lua_newtable(ls);
          self->tableRef = luaL_ref(ls, LUA_REGISTRYINDEX);
        }
        /* Push our table and copy buffer PCM (int16 -> double) */
        lua_rawgeti(ls, LUA_REGISTRYINDEX, self->tableRef);
        for (i = 0; i < len; i++) {
          lua_pushnumber(ls, ((double) self->buf[i]) * 3.0517578125e-05);
        ...
        if (!lua_toboolean(ls, -1)) {
          luaL_unref(ls, LUA_REGISTRYINDEX, self->callbackRef);
          self->callbackRef = LUA_NOREF;
        }
        lua_pop(ls, 1); /* Pop function return value */
        /* Copy contents of table back to buffer (double -> int16) */
        n = lua_rawlen(ls, -1);
        for (i = 0; i < n; i++) {
          lua_rawgeti(ls, -1, i + 1);
          self->buf[i] = lua_tonumber(ls, -1) * 32768.;
          lua_pop(ls, 1);
        }
        /* Pop PCM table */
        lua_pop(ls, 1);
...
static int l_source_fromData(lua_State *ls) {
  Data *data = luaL_checkudata(ls, 1, DATA_CLASS_NAME);
  Source *self = newSource(ls);
  /* Init data reference */
  self->data = data;
  lua_pushvalue(ls, 1);
  self->dataRef = luaL_ref(ls, LUA_REGISTRYINDEX);
  ...
  luaL_error(ls, "could not init Source; bad Data format?");
  /* Init stream */
init:;
...
static int l_source_fromBlank(lua_State *ls) {
  Source *self = newSource(ls);
  /* Init */
  self->dest = master;
  /* Issue "add" command to push to `sources` vector */
...
static int l_source_gc(lua_State *ls) {
  Source *self = checkSource(ls, 1);
...
static int l_source_getLength(lua_State *ls) {
  Source *self = checkSource(ls, 1);
  lua_pushnumber(ls, getBaseRate(self) * self->length / self->samplerate);
  return 1;
}


static int l_source_getState(lua_State *ls) {
  Source *self = checkSource(ls, 1);
  switch (self->state) {
    case SOURCE_STATE_PLAYING : lua_pushstring(ls, "playing"); break;
    case SOURCE_STATE_PAUSED  : lua_pushstring(ls, "paused");  break;
    case SOURCE_STATE_STOPPED : lua_pushstring(ls, "stopped"); break;
    default                   : lua_pushstring(ls, "?");       break;
  }
  return 1;
}


static int l_source_setCallback(lua_State *ls) {
  Source *self = checkSource(ls, 1);
  if (!lua_isnoneornil(ls, 2) && lua_type(ls, 2) != LUA_TFUNCTION) {
    luaL_argerror(ls, 2, "expected function");
  }
  Command c = command(COMMAND_SET_CALLBACK, self);
  if (!lua_isnoneornil(ls, 2)) {
    lua_pushvalue(ls, 2);
    c.i = luaL_ref(ls, LUA_REGISTRYINDEX);
  } else {
    c.i = LUA_NOREF;
  }
  pushCommand(&c);
...
static int l_source_setDestination(lua_State *ls) {
  Source *self = checkSource(ls, 1);
  Source *dest = master;
  if (!lua_isnoneornil(ls, 2)) {
    dest = checkSource(ls, 2);
  }
  /* Master? Disallow */
  if (self == master) {
    luaL_argerror(ls, 1, "master cannot be rerouted");
  }
  /* Check for feedback loop */
  Source *s = dest;
  while (s) {
    if (s == self) {
      luaL_error(ls, "routing results in a feedback loop");
    }
    s = s->dest;
  }
  /* Do routing */
  ...
  if (!lua_isnoneornil(ls, 2)) {
    ...
    lua_rawgeti(ls, LUA_REGISTRYINDEX, masterRef);
  }
  c.i = luaL_ref(ls, LUA_REGISTRYINDEX);
  c.p = dest;
  pushCommand(&c);
...
static int l_source_setGain(lua_State *ls) {
  Source *self = checkSource(ls, 1);
  double gain = luaL_optnumber(ls, 2, 1.);
...
static int l_source_setPan(lua_State *ls) {
  Source *self = checkSource(ls, 1);
  double pan = luaL_optnumber(ls, 2, 0.);
...
static int l_source_setRate(lua_State *ls) {
  Source *self = checkSource(ls, 1);
  double rate = luaL_optnumber(ls, 2, 1.);
  if (rate < 0) {
    luaL_argerror(ls, 2, "expected value of zero or greater");
  }
  if (rate > 16) {
    luaL_argerror(ls, 2, "value is too large");
  }
...
static int l_source_setLoop(lua_State *ls) {
  Source *self = checkSource(ls, 1);
  int loop = luax_optboolean(ls, 2, 0);
...
static int l_source_play(lua_State *ls) {
  Source *self = checkSource(ls, 1);
  int reset = luax_optboolean(ls, 2, 0);
...
static int l_source_pause(lua_State *ls) {
  Source *self = checkSource(ls, 1);
...
static int l_source_stop(lua_State *ls) {
  Source *self = checkSource(ls, 1);
...
int luaopen_source(lua_State *ls) {
  luaL_Reg reg[] = {
    { "__gc",           l_source_gc             },
  ...
  ASSERT( luaL_newmetatable(ls, CLASS_NAME) );
  luaL_setfuncs(ls, reg, 0);
  lua_pushvalue(ls, -1);
  ...
  /* Init master */
  master = newSource(ls);
  masterRef = luaL_ref(ls, LUA_REGISTRYINDEX);
