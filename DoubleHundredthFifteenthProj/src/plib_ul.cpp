...
ulDir* ulOpenDir ( const char* dirname )
{
  ...
  if ( dir != NULL )
  {
    ...
    dir->first = true;
    dir->done = false;
    dir->hFind = FindFirstFile((LPCWSTR)search, &dir->data);
...
ulDirEnt* ulReadDir ( ulDir* dir )
{
  ...
  strcpy( (char *)dir->curr.d_name, (const char *)dir->data.cFileName ) ;
  //strncpy( dir->curr.d_name, dir->data.cFileName , sizeof(dir->curr.d_name)) ;
#else
