...
#include "mc_lua_exception.h"
#include "mc_lua.hpp"
#include <sstream>
#include "mc_log.h"
#include "mc_lua_stack_impl.h"
#include "mc_lua_command.h"
...
auto Path(lua_State* state, std::filesystem::path const& path) -> void
{
  std::stringstream stream;
...
auto InitState(std::filesystem::path const& path) -> lua_State* 
{
...
namespace lua
{
StackImpl::StackImpl(std::filesystem::path const& path) : state_(InitState(path))
{
...
auto StackImpl::Load(std::filesystem::path const& file) -> void
{
...
auto Stack::Load(std::filesystem::path const& file) -> void
{
  impl_->Load(file);
}
...
Stack::Stack(std::filesystem::path const& path) : impl_(std::make_shared<StackImpl>(path))
{
}
