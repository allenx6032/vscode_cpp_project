...
Player::Impl::Impl(lua::Stack& lua, display::Window& window, std::filesystem::path const& path) : next_(0), redraw_(true), sign_(0)
{
  {
...
Player::Player(lua::Stack& lua, display::Window& window, std::filesystem::path const& path) : impl_(std::make_shared<Impl>(lua, window, path))
{
}
