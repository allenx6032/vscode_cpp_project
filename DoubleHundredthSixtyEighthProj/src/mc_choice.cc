...
{
public:
  Impl(lua::Stack& lua, display::Window& window, event::Queue& queue, std::filesystem::path const& path);
  auto Render() -> void;
...
Choice::Impl::Impl(lua::Stack& lua, display::Window& window, event::Queue& queue, std::filesystem::path const& path) : window_(window), paused_(true), count_(0), queue_(queue)
{
...
Choice::Choice(lua::Stack& lua, display::Window& window, event::Queue& queue, std::filesystem::path const& path) : impl_(std::make_shared<Impl>(lua, window, queue, path))
{
}
}