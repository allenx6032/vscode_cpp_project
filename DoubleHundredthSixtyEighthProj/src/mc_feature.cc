...
Feature::Impl::Impl(lua::Stack& lua, display::Window& window, std::filesystem::path const& path)
{
  {
...
Feature::Feature(lua::Stack& lua, display::Window& window, std::filesystem::path const& path) : impl_(std::make_shared<Impl>(lua, window, path))
{
}
