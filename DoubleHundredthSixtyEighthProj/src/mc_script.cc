...
namespace game
{
Script::Impl::Impl(display::Window& window, event::Queue& queue, std::filesystem::path const& path, float volume) : window_(window), queue_(queue), paused_(true), fade_(queue, window), begun_(false), path_(path), lua_(path), volume_(volume)
{
}

auto Script::Impl::Init(std::filesystem::path const& file) -> void
{
...
Script::Script(std::filesystem::path const& file, display::Window& window, event::Queue& queue, std::filesystem::path const& path, float volume) : impl_(std::make_shared<Impl>(window, queue, path, volume))
{
  impl_->Init(file);
}
}