...
Subtitle::Impl::Impl(lua::Stack& lua, display::Window& window, std::filesystem::path const& path) : window_(window)
{
  {
...
{
  text_ = display::Texture(text, font_, clip_.w(), window_, display::BoundingBox(0.f, 0.f, clip_.w(), clip_.h()));
...
Subtitle::Subtitle(lua::Stack& lua, display::Window& window, std::filesystem::path const& path) : impl_(std::make_shared<Impl>(lua, window, path))
{
}
}