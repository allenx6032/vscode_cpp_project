...
{
public:
  Impl(std::filesystem::path const& file);
  ~Impl();
...
Saves::Impl::Impl(std::filesystem::path const& file) : file_(file), playing_(false)
{
  lua::Stack lua("");
  lua.Load(file);
...
Saves::Saves(std::filesystem::path const& file) : impl_(std::make_shared<Impl>(file))
{
}
}