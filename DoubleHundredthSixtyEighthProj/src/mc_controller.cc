...
auto Chapters(lua::Stack& lua, Strings& names, Paths& files, std::filesystem::path const& path) -> void
{
...
{
public:
  Impl(lua::Stack& lua, event::Queue& queue, std::filesystem::path const& path);
  auto Init() -> void;
...
Controller::Impl::Impl(lua::Stack& lua, event::Queue& queue, std::filesystem::path const& path) : state_(State::Start), queue_(queue), path_(path), sign_(0)
{
  {
...
Controller::Controller(lua::Stack& lua, event::Queue& queue, std::filesystem::path const& path) : impl_(std::make_shared<Impl>(lua, queue, path))
{
  impl_->Init();
}
