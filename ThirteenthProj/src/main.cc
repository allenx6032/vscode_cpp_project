...
int main(int argc, char** argv)
{
	...
	lua_pushnumber(L, 10);
    printf("lua_pushnumber(L, 10);");
	lua_pushnil(L);
    printf("lua_pushnil(L);");
	// 向stack中push字符串的时候，stack会对字符串进行复制!!!
	char * str = (char *)malloc(sizeof(char) * 20);
    printf("char * str = (char *)malloc(sizeof(char) * 20);");
	strcpy(str, "hello");
    printf("strcpy(str, \"hello\");");
	lua_pushstring(L, str);
    printf("lua_pushstring(L, str);");
	// free(str);
