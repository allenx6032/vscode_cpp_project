...
BL_HIDDEN extern const BLTag tableIdToTagTable[];
...
BL_HIDDEN extern const BLTag scriptIdToTagTable[];
...
BL_HIDDEN extern const BLTag languageIdToTagTable[];
...
BL_HIDDEN extern const BLTag baselineIdToTagTable[];
...
static BL_INLINE uint32_t tableTagToId(uint32_t tag) noexcept {
  static const uint8_t hashTable[128] = {
...
static BL_INLINE uint32_t scriptTagToId(uint32_t tag) noexcept {
  ...
  if (scriptIdToTagTable[i1] == tag)
    index = i1;

  if (scriptIdToTagTable[i2] == tag)
    index = i2;

  return index;
}

static BL_INLINE uint32_t languageTagToId(uint32_t tag) noexcept {
  ...
  if (languageIdToTagTable[i1] == tag)
    index = i1;

  if (languageIdToTagTable[i2] == tag)
    index = i2;
...
static BL_INLINE uint32_t baselineTagToId(uint32_t tag) noexcept {
  static const uint8_t hashTable[16] = {
  ...
  if (baselineIdToTagTable[i1] == tag)
    index = i1;
