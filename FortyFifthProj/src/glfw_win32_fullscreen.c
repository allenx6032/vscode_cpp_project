...
void _glfwSetVideoMode( int *w, int *h, int r, int g, int b, int refresh )
{
	...
	// Dmitri: If the target resolution equals to the desktop resolution, don't actually set the video mode
	if (_glfwLibrary.desktopMode.Width == *w && _glfwLibrary.desktopMode.Height == *h && _glfwLibrary.desktopMode.RedBits == r && _glfwLibrary.desktopMode.GreenBits == g && _glfwLibrary.desktopMode.BlueBits == b && 0 == refresh)
		return;
