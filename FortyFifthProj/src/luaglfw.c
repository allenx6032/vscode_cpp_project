...
//------------------------------------------------------------------------
// $Id: luaglfw.c,v 1.9 2005/03/14 21:09:48 marcus256 Exp $
//========================================================================
...
static int windowsize_ref;
static int windowclose_ref;
static int windowrefresh_ref;
static int mousebutton_ref;
static int mousepos_ref;
static int mousewheel_ref;
static int key_ref;
static int char_ref;
...
void GLFWCALL luaglfw_windowsizefun( int w, int h )
{
    ...
    lua_rawgeti( L, LUA_REGISTRYINDEX, windowsize_ref );
...
int GLFWCALL luaglfw_windowclosefun( void )
{
    ...
    lua_rawgeti( L, LUA_REGISTRYINDEX, windowclose_ref );
    if( lua_isfunction( L, -1 ) )
    {
        lua_pcall( L, 0, 1, 0 );
...
void GLFWCALL luaglfw_windowrefreshfun( void )
{
    ...
    lua_rawgeti( L, LUA_REGISTRYINDEX, windowrefresh_ref );
    if( lua_isfunction( L, -1 ) )
    {
        lua_pcall( L, 0, 0, 0 );
    }
...
void GLFWCALL luaglfw_mousebuttonfun( int button, int action )
{
    ...
    lua_rawgeti( L, LUA_REGISTRYINDEX, mousebutton_ref );
    if( lua_isfunction( L, -1 ) )
    {
...
void GLFWCALL luaglfw_mouseposfun( int x, int y )
{
    ...
    lua_rawgeti( L, LUA_REGISTRYINDEX, mousepos_ref );
    if( lua_isfunction( L, -1 ) )
    {
...
void GLFWCALL luaglfw_mousewheelfun( int wheelpos )
{
    ...
    lua_rawgeti( L, LUA_REGISTRYINDEX, mousewheel_ref );
    if( lua_isfunction( L, -1 ) )
    {
...
void GLFWCALL luaglfw_charfun( int key, int action )
{
    ...
    lua_rawgeti( L, LUA_REGISTRYINDEX, char_ref );
    if( lua_isfunction( L, -1 ) )
    {
        lua_pushnumber( L, (lua_Number)key );
        lua_pushnumber( L, (lua_Number)action );
...
// Video modes
//========================================================================
#define VID_MODES_LEN 25

static GLFWvidmode vid_modes[VID_MODES_LEN];

static int glfw_GetVideoModes( lua_State *L )
{
  int mode_count = glfwGetVideoModes( vid_modes, VID_MODES_LEN );
  int i;
  
  // create the array of video modes
  lua_newtable( L );
  
  for( i=0; i<mode_count; i++ )
  {
    GLFWvidmode mode = vid_modes[i];
    
    // create a table for this element in the array
    lua_newtable( L );
    ...
    // set the table we just created into an index
    // of the array
    lua_rawseti( L, -2, i );
  }
...
static int glfw_SetWindowSizeCallback( lua_State *L )
{
    ...
    if( lua_isfunction( L, 1 ) )
    {
        windowsize_ref = luaL_ref( L, LUA_REGISTRYINDEX );
...
static int glfw_SetWindowCloseCallback( lua_State *L )
{
    ...
    if( lua_isfunction( L, 1 ) )
    {
        windowclose_ref = luaL_ref( L, LUA_REGISTRYINDEX );
...
static int glfw_SetWindowRefreshCallback( lua_State *L )
{
    ...
    if( lua_isfunction( L, 1 ) )
    {
        windowrefresh_ref = luaL_ref( L, LUA_REGISTRYINDEX );
...
static int glfw_SetMouseButtonCallback( lua_State *L )
{
    ...
    if( lua_isfunction( L, 1 ) )
    {
        mousebutton_ref = luaL_ref( L, LUA_REGISTRYINDEX );
...
static int glfw_SetMousePosCallback( lua_State *L )
{
    ...
    if( lua_isfunction( L, 1 ) )
    {
        mousepos_ref = luaL_ref( L, LUA_REGISTRYINDEX );
...
static int glfw_SetMouseWheelCallback( lua_State *L )
{
    ...
    if( lua_isfunction( L, 1 ) )
    {
        mousewheel_ref = luaL_ref( L, LUA_REGISTRYINDEX );
...
static int glfw_SetKeyCallback( lua_State *L )
{
    ...
    if( lua_isfunction( L, 1 ) )
    {
        key_ref = luaL_ref( L, LUA_REGISTRYINDEX );
        fun = luaglfw_keyfun;
    }
...
static int glfw_SetCharCallback( lua_State *L )
{
    ...
    if( lua_isfunction( L, 1 ) )
    {
        char_ref = luaL_ref( L, LUA_REGISTRYINDEX );
        fun = luaglfw_charfun;
    }
...
#define luaL_newlib(L,f) luaL_register(L,"luaglfw",f)
#endif
